            ...
            if (bpName.indexOf("tree:") === 0) {
                ...
                try {
                    ...
                    if (rig && rig.mesh) {
                        ...
                        rig.mesh.updateMatrixWorld(true);
                        if (window.__skel) {
                            // KONVERGENZ V18.454: das 8-KH-Skelett (_humanoidSkeleton) ist GEFALLEN —
                            // der __skel-Detail-Ast zeigt LAUT nichts statt toter Maschine.
                            window.__treeInfo = (window.__treeInfo || "") + " __skel-GEFALLEN(V18.454)";
                        } else {
                            grp.add(rig.mesh);
                        }
...
(async () => {
    ...
    try {
        ...
        for (const [bp, file, view] of [
            ...
            ["humanoidrig:rest:0", "werk-rig-rest-brust.png", "torso"], // Hals/Klavikel/Brust-Top nah
