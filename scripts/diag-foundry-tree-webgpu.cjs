...
(async () => {
    ...
    const out = await page.evaluate(async () => {
        ...
        res.meshKinds = grp.children.map(
            (c) => (c.material && c.material.userData && c.material.userData.foundryKind) || "?"
        );
        // Szene + Licht (AnazhRealm-nah: Hemi + Key). Kamera auf den Baum gerahmt.
        const sc = new T.Scene();
        sc.add(new T.HemisphereLight(0xdfeecc, 0x2a2a1a, 0.7));
        const key = new T.DirectionalLight(0xfff2d9, 2.2);
        ...
        cam.position.set(ctr.x + d * 0.6, box.min.y + sz.y * 0.55, ctr.z + d * 0.7);
        ...
        try {
            ...
            const rowB = W * 4;
            for (let y = 0; y < W; y++) {
                const srcr = (W - 1 - y) * rowB,
                    dstr = y * rowB;
                for (let i = 0; i < rowB; i++) img.data[dstr + i] = buf[srcr + i];
            }
            ctx.putImageData(img, 0, 0);
            // Nicht-Hintergrund-Pixel zaehlen (Sanity).
            let nonBg = 0;
            for (let i = 0; i < buf.length; i += 4) {
                if (Math.abs(buf[i] - 0xbc) > 24 || Math.abs(buf[i + 1] - 0xd2) > 24) nonBg++;
            }
            res.nonBgPx = nonBg;
            res.totalPx = buf.length / 4;
            res.dataURL = cv.toDataURL("image/png");
        } catch (e) {
            res.err = "RTT/readback: " + String((e && e.message) || e);
        }
        return res;
    });
    ...
    if (dataURL) {
        fs.writeFileSync(
            path.join(ART, "foundry-tree-webgpu.png"),
            Buffer.from(dataURL.replace(/^data:image\/png;base64,/, ""), "base64")
        );
        console.log("-> artifacts/foundry-tree-webgpu.png");
    }
    await browser.close();
    server.close();
