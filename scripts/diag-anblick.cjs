...
const flush = () => { try { fs.writeFileSync(path.join(ART, "anblick-progress.json"), JSON.stringify(progress, null, 2)); } catch (_e) {} };
const T0 = Date.now();
const mark = (phase) => { progress.phase = phase; progress.t = ((Date.now() - T0) / 1000).toFixed(1); flush(); console.log(`[${progress.t}s] ${phase}`); };

(async () => {
    if (!fs.existsSync(ART)) fs.mkdirSync(ART, { recursive: true });
    await new Promise((r) => server.listen(PORT, r));
    mark("launch-browser");
    const browser = await puppeteer.launch({ headless: true, protocolTimeout: 595000, args: ["--use-angle=swiftshader", "--enable-unsafe-swiftshader", "--enable-webgl", "--ignore-gpu-blocklist", "--no-sandbox", "--disable-setuid-sandbox"] });
    ...
    mark("settle");
    const settleInfo = await page.evaluate(async (opts) => {
        let stubbed = false;
    ...
    const realRender = async () => page.evaluate(async () => {
        ...
        } catch (e) { return { err: String(e.message || e), ms: Math.round(performance.now() - t0) }; }
    ...
    const tinfo = await page.evaluate(() => {
        ...
        let spawned = false, parts = 0, anchors = 0, foliageVerts = 0, key = "";
        try {
            ...
            if (bp) {
                const px = pm.position.x + 13, pz = pm.position.z + 2;
                ...
                if (fg && fg.leaves) for (const lf of fg.leaves) { const g = lf.geom; if (g && g.attributes && g.attributes.position) foliageVerts += g.attributes.position.count; }
                ...
                if (Number.isFinite(cam.fov)) { cam.fov = 30; cam.updateProjectionMatrix(); }
    ...
    progress.tree = tinfo;
    flush();
    console.log(`  tree key=${tinfo.key} spawned=${tinfo.spawned} parts=${tinfo.parts} anchors=${tinfo.anchors} foliageVerts=${tinfo.foliageVerts} ${tinfo.err || ""}`);
    await shoot("baum");

    if (process.env.ANBLICK_ONLY_BAUM === "1") {
        progress.phase = "done-baum-only"; flush();
    ...
    for (const [yaw, tag] of [[35, "ground-a"], [200, "ground-b"]]) {
        ...
        await page.evaluate((yaw) => {
            const r = window.anazhRealm, s = r.state, pm = s.playerMesh, cam = s.camera;
            const y = (yaw * Math.PI) / 180, ex = pm.position.x, ey = pm.position.y + 1.7, ez = pm.position.z;
    ...
    const counts = await page.evaluate(() => {
        const r = window.anazhRealm, s = r.state;
        let instances = 0, tris = 0, draws = 0, meshes = 0, instMeshes = 0;
        s.scene.traverse((o) => {
            if (o.isInstancedMesh) {
                const c = o.count || 0; instances += c; draws++; instMeshes++;
                const g = o.geometry; const t = g && g.index ? g.index.count / 3 : g && g.attributes.position ? g.attributes.position.count / 3 : 0;
                tris += t * c;
            } else if (o.isMesh) {
                meshes++; draws++;
                const g = o.geometry; const t = g && g.index ? g.index.count / 3 : g && g.attributes && g.attributes.position ? g.attributes.position.count / 3 : 0;
                tris += t;
            }
        });
        return { instances, tris: Math.round(tris), draws, meshes, instMeshes, chunks: s.voxelChunks ? s.voxelChunks.size : 0 };
    });
