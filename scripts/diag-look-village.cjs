    ...
    const placed = await page.evaluate(() => {
        ...
        for (let d = 20; d <= 60 && !spot; d += 4) {
            for (let off = -20; off <= 20; off += 5) {
                const x = pm.position.x + off,
                    z = pm.position.z + d;
                const g = th(x, z);
                if (g > wl(x, z) + 1 && Math.abs(th(x + 3, z) - g) < 2 && Math.abs(th(x, z + 3) - g) < 2) {
                    spot = { x, z, g };
                    break;
                }
            }
        ...
        if (!spot) {
            const g = th(pm.position.x, pm.position.z + 26);
            spot = { x: pm.position.x, z: pm.position.z + 26, g };
        }
        // AUSLÖSCHUNGS-WELLE: der village-Bauplan ist gefallen (das Dorf ist Studio/
        // spawnSettlement, braucht warme Foundry) — die LOOK-Szene baut ein Mini-Dorf
        // aus DREI Haeusern der eingefrorenen Substanz (KIND_SUBSTANCE.haus_basis).
        const KS = r.constructor.KIND_SUBSTANCE || {};
        if (!r.state.blueprints._look_haus && KS.haus_basis) {
            r.state.blueprints._look_haus = {
                name: "_look_haus",
                parts: JSON.parse(JSON.stringify(KS.haus_basis.parts)),
            };
        }
        let entry = null;
        try {
            for (const [dx, dz] of [
                ...
                const e = r.spawnArchitecture(
                    "_look_haus",
                    { x: spot.x + dx, y: th(spot.x + dx, spot.z + dz) + 0.5, z: spot.z + dz },
                    { seed: 7 }
                );
                entry = entry || e;
            }
        } catch (e) {
            return { err: "spawn: " + e.message };
        }
        // Spieler ~14m vor das Dorf stellen, auf Augenhöhe
        const px = spot.x,
            pz = spot.z - 14,
            py = th(spot.x, spot.z - 14);
