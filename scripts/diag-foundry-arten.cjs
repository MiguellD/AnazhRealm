...
(async () => {
    ...
    const out = await page.evaluate(async () => {
        ...
        const r = window.anazhRealm; const o = {};
        ...
        o.hasRecipes = !!(f && f.recipes); o.recipeCount = f && f.recipes ? Object.keys(f.recipes).length : 0;
        ...
        if (f && f.recipes) {
            f.recipes.findling = { kind: "tree" }; o.findlingReclassTree = r._foundryPresetIsTree("findling");
            f.recipes.eiche = { kind: "rock" }; o.eicheReclassRock = r._foundryPresetIsTree("eiche");
        }
        return o;
    });
    ...
    const ok = out && out.hasRecipes && out.recipeCount >= 15 && out.eicheKind === "tree" &&
        out.eicheIsTree === true && out.findlingIsTree === false && out.strauchIsTree === true &&
        out.findlingReclassTree === true && out.eicheReclassRock === false;
