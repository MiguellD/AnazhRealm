    ...
    const info = await page.evaluate(async () => {
        ...
        for (let i = 0; i < arts.length; i++) {
            ...
            const px = Math.cos(ang) * dist,
                pz = Math.sin(ang) * dist - 10; // vor dem Blick (−z)
        ...
        for (let it = 0; it < 500; it++) {
            ...
            const done = archs.filter(
                (e) => e && r._foundryPresetForEntry && r._foundryPresetForEntry(e) && e.instFoundry
            ).length;
            if (done >= 6) break;
        }
        const archs = r.state.architectures || [];
        o.treesTotal = archs.filter((e) => e && r._foundryPresetForEntry && r._foundryPresetForEntry(e)).length;
        o.treesFoundry = archs.filter(
            (e) => e && r._foundryPresetForEntry && r._foundryPresetForEntry(e) && e.instFoundry
        ).length;
        o.treesClassic = archs.filter(
            (e) =>
                e &&
                r._foundryPresetForEntry &&
                r._foundryPresetForEntry(e) &&
                !e.instFoundry &&
                (e.instanced || e.mesh)
        ).length;
        // Kamera waagrecht in den Baum-Ring blicken (−z).
        if (r.state.camera) {
            r.state.camera.position.set(0, pm ? pm.y : 20, 0);
            r.state.camera.lookAt(0, (pm ? pm.y : 20) - 0.2, -12);
        }
        return o;
    });
