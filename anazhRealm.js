...
addWallCollisions(heightData, width, depth, scaleX, scaleZ, chunkX, chunkZ) {
    try {
        if (!this.state.physicsWorld) {
            throw new Error("Physik-Welt nicht initialisiert – initPhysics() muss zuerst aufgerufen werden");
        }
        ...
        if (!this.state.playerMesh) {
            this.log("addWallCollisions: Spieler nicht initialisiert – überspringe Kollisionsboxen", "WARNING");
            return;
        }
        ...
        const endZ = Math.min(startZ + chunkSize + 1, depth);

        // Chunk-Mittelpunkt berechnen
        const chunkCenterX = ((startX + endX - 1) / 2 / (width - 1)) * 300 - 150;
        const chunkCenterZ = ((startZ + endZ - 1) / 2 / (depth - 1)) * 300 - 150;

        // Distanzen zu Spieler und Kreaturen berechnen
        ...
        const loadRadius = 50; // Kollisionsboxen nur innerhalb dieses Radius laden
        if (distToPlayer > loadRadius && minDistToCreature > loadRadius) {
            this.log(`Chunk (${chunkX}, ${chunkZ}) außerhalb des Lade-Radius (${loadRadius}): Distanz zu Spieler=${distToPlayer.toFixed(2)}, zu Kreaturen=${minDistToCreature.toFixed(2)} – überspringe Kollisionsboxen`, "DEBUG");
            return;
        }
        ...
        for (let z = startZ; z < endZ - 1; z++) {
            for (let x = startX; x < endX - 1; x++) {
                const idx = z * width + x;
                const idxRight = z * width + (x + 1);
                const idxDown = (z + 1) * width + x;

                const h = heightData[idx];
                const hRight = x < width - 1 ? heightData[idxRight] : h;
                const hDown = z < depth - 1 ? heightData[idxDown] : h;

                const steepnessX = Math.abs(h - hRight);
                const steepnessZ = Math.abs(h - hDown);
                totalSteepness += Math.max(steepnessX, steepnessZ);
                steepnessCount++;
            }
        }

        const avgSteepness = totalSteepness / steepnessCount;
        const steepnessFactor = Math.min(2.0, avgSteepness / 3.0); // Skaliere die Anzahl der Boxen basierend auf Steilheit
        const baseMaxBoxes = 500;
        const maxCollisionBoxes = Math.min(1000, Math.floor(baseMaxBoxes * steepnessFactor / (avgSteepness + 1)));
        let collisionBoxCount = 0;

        const steepnessThreshold = 3.0; // V7.42-Wert
        const wallHeightFactor = 2.0;
        const wallThickness = 2.0; // V7.42-Wert

        // Kollisionsboxen hinzufügen
        for (let z = startZ; z < endZ - 1; z++) {
            for (let x = startX; x < endX - 1; x++) {
                if (collisionBoxCount >= maxCollisionBoxes) break;
                ...
                const xPos = (x / (width - 1)) * 300 - 150;
                const zPos = (z / (depth - 1)) * 300 - 150;

                // Prüfe Distanz zu Spieler und Kreaturen für diese Position
                const posDistToPlayer = Math.sqrt(
                    Math.pow(playerPos.x - xPos, 2) + Math.pow(playerPos.z - zPos, 2)
                );
                let posMinDistToCreature = Infinity;
                this.state.creatures.forEach(creature => {
                    const dist = Math.sqrt(
                        Math.pow(creature.position.x - xPos, 2) + Math.pow(creature.position.z - zPos, 2)
                    );
                    posMinDistToCreature = Math.min(posMinDistToCreature, dist);
                });

                if (posDistToPlayer > loadRadius && posMinDistToCreature > loadRadius) {
                    continue; // Überspringe diese Position, wenn sie außerhalb des Radius liegt
                }

                // Kollisionsbox in X-Richtung
                if (Math.abs(h - hRight) > steepnessThreshold) {
                    const avgHeight = (h + hRight) / 2;
                    const wallHeight = Math.abs(h - hRight) * wallHeightFactor;
                    const wallGeometry = new THREE.BoxGeometry(wallThickness, wallHeight, scaleZ);
                    const wallMaterial = new THREE.MeshBasicMaterial({ color: 0xff00ff, transparent: true, opacity: 0.5 });
                    const wallMesh = new THREE.Mesh(wallGeometry, wallMaterial);
                    wallMesh.position.set(xPos + scaleX / 2, avgHeight, zPos);
                    this.state.scene.add(wallMesh);
                    this.state.wallBoxes.push(wallMesh);

                    const wallShape = new Ammo.btBoxShape(new Ammo.btVector3(wallThickness / 2, wallHeight / 2, scaleZ / 2));
                    this.addRigidBody(wallMesh, 0, wallShape);
                    collisionBoxCount++;
                }

                // Kollisionsbox in Z-Richtung
                if (Math.abs(h - hDown) > steepnessThreshold) {
                    const avgHeight = (h + hDown) / 2;
                    const wallHeight = Math.abs(h - hDown) * wallHeightFactor;
                    const wallGeometry = new THREE.BoxGeometry(scaleX, wallHeight, wallThickness);
                    const wallMaterial = new THREE.MeshBasicMaterial({ color: 0xff00ff, transparent: true, opacity: 0.5 });
                    const wallMesh = new THREE.Mesh(wallGeometry, wallMaterial);
                    wallMesh.position.set(xPos, avgHeight, zPos + scaleZ / 2);
                    this.state.scene.add(wallMesh);
                    this.state.wallBoxes.push(wallMesh);

                    const wallShape = new Ammo.btBoxShape(new Ammo.btVector3(scaleX / 2, wallHeight / 2, wallThickness / 2));
                    this.addRigidBody(wallMesh, 0, wallShape);
                    collisionBoxCount++;
                }
            }
        ...
        this.log(`Kollisionsboxen für steile Wände in Chunk (${chunkX}, ${chunkZ}) hinzugefügt: ${collisionBoxCount} Boxen (Max: ${maxCollisionBoxes}, Steepness Threshold: ${steepnessThreshold.toFixed(2)}, Steepness Factor: ${steepnessFactor.toFixed(2)})`);
    } catch (error) {
        this.logError(error);
        throw error;
    }
...
isPlayerGrounded() {
    try {
        if (!this.state.playerMesh || !this.state.playerBody || !this.state.physicsWorld) {
            this.log("isPlayerGrounded: Spieler oder Physik-Welt nicht initialisiert", "ERROR");
            return false;
        }

        const playerPos = this.state.playerMesh.position;
        const rayStart = new Ammo.btVector3(
            playerPos.x / this.state.scaleFactor,
            playerPos.y / this.state.scaleFactor,
            playerPos.z / this.state.scaleFactor
        );
        const rayEnd = new Ammo.btVector3(
            playerPos.x / this.state.scaleFactor,
            (playerPos.y - 1.0) / this.state.scaleFactor, // Prüfe 1 Einheit unter dem Spieler
            playerPos.z / this.state.scaleFactor
        );
        ...
        const isGrounded = rayCallback.hasHit();
        if (isGrounded) {
            const hitPoint = rayCallback.get_m_hitPointWorld();
            const distance = Math.abs(hitPoint.y() * this.state.scaleFactor - playerPos.y);
            if (distance > 0.5) {
                Ammo.destroy(rayCallback);
                return false; // Zu weit vom Boden entfernt
            }
        }
...
getTerrainHeightAt(x, z) {
    try {
        if (!this.state.groundHeightField) {
            this.log("getTerrainHeightAt: groundHeightField nicht initialisiert", "ERROR");
            return 0;
        }

        const zIndex = Math.floor((z + 150) / 300 * 255);
        const xIndex = Math.floor((x + 150) / 300 * 255);
        const idx = Math.min(Math.max(zIndex, 0), 255) * 256 + Math.min(Math.max(xIndex, 0), 255);
        const height = this.state.groundHeightField[idx] || 0;

        if (isNaN(height) || !isFinite(height)) {
            this.log(`Ungültige Terrain-Höhe bei (${x}, ${z}): ${height}. Setze auf 0.`, "ERROR");
            return 0;
        }
        return height;
    } catch (error) {
...
calculateTerrainSteepness(x, z) {
    try {
        if (!this.state.groundHeightField) {
            this.log("calculateTerrainSteepness: groundHeightField nicht initialisiert", "ERROR");
            return 0;
        }
        ...
        const idx = Math.min(Math.max(zIndex, 0), 255) * 256 + Math.min(Math.max(xIndex, 0), 255);
        const idxRight = Math.min(Math.max(zIndex, 0), 255) * 256 + Math.min(Math.max(xIndex + 1, 0), 255);
        const idxDown = Math.min(Math.max(zIndex + 1, 0), 255) * 256 + Math.min(Math.max(xIndex, 0), 255);

        const h = this.state.groundHeightField[idx] || 0;
        const hRight = this.state.groundHeightField[idxRight] || h;
        const hDown = this.state.groundHeightField[idxDown] || h;

        const steepnessX = Math.abs(h - hRight);
        const steepnessZ = Math.abs(h - hDown);
        const steepness = Math.max(steepnessX, steepnessZ);

        if (isNaN(steepness) || !isFinite(steepness)) {
            this.log(`Ungültige Steilheit bei (${x}, ${z}): ${steepness}. Setze auf 0.`, "ERROR");
            return 0;
        }
        return steepness;
    } catch (error) {
