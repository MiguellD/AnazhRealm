...
**Stand**: V9.52-e (23.05.2026) — **Playtest-Pflege-Bogen Sub-Welle e: Band 4 ist drin, die `### Invarianten`-Sektion ist VOLLSTÄNDIG strukturiert**. `scripts/playtest.cjs` war eine 31 574-Zeilen-async-IIFE (~3000 Invarianten in einem Topf — die zweitgrößte Reibung des Projekts). Sub-Wellen a-e zerlegen alle 193 Original-Sektionen in 41 benannte Band-Funktionen; der `else { ... }`-Block des Orchestrators ist jetzt eine **schlanke Liste von 40 `await checkBandX(ctx)`-Calls** (3 a + 11 b + 8 c + 8 d + 10 e), kein Inline-Code mehr. Plan-treu zu `docs/playtest-hygiene.md` §5 (Ziel ~25-35; 41 ist leicht über, aber soft — Akzeptanz ist Navigierbarkeit + Kohärenz). Verhaltens-identisch bewiesen über 10 Baseline-Läufe (3060-Invarianten-Satz bit-identisch, nur Drift-Bänder). Datei: 31 574 → 30 137 Z. (−1437 netto). Sub-Welle f bleibt: Helfer-Durchzug (`safeEvaluate`-Roll-out innerhalb der Band-Funktionen) + ehrliche Rest-Grenze.
