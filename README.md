...
**Stand**: V9.56-h (23.05.2026) — **Kontrollfluss-Etappe Sub-Welle h**. `processChatCommand` 285 → 21 Zeilen Orchestrator (5-Stufen-Pipeline Setup → Voxel-Debug → Portal → DSL-Parse → Legacy-Dispatch) + neun Themen-Helfer entlang der natürlichen Kommando-Kategorien: `_chatRecordInputForLearning` 12 Z. (Schicht 1), `_chatTryVoxelDebugCommand` 32 Z., `_chatTryPortalRoute` 12 Z., `_chatTryDslParse` 16 Z., `_chatDispatchLegacyCommand` 7 Z. (Themen-Dispatcher), `_chatTryAbilityCommand` 23 Z., `_chatTryPersistenceCommand` 26 Z., `_chatTryWorldCommand` 81 Z. (Ring 8 Welt-Verwaltung, der größte), `_chatTrySystemCommand` 61 Z., `_chatHandleConversationalFallback` 32 Z. (Kreatur → LLM → P2P → Hilfe). Median ~26 Z., max 81. Try-Pattern-Konvention (`_chatTryXxx` returns true wenn handled). Cleanup-Asymmetrie bit-identisch preserviert (voxel-test räumt nicht, Portal+DSL räumen selbst, Legacy fällt durch). Verhaltensneutral, Playtest grün vor + nach. Datei +76 Z.
