...
**Stand**: V13.4 (29.05.2026) — **V13-Wasser-Profi-Bogen: das Wasser wie die Riesen, effizient + korrekt.** Schöpfer-Browser-Befund: Wasser klebte als Schatten an Hängen, Streaming-Hitch, würfeliges Mesh. Fünf Wellen, jede im Code verifiziert statt spekuliert: **V13.0** Diagnose (`scripts/diag-water-perf.cjs`, Vorher-Zahl) · **V13.1** Klassifikation `_atlasWaterLevelAt` + Depth-Gate (der Hang-Schatten 51,7→0,1 %, war der 16-m-See-Dilatations-Bleed) · **V13.1.1** Worker-Cache-Buster (der Fix erreichte den Browser erst dann — `new Worker(...?v=VERSION)`) · **V13.2** Grenzflächen-Meshing statt Surface-Nets (~150→~3 ms/Chunk, ~18× weniger Dreiecke) · **V13.3** Flow-Bias geheilt (Schöpfer-Hypothese gemessen: D8 wählte steilsten DROP statt GEFÄLLE → Diagonal-Grain; `drop/dist` → isotrop, determinismus-brechend) · **V13.4** Wasser-Glättung (`_weldAndSmoothWater`). **Reflexion (V13.4):** der Schöpfer fing einen „Loch stopfen, anderes entsteht"-Thrash — ich reparierte die Geometrie für ein Erscheinungs-Problem. Die saubere Architektur: Wasser = **(1) Wahrheit/Zellen** (reaktiv = lebendige Substanz) + **(2) dumme Geometrie** + **(3) Tiefenpuffer-Shader** (weiche Uferlinie/„Oberflächenspannung" via `viewportLinearDepth` + Emotions-Kopplung). **Nächste Welle: V13.5 — Schicht-3-Tiefen-Ufer-Shader** (+ V13.4-Glättung zurück). Render-Korrektheit (V13) ist Fundament für das lebendige Ultiversum (V14: Emotion/Kreatur/Cluster-Kopplung), nicht Selbstzweck. Davor der V12-Genie-Bogen (Three.js r184 + WebGPU-required, 119 FPS steady) + V10.0-Bogen (NodeMaterials/TSL). Volle Chronik in `docs/handover.md`.

**Davor — V10.0-j (27.05.2026, 10 Sub-Wellen .a..j)** — Shadow-Profi-Pattern + WebGPU-Buffer-Lifecycle-Race-Workaround. Drei strukturelle Schatten-Wurzeln aus Vendor-Code geheilt (`AnalyticLightNode.setupShadow/updateShadow`); danach 7 Sub-Wellen Buffer-Lifecycle-Heilung (setTimeout(0) → 2-Frame-Counter → onSubmittedWorkDone → Set-Dedup → Material-Skip → DynamicDrawUsage-Removal → Mesh-Memory-Akzeptanz). Vier permanente Lehren in `CLAUDE.md/Gotchas`: Override-Material+castShadow-Filter, Workaround-Audit nach struktureller Heilung, Mesh-Memory > Race-Crashes, fremder-Code-Augen-Disziplin nach 3+ Wellen.

**Davor — V10.0-g (27.05.2026)** — MeshToonMaterial → MeshBasicNodeMaterial + manuelles Half-Lambert-Toon-Lighting im colorNode. Cel-Stufen via gradientMap-Texture-Lookup, Sun-Direction-Sync via `state.toonLightUniforms`.
