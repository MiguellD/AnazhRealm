...
**Stand**: V9.43-c (21.05.2026) — **Flüsse + Seen + Wasserfälle werden sichtbar**. `_buildHydrosphereMeshes()` rendert das V9.43-b-Drainage-Netz: See-Planes (ein Quad je See-Zelle auf der Füll-Höhe), Fluss-Ribbon-Meshes (Quad-Streifen, Breite ∝ √Akkumulation, der Flow steckt im per-Vertex-`aFlow`-Attribut) + Wasserfall-Planes, die gegen die echte Voxel-Surface re-verankert sind (ein Fluss kreuzt eine Klippe — der per-Chunk-Zufalls-Wasserfall-Spawner aus V9.32/V9.43-a ist gelöscht). See + Fluss teilen EIN horizontales Wasser-`ShaderMaterial`; Meer, See, Fluss, Wasserfall sind eine Wasser-Sprache (Vision §1.3 fraktal). Ehrlicher Browser-Befund: das Wasser rendert sichtbar (große, klare Seen), aber das Netz dieser Welt ist see-dominant — 12 Seen (~20 % der Region), die Flüsse sind kurze 32-48-m-Verbinder; lange, frei fließende Flüsse sind eine Netz-Qualitäts-Frage für eine Folge-Welle. Offen: V9.43-d (Flüsse carven echte Betten ins Terrain), V9.43-e (Klang). **Davor — V9.43-b** lieferte den Hydrosphären-Atlas (das Drainage-Netz als reine Daten: Surface-Sampling → Priority-Flood → D8-Flow-Direction → Flow-Accumulation → Netz-Extraktion); **der Stamm-Pflege-Bogen V9.44-a..f** heilte strukturelle Reibung als reines verhaltensneutrales Refactoring. Pläne: `docs/hydrosphere.md`, `docs/code-hygiene.md`. Nächste Welle: V9.43-d — die Flüsse carven echte Betten.

**Davor — Voxel-Surface-Politur V9.40–V9.42** (fünf Schöpfer-Browser-Befunde nach V9.39): V9.40 a–f heilte die Chunk-Löcher beim Carve/Fill (Async-Rebuild + Pre-Build-Pattern + die Wurzel: vendored `ammo.wasm` auf 256 MB growable gepatcht) + den fehlenden P2P-Sync der Maus-Voxel-Edits. V9.41 + V9.41-b heilte die Trapeze/Linien auf flachen Hügeln (Schach-Brett-Diagonalen + Laplacian-Smooth gegen die Surface-Nets-Treppen). V9.42 a–d die Insel-Inkonsistenz (Inseln teilen die Surface-Nets-Pipeline + MeshToon-Material + Naht-pad-Smooth). V9.43-b ✅ liefert den Hydrosphären-Atlas (das Drainage-Netz als Daten), V9.43-c ✅ rendert es (Flüsse/Seen/Wasserfälle sichtbar); offen: V9.43-d/e — echte Fluss-Betten ins Terrain carven, Klang. Ausführliche Planung in `docs/hydrosphere.md`. Die volle Versions-Historie lebt in `CLAUDE.md` (kanonisch); `docs/roadmap.md` §3 trägt den Wellen-Plan, `docs/handover.md` die Erstorientierung.
...
npm run playtest        # ~2995 Headless-Invarianten (~60s)
