...
**Nächste Welle: V13.9 — Wasser-Finish im Browser-Loop** (dünnes Wand-Bluten pro Pixel im Tiefenpuffer-Shader cullen + tiefes Wasser an Strukturen + Carve-Naht, mit dem Schöpfer im Browser justiert). Danach V13.10 Sub-Region-Edit, V13.11 LOD, V13.12 Audit; dann V14 — die lebendige Welt (System-Kopplungen E/F/G). Reflexions-Leitsätze: Wasser ist drei Schichten (Zellen + Geometrie + Shader); das Ufer ist ein 16-m-vs-Voxel-Tradeoff → Glätte im Shader, keine 3D-Fluid-Sim; Wasser-Render nur im Browser-Loop (Headless ist pixel-blind).
