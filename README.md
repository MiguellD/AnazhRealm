...
**Stand**: V9.52-d (23.05.2026) — **Playtest-Pflege-Bogen Sub-Welle d: Band 3 ist drin, drei Viertel der Test-Datei sind strukturiert**. `scripts/playtest.cjs` war eine 31 574-Zeilen-async-IIFE (~3000 Invarianten in einem Topf — die zweitgrößte Reibung des Projekts). Sub-Wellen a-d zerlegen die ersten ~22 256 Zeilen der `### Invarianten`-Sektion in 31 benannte Funktionen: a (Gerüst + 4 Funktionen) + b (11 Band-Funktionen Ring 2 Phase 3 .. Ring 11.5) + c (8 Band-Funktionen Welle 6.A .. 6.H Phase 2C) + d (8 Band-Funktionen Welle 6.X Audit + 6.G3/G4 Atmosphäre + V8.24-V8.49 + W12-W14 Welt-Portal-Bogen + KI-Übersetzer + V8.70-72 Untrusted-Tor/Vendor + späte 6.X.4-Fixes, je 498-1190 Z.). Plan-treu zu `docs/playtest-hygiene.md` §5 (Band-Funktionen, nicht per-Sektion; Ziel ~25-35 total — die 31 Funktionen sind im Soll). Verhaltens-identisch bewiesen über 8 Baseline-Läufe (3060-Invarianten-Satz bit-identisch, nur Drift-Bänder). Datei: 31 574 → 30 415 Z. (−1159 netto). Sub-Welle e zieht die End-Sektionen nach (Welle 6.H 2B.2 .. Datei-Ende), Sub-Welle f der Helfer-Durchzug + Politur.
