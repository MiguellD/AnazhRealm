...
**Stand**: V9.56-i (23.05.2026) — **Kontrollfluss-Etappe Sub-Welle i**. `_applyDayNightToScene` 305 → 16 Zeilen Orchestrator + neun Helfer entlang der natürlichen Phasen-Hierarchie: `_dayNightComputeTint` 93 Z. (gekoppelte 3-Schicht-Akkumulation Wetter/Welt-Feld/Emotion) + acht unabhängige Fanout-Apply-Helfer (Skybox 7, StarField 8, DirLight 14, Ambient 7, HemiFog 45, TerrainShader 19, WaterMaterials 18, SunDirection 7). Median 14 Z. Tint-Objekt schliesst Derivate (skyR/G/B) ein — eine Berechnung, zwei Konsumenten. **Duplikat-Konsolidierung** (Hydro-Material-Doppel-Apply, V8.29.1+V9.43-c, identische idempotente Writes) → `_dayNightApplyWaterMaterials` ruft die applyTo-Closure einmal pro Material. **Datei −30 Z. netto-negativ!** Strukturelle Test-Invarianten an die neuen Helfer angepasst (V8.32/V8.46/V8.48-Patterns wandern zu `_dayNightApplyHemiAndFog`/`_dayNightApplyTerrainShaderUniforms`/`_dayNightApplySkybox`). Verhaltensneutral, Playtest grün vor + nach.
