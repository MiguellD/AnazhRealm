...
**Stand**: V9.52-b (23.05.2026) — **Playtest-Pflege-Bogen Sub-Welle b: Band 1 ist drin**. `scripts/playtest.cjs` war eine 31 574-Zeilen-async-IIFE (~3000 Invarianten in einem Topf — die zweitgrößte Reibung des Projekts). Sub-Welle a (das Gerüst, `safeEvaluate`/`ctx` + 4 erste Funktionen) + Sub-Welle b (Band 1: 11 thematische Band-Funktionen für Ring 2 Phase 3 .. Ring 11.5, je 265-1260 Z.) zerlegen die ersten 8010 Zeilen der `### Invarianten`-Sektion in 15 benannte Funktionen. Plan-treu zu `docs/playtest-hygiene.md` §5: **Band-Funktionen, nicht per-Sektion** (das wäre Über-Zerlegung; Ziel ~25-35 Funktionen total). Verhaltens-identisch bewiesen über 6 Baseline-Läufe (3060-Invarianten-Satz bit-identisch, gleiche Reihenfolge, gleiche Pass/Fail — nur die pre-existing Drift-Bänder bleiben). Sub-Wellen c-f ziehen den Rest nach.
