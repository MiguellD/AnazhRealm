...
**Stand**: V14.6 (30.05.2026) — **Terrain-Bogen „Die wahre Tiefe des Geländes" (epische Geomorphologie, V14.0–.6, Browser-Audit-getrieben)**: das Makro-Relief von uniform-spitz-alpin zu abwechslungsreich-wie-die-Erde — kontinentale Basis-Oktave (V14.1, lange Flüsse) + thermische Erosion (V14.2, Talus glättet) + regionale Terrain-Typ-Differenzierung (V14.3, Vielfalt) + echte Ebenen (V14.4, `mtn` moduliert alle Oktaven) + Harmonie-Heilung (V14.5, Wurm-Höhlen-Decke an die flache Welt nachgezogen) + Fern-Heilung (V14.6, Voxel-Decke dimY 124→136 + tanh-Soft-Clamp + stale GPU-Mirror abgeklemmt — die Welt zerfiel „in eine Richtung", weil cont0 fern bis 235 m über die 133-m-Decke wuchs). **Über den Bogen (gemessen): Anteil steil (>45°) 26 %→4,4 %, Median-Hangneigung 35°→18,7° (sanfte Welt), sanft-Anteil 6,7 %→24,1 % (echte Ebenen), Plateau-Anteil 2,7 %→11,2 % (Hochebenen), 9 lange Flüsse, FPS 119.** Davor: V13-Wasser-Profi-Bogen (pausiert @ V13.14, Finish nach dem Terrain) + V12-Genie-Bogen (Three.js r184 + WebGPU-required, 119 FPS steady) + V10.0-NodeMaterial/TSL-Bogen.

**Nächste Welle: V14.7 — Maßstab-Streckung** (die `ranges`-Wellenlängen strecken λ77→~150–250 m → Feature-Größe 176→>400 m + längere Flüsse; `terrainSteepness` verdrahten/beerdigen). Danach V14.8 (gerichtete Uplift-Maske: Anden-Ketten über weite Distanzen), V14.9 (Browser-Audit-Schluss); dann V15 (Wasser-Finish: die drei Render-Sync-Wurzeln gegen die neue Topologie), V16 (lebendige Welt: System-Kopplungen E/F/G). Reflexions-Leitsätze: episches Terrain ist Worldgen-Mathematik — die dominante Skala muss die größte sein; **wer die Terrain-HÖHE ändert, muss ALLE höhen-co-getuneten Annahmen nachziehen** (surf-relative Schichten: Höhlen-Decke/Wasser-Band/Tarn — V14.5; die Voxel-DECKE + jeden parallelen Density-Mirror Worker+GPU — V14.6); Decken-Diagnose radial bis > λ_cont0 (7100 m); Render/visuelle Qualität nur im Browser-Loop justieren (Headless ist pixel-blind).
