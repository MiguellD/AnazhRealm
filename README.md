...
**Stand**: V9.86 (25.05.2026) — **Welle Perf-2 ext.a: drei Heilungen nach V9.85-Browser-Audit, pre-existierender C.3-Flake nebenbei geheilt**. (1) `_remeshVoxelChunksAround` bekommt `skirt`-Parameter — Architektur-Spawn ruft mit `skirt=0` → ~89 % weniger dirty-Chunks → FPS-Drops beim Bauen drastisch reduziert (vorher 119→36 bei kristall_geode/felsturm-Kaskade). (2) Mountain-Mulden-Filter via Atmosphären-Konnektivität (Top-Down-Pass mit `airAbove`-Flag pro xz-Spalte) ersetzt den V9.85-„SOLID-darüber"-Check — semantisch korrekt („kann diese WATER-Cell die Atmosphäre erreichen?"), bewahrt V9.74-Carve-Vision, heilt V9.84-Bergwand-Audit, heilt pre-existierenden C.3-Carve-Flake (vorher ~50 % Failure-Rate, jetzt 4/4 grün). (3) Klassen-Konstante `AnazhRealm.VERSION` — log() + Init-Message lesen sie, kein hardcoded „V7.82" mehr.
