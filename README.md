...
**Stand**: V9.56-e (23.05.2026) — **die Kontrollfluss-Etappe des Hygiene-Bogens 2 ist ERÖFFNET**. `fuseWorlds` 239 → 114 Zeilen Orchestrator + acht Helfer entlang der natürlichen Sub-Compute-Blöcke: `_fusionGenerateIdentity` 17 Z. (UUID/Slugs/Seed/bornAt), `_fusionMergeInventory` 22 Z. (blueprints/materials/tools/playerTools + Querverweis-Rewire), `_fusionMergeEmotions` 14 Z. (strategie-abhängig), `_fusionMergeDslHistory` 21 Z. (random-mix Münzwurf), `_fusionMergePatternMemory` 14 Z. (per-Keyword Cap 8), `_fusionMergeAbilities` 15 Z. (Union per Name), `_fusionMergePathBuckets` 15 Z. (Mittelung pro Achse), `_fusionBuildJournalEntries` 19 Z. (Genesis + 4-pro-Eltern). Median ~16 Z. Das große Snap-Object (30+ Felder) bleibt INLINE im Orchestrator — V9.44-Heilige-Lektion-Schutz vor „noch einer Schicht für Symmetrie". Math.random-Reihenfolge bit-identisch (Identity vor DSL-History). Verhaltensneutral, Playtest grün vor + nach. Datei +37 Z.
