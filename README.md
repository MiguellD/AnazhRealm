...
**Stand**: V13.7 (30.05.2026) — **V13-Wasser-Profi-Bogen**: das Voxel-Wasser ist effizient, korrekt, in Synergie mit dem Terrain UND verbunden — es teilt den Terrain-Mesher (Surface-Nets-Iso → fließt wie die Landschaft) und füllt die Mulde bis zum Ufer, bleibt aber zell-basiert + reaktiv. V13.1 Klassifikation · V13.3 Flow-Bias · V13.5 Schicht-3-Tiefen-Ufer-Shader · V13.6 Synergie zurück (gemeinsame Surface-Nets-Iso, band-limitiert) · V13.7 verbundenes Wasser (`terrainTopY < rim` → die vertikalen Ufer-Wände weg, tiefe Wände 33 %→0 %; Carve nahe Quelle füllt sich reaktiv; kein Hang-Schatten/Phantom). Davor: V12-Genie-Bogen (Three.js r184 + WebGPU-required, 119 FPS steady) + V10.0-NodeMaterial/TSL-Bogen.

**Nächste Welle: V13.8 — Sub-Region-Edit-Remesh** (nur die berührte Zell-Sektion neu statt des ganzen Chunks). Danach V13.9 LOD, V13.10 Audit; dann V14 — die lebendige Welt (System-Kopplungen E/F/G). Reflexion-Leitsatz: Wasser ist drei Schichten — Wahrheit/Zellen (verbunden) + Geometrie (Iso, Synergie) + Shader-Erscheinung; eine Perf-Optimierung darf keine Vision-Qualität wegoptimieren.
