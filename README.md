...
**Stand**: V17.27 (01.06.2026) — **der HEILUNG-Bogen DURCH + der ERSTE FELD-Bogen RUND, mit beiden Hälften (lesen UND schreiben): V17.19 Werkstatt-Auge + V17.20 tote Glieder (~−1430 Z.) + V17.21 `auraAt(x,z,t)` (die Lese-Seite) + der Kreis Welt→Spieler-Emotion + V17.22 der Nexus bekommt Augen (komponiert resonant) + V17.23 HARMONIE statt Revert (Wetter/Himmelsfarbe/Spawn verschmelzen parallel, ohne zu überschreiben, und faden) + V17.24 Sky-Farb-Cross-Fade (nichts snappt) + V17.25 viele Leser ans Feld (§5: Logik-Konsumenten lesen `auraAt` + volle Achsen — glut DÄMPFT) + V17.26 dem Nexus die FELD-RICHTUNG (er trägt Leben zum erkannten BEDARF) + V17.27 die SCHREIB-Seite (ein Leben-Overlay über dem frozen Kern — eine Geburt hebt lebendig, es zerfällt zurück → der at_field_need-Loop schließt sich ECHT: „die Welt heilt sich" ist jetzt eine geschlossene Regel, kein Vektor). Damit ist der erste Feld-Bogen rund: lesen · Kreis Welt→Spieler · Nexus-Augen · Harmonie/Fade · viele Leser · gezielte Heilung · der geschlossene Loop. Davor: der Render-/Tiefe-Bogen + die Architektur-Aufwertung sind durch; die Welt-OBERFLÄCHE lebt (Ghibli-malerisch).** V17.0–.13 Render-Realismus (Post-FX-Bloom/Grading · fraktale Klein-Vegetation · gemalter Wolkenhimmel · Entgrauen · Böen+Pollen · `mx_noise`-Wolken · Ghibli-Bäume · triplanar Terrain-Makrotextur · Unsharp-Strukturkontrast); V17.14–.15 Halm-Variation + Dorf/Tempel-Binnenstruktur; **V17.16.2 REVERT** (eine Architektur-Aufwertung killte das Baum-Spawning — die Wurzel war eine entfesselte Material-Resonanz, nicht ein Crash); **V17.17** disziplinierter Neuanlauf (Turm + Steintor tag-neutral angereichert) + Affinitäts-Tag-Wächter; **V17.18** der Turm wird die Antenne (felsturm stein→eisen → `broadcasting`-Relais). Davor: V14 Terrain-Bogen (epische Geomorphologie, Feature-Größe 176→464 m, Anden-Ketten, DREI Region-Typen) · V15 Render-Bogen · V16 Lebende-Umgebung/Gras-Riese · V13 Wasser-Profi-Bogen (pausiert @ V13.14) · V12 Genie-Bogen (r184 + WebGPU, 119 FPS).
