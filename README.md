...
**Stand**: V10.0-j.j (27.05.2026) — **V10.0-Bogen WIRKLICH vollendet: Welt rendert dauerhaft auf WebGPU mit voller Visual-Tiefe**. 23 Sub-Wellen vom Vendor-Wechsel über WebGPU-Foundation, Material-Migration und Shadow-Profi-Pattern zum WebGPU-Buffer-Lifecycle-Workaround: **V10.0-a..d** Vendor + ESM-Loading + WebGPURenderer-Addon + Renderer-Auswahl · **V10.0-e..f-6** Smart Hot-Swap + 4 ShaderMaterials zu TSL + Canvas-Replacement + Animation-Loop-Restore · **V10.0-g** MeshToonMaterial → MeshBasicNodeMaterial mit Half-Lambert-Toon-Lighting · **V10.0-h..i** Shadow-Pass-Foundation + manuelle Shadow-Map-Sampling + Sterne als InstancedMesh-Billboards + Gras-Wind via TSL-positionNode (kein onBeforeCompile mehr) · **V10.0-j (10 Sub-Wellen .a..j)** Shadow-Profi-Pattern (Override-Material + castShadow-Filter + Pure-Depth-LessCompare + Hardware-PCF via `texture.compare(refZ)`) + WebGPU-Buffer-Lifecycle-Race-Workaround. **Welt rendert dauerhaft auf WebGPU**, alle Visual-Schichten aktiv (Cel-Stufen + Sun-Lighting + Schatten + Sterne-Billboards + Gras-Wind). **Memory-Trade-Ehrlichkeit**: ~500 KB GPU-Heap akzeptiert (Gras-Geometry nicht disposed) gegen einen Three.js-v160-Vendor-Race der nach 10 Sub-Wellen Heilungs-Versuchen nicht voll workaroundbar war ohne Vendor-Patch. **Profi-Pattern Genshin/BotW**: Mesh-Memory > Race-Crashes. **V11+-Backlog**: echtes Mesh-Pool-Pattern (Genshin's instancing-system, ~3-4h, eigene Welle). Vor V10.0-Bogen lag der Welle-Perf-3-Bogen (V9.87-V9.96); volle Chronik in `docs/handover.md`.
