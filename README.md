...
**Stand**: V9.83 (25.05.2026) — **CI-Playtest-Flake an der Wurzel geheilt + Performance-Welle eingelegt**. Der Welle-C-Bogen (V9.71-V9.82, Wasser-Substanz-Vereinigung) ist vollendet; V9.83 heilt den CI-Flake (rAF-Drosselung im Headless + Per-Chunk-Cost-Anstieg durch die Iso-Mesh-Welle) via synchroner `_gameLoopTick`-Pumpe im Test-Warmup + PLAYTEST_SECONDS-Budget von 20 → 30 s. Parallel haben drei Code-Audits (Game-Loop-Allokationen, Voxel-Chunk-Build, Renderer-Settings) die Performance-Welle eingeplant — 14 priorisierte Heilungen in drei Wellen geschnitten (Perf-1 V9.84 billige Twists, Perf-2 V9.85 Chunk-Build-Hebel, Perf-3 optional V9.86+ tiefe Operationen). Voller Plan in `docs/performance.md`. Performance-Welle blockiert die System-Kopplungs-Wellen D/E/F/G — Vision-Wellen kommen NACH dem Fundament-Schliff. ~3000 Playtest-Invarianten grün, Audit-Strict 0 Failures.
