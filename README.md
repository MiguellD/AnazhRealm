...
**Stand**: V14.7 (30.05.2026) — **Terrain-Bogen „Die wahre Tiefe des Geländes" (epische Geomorphologie, V14.0–.7, Browser-Audit-getrieben)**: das Makro-Relief von uniform-spitz-alpin zu abwechslungsreich-wie-die-Erde — kontinentale Basis (V14.1) + thermische Erosion (V14.2) + regionale Differenzierung (V14.3) + echte Ebenen (V14.4) + Harmonie-Heilung (V14.5, Höhlen-Decke) + Fern-Heilung (V14.6, Voxel-Decke + tanh-Soft-Clamp + stale GPU-Mirror ab) + Maßstab-Streckung (V14.7, eine glatte asymmetrische λ2860-Hub-Oktave macht die DOMINANTE Skala die GRÖSSTE → **Feature-Größe 176→464 m kontinental**; `terrainSteepness` verdrahtet, tect-Kommentar-Lüge geheilt). **Über den Bogen (gemessen): Feature-Größe 112→464 m, Anteil steil (>45°) 26 %→0 %, Median-Hangneigung 35°→14,6°, sanft-Anteil 6,7 %→33,6 %, Plateau 2,7 %→14,8 %, Hochland-Kohäsion →0,95, 12 lange Flüsse, FPS 119.** Davor: V13-Wasser-Profi-Bogen (pausiert @ V13.14, Finish → V15) + V12-Genie-Bogen (Three.js r184 + WebGPU-required, 119 FPS steady) + V10.0-NodeMaterial/TSL-Bogen.

**Nächste Welle: V14.8 — Gerichtete Uplift-Maske** (lineare/kurvige Anden-Ketten über weite Distanzen statt isotropes Noise). Danach V14.9 (Browser-Audit-Schluss); dann V15 (Wasser-Finish: die drei Render-Sync-Wurzeln + ±1024-Wasser-Region gegen die neue Topologie), V16 (lebendige Welt: System-Kopplungen E/F/G). Reflexions-Leitsätze: episches Terrain ist Worldgen-Mathematik — **die dominante Skala muss die größte sein** (Feature-Größe ∝ Wellenlänge, nicht Amplitude — eine glatte asymmetrische Oktave baut Hochland ohne Flutung); **wer die Terrain-Höhe/-Skala ändert, muss ALLE co-getuneten Annahmen nachziehen** (surf-relative Schichten, Voxel-Decke, jeden Density-Mirror Worker+GPU, UND die Foundation-Tests surf-relativ); Render nur im Browser-Loop justieren (Headless ist pixel-blind).
