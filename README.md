...
**Stand**: V9.52-a (23.05.2026) — **der Playtest-Pflege-Bogen beginnt**. `scripts/playtest.cjs` war eine 31 574-Zeilen-Funktion (eine async-IIFE, ~3000 Invarianten in einem Topf — die zweitgrößte Reibung des Projekts, schlimmer als die V9.44-Code-Wurzel). Sub-Welle a setzt das Gerüst: ein `safeEvaluate(page, fn)`-Helfer + `ctx = { page, check, logs, errors, finalState }`, und die ersten drei Sektionen (Initial-State, Ring 1 Grok, Ring 2 DSL+Terrain-Erweiterung) als benannte Module-Scope-Funktionen — `gatherInitialFinalState`, `checkInitialState`, `checkRing1Grok`, `checkRing2Dsl`. Verhaltens-identisch bewiesen über zwei Baseline-Läufe (3060 Invarianten, gleicher Satz, gleiche Reihenfolge — nur die pre-existing Drift-Bänder bleiben). Sub-Wellen b-e ziehen die übrigen Bänder nach. Plan: `docs/playtest-hygiene.md`.
