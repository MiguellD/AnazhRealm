...
Stand: 18.05.2026 (laufend aktualisiert; ursprünglich bei V8.52 geschrieben — die Umsetzungs-Noten unten reichen bis V8.72). Diese Doku ist **keine Implementierungs-Spec**, sondern ein **Vision-Anker**. Sie hält die Frage offen: was wird AnazhRealm, wenn der Spieler nicht nur Welten innerhalb einer Engine baut, sondern durch Welten zwischen Engines geht?
...
Stand V8.72 — die Sequenz bis zur Bibliothek + der Fremd-Engine-Bogen (W15 gebaut, W16/W17 offen):
...
| **16** | **Mesh-Welt-Verteilung** (vendorte Welten reisen peer-to-peer) | offen — Detail in `roadmap.md` §3 |
| **17** | **Multiplayer-Sub-Welten** (Transport-Shim + Mesh-als-Server) | offen — Detail in `roadmap.md` §3 |

Welle 12 war das **Proof of Concept** (bewiesen). Welle 13 ✅ + 14 ✅ + 7 ✅ sind die **Skalierung zur Bibliothek** — alle gebaut. Der **KI-Übersetzer** ist mit Phase 1 (V8.68) + Phase 2 (V8.69) vollständig: eine frei beschriebene fremde Welt dockt automatisch an, browsbar UND betretbar. **V8.70 — das Untrusted-Welt-Tor** — lässt zudem eine echte, ungeprüfte fremde Engine null-origin sandgesichert laufen (`worlds/schwarm/`). **V8.71 + V8.72 — der Auto-Vendor-Pfad (W15) komplett** — ein fremdes Welt-Bündel dockt ohne Handarbeit an, aus einem lokalen Ordner (Phase 1) ODER direkt aus einem GitHub-Repo (Phase 2, der save-server holt die Dateien selbst). Offen ist der Rest des Fremd-Engine-Bogens: die Mesh-Welt-Verteilung (W16 — vendorte Welten reisen peer-to-peer) + die Multiplayer-Sub-Welten (W17) — detailliert in `roadmap.md` §3 („Der Fremd-Engine-Bogen, W15–W17").
