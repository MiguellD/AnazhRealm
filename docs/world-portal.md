...
> **W14 komplett (V8.58-V8.61)**: der „Bibliothek"-Tab + „Portal holen" (P1), signierte Welt-Manifeste (P2), fremde Welten empfangen (P3). **KI-Übersetzer Phase 1 live (V8.68)**: eine neue Sektion „KI-Übersetzer" im Bibliothek-Drawer — ein LLM übersetzt eine frei beschriebene fremde Welt in ein Portal-Manifest (`translateWorldManifest` ruft `llmCall`, `_parseManifestResponse` liest das Manifest per `JSON.parse`, `_sanitizeImportedManifest` säubert es — die erprobte W14-P3-Wand). Der Spieler prüft den KI-Vorschlag + nimmt ihn auf; eine übersetzte Welt ist browsbar, nicht betretbar (`reachable:false`), bis Phase 2 ihre Engine vendort. Detail in `CLAUDE.md` V8.68.
