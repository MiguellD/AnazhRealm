...
Stand V8.78 — die Sequenz bis zur Bibliothek + der Fremd-Engine-Bogen (W15 + W16 + W17 komplett):
...
| **15** | **Auto-Vendor-Pfad** (ein fremdes Repo dockt ohne Handarbeit an) | ✅ **komplett (V8.71/V8.72)** — der save-server `/api/vendor-world`-Endpunkt nimmt ein lokales Bündel (P1) ODER eine GitHub-Repo-URL (P2 — Trees-API + Raw-Fetch, kein SSRF); die vendorte Welt läuft `trust:"sandboxed"` |
| **16** | **Mesh-Welt-Verteilung** (vendorte Welten reisen peer-to-peer) | ✅ **komplett (V8.73/V8.74)** — Phase 1: der Bündel-Transport (`world-bundle-pull`/`world-bundle-chunk`, ein Mitspieler holt eine vendorte Welt p2p). Phase 2: der browsbare Welt-Katalog (Peers annoncieren ihre `customWorlds` über den `soul`-Kanal) + ein sha256-Content-Hash (peer-unabhängige Identität + Dedup) |
| **17** | **Multiplayer-Sub-Welten** (Transport-Shim + Mesh-als-Server + Gruppen-Portal) | ✅ **Phase A + B-Relay + C + Multiplayer-Welt-Deklaration live (V8.75–V8.78)** — der Transport-Shim trägt den `WebSocket`-Verkehr über die Sandbox-Grenze, B-Relay verteilt ihn übers W7-Mesh (`subworld-net`-Kanal), das Gruppen-Portal bringt eine Gruppe gemeinsam hindurch (`portal-invite`), eine vendorte Welt erklärt sich selbst mehrspielerfähig. Offen im Bogen nur B-JS-Compute — Detail in `roadmap.md` §3 |

Welle 12 war das **Proof of Concept** (bewiesen). Welle 13 ✅ + 14 ✅ + 7 ✅ sind die **Skalierung zur Bibliothek** — alle gebaut. Der **KI-Übersetzer** ist mit Phase 1 (V8.68) + Phase 2 (V8.69) vollständig: eine frei beschriebene fremde Welt dockt automatisch an, browsbar UND betretbar. **V8.70 — das Untrusted-Welt-Tor** — lässt zudem eine echte, ungeprüfte fremde Engine null-origin sandgesichert laufen (`worlds/schwarm/`). **V8.71 + V8.72 — der Auto-Vendor-Pfad (W15) komplett** — ein fremdes Welt-Bündel dockt ohne Handarbeit an, aus einem lokalen Ordner (Phase 1) ODER direkt aus einem GitHub-Repo (Phase 2, der save-server holt die Dateien selbst). **V8.73 + V8.74 — W16 die Mesh-Welt-Verteilung komplett** — Phase 1 (V8.73): eine vendorte Welt reist peer-to-peer, ein Mitspieler holt ihr Bündel über das Mesh (`world-bundle-pull`/`world-bundle-chunk`, Spiegel von W7 P2; der Empfänger reicht es an die erprobte `vendorWorldBundle`-Schreib-Seite). Phase 2 (V8.74): der browsbare Welt-Katalog — jeder Mitspieler annonciert seine vendorten Welten über den `soul`-Kanal, der Bibliothek-Drawer zeigt „Mitspieler X hat: …" mit einem Holen-Knopf; ein sha256-Content-Hash gibt einer Welt eine peer-unabhängige Identität (Dedup über id ODER Hash). **V8.75 — W17 Phase A: der Transport-Shim** — eine fremde Multiplayer-Welt netzwerkt nur über `WebSocket`; der save-server-injizierte Shim ersetzt es durch eine postMessage-Brücke, der Verkehr quert die Sandbox-Grenze zu AnazhRealm. **V8.76 — W17 Phase B-Relay: das Mesh-als-Server** — der Phase-A-Loopback wird durch die Mesh-Verteilung ersetzt: ein `ws-send` aus dem Sub-Welt-iframe wird ein `subworld-net`-Mesh-Broadcast (`_portalNetReceive`), jedes andere AnazhRealm im selben Sub-Welt-Raum (Schlüssel: der Welt-Pfad) stellt es als `ws-recv` in SEIN iframe zu (`_portalNetDeliver`) — das Mesh IST der Server, kein Host. Trägt Relay-Welten (Server = blosser Rebroadcast). **V8.77 — W17 Phase C: das Gruppen-Portal** — betritt ein Spieler ein Multiplayer-Portal, broadcastet er einen `portal-invite`; die Mitspieler bekommen einen In-Game-Banner „X öffnete ein Tor nach <Welt> — mitkommen?", „Mitkommen" holt das Portal + betritt es (der Overlay erzwingt `multiplayer:true` — der Empfang der Einladung beweist es), die B2-Sub-Raum-Eingrenzung verbindet die Gruppe. **V8.78 — W17 die Multiplayer-Welt-Deklaration** — eine vendorte Welt erklärt sich selbst mehrspielerfähig: ein `multiplayer`-Flag fliesst durch den ganzen Vendor-/Mesh-Pfad (`vendorWorldBundle` → `customWorlds`-Eintrag → `aimBlueprintAtWorld` → `portalMeta`; Welt-Katalog + Bündel-Transfer), eine „Multiplayer-Welt"-Checkbox beim Andocken — `obtainPortalForWorld` produziert damit für eine vendorte Multiplayer-Welt ein Multiplayer-Portal, das W17-Gruppen-Portal greift end-to-end. Damit ist W17 Phase A+B-Relay+C + die Multiplayer-Welt-Deklaration komplett. Offen im Fremd-Engine-Bogen nur W17 Phase B-JS-Compute (ein Peer wird Compute-Host) — detailliert in `roadmap.md` §3 („Der Fremd-Engine-Bogen, W15–W17").
