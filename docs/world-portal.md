...
Stand: 18.05.2026 (laufend aktualisiert; ursprünglich bei V8.52 geschrieben — die Umsetzungs-Noten unten reichen bis V8.74). Diese Doku ist **keine Implementierungs-Spec**, sondern ein **Vision-Anker**. Sie hält die Frage offen: was wird AnazhRealm, wenn der Spieler nicht nur Welten innerhalb einer Engine baut, sondern durch Welten zwischen Engines geht?
...
Stand V8.74 — die Sequenz bis zur Bibliothek + der Fremd-Engine-Bogen (W15 + W16 komplett, W17 offen):
...
| **17** | **Multiplayer-Sub-Welten** (Transport-Shim + Mesh-als-Server) | offen — Detail in `roadmap.md` §3 |

Welle 12 war das **Proof of Concept** (bewiesen). Welle 13 ✅ + 14 ✅ + 7 ✅ sind die **Skalierung zur Bibliothek** — alle gebaut. Der **KI-Übersetzer** ist mit Phase 1 (V8.68) + Phase 2 (V8.69) vollständig: eine frei beschriebene fremde Welt dockt automatisch an, browsbar UND betretbar. **V8.70 — das Untrusted-Welt-Tor** — lässt zudem eine echte, ungeprüfte fremde Engine null-origin sandgesichert laufen (`worlds/schwarm/`). **V8.71 + V8.72 — der Auto-Vendor-Pfad (W15) komplett** — ein fremdes Welt-Bündel dockt ohne Handarbeit an, aus einem lokalen Ordner (Phase 1) ODER direkt aus einem GitHub-Repo (Phase 2, der save-server holt die Dateien selbst). **V8.73 + V8.74 — W16 die Mesh-Welt-Verteilung komplett** — Phase 1 (V8.73): eine vendorte Welt reist peer-to-peer, ein Mitspieler holt ihr Bündel über das Mesh (`world-bundle-pull`/`world-bundle-chunk`, Spiegel von W7 P2; der Empfänger reicht es an die erprobte `vendorWorldBundle`-Schreib-Seite). Phase 2 (V8.74): der browsbare Welt-Katalog — jeder Mitspieler annonciert seine vendorten Welten über den `soul`-Kanal, der Bibliothek-Drawer zeigt „Mitspieler X hat: …" mit einem Holen-Knopf; ein sha256-Content-Hash gibt einer Welt eine peer-unabhängige Identität (Dedup über id ODER Hash). Offen: die Multiplayer-Sub-Welten (W17) — detailliert in `roadmap.md` §3 („Der Fremd-Engine-Bogen, W15–W17").
