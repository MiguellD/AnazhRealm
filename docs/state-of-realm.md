...
| ✅ **Welt-Identität + Multi-Welt + Tor + Fusion** (Ring 8 / 8.1 / 8.2 / 9 / 10) | **vollständig live — Mai 2026**. `worldMeta` mit `worldId`, `slug`, `creator`, `visibility`, `parentWorlds`, `seed`, `bornAt`, `fusionStrategy`, `schemaVersion` (`8.0-multiworld-v1` für Standard, `9.0-tor-v1` für importierte, `10.0-fusion-v1` für fusionierte Welten). Drei localStorage-Keys spannen das System. **Methoden**: `createNewWorld`, `switchToWorld`, `deleteWorld`, `importWorldBeside`, `fuseWorlds(idA, idB, strategy)`. **Welt-Tor (Ring 9)**: Import-JSON läuft durch `<dialog id="world-tor-dialog">` mit Drei-Wahl (Ersetzen / Daneben legen / Fusionieren-mit-aktueller). „Daneben legen" trägt Original-ID in `parentWorlds`. **Fusion (Ring 10)**: `<dialog id="world-fusion-dialog">` mit 2-Spalten-Eltern + Strategie-Radio. Drei Strategien — `sequence` (History konkateniert, Emotionen aus A), `random-mix` (History 50:50 interleaved, Emotionen Mittel), `tag-merge` (Emotionen `Math.max` axis-wise — Vereinigung statt Mittelung). Inventar (Materialien/Werkzeuge/Baupläne) wird als Union gemerged mit `-fusion`-Suffix bei Kollision. Stammbaum-Sektion `#world-lineage` im Welt-Drawer zeigt Eltern-Welten als klickbare Links — Schöpfer kann jederzeit zur Eltern-Welt zurückwechseln. Ring 11 (Multi-User P2P-Sync) bleibt offen. |
