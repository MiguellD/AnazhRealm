...
| ✅ Welt-Generierung (Voxel-Terrain, Inseln, Vegetation, Hydrosphäre) | vollwertig — Voxel-Bogen V9.07+, Welle C VOLLENDET (V9.71-V9.82), Welle Perf-1 GEBAUT (V9.84) | 3D-Dichte-Feld + Surface-Nets-Mesher (formbarer Boden, Höhlen, Überhänge), Drainage-Netz mit Flüssen/Seen/Wasserfällen. **Welle C vollendet (V9.82)**: Wasser ist ein Voxel-Cell-Zustand (air/water/solid), naht-frei per Pad+Crop, nur Wasser-Luft-Iso (kein Floating-Plane), Density-Grid mit Boden-Mesher geteilt (~50× Speedup), Streaming-Pfad geheilt (Cells beim ersten Stream). **V9.83**: CI-Playtest-Flake an der Wurzel geheilt (Headless-rAF auf ~1 Hz gedrosselt → `_gameLoopTick`-Pumpe statt passivem `setTimeout`). **V9.84 Welle Perf-1**: sechs Sub-Wellen mit den billigen Twists (Shadow-PCF + Material-Singleton + Frustum-Pool + Color-Pool + Task-Direktionen-Scratch + Visual-LOD + Spatial-Hash für Flocking) — ~600 weniger Allokationen/Sekunde, Flocking 10–20× schneller, GPU ~10–20 % entlastet. **V9.85 Welle Perf-2 Kern**: vier weitere Sub-Wellen nach dem V9.84-Browser-Audit (Frame-Time-Budget für Streaming + Stable Shadow Maps via Texel-Snap + Mountain-Mulden-Filter für Bergwand-Wasser + Gradient-Normals teilen das `preDensity`-Grid) — Streaming smoother, Schatten beim WASD-Laufen stabil, Bergwand-Wasserflecken strukturell weg. **V9.86 Perf-2 ext.a**: drei Heilungen nach V9.85-Browser-Audit (Skirt=0 für Architektur-Spawn → -89 % dirty-Chunks + Konnektivitäts-Filter ersetzt SOLID-darüber-Check → C.3-Flake geheilt + Klassen-Konstante VERSION → kein hardcoded V7.82 mehr). **Welle Perf-3 als epochaler Bogen geplant** (V9.87+, Master-Plan in `docs/performance.md` §4): vier Sub-Wellen (Atlas-Strict-Wasser-Gate, Distance-LOD für ferne Chunks, Frustum-priorisiertes Build, Worker-Migration) — der Schöpfer-Auftrag: „die Bibliothek von Alexandria der Neuzeit muss selbst die AAA-Titel alt aussehen lassen". Welle Perf-3 zieht die WURZEL der Frame-Drops (synchroner Main-Thread-Build) strukturell weg. Detail in `docs/roadmap.md` Welle C + `docs/hydrosphere.md` §16 + `docs/performance.md`. |
