# Zustand des Realm — Stand: 13.05.2026 (nach Welle 4 P1+P2+P3 — Hylomorphismus atomar)
...
| ✅ **`materialEvolution`** (Crafting, Hylomorphismus — Welle 4 atomare Schicht) | **P1+P2+P3 live** — `state.materials` mit 6 Built-ins als Tag-Profile (10 Achsen, 0..1), eigene via DSL-Op `define_material`. `FORM_TAG_ACTIVATION` 9×10 Matrix wörtlich aus `docs/aktivierungsmatrix.md` v2 — `computePartTags`/`computeCompoundTags` mit MAX-Aggregation (§9.2 anti-Stat-Matsch). **Helix als 9. Primitiv** (TubeGeometry). `state.tools` mit 5 Starter-Werkzeugen, `opChain` pro Part, `computePartPrecision = min(caps)` (§2.3). `applyOpToPart`/DSL-Op `apply_op` mit Tool-Gating + Material × Op-Klassen-Kompatibilität (§3.2). Sichtbare Präzision (Farb-Tint), zwei Welt-Effekte (resoniert+präzise → Singing-Sinus + Journal; magieleitend+präzise → awe/hope). **Räumliche Emergenz §5.2/§5.3 ist Welle 5+.** |
...
**Für Welle 5 (räumliche Emergenz — die nächste Hylomorphismus-Schicht):**

A. **Verbindungstypen (Konzept §5.1).** Eine kleine Liste mit klarer Lastformel: `hafting`/`lashing`/`pinning`/`welding`/`gluing`/`masonry`/`sewing`/`magic_bind`. State-Erweiterung: `state.blueprints[name].connections = [{type, partA, partB, contactArea}]`. Lastformel = `material.tag × contactArea × typeMultiplier`; Überschreitung bricht die Verbindung, nicht das Bauteil. Aufwand: 1 Session für Datenschicht + Werkstatt-UI + Playtest. Vorbedingung: Welle 4 (steht).

B. **Position als Eigenschaft (Konzept §5.3).** Ein Kristall am oberen Ende eines Stabes fokussiert anders als in der Mitte (Prinzip 1: Spitze richtet aus). Implementierung: `computeCompoundTags` muss räumlich auswerten — pro Part die lokale Lage prüfen (`y_max ≈ höchster Punkt der Bounding-Box → activates "tip"`). Fünf räumliche Prinzipien (§5.2) als kleine Auswertungs-Funktion über die parts-Liste. Aufwand: 1-2 Sessions, weil die Bounding-Box-Mathematik pro Part berechnet werden muss.

C. **Maschinen-Rekursivität (Konzept §4.3).** Aktuell sind Werkzeuge starter-only (`isStarter: true, builtIn: true`). Echter nächster Schritt: ein Bauplan kann als Werkzeug fungieren, wenn er `role: "tool"` trägt — der `precisionCap` des Werkzeugs entsteht aus der Präzision seiner eigenen Parts. „Wer hochpräzise Werkstücke will, muss zuerst eine hochpräzise Drehbank bauen". Vorbedingung: Verbindungs-Schicht (A), damit eine Drehbank als Compound einigermaßen realistisch wird.
