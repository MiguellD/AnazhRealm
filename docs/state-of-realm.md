...
| ✅ **V13 — Wasser-Profi-Bogen** (V13.0–.6 durch, V13.7+ offen) | Voxel-Wasser effizient, korrekt + in Synergie mit dem Terrain: Klassifikation (`_atlasWaterLevelAt`, Hang-Schatten 51,7 → 0,1 %) + Flow-Bias + **Schicht-3-Tiefen-Ufer-Shader** (weiche Uferlinie via `viewportLinearDepth` pro-Pixel + Tiefen-Farbe + Emotions-Haken) + **V13.6 Surface-Nets-Iso wie das Terrain** (band-limitiert aufs `hydroBand`; teilt den Boden-Mesher → fließt; der V13.2-Boundary-Mesher hatte die Synergie geopfert). Zellen bleiben die reaktive Wahrheit (kein Sheet). **Nächste Welle V13.7**: Sub-Region-Edit-Remesh. Detail in `docs/roadmap.md §1.4`. |
