...
| ✅ **V13 — Wasser-Profi-Bogen** (V13.0–.8 durch, V13.9+ offen) | Voxel-Wasser effizient, korrekt, in Synergie + 3D-verbunden: Flow-Bias + **Schicht-3-Tiefen-Ufer-Shader** (`viewportLinearDepth` pro-Pixel) + **V13.6 Surface-Nets-Iso wie das Terrain** (teilt den Boden-Mesher → fließt) + **V13.8 3D-Flood-Fill von Quellen** (Wasser nur wo durch nicht-soliden Raum unter dem Spiegel mit einer Quelle verbunden → kein Bergwand-Bluten, keine Unterwasser-Luftblasen, Damm blockt, Carve flutet reaktiv). Zellen bleiben die reaktive Wahrheit (kein Sheet). **Nächste Welle V13.9**: Sub-Region-Edit-Remesh. Detail in `docs/roadmap.md §1.4`. |
