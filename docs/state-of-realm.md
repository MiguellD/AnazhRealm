# Zustand des Realm — Stand: 18.05.2026 (V8.71)

**V8.71 — W15 Phase 1 (Auto-Vendor-Pfad)**: V8.70 baute den Schlüsselstein (eine fremde Engine läuft null-origin sandgesichert), aber `worlds/schwarm/` war hand-vendort. W15 automatisiert das Andocken. Der save-server bekommt einen zweiten Schreib-Pfad `/api/vendor-world` — er nimmt `{worldId, files}` und schreibt das Bündel nach `worlds/<id>/`, mit einer strengen Wand (op-förmige id, keine Built-in-Welt, kein `..`-Ausbruch, Endung-Whitelist, Größen-Deckel, NIE wird etwas ausgeführt). Clientseitig: `_vendorSanitizeBundle` prüft vorab, `_vendorPostBundle` POSTet, `vendorWorldBundle` registriert die Welt als `customWorlds`-Eintrag mit `trust:"sandboxed"` + `vendored:true` (eine vendorte Welt läuft IMMER null-origin — `_sanitizeImportedManifest` erzwingt es, unforgeable). Eine „Welt andocken"-Sektion im Bibliothek-Drawer nimmt einen Welt-Ordner. Phase 1 ist die netzfreie Hälfte (ein lokales Bündel); der GitHub-Fetch ist Phase 2. +20 Invarianten 2597→2617, browser-verifiziert (`smoke-vendor.cjs`). Voller Eintrag in `CLAUDE.md` V8.71.

**V8.68–V8.70 — der Fremd-Engine-Bogen beginnt**: der KI-Übersetzer ist vollständig — V8.68 übersetzt eine fremde Welt in ein Portal-Manifest, V8.69 in eine deklarative Szene, die der generische `worlds/translated/`-Renderer betretbar macht (der LLM-Output bleibt durchgehend DATEN, nie ausgeführter Code — `JSON.parse` + Sanitizer-Wand). V8.70 — das **Untrusted-Welt-Tor**: eine Welt trägt eine Vertrauensstufe (`portalMeta.trust`); `sandboxed` → das Portal-iframe läuft `allow-scripts` allein (opake null-Herkunft), eine echte, ungeprüfte fremde Engine läuft voll, kann AnazhRealms localStorage/DOM/Cookies aber nicht berühren — `worlds/schwarm/` (eine eigenständige 2D-Boids-Engine) beweist es. Offen ist der Rest des Fremd-Engine-Bogens: der Auto-Vendor-Pfad **Phase 2 (GitHub-Fetch)**, die Mesh-Welt-Verteilung + die Multiplayer-Sub-Welten — detailliert geplant in `docs/roadmap.md` §3. Volle Wellen-Einträge in `CLAUDE.md` V8.68–V8.71; 2617 Playtest-Invarianten grün, 0 Audit-Strict-Failures.
