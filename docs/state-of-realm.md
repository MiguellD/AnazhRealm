...
| ✅ **V13 — Wasser-Profi-Bogen** (V13.0–.5 durch, V13.6+ offen) | Voxel-Wasser effizient + korrekt wie die Riesen: Diagnose + Klassifikation (`_atlasWaterLevelAt`, Hang-Schatten 51,7 → 0,1 %) + Grenzflächen-Meshing (~150 → ~3 ms/Chunk) + Flow-Bias + Glättung + **Schicht-3-Tiefen-Ufer-Shader** (weiche Uferlinie via `viewportLinearDepth` pro-Pixel + Tiefen-Farbe + Emotions-Haken; Geometrie dumm-flach). **Nächste Welle V13.6**: Sub-Region-Edit-Remesh. Detail in `docs/roadmap.md §1.4`. |
