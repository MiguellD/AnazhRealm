...
| ✅ **V13 — Wasser-Profi-Bogen** (V13.0–.7 durch, V13.8+ offen) | Voxel-Wasser effizient, korrekt, in Synergie + verbunden: Klassifikation (`_atlasWaterLevelAt`) + Flow-Bias + **Schicht-3-Tiefen-Ufer-Shader** (`viewportLinearDepth` pro-Pixel + Tiefen-Farbe + Emotions-Haken) + **V13.6 Surface-Nets-Iso wie das Terrain** (band-limitiert; teilt den Boden-Mesher → fließt) + **V13.7 verbundenes Wasser** (`terrainTopY < rim` → füllt die Mulde bis zum Ufer, vertikale Wände 33 %→0 %; Carve nahe Quelle füllt reaktiv; kein Hang-Schatten/Phantom). Zellen bleiben die reaktive Wahrheit (kein Sheet). **Nächste Welle V13.8**: Sub-Region-Edit-Remesh. Detail in `docs/roadmap.md §1.4`. |
