...
| ✅ Welt-Generierung (Voxel-Terrain, Inseln, Vegetation, Hydrosphäre) | vollwertig — Voxel-Bogen V9.07+, Welle C VOLLENDET (V9.71-V9.82), Performance-Welle eingelegt ab V9.84 | 3D-Dichte-Feld + Surface-Nets-Mesher (formbarer Boden, Höhlen, Überhänge), Drainage-Netz mit Flüssen/Seen/Wasserfällen. **Welle C vollendet (V9.82)**: Wasser ist ein Voxel-Cell-Zustand (air/water/solid), naht-frei per Pad+Crop, nur Wasser-Luft-Iso (kein Floating-Plane), Density-Grid mit Boden-Mesher geteilt (~50× Speedup), Streaming-Pfad geheilt (Cells beim ersten Stream). **V9.83**: CI-Playtest-Flake an der Wurzel geheilt (Headless-rAF auf ~1 Hz gedrosselt → `_gameLoopTick`-Pumpe statt passivem `setTimeout`). **Performance-Welle eingelegt** (Perf-1 V9.84 billige Twists, Perf-2 V9.85 Chunk-Build-Hebel, optional Perf-3 V9.86+) — drei Code-Audits identifizierten Allokations-Sturm im Hot-Path, redundante Density-Resamples + Renderer-Settings auf High-End geklemmt. Detail in `docs/roadmap.md` Welle C + `docs/hydrosphere.md` §16 + `docs/performance.md`. |
