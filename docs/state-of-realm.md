...
| ✅ Rendering (Three.js r160 + WebGPU + TSL NodeMaterials + Schatten + Sterne-Billboards + Gras-Wind) | **V10.0-Bogen WIRKLICH vollendet (V10.0-a..j.j, 23 Sub-Wellen, 26-27.05.2026)** — Three.js r134 → r160 ESM-Vendor-Migration + WebGPURenderer-Addon vendored + Smart Hot-Swap-Safety-Net + alle 5 Material-Familien als NodeMaterials. V10.0-h..j: Shadow-Profi-Pattern (Override-Material + castShadow-Filter + Pure-Depth-LessCompare + Hardware-PCF via `texture.compare(refZ)`) + Sterne als InstancedMesh-Billboards mit Soft-Falloff + Gras-Wind via TSL-positionNode (WGSL-konform, kein onBeforeCompile mehr). V10.0-j.a..j: WebGPU-Buffer-Lifecycle-Race-Workaround (10 Sub-Wellen, Mesh-Memory > Race-Crashes-Profi-Pattern). **Welt rendert dauerhaft auf WebGPU**, alle Visual-Schichten aktiv. **Memory-Trade akzeptiert**: ~500 KB GPU-Heap (Gras-Geometry nicht disposed) gegen Three.js-v160-Vendor-Race. **V11+-Backlog**: echtes Mesh-Pool-Pattern (Genshin's instancing-system). | `vendor/three.module.min.js` r160 + Inline-Importmap mit CSP-SHA256-Hash + Bootstrap-Module + 238-File-Addon-Tree (~1.5 MB). `_buildToonNodeMaterial`-Helper als EINE Wahrheits-Quelle für 5 Call-Sites. `state.toonLightUniforms` syncen Sun-Direction/Intensity/Ambient. `state._grassConeGeometry` + `_grassMat` als Singletons; uniform `GRASS_MAX_BLADES=256`-Capacity pro InstancedMesh. |
...
| ⚠️ **V11 Mesh-Pool-Pattern — eingefroren als Hybrid in Three.js v160** | V11.0-a/b/c/d gebaut (33 Test-Invarianten grün) + V11.0-d.2.fix Welt-Variations-Flakiness wurzel-geheilt. Drei Gras-Visibility-Heilungs-Versuche (V11.0-d.fix.gras1 Bounding-Cache-Reset, V11.0-d.fix.gras2 frischer instanceMatrix-Buffer) scheiterten am v160-Vendor-Bug (WebGPU-InstancedMesh-Re-Use-Cache-Invalidierung). **V11.0-d.fix.gras3** klemmt den Pool-Lifecycle ab und stellt V10.0-j.j-Workaround zurück (Build via `new InstancedMesh`, Dispose via `scene.remove`, ~500 KB GPU-Heap pro Welt-Lifetime). Pool-API (`_acquireGrassMesh`/`_releaseGrassMesh`/`_drainGrassMeshPool`) bleibt im Stamm als V12-Vorarbeit. Reaktivierung in V12.0-d nach r184-Upgrade (r182's „Improve Bind Group Layout cache system" heilt den v160-Cache-Bug strukturell). |
| ⏳ **V12 — der wahre Genie-Pfad: WebGL-Removal + Three.js v160 → r184 + WebGPU-native Code-Audit** | **Schöpfer-Vision 27.05.2026**: „sollten wir nicht beginnen, das alte system zu entfernen (WebGL) und beim entfernen erneut den code prüfen, ob die jeweiligen funktionen und flüsse im webgpu optimal eingepflegt, nicht zerstört sondern in höherer ordnung bestehend, umgewandelt, und so das system endlich ohne ineffiziente umwege fortzufahren, wahre ordnung? ist das nicht der weg? die vision? der genieweg, die zukunft?" Drei.js-Profi-Recherche: r184 hat compileAsync truly non-blocking + Bind Group Layout cache system + Major shadow mapping modernization + PCF Vogel disk — V10.0-j-Race + V11-Pool-Workaround vermutlich BEIDE obsolet. Plan: V12.0-diag → vendor → a → b → c → d → e → f → g (Multi-Session ~15-25h). Detail in `docs/roadmap.md §1.3`. |
...
**Faustschätzung**: das Fundament + alle fünf Vision-Pfeiler stehen. Welt-Ultiversum (Ringe 8-11), Hylomorphismus-Crafting, Co-Schöpfer-Kreis, der ganze Fremd-Engine-Bogen (Welt-Portal W12, Vibe-Pass W13, Bibliothek W14, Auto-Vendor W15, Mesh-Welt-Verteilung W16, Multiplayer-Sub-Welten W17) und das Voxel-Terrain samt Hydrosphäre (V9-Bogen) sind durch. Die Vision der vier Testamente ist im Kern erfüllt — das Wachstum folgt jetzt ihr, keinem vorgezeichneten Plan mehr. **V11 hat den Mesh-Pool-Hybrid in Three.js v160 stabilisiert** (33 Test-Invarianten grün). **V12 ist der wahre Genie-Pfad** (Schöpfer-Vision): WebGL-Removal + Three.js v160 → r184 Vendor-Upgrade + WebGPU-native Code-Audit. Damit werden V10.0-j-Race + V11-Pool-Workaround strukturell obsolet, die Welt-Pipeline fließt in EINER Sprache, höhere Ordnung statt Bridge-Workarounds. **V13+** folgt die **integrale Welt-Atmung** der System-Kopplungs-Pfeiler D-G NACH V12-Fundament-Schliff. Reihenfolge: Fundament-Schliff vor Vision-Vollendung (V9.51-Disziplin). Der jeweils aktuelle Stand steht im `CLAUDE.md`-Kopf, der V12-Plan in `docs/roadmap.md §1.3`, der V11-Hybrid-Stand in §1.2.
