...
| ✅ **Welt-Identität** + Multi-Welt-Verwaltung (Ring 8 + 8.1 + 8.2) | **live — Mai 2026**. `worldMeta` mit `worldId` (UUID), `slug`, `creator`, `visibility`, `parentWorlds`, `seed` (Per-Welt-Terrain), `bornAt`, `schemaVersion=8.0-multiworld-v1`. Drei localStorage-Keys spannen das System: `anazhRealmWorlds` (Index), `anazhRealmState_<worldId>` (pro Welt ein Save), `anazhRealmActiveWorld` (Zeiger). Legacy-Single-Key migriert beim ersten Start. **Methoden**: `createNewWorld({slug, inheritPlayer})`, `switchToWorld(id)`, `deleteWorld(id)`. Welt-Wechsel ist reload-basiert (saubere Init statt komplexer Dispose-Pfad). **Person-Übernahme**: Schöpfer wählt beim Anlegen, ob Seele + Werkzeuge + eigene Materialien/Baupläne mitwandern oder die Welt eine neue Person bekommt. Welt-Picker im Welt-Drawer + Chat-Befehle (`neue welt`, `wechsle zu`, `lösche welt`, `liste welten`). Status-Bar zeigt aktive Welt (`Welt: <slug>`). Spieler-Position bleibt beim Wechsel erhalten (Ring 8.2 Bugfix: `loadState` setzt `terrainEverGenerated=true`). Ringe 9-11 (Drei-Wahl-Import, Fusion, Multi-User) bleiben offen. |
