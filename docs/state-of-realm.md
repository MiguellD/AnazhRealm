...
| ✅ **Welt-Identität** + Multi-Welt + Welt-Tor (Ring 8 + 8.1 + 8.2 + 9) | **live — Mai 2026**. `worldMeta` mit `worldId` (UUID), `slug`, `creator`, `visibility`, `parentWorlds`, `seed` (Per-Welt-Terrain), `bornAt`, `schemaVersion=8.0-multiworld-v1` (Active) bzw. `9.0-tor-v1` (durch Welt-Tor importierte). Drei localStorage-Keys: `anazhRealmWorlds` (Index), `anazhRealmState_<worldId>` (pro Welt), `anazhRealmActiveWorld` (Zeiger). Legacy-Single-Key migriert beim ersten Start. **Methoden**: `createNewWorld`, `switchToWorld`, `deleteWorld`, `importWorldBeside`. Welt-Wechsel reload-basiert (saubere Init statt Dispose-Pfad). **Person-Übernahme** beim Erschaffen: Seele + Werkzeuge + eigene Materialien/Baupläne wandern mit, Pfad-Buckets + Emotionen + Journal bleiben welt-spezifisch. Welt-Picker im Welt-Drawer + Chat-Befehle. Status-Bar zeigt aktive Welt. Spieler-Position bleibt beim Wechsel erhalten. **Welt-Tor (Ring 9)**: Import einer fremden Welt-JSON öffnet einen Drei-Wahl-Dialog (Ersetzen / Daneben legen / Fusionieren-stub) — „Daneben legen" vergibt neue worldId, behält Source-Seed (Look bleibt), trägt Original-ID in `parentWorlds` als Provenienz-Spur, löst Slug-Kollisionen mit `-2`/`-3`, fügt einen Witness-Journal-Eintrag in die importierte Welt. Ringe 10-11 (Fusion, Multi-User) bleiben offen. |
