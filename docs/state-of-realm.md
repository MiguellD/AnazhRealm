...
| ✅ **V13 — Wasser-Profi-Bogen** (V13.0–.4 durch, V13.5+ offen) | Voxel-Wasser effizient + korrekt wie die Riesen: Diagnose + Klassifikation (`_atlasWaterLevelAt`, Hang-Schatten 51,7 → 0,1 %) + Grenzflächen-Meshing (~150 → ~3 ms/Chunk) + Flow-Bias-Heilung + Glättung. **Nächste Welle V13.5**: Schicht-3-Tiefen-Ufer-Shader (Drei-Schichten-Reflexion). Detail in `docs/roadmap.md §1.4`. |
