...
| ✅ **V13 — Wasser-Profi-Bogen** (V13.0–.8 durch, V13.9+ offen) | Voxel-Wasser effizient, korrekt, in Synergie + 3D-verbunden: Flow-Bias + **Schicht-3-Tiefen-Ufer-Shader** (`viewportLinearDepth` pro-Pixel) + **V13.6 Surface-Nets-Iso wie das Terrain** (teilt den Boden-Mesher → fließt) + **V13.8 3D-Flood-Fill von Quellen** (Wasser nur wo durch nicht-soliden Raum unter dem Spiegel mit einer Quelle verbunden; Damm blockt, Carve flutet reaktiv; Luftblasen weg). Schöpfer-Audit V13.8: Rest-Wand-Bluten ist DÜNN (16-m-Atlas-vs-Voxel-Spill) + tiefes Wasser an Strukturen + Carve-Naht offen → **V13.9 Wasser-Finish im Browser-Loop** (dünnes Wasser im Tiefenpuffer-Shader cullen). **Lehre: Ufer ist ein 16-m-vs-Voxel-Tradeoff — Glätte im Shader, keine 3D-Fluid-Sim; Render nur im Browser-Loop justieren.** Zellen bleiben die reaktive Wahrheit (kein Sheet). Detail in `docs/roadmap.md §1.4`. |
| ⏳ **V14 — Vision-Vollendung: Pfeiler D-G** | NACH dem V13-Wasser-/Render-Fundament. Pfeiler D Kern ✅ + D-Polish, E (Emotion ↔ lokale Welt), F (Cluster-Resonanz), G (Multi-Spieler-Vibe) als Pfeiler-Plan. JEDE Welle additiv + klein-fokussiert (V9.56-Bogen-Disziplin), keine Meta-Schicht (Heilige Lektion). |

**Faustschätzung**: das Fundament + alle fünf Vision-Pfeiler stehen. Welt-Ultiversum (Ringe 8-11), Hylomorphismus-Crafting, Co-Schöpfer-Kreis, der ganze Fremd-Engine-Bogen (Welt-Portal W12, Vibe-Pass W13, Bibliothek W14, Auto-Vendor W15, Mesh-Welt-Verteilung W16, Multiplayer-Sub-Welten W17) und das Voxel-Terrain samt Hydrosphäre (V9-Bogen) sind durch. Die Vision der vier Testamente ist im Kern erfüllt — das Wachstum folgt jetzt ihr, keinem vorgezeichneten Plan mehr. Der **V12-Genie-Bogen ist vollendet** (Three.js r184, WebGPU-required, Architektur-Instancing, 119 FPS steady — Render gelöst), der **V13-Wasser-Profi-Bogen ist bei V13.8** (effizient, korrekt, in Synergie + 3D-verbunden via Flood-Fill — teilt den Surface-Nets-Mesher, Schicht-3-Tiefen-Shader, Damm blockt, Carve flutet; Rest-Feinschliff Ufer im Browser-Loop). **Als Nächstes**: V13.9 Wasser-Finish (dünnes Wand-Bluten im Shader cullen + Struktur-Tiefwasser + Carve-Naht, mit dem Schöpfer im Browser), dann V13.10 Sub-Region-Edit, V13.11 LOD, V13.12 Audit, dann **V14 — die integrale Welt-Atmung** der System-Kopplungs-Pfeiler D-G. Reihenfolge: Fundament-Schliff vor Vision-Vollendung (V9.51-Disziplin). Der jeweils aktuelle Stand steht im `CLAUDE.md`-Kopf, der Plan vorwärts in `docs/roadmap.md` (V13 in §1.4, V14-Kandidaten in §1.1).
