...
| -------- | ---------------------------------------------------------------------------------------------------------------------------------------------------| ------------------------------- |
| **W-A1** | ✅ (10.07., V18.437) REZEPT-KATALOG + generische B4-Regler (`ov`-Roundtrip cache-frei) + Donor-Abschied + ehrliches Interim — `gate:rezept-katalog` (CI). **PHYSISCHER ABSCHIED ✅ (Auslöschungs-Welle V18.446, Schöpfer „alles raus, wie bei den bäumen"):** die Donoren sind nicht mehr donorOnly-versteckt, sondern PHYSISCH gefallen — die Substanz wanderte byte-treu nach `AnazhRealm.KIND_SUBSTANCE` (frozen, das Baum-Muster SPECIES_TAG_REFERENCE), 5 Genom-Maschinen + 10 Part-Konstanten (inkl. templeParts/villageParts/avatarWaechterParts/reittierHolzrossParts) geschnitten, Buch 141→134 Blueprints/donorOnly 4→0; `_foundryAutoRegisterSpecies` liest `bps[k] || KIND_SUBSTANCE[k]` fail-closed; alte Körper auf Studio-Guss (waechter→koerperstudio, holzross→tetrapoda) | „regelbar, alle Assets“ ✅      |
