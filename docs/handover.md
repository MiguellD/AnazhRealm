...
## Schnell-Lage (Stand 20.05.2026, V9.27)

**Du erbst eine sehr lebendige Welt**. **~2928 Playtest-Invarianten grün + 0 Audit-Strict-Failures**, ~36000 Zeilen in einer Datei, alles produktiv. (Der Playtest-Konsolen-Zähler driftet ±2-3 je Lauf — einige Checks sind bedingt; „Alle Invarianten OK" ist die Wahrheit, nicht die exakte Zahl.)

**Jüngste Welle — V9.27 (Voxel-Terrain-Bogen Phase 5c.1: Heightfield-Init für Voxel-Welten übersprungen)**: der erste ehrliche Schritt der grossen 5c-Aufräumarbeit (V9.26 ehrlich benannt). Eine voxel-basierte Welt baut die initialen 64 Heightfield-Chunks GAR NICHT mehr — vorher bauten sie + `_restoreVoxelTerrain` legte sie Sekunden später via `_setHeightfieldDormant` schlafen (wasteful). Der Skip-Gate in `generateTerrainWithParameters` liest `worldMeta.voxelTerrain` als persistente Wahrheit (von `_preloadActiveWorldMeta` hydratisiert, von V9.26 für migrierte Welten gesetzt, BEVOR `generateNewWorld` läuft). Das Heightfield-`terrainMaterial` bleibt erhalten — wer per Chat `voxel terrain off` toggelt, bekommt Chunks vom Streaming-Ring nachgeladen (Reversibilität gewahrt). Die fresh-Eingangs-Welt (Playtest) bleibt heightfield (V9.26-Disziplin) → bestehende Test-Schicht unberührt. +3 Invarianten. **Noch offen in 5c**: (a) `heightData/caveData/volcanoData`-Allokation (768 KB; kleine Politur); (b) `_setHeightfieldDormant`-Code (no-op auf leerer chunkMap, lebt aber); (c) die grosse Heightfield-Code-Entfernung (`_terrainHeightAtWorld`, `ensureChunkAt`, `generateChunk`, `groundHeightField`) + Test-Suite-Umschreibung — sobald die Migrations-Erfahrung gefestigt ist + die Eingangs-Welt geflippt wird. Davor V9.26 (Phase 5c-Start — Migrations-Flip + Chunk-Loch-Heilung).
