...
## Schnell-Lage (Stand 19.05.2026, V9.11)

**Du erbst eine sehr lebendige Welt**. **~2879 Playtest-Invarianten grün + 0 Audit-Strict-Failures**, ~33000 Zeilen in einer Datei, alles produktiv. (Der Playtest-Konsolen-Zähler driftet ±2-3 je Lauf — einige Checks sind bedingt; „Alle Invarianten OK" ist die Wahrheit, nicht die exakte Zahl. Die Suite ist seit V8.96 deterministisch — der letzte benannte flaky Test ist geheilt.)

**Jüngste Welle — V9.11 (Voxel-Terrain Naht-Wurzelheilung — der Index-Aliasing-Bug)**: ein Schöpfer-Browser-Befund nach V9.10 — „immer noch unsaubere Nähte, falsche effektive Chunk-Grösse?". Die Intuition traf den Kern. In `_voxelChunkGeometry` Pass 2 aliaste `ci(dim,j,k) = dim + j*dim + k*dim²` an der `i/j/k=dim`-Randebene in einen FREMDEN Zell-Slot (oder out-of-bounds) → ein Streck-Dreieck quer durch den Chunk an jeder Chunk-Naht. Der Fix: ein `cv(i,j,k)`-Helfer liefert -1 für jeden out-of-range Index, kein Quad referenziert je einen aliasten Slot. Der V9.10-Skirt war konzeptionell korrekt — der Bug lag darunter im Mesher. +1 Invariante. Davor V9.10 (Naht-Skirt + per-Vertex-Welt-Feld-Farbe), V9.09 (Voxel-Chunk-Ring — Voxel-Chunks streamen um den Spieler, `voxel terrain on`/`off`). **Nächste Welle: Phase 2c — per-Welt-Persistenz** (`worldMeta.voxelTerrain` persistiert das Flag; eine neue Welt kann voxel-basiert sein). Danach Phase 3 (3D-`modify_terrain` — echtes Graben), Phase 4 (Höhlen/Überhänge in der Generierung), Phase 5 (Materialien/Shader/Ablösung). Der volle Plan steht in `docs/roadmap.md` „Der Voxel-Terrain-Bogen". Disziplin: parallel bauen + beweisen, dann das Heightfield ablösen — nie das Funktionierende brechen. **Lies `CLAUDE.md` V9.11 ZUERST.**
