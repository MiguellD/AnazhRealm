...
### Davor: V9.41-b (Laplacian-Smooth, Voxel-Treppen geglättet)

**V9.41-b — Voxel-Surface-Politur Welle B.2 (Laplacian-Smooth glättet die Voxel-Treppen-Geometrie)**: Schöpfer-Browser-Test nach V9.41: „scheint noch wie zuvor, sehe keine änderung". V9.41 funktionierte technisch perfekt (Schach-Brett 92% balanciert am echten Welt-Mesh), aber das Schöpfer-Auge sah keine Wirkung. Diagnose: die sichtbaren Streifen sind nicht die Quad-Diagonal-Falten, sondern **Voxel-Treppen aus der Surface-Nets-Auflösungs-Grenze**. Cel-Stufen-Slider-Test des Schöpfers schloss Cel-Banding als Quelle aus. **Heilung**: zwischen Pass-2 (Quad-Emit) und Normalen-Bau ein **Laplacian-Smooth-Pass** über die Mesh-Topologie. Pro Vertex: Mittelwert seiner topologischen Nachbarn aus dem Index-Buffer; Vertex bewegt sich mit Lambda 0.5 auf halben Weg zum Mittel. 1 Iteration, Normalen bleiben V9.16-Dichte-Gradient (an der NEUEN Position gesampelt — die echte Iso-Fläche ist unverschoben, nur das Mesh-Approx). Test-Mesh Y-Abweichung 0.13 (Schwelle 0.25, step 1.8); echtes Welt-Mesh ~0.11 (war pre-Smooth 0.3-0.5). 1 neue Invariante, alle grün. **Meta-Lehre**: ein messbar-perfekter Headless-Beweis ist nicht dasselbe wie eine wirksame Welle — V9.41 fixte die Diagonal-Wahl, das Schöpfer-Auge sah die Treppen. Wer drei plausible Wurzeln hat, prüft sie in Reihenfolge der Kosten (Cel-Slider-Test → kleine Welle → grosse Welle). Nächste Welle: Schöpfer-Browser-Test der V9.41-b entscheidet. Falls Streifen weg → V9.42 (Inseln als Voxel-Chunks). Falls Spur sichtbar → V9.41-c (2 Iterationen oder Taubin-Smooth).

### Davor: V9.41 (Schach-Brett-Diagonalen, technisch wirksam aber Schöpfer-Wahrnehmung unverändert)

 Schöpfer-Audit-Befund #3 nach V9.39-Browser-Test: „Trapeze/Linien auf flachen Hügeln" — auf welligen, hellen Flächen erschienen sichtbare DIAGONAL-STREIFEN. Wurzel an Surface-Nets-Quad-Trianguliering gemessen: `quad(a,b,c,d)` emittierte IMMER `indices.push(a,b,c, a,c,d)` — alle Diagonal-Falten zeigten in dieselbe Richtung. Auf nicht-planaren Quads sind die Falten als Geometrie sichtbar; bei uniformer Wahl addieren sie sich zu langen Linien. V9.16 fixte die LICHT-Facetten via Dichte-Gradient-Normalen — die GEOMETRIE-Falte blieb. **Fix in einer Funktion**: `quad()` nimmt jetzt `parity`-Argument; gerade Parität triangulieren a→c (alt), ungerade triangulieren b→d. Drei Emit-Stellen geben `parity = (i+j+k) & 1` mit — die Schach-Brett-Parität bricht die Streifen. Geometrie-Kosten null, Winding bleibt CCW. Browser-Headless-Beweis: 964 a-c-Diagonalen vs 885 b-d (~52/48 balanciertes Schach-Brett); pre-V9.41 wären 1849/0. **Meta-Lehre**: eine Wurzel kann hinter einer kosmetisch-anmutenden Welle verborgen liegen — V9.16 verbarg das Licht-Banding, die zugrundeliegende Geometrie blieb. Eine 1-Bit-Variation in der Generation kann ein ganzes Symmetrie-Muster brechen. **Nächste Welle**: V9.42 (Inseln als Voxel-Chunks vereinheitlichen) oder V9.41.b Laplacian-Smooth-Pass, falls Schöpfer-Browser-Test sagt „noch nicht genug".

### Davor: V9.40-f (Ammo-Heap an der WASM-Wurzel)

Schöpfer-Browser-Test nach V9.40-e: „weiter gehts, doch die fehlerhaften Chunks scheinen zum Teil noch zu bestehen". Im Screenshot hatte er Sicht-Ring **17×17** (Maximum, Ring 8 = 289 Chunks gleichzeitig — Default ist Ring 4 = 81). Weiterhin `_buildStaticTriMeshCollision: Aborted(OOM)` + V9.40-e-Empty-Meldungen. **Wurzel an WASM-Schicht gemessen**: das vendored `ammo.wasm.wasm` ist Emscripten ohne `-s ALLOW_MEMORY_GROWTH=1` gebaut — Memory-Section `initial=max=1024 pages` (64 MB, NICHT growable) UND der C-Allocator ruft `memory.grow()` ohne Build-Flag NIE selbst. V9.40-a..e haben jeden Symptom-Pfad geheilt; die WAHRE Wurzel war die WASM-Konstante, die nie geprüft wurde. **Zwei-Schicht-Heilung**: (1) `scripts/patch-ammo-memory.cjs` patcht den WASM-Memory-Header `max 1024→4096 pages` (256 MB, growable). 2-Byte-LEB128-Edit, idempotent, `npm run patch:ammo` + automatisch via `postinstall`. (2) `vendor/ammo-bootstrap.js` setzt `Module.instantiateWasm`, das die Memory direkt nach `WebAssembly.instantiate` auf 256 MB pre-grow't — BEVOR Emscripten irgendetwas allokiert. **Drei subtile Naht-Fallen geheilt**: (a) CSP `script-src 'self'` blockt inline → externes Script Pflicht; (b) Ammo's IIFE `function(moduleArg = {})` liest NUR sein Argument → `anazhRealm.js` muss `Ammo(window.Module)` rufen statt `Ammo()`; (c) `ArrayBuffer.maxByteLength` ist Snapshot-Wert, der echte Beweis ist `mem.grow()`-Test. **Browser-Stress-Verifikation**: Default Ring 4 (81 Chunks) → **0 OOM-Aborts, 0 empty Chunks**. Extremer Ring 8 (289 Chunks) → OOMs von 716 → 222 (-69 %), empty von 119 → 37 (-69 %). Default-Pfad voll geheilt. **Meta-Lehre**: wer eine Bug-Klasse an Symptom-Ebene 5× heilt und sie nicht weg geht, MUSS unter die Schicht schauen. Hier war's WASM-Memory-Header — drei Klick + ein 6-Byte-Lookup. **V9.40 ist damit vollständig** (a..f). Nächste Welle laut Roadmap: **V9.41** (Trapeze/Linien auf flachen Hügeln, Surface-Nets-Diagonalen).

### Davor: V9.40-e (Y-Bound + Stream-Retry-Symmetrie)

nach V9.40-d funktionierte Edit-Pfad sauber, aber „neu generierte Chunks scheinen teils noch fehlerhaft" + Log `Voxel-Chunk 0,-6: 3× OOM, als empty markiert`. Schöpfer-Hinweis: „du hast nicht das min und max in der höhe begrenzt" — goldrichtig. Drei Heilungen: (1) **`_addVoxelEdit` clampt Y** auf den Chunk-Bereich + verwirft Edits weit außerhalb (vor V9.40-e wurde jedes Y akzeptiert + erzeugte unnötige Re-Meshes über `_remeshVoxelChunksAround`, das Y ignoriert). (2) **`_disposeVoxelChunk` räumt `voxelRebuildAttempts`** mit (Memory-Hygiene). (3) **`_ensureVoxelChunkAt` (Streaming-Pfad) bekommt Retry-Counter** wie der Re-Mesh-Pfad — vor V9.40-e war das eine Asymmetrie: Edit-Chunks bekamen 3 Versuche, Stream-Chunks 1 → der Schöpfer-Befund „neu generierte Chunks fehlerhaft" zeigte direkt darauf. Jetzt: bei Stream-OOM wird der Map-Eintrag NICHT gesetzt, nächster Tick versucht's wieder; nach 3 Fails empty.
...
**Jüngste Welle — V9.39 (Voxel-Phase 5c.2.c.3.b.iii: die letzte grosse Aufräum-Welle der Heightfield-Pipeline)**: nach V9.37 (drei Dispatch-`else`-Pfade) + V9.38 (fünf `else`-Hälften + `calculateTerrainSteepness`) ist V9.39 der Schlussstrich. **Sieben tote Methoden gelöscht**: `pruneDistantChunks`, `_disposeChunkPhysics`, `populateChunkVegetation`, `_buildChunkGrass`+`_disposeChunkGrass`, `_terrainHeightAtWorld`, `_chunkGeometry`, `ensureChunkAt` (~480 Z. Methoden-Code). **Plus** der `state.terrainMaterial`-Setzer + der lokale Material-Builder in `generateTerrainWithParameters` (das Insel-Material teilt sich den Shader-String weiter), der `_buildWaterPlane`-Heightfield-Branch + die SimplexNoise-Instanzen darin, der Game-Loop-`pruneDistantChunks`-Aufruf + der `generateTerrainWithParameters`-Cleanup-Loop. **Plus** der Legacy-`onMouseClick`-Block-Place-Handler (V6-Vorgänger des echten Bau-Systems): sein `[state.groundMesh]`-Fallback crashte mit `null.layers` — der V9.39-Lösch deckte diesen pre-existing Bug auf, der Audit-Strict zeigte 3× Page-Error mit voller Stack-Trace (V9.34-Trust-but-verify-Disziplin). **Plus** `toggleTerrain` voxel-aware (die DSL-Op `set_visible terrain` wirkt jetzt auf `state.voxelChunks` statt dem toten `state.groundChunks`). **Tests**: 25+ Heightfield-Pipeline-Tests gestrichen (V9.29-Disposal, V9.30/V9.34-Regression-Schutzanker, V8.27/V8.28/V8.31/V8.43-V8.48-Terrain-Shader-Tests, V9.27-`voxelMaterialKept`), oder auf Voxel-Pendant umgewidmet (V8.29-chunk-Gras → `state.voxelChunkGrass`/`_buildVoxelChunkGrass`, Welle 6.G P2/P3 → `_populateVoxelChunkVegetation`, Phase 3b Boden-Toggle → `state.voxelChunks`-Iteration). 14 V9.39-Lösch-Beweise neu (sieben Methoden gelöscht + terrainMaterial-null + waterPlane-voxel-only + sechs Vision-Anker leben weiter). ~500 Z. Code netto weg, 2952 → 2931 grün. **Phase 5c.2.c.3 ist mit V9.39 abgeschlossen** — die heightfield-only Chunk-Pipeline ist vollständig zurückgebaut; die Voxel-Welt trägt alles allein. **Lehre**: (a) eine grosse Welle wird in disziplinierten Schritten ausgeführt — zuerst Aufrufer-Pfade säubern, DANN Methoden löschen; wer die Reihenfolge tauscht, hat ein Bug-Reservoir an dangling-Aufrufen; (b) ein Audit-Page-Error mit Stack-Trace ist die EHRLICHE Diagnose — pre-existing Bugs werden durch eine grosse Welle SICHTBAR, V9.31-Lehre wieder bestätigt; (c) eine Test-Umwidmung ist eine Vision-Geste — der Anker wandert auf den neuen Wohnort, die Vision lebt; (d) Source-Scan-Proben matchen auf echte Aufruf-Patterns (`this.X\s*\(`), NICHT auf blossen Methoden-Namen (V9.36-Test-Kommentar-Falle, wieder bestätigt).

**Vor V9.39 — V9.38 (Voxel-Phase 5c.2.c.3.b.ii: fünf weitere tote Heightfield-`else`-Pfade entfernt + `calculateTerrainSteepness` gelöscht)**: Folge zu V9.37 (drei Dispatch-`else`-Pfade weg). Mit V9.35 ist `worldMeta.voxelTerrain` permanent true → fünf weitere `else`-Hälften von voxel-Gates wurden unreachable. V9.30/V9.37-Disziplin angewandt: (1) heightData-Allokations-`else` in `generateTerrainWithParameters` (~46 Z. inkl. SimplexNoise-Init + 65k-Iterationen); (2) 256×256-Wasserfall-Partikel-Loop (~80 Z. — las `heightData[idx]` direkt — eine Voxel-Welt baut Wasserfälle über `_buildVoxelChunkWaterfalls` V9.32 am Voxel-Chunk-Lifecycle); (3) „Neues Terrain generiert"-Log-Gate; (4) Heightfield-Pfad in `getTerrainHeightAt` (~35 Z. inkl. groundHeightField-Index-Lookup + Raycast-Fallback via `_runRaycast`); (5) Heightfield-Pfad in `findSurfaceAbove` (~30 Z. inkl. floatingIslands-Loop). Plus `calculateTerrainSteepness` ganz gelöscht — sein einziger Aufrufer war der Wasserfall-Loop (Punkt 2); `grep -n` bestätigte Null Konsumenten. **Verschlankte Funktionen**: `getTerrainHeightAt` + `findSurfaceAbove` sind jetzt voxel-only mit `terrainBaseHeight || 0`-Fallback für `_voxelSurfaceY`-nicht-finit-Edge-Case. `findSurfaceAbove`s `_currentY`-Parameter bleibt mit Unterstrich-Präfix (33 Aufrufer, deren Signaturen unverändert bleiben). **Tests**: 7 V9.38-Lösch-Beweise neu (5 Lösch-Marker + 2 Regression-Schutzanker für „liefert weiter eine endliche Zahl"); 1 V9.34-Test umgewidmet (caveNoise/volcanoNoise leben jetzt in `ensureChunkAt` + `_buildChunkGrass` statt `generateTerrainWithParameters`, weil der heightData-Loop, der sie konstruierte, weg ist — der Vision-Ziel-Anker „Höhen-Modulatoren leben weiter" hält, der Source-Scan zieht auf die neuen Wohnorte um). ~200 Zeilen Code netto weg, 2945 → 2952 grün. **Lehre**: (a) wenn ein Test-Mechanismus altert, wandert er mit dem Code mit — Vision-Ziel-Anker bleibt, Source-Scan zieht auf die neuen Wohnorte um; (b) ein verschlankter Pfad braucht keinen Behavior-Beweis, nur einen Regression-Schutzanker; (c) ein unused-Parameter mit Unterscheid-Präfix ist die ehrliche Form, wenn die Signatur 33 Aufrufer hat.

**Vor V9.38 — V9.37 (Voxel-Phase 5c.2.c.3.b.i: drei tote Heightfield-Dispatch-Pfade entfernt)**: Folge zu V9.36 (`modify_terrain` weg). V9.35 (Voxel permanent + Zwangs-Migration in `ensureWorldMeta`) hatte drei `if (voxelActive/voxelWorld) ... else { heightfield-pfad }`-Dispatcher per Konstruktion tot gemacht; V9.37 löscht die else-Hälften ehrlich (V9.30-Disziplin „pure Dead-Code-Welle"). (1) Game-Loop-Streaming-Ring (Z. 34005, 4-fach-nested-Loop um den Spieler-Chunk via `ensureChunkAt` mit `RING_RADIUS`/`MAX_PER_FRAME`-Drossel) — der Voxel-Streaming-Ring trägt jeden Spieler-Schritt allein; (2) `generateTerrainWithParameters` initial 64-Chunk-Loop (Z. 13573) — `CHUNKS_X`/`CHUNKS_Z`-Konstanten fallen mit (ungenutzt nach Lösch); (3) `populateChunkVegetation`-Initial-Loop Heightfield-Pass (Z. 13978) — Vegetation streamt jetzt nur noch am Voxel-Chunk-Lifecycle (`_populateVoxelChunkVegetation` in `_ensureVoxelChunkAt`, V9.24-Verdrahtung). **Die Pipeline-Methoden selbst bleiben** (`ensureChunkAt`, `populateChunkVegetation`, `pruneDistantChunks`, `_disposeChunkPhysics`) — die V9.29/V8.27/V8.28-Tests rufen sie noch direkt via `ensureChunkAt(40, 40)`/`(50, 50)`/`(60, 60)`, ein ehrlicher 5c.2.c.3.b.ii muss diese Test-Anker umwidmen, bevor die Methoden selbst sterben können. **Tests**: 4 stale Heightfield-Regression-Tests gestrichen (V9.27 „eine Heightfield-Welt baut alle 64 Chunks weiter" + V9.28 zwei Tests „heightData/minHeight gefüllt" — sie schützten einen Pfad, der seit V9.35 unreachable war); 1 V9.37-Lösch-Beweis neu (auch `voxelTerrain = false`-Mutation in Tests baut keine Heightfield-Chunks mehr). ~50 Z. Code netto weg, 2956 → 2945 grün. **Lehre**: (a) eine grosse Welle in ehrlich kleinste Schritte zerlegen — 5c.2.c.3.b ist „heightfield-Pipeline ablösen", insgesamt ~500 Z. Code in 26 Stellen + 4 lebende Pipeline-Methoden; der ehrlich-kleinste Schritt schneidet zunächst nur die toten Dispatch-Pfade (~50 Z.) ab. Mehrere kleine Sub-Wellen statt einer grossen halb-fertigen. (b) Eine tote `else`-Hälfte ist die billigste Dead-Code-Welle — der `if`-Gate war konstant-true seit V9.35, die else-Hälfte unreachable. Es braucht keinen Beweis, dass Totes tot ist; nur den Mut zum Löschen. (c) Ein Regression-Schutz-Test für eine unreachable Schicht ist Investition in Sicherheit, die niemand bekommt — ehrlich streichen, nicht heilen.

**Vor V9.37 — V9.36 (Voxel-Phase 5c.2.c.3.a: `modify_terrain` stirbt)**: Folge zu V9.35 (Toggle-Tod). Die ganze Ring-10.5-Heightfield-Welt-Mod-Pipeline (V7.68-Vision: pro-Chunk-DSL-Delta, Spieler-Mods überleben Reload) ist abgelöst — ~289 Zeilen netto weg, ersetzt durch zwei schlanke broadcastable DSL-Ops im 3D-Voxel-Feld (V9.14/V9.15). Sieben Sub-Schritte: (1) zwei neue DSL-Ops `voxel_carve(x, y, z, r)` + `voxel_fill(x, y, z, r)` (broadcastable wie spawn-Ops, NICHT im `dslComposeAtomic`-Pool — Nexus-Disziplin). (2) Chat-Patterns `grabe loch`/`hebe hügel` umgeroutet (rufen die neuen Ops mit Spieler-Position embedded). (3) `tryMouseBreak`-`else if (modify_terrain)`-Fallback weg (war seit V9.35 toter Pfad — `voxelTerrainActive` permanent true). (4) Sechs Helpers gelöscht (~150 Z.): `_chunksTouchedByDisc`, `_appendChunkDeltaOp`, `_sanitizeChunkDeltas`, `_applyModifyOpToChunk`, `_rebuildChunkPhysics`, `applyChunkDelta`. (5) `chunkDeltas`-Feld + `CHUNK_DELTA_OPS_CAP`-Konstante gelöscht (alte Saves mit dem Feld werden defensiv ignoriert, KEIN Migrations-Pfad — Heightfield-Höhen-Deltas haben in einer Voxel-Welt keinen Sinn). (6) `describeProgram` aktualisiert. (7) CREATURE_PROPOSED_OPS-Whitelist: `modify_terrain` raus, `voxel_carve`+`voxel_fill` rein. **Tests**: 31 alte Ring-10.5-Checks sterben (state-Feld, alle 6 Helpers, DSL-Op, FIFO-Cap, Sanitize, Replay-Mechanik), 22 V9.36-Lösch-Beweise neu (Methoden gelöscht, neue Ops registriert + funktional, Chat-Pattern-Umroutung, Edits landen in `voxelEdits`, NaN-Validation, CREATURE_PROPOSED_OPS, tryMouseBreak-Kommentar-frei). Plus ~7 umgewidmet: Ring 11 V2 „Remote modify_terrain → chunkDeltas wachsen" → „Remote voxel_carve → voxelEdits wachsen" (Vision-Punkt unverändert, Schicht gewechselt); V8.36 Heightfield-Höhen-Clamp-Test → „_applyModifyOpToChunk gelöscht (obsolet)"; 6.H P2E V3 Whitelist `modify_terrain` → `voxel_carve`. 2956 → 2947 grün. **Lehre**: (a) eine ehrlich geschnittene grosse Welle ersetzt ein ganzes Subsystem durch ein schlankeres + erbt seinen Wert (Ring 10.5 trug eine substanzielle Vision — Welt-Mods überleben Reload, sind multi-user-synchron — V9.36 trägt sie ins Voxel-Feld + verschlankt sie radikal: 6 Helpers + 1 DSL-Op + 1 Feld + 1 Konstante → 2 DSL-Ops + ein schon-existierendes `voxelEdits`-Feld). (b) Ein neuer broadcastable DSL-Op ist eine bewusste Vision-Wahl — Ring 11 V2 lehrte, dass geteilte Welten geteilte Welt-Mods haben sollen; `voxel_carve`/`voxel_fill` werden darum NICHT in NON_BROADCASTABLE_OPS gepackt. (c) Test-Kommentar-Falle — ein `breakSrc.toString().test(/modify_terrain/)`-Test fängt auch Kommentare; entweder String-Literale prüfen oder den Kommentar des betroffenen Quellcodes reinigen. (d) Alte Saves verlieren ihre `chunkDeltas` still — KEIN Migrations-Pfad nötig: die Daten waren Heightfield-Höhen-Modifikationen, die in einer Voxel-Welt keinen Sinn haben. Stille ist die ehrliche Form, wenn „nichts" die richtige Übersetzung ist.

**Vor V9.36 — V9.35 (Voxel-Phase 5c.2.c.2: der Toggle-Tod)**: Voxel ist **permanent + irreversibel**. Drei Methoden gelöscht (`setVoxelTerrainActive`, `_setHeightfieldDormant`, `_restoreVoxelTerrain`), Zwangs-Migration für alte V9.23-Heightfield-Opt-out-Welten, V9.23-Checkbox + `createNewWorld`s `voxelTerrain`-Parameter entfernt. ~75 Zeilen Reversibilitäts-Code weg.

**Vor V9.35 — V9.34 (Voxel-Phase 5c.2.c.1: drei tote Heightfield-Schichten gelöscht)**: der erste ehrliche Schritt der grossen Phase 5c.2.c (Heightfield-Code-Entfernung). Per V9.30-Disziplin (pure Dead-Code, keine Verhaltens-Änderung): nur Schichten, die NACHWEISLICH keinen Aufrufer/Leser haben. (1) `extendTerrain` (Legacy-direction-API, ~38 Z.) — NULL Aufrufer; (2) `caveData`-Float-Array (256×256, ~128 KB) — 2× geschrieben, NIE gelesen; (3) `volcanoData` — dieselbe Klasse. **caveNoise + volcanoNoise (die SimplexNoise-Instanzen) leben weiter** — sie modulieren die Höhe in `_terrainHeightAtWorld` (echte Konsumenten). +6 Invarianten. **Lehre**: eine grosse Welle wird ehrlich in ihre kleinsten Schritte zerlegt, BEVOR der erste Code-Block fällt; ein Versprechen aus einem Kommentar IST eine Welle in Wartestellung; Trust-but-verify am echten Source, nicht am Audit-Bericht.

**Vor V9.34 — V9.33 (Voxel-Phase 5c.2.b: der Eingangs-Welt-Flip)**: V9.31 nannte das Versprechen — „der Flip (1 Zeile in `ensureWorldMeta`) + die ~25 heightfield-spezifischen Test-Heilungen, eine eigene saubere Welle". V9.33 liefert beides plus fünf pre-existing Bugs, die der Flip ans Licht zog. Die fresh-Eingangs-Welt ist jetzt voxel-default. Ein Commit, 5/5 Playtest-Runs grün, Audit-Strict 0 Failures. Sieben surgische Heilungen: (1) `ensureWorldMeta` ohne `!fresh`-Gate — fresh-Welten erben den Voxel-Default; (2) `createNewWorld` setzt `voxelTerrain` EXPLIZIT (sonst trägt der Spread der laufenden Voxel-Welt den Wert in eine Opt-out-Welt — die V8.59-Spread-Lehre wieder angewandt); (3) `chunkMap.clear()` in `generateTerrainWithParameters`-Cleanup (pre-existing Bug seit `ensureChunkAt`-Geburt — `groundChunks=[]` leerte das Array aber nicht die Map); (4) `generateNewWorld` synchronisiert `voxelTerrainActive` mit `worldMeta.voxelTerrain` (sonst akkumulierten Voxel-Chunks bei Test-Welt-Regens, OOMs durch die Decke); (5) `_buildStaticTriMeshCollision` mit Partial-Build-Cleanup im catch+finally-Pfad (vorher leakten partial-OOMs ihren Heap → Snowball); (6) `state.chunkGrass = new Map()` explizit in init (Audit-Strict-Heilung — voxel skippt Heightfield-Lazy-Init); (7) ~7 Playtest-Tests voxel-aware (Pre-P1-Reset, 6.A3-Cache invalidieren, 6.A4 Tempel auf y=200, W10 natürliche Strahler cleanup, 6.G3.b weather-200s-Offset). **Lehre**: ein Eingangs-Welt-Flip macht pre-existing Bugs sichtbar — V9.31 ahnte das („welche Schichten hat sie SICHTBAR gemacht?"); V9.33 zieht ALLE Folgen ehrlich nach. Ein partial-allocation-cleanup-Pattern ist die Ammo-Disziplin schlechthin — wer eine Sequenz von new-Allokationen macht, MUSS sie in einem trackbaren Array sammeln + im finally-Block ALLE destroyed.

**Vor V9.33 — V9.32 (Voxel-Phase 5d-Mini: Wasserfälle aus der Voxel-Klippen-Steilheit)**: eine voxel-basierte Welt hatte seit V9.28 keine Wasserfälle (der heightfield-256×256-Loop war voxel-gegated, da er `heightData[idx]` direkt las). V9.32 baut den Voxel-Pendant: **`_buildVoxelChunkWaterfalls(cx, cz)`** sampelt pro Voxel-Chunk 6×6 Stellen, misst die Steilheit via `_voxelSurfaceY`-Gradient zu vier Nachbar-Punkten (4 m Distanz), spawnt einen Partikel-Wasserfall bei ≥4 m Höhen-Drop mit ~12 % chunk-key-RNG-Wahrscheinlichkeit. Die Partikel-Schicht ist Zeile für Zeile die Heightfield-Variante (`THREE.Points` + `velocity`-Attribut + `userData.{baseHeight, minY}`) — sie landet in `state.waterfalls`, der bestehende Animations-Loop animiert sie geschenkt. Pro-Chunk-Lifecycle via `state.voxelChunkWaterfalls`-Map analog Gras (V9.22) + Vegetation (V9.24) — am `_ensureVoxelChunkAt`/`_disposeVoxelChunk`-Lifecycle aufgehängt, deterministisch, idempotent. Vierte Voxel-Chunk-Schicht in derselben Spawn-Sprache. +6 Invarianten. **Lehre**: eine V9.28-ehrlich-benannte Lücke wird zu einer ehrlich-kleinen Mini-Welle, wenn man die Spawn-Sprache aus dem bestehenden Pattern zieht (`_buildVoxelChunkGrass` + `_populateVoxelChunkVegetation` lehrten schon, wie man am Voxel-Chunk-Lifecycle eine Schicht hängt). Wer eine etablierte Sprache hat, baut die nächste Schicht in einer halben Welle.

**Vor V9.32 — V9.31 (Vorbereitung 5c.2.b — Ammo-motionState-Leck-Heilung + voxel-aware Boden-Selbstanalyse)**: der erste Versuch des Eingangs-Welt-Flips (5c.2.b) deckte ZWEI vorbestehende Bugs auf, die ihn blockierten. (1) `_buildStaticTriMeshCollision` (Inseln + Voxel-Chunks + Voxel-Test) erzeugte einen `btDefaultMotionState`, der NIE destroyed wurde — V8.26-§6.4-Muster + V9.29-`_disposeChunkPhysics`-Disziplin auf statische Tri-Mesh-Kollisionen NICHT angewandt; mit 81 Voxel-Chunks initial × Test-Welt-Regens summierte sich das zum OOM-Crash (`_buildStaticTriMeshCollision: Aborted(OOM)`). V9.31 speichert motionState in `userData.collision` + räumt ihn im Dispose. (2) `selfAwarenessAnalyze` + Game-Loop-Boden-Check prüften NUR `groundChunks.length === 0` — in einer Voxel-Welt LEGITIM leer (V9.27-Skip); der Check triggerte sonst eine Welt-Regen-Death-Spiral. Beide Checks jetzt voxel-aware. +3 Invarianten. **Phase 5c.2.b (Eingangs-Welt-Flip + ~25 Test-Migrationen) bleibt benannt** für eine eigene saubere Welle, jetzt mit reduziertem Heap-Druck als Voraussetzung. **Lehre**: die erste Schätzung einer grossen Welle kann mit einem unentdeckten vorbestehenden Bug kollidieren — der Eingangs-Welt-Flip schien straight-forward, aber 22× OOM zeigte den seit V9.08 latenten motionState-Leck. Wer eine grosse Welle wagt + sie zerbricht, soll fragen: welche vorbestehenden Schichten hat sie SICHTBAR gemacht? Diese sind die ehrlich-kleinen Vorwellen.

**Vor V9.31 — V9.30 (Phase 5c.2.a — Heightfield-Code-Entfernung Schritt 1: tote Methoden)**: die Schöpfer-Wahl Phase 5c.2 (Heightfield-Code-Entfernung) in 5-10 ehrliche Schritte zerlegt (V9.27-Lehre); V9.30 ist der kleinste mögliche erste Schritt. **`generateChunk`** (~97 Zeilen) und **`addTerrainPhysics`** (~104 Zeilen) sind als nachweislich tote Methoden gelöscht — der V9.28-Eintrag hatte sie ausdrücklich so benannt; das Versprechen aus dem Kommentar bei Zeile 13549 („werden in einem späteren Cleanup entfernt") ist erfüllt. ~200 Zeilen Heightfield-Altcode verschwunden, NULL Verhaltens-Änderung. +5 Invarianten (Methoden gelöscht + 3 Regression-Schutzanker für `ensureChunkAt`/`_terrainHeightAtWorld`/`_setHeightfieldDormant`). **Lehre**: ein Versprechen aus einem Kommentar IST eine Welle in Wartestellung — `grep -n "wird in einem späteren\|TODO\|FIXME\|obsolet"` durchgehen findet die wartenden ehrlichen kleinen Wellen.

**Vor V9.30 — V9.29 (Ammo-Heap-Leck-Fix der Heightfield-Chunks + Gotcha-Sektion-Hygiene)**: der in V9.28 entdeckte pre-existing Bug ehrlich geheilt. `Ammo.destroy(body)` cascadiert NICHT zu seinen `btBvhTriangleMeshShape`/`btDefaultMotionState`/`btTriangleMesh`-Auxiliars. V9.29 zentralisiert den Cleanup in **`_disposeChunkPhysics(mesh)`** — drei Konsumenten teilen ihn (Welt-Regen, Streaming-Prune, Ring 10.5 terrain-Mod). Build-Pfade legen jetzt `physicsShape`+`physicsMotionState` in userData ab. **Lehre**: ich verletzte in V9.28 die V8.65/V9.27-Meta-Lehre („dokumentierte Lektion schützt nur, wenn aktiv abgerufen"). Disziplin: vor JEDER Ammo-Allokations-Welle `grep -n 'new Ammo\.'` rufen, ZUERST den Dispose-Helper bauen, DANN den Body.

**Vor V9.29 — V9.28 (Phase 5c.1+ — V9.25 Phase 5b ehrlich abgeschlossen + heightData-Skip)**: zwei surgische Schnitte. (1) `updateCreatures` war der letzte missed Höhen-Konsument aus V9.25 — es las `state.groundHeightField` direkt am voxel-aware `getTerrainHeightAt` vorbei; jetzt route auch dieser Pfad voxel-aware. (2) Nach (1) liest in einer Voxel-Welt niemand mehr `groundHeightField`/`minHeight`/`maxHeight` → die 256×256×3-Float-Allokation (~768 KB) wird komplett übersprungen. Pre-existing Ammo-Heap-Leck der btBvhTriangleMeshShape-Auxiliars entdeckt (V9.29-Fix).

**Vor V9.28 — V9.27 (Voxel-Terrain-Bogen Phase 5c.1: Heightfield-Init für Voxel-Welten übersprungen)**: der erste ehrliche Schritt der grossen 5c-Aufräumarbeit. Eine voxel-basierte Welt baut die initialen 64 Heightfield-Chunks GAR NICHT mehr. Davor V9.26 (Phase 5c-Start — Migrations-Flip + Chunk-Loch-Heilung), V9.25 (Phase 5b — Konsumenten voxel-aware), V9.24 (Verbindungen geheilt — Sicht-Ring + Vegetation), V9.23 (Voxel als Default für neue Welten), V9.22 (Voxel-Terrain grünt — Instanced-Gras), V9.21 (Voxel-Welt-Geburt), V9.20 (Grösse + Hierarchie), V9.19 (Phase 4 Politur), V9.18 (Phase 4 — Höhlen/Überhänge).

**Nächste Welle: offen.** Phase 5c.2.c.3.b.ii ist mit V9.38 erledigt (fünf weitere tote `else`-Pfade weg + `calculateTerrainSteepness` weg). Übrig in der grossen 5c.2.c-Aufräumarbeit: **5c.2.c.3.b.iii** (die Pipeline-Methoden selbst — `ensureChunkAt`, `populateChunkVegetation`, `_buildChunkGrass`, `_terrainHeightAtWorld`, `_disposeChunkPhysics`, `pruneDistantChunks`; verlangt vorher Test-Migration der V9.29/V8.27/V8.28-Tests auf Voxel-Material-Probe — sie rufen die Methoden heute direkt via `ensureChunkAt(40, 40)`/etc.). Plus der `terrainMaterial`-ShaderMaterial-Bau in `generateTerrainWithParameters` lebt weiter — er wird vom `ensureChunkAt`-Pfad gelesen + von den V8.27-Cel-Stufen-Regler-Tests verifiziert; er stirbt mit 5c.2.c.3.b.iii zusammen. Daneben Phase 5d-vollständig (Wasser-/Schluchten-Drama-Schicht) oder eine andere Vision-Richtung. Davor — historische 5c.2.c-Aufräum-Substanz: **Lies `CLAUDE.md` V9.38 ZUERST.** Disziplin: parallel bauen + beweisen, nie das Funktionierende brechen, ehrliche kleine Schritte vor grossen Würfen.
