...
## Schnell-Lage (Stand 19.05.2026, V9.19)

**Du erbst eine sehr lebendige Welt**. **~2910 Playtest-Invarianten grün + 0 Audit-Strict-Failures**, ~33000 Zeilen in einer Datei, alles produktiv. (Der Playtest-Konsolen-Zähler driftet ±2-3 je Lauf — einige Checks sind bedingt; „Alle Invarianten OK" ist die Wahrheit, nicht die exakte Zahl.)

**Jüngste Welle — V9.19 (Voxel-Terrain-Bogen Phase 4 Politur)**: zwei Schöpfer-Browser-Befunde nach V9.18 geheilt. (1) „Die Berge wurden flacher, nur noch Hügel, keine Überhänge" — V9.18 hatte beim Phase-4-Umbau das feine Roughness-Band (`noise3D(x*0.05) × 7`) still mitentfernt; V9.19 stellt beide V9.17-Oberflächen-Bänder wörtlich wieder her — die Oberfläche ist Zeile für Zeile die von V9.17, das V9.18-Überhang-Band ist weg (die zwei vollen Bänder erzeugen Überhänge ohnehin). (2) „Höhlen viele kleine Spalten, nicht zugänglich" — V9.18 carvte mit dem Produkt zweier ridged-Felder, ihr Schnitt zerfiel in verstreute Flecken; V9.19 nutzt EIN ridged-Noise-Feld (`ridge = 1−|noise3D|`), dessen Grat der Noise-Nullfläche folgt → eine zusammenhängende, begehbare Kavern-Ebene. Die Tiefen-Hüllkurve bleibt (Höhlen zwischen `surf-6` und `base-28`, `base-35` beweisbar fest). Plus: die `index.html`-Versions-Marker auf v9.19 gezogen (V9.18 vergass sie). Invarianten-Zahl unverändert. Davor V9.18 (Phase 4 — Höhlen/Überhänge in der Generierung), V9.17 (Material-Erhaltungs-Kreis), V9.16 (Dichte-Gradienten-Normalen), V9.15 (Aufschütten), V9.14 (3D-Graben), V9.13 (per-Welt-Persistenz), V9.12 (72-m-Chunk-Säule), V9.11 (Naht-Wurzelheilung), V9.10 (Naht-Skirt + Welt-Feld-Farbe).

**Nächste Welle: Phase 5 — Materialien/Shader + die Ablösung des Heightfields** (`aField` per Vertex auf dem Voxel-Mesh, Terrain-Material-Harvest, Cel-Shading; das Heightfield wird abgelöst, alte Welten migrieren). Der volle Plan steht in `docs/roadmap.md` „Der Voxel-Terrain-Bogen". Disziplin: parallel bauen + beweisen, dann das Heightfield ablösen — nie das Funktionierende brechen. **Lies `CLAUDE.md` V9.19 ZUERST.**
