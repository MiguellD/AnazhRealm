...
## Schnell-Lage (Stand 21.05.2026, V9.44-e)
...
**Empfehlung für den nächsten Agenten**: die nächste Welle ist **V9.44-f — der Game-Loop bekommt Phasen-Struktur (Stamm-Pflege-Bogen Sub-Welle 6/6, die letzte)**. V9.44-a/b/c/d/e sind erledigt (a: Persistenz-Schema; b: `_applyPlayerSpeed`; c: Mesh-Router-Dispatch-Table; d: Infrastruktur-Konstanten + getrennter Flaky-Heilungs-Commit; e: `_renderWorkshopDOM` → Orchestrator + 8 `_workshopRender<X>`-Methoden + `_wireCmdDelegation`). Vor dem ersten Code-Block **`docs/code-hygiene.md`** ganz lesen (Befund, Vision-Abgleich — KEIN neues Modul, kein State-Manager; sechs Sub-Wellen a-f; bewusste Nicht-Wellen). V9.44-f zerlegt `startEternalLoop` (~696 Z.) — der Loop-Body ist eine einzige Closure mit ~660 Z. Inline-Logik (Physik-Sync, Bewegung, Sprung, Kamera-Kollision, Voxel-Streaming, Killplane). Die Inline-Blöcke werden benannte Phasen-Methoden (`_loopPhysicsSync`/`_loopPlayerMovement`/`_loopCamera`/…); die ~25 schon existierenden `tick*/update*`-Aufrufe bleiben. **Herausforderung**: der Loop-Body teilt lokale Variablen (`delta`, `currentTime`, `playerPos`) über die Blöcke — die müssen explizite Parameter werden. Vor jeder Extraktion prüfen: welche lokalen Werte fliessen rein/raus (anders als bei V9.44-e, wo die UI-Sektionen NICHT-geteilt waren). **Browser-Test verpflichtend** — der Loop ist die heisseste Stelle, ein Refactoring-Bug dort zeigt sich nur im echten Lauf. Disziplin: reines verhaltensneutrales Refactoring, die ~2974 bestehenden Invarianten sind der Beweis (V9.30-Disziplin — keine neuen Invarianten nötig, nur alle alten grün + Audit-Strict 0). Mit V9.44-f ist der Stamm-Pflege-Bogen vollständig. **Erst NACH dem Stamm-Pflege-Bogen** folgt die Hydrosphäre — V9.43-b der Hydrosphären-Atlas (`_computeHydrosphere()`: Surface-Sampling → Priority-Flood → D8-Flow → Flow-Accumulation → Netz-Extraktion), dann V9.43-c/d/e; `docs/hydrosphere.md` bleibt der Plan-Anker dafür. Bekannter Edge-Case aus V9.42-d: **5 Vertices an Vier-Chunk-Ecken** mit ~1 m Rest-Naht-Versatz — falls der Schöpfer sie sieht, eine kleine V9.42-e davorziehen, sonst ignorierbar. Detail in `docs/roadmap.md` §3 „Der Stamm-Pflege-Bogen" + `docs/code-hygiene.md`.
