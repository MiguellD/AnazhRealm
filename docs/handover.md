...
**Die grossen Roadmap-Ringe sind gebaut**, und der echte Fremd-Engine-Bogen läuft: der KI-Übersetzer ist vollständig (V8.68/V8.69), **V8.70 öffnete das Untrusted-Welt-Tor** (eine echte fremde Engine läuft null-origin sandgesichert), **V8.71 + V8.72 bauten den Auto-Vendor-Pfad komplett** (ein fremdes Bündel ODER ein GitHub-Repo dockt sandgesichert an), **V8.73 + V8.74 die Mesh-Welt-Verteilung komplett** (eine vendorte Welt reist peer-to-peer + die Mesh-Bibliothek ist browsbar). Offen in diesem Bogen: nur noch die Multiplayer-Sub-Welten (W17). Der aktuelle Stand steht im Block „Aktuelle Roadmap" weiter unten und in `docs/roadmap.md` §3.
...
| **W17 — Multiplayer-Sub-Welten** | Der Transport-Shim (`WebSocket`/`fetch` der fremden Welt → AnazhRealms Mesh) + das Mesh-als-Server + das Gruppen-Portal. So taucht eine Gruppe gemeinsam in eine Server-Welt — auch voxelize/Shooter. | ~6-8 Sessions, mehrere Phasen | sehr hoch |

**Empfehlung**: **W17 — Multiplayer-Sub-Welten** — die letzte grosse Schicht des Fremd-Engine-Bogens. Der Bogen hat mit W12-W16 alles: das Portal trägt fremde Engines, der Avatar ist eine souveräne Identität, die Bibliothek steht, ein fremdes Repo dockt automatisch an, und eine vendorte Welt reist peer-to-peer + ist browsbar. W17 macht das Tor mehrspielerfähig: ein **Transport-Shim** überschreibt `WebSocket`/`fetch`/`RTCPeerConnection` der fremden Welt-`index.html` (BEVOR ihr Code läuft), sodass ihr Verkehr durch AnazhRealms Mesh fliesst statt zu einem echten Server — ein null-origin-iframe braucht dafür GAR KEINE Netz-Berechtigung (`postMessage` quert die Sandbox-Grenze ohne CSP). Dann das **Mesh-als-Server** (Relay-Welt: das Mesh broadcastet; JS-Compute-Welt: ein Peer wird Compute-Host) + das **Gruppen-Portal** (öffnet einer ein Tor, bekommen die anderen einen „mitkommen?"-Prompt). W17 ist mehrere Phasen (A Shim → B-Relay → C Gruppen-Portal → B-JS-Compute → B-WASM) — jede eine eigene, browser-verifizierte Welle; schneide Phase A sauber zuerst. `docs/roadmap.md` §3 („W17 — Multiplayer-Sub-Welten: der Transport-Shim + das Mesh-als-Server") + `docs/world-portal.md` ZUERST lesen.
