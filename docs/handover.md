...
## Schnell-Lage (Stand 19.05.2026, V9.22)
...
**Jüngste Welle — V9.22 (Voxel-Terrain grünt)**: eine voxel-basierte Welt war kahl — `_setHeightfieldDormant` legt das Heightfield-Gras schlafen, der Voxel-Boden trug keines. V9.22 gibt dem Voxel-Terrain eigenes Instanced-Gras: `_buildVoxelChunkGrass(cx,cz)` spiegelt das Heightfield-`_buildChunkGrass` (16×16-Raster, Gras-Dichte aus `worldFieldAt.lebendig`), jeder Halm sitzt auf der Voxel-Oberfläche (`_voxelSurfaceY` — die oberste Fest/Luft-Grenze, je Sample-Zelle einmal gescannt). Am Voxel-Chunk-Lifecycle aufgehängt (`_ensureVoxelChunkAt` baut, `_disposeVoxelChunk` räumt) → das Gras lebt exakt so lange wie sein Chunk. +5 Invarianten. Davor V9.21 (Voxel-Welt-Geburt — Checkbox im Neue-Welt-Dialog), V9.20 (Grösse + Hierarchie — drei Oktaven, 122-m-Chunk-Säule), V9.19 (Phase 4 Politur), V9.18 (Phase 4 — Höhlen/Überhänge), V9.17 (Material-Erhaltungs-Kreis), V9.16 (Dichte-Gradienten-Normalen), V9.15 (Aufschütten), V9.14 (3D-Graben), V9.13 (per-Welt-Persistenz).

**Nächste Welle: Phase 5 — Materialien/Shader + die Ablösung des Heightfields** (`aField` per Vertex auf dem Voxel-Mesh, Terrain-Material-Harvest, Cel-Shading; das Heightfield wird abgelöst, alte Welten migrieren). Der volle Plan steht in `docs/roadmap.md` „Der Voxel-Terrain-Bogen". Disziplin: parallel bauen + beweisen, dann das Heightfield ablösen — nie das Funktionierende brechen. **Lies `CLAUDE.md` V9.22 ZUERST.**
