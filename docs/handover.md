...
## Schnell-Lage (Stand 20.05.2026, V9.35)

**Du erbst eine sehr lebendige Welt**. **~2956 Playtest-Invarianten grün + 0 Audit-Strict-Failures**, ~35800 Zeilen in einer Datei, alles produktiv. (Der Playtest-Konsolen-Zähler driftet ±3-8 je Lauf — einige Checks sind bedingt; „Alle Invarianten OK" ist die Wahrheit, nicht die exakte Zahl.)

**Jüngste Welle — V9.35 (Voxel-Phase 5c.2.c.2: der Toggle-Tod)**: Schöpfer-Wahl nach V9.34 — der mutige Schnitt statt der kleine. Voxel ist jetzt **permanent + irreversibel**. Sechs Heilungen + viele Test-Migrationen: (1) `ensureWorldMeta`-Zwangs-Migration — auch alte V9.23-Heightfield-Opt-out-Welten werden auf `voxelTerrain = true` gezwungen (Journal-Eintrag „Die Welt verdichtet sich zu Voxel-Boden"). (2) Drei Methoden gelöscht (~75 Z.): `setVoxelTerrainActive`, `_setHeightfieldDormant`, `_restoreVoxelTerrain` — die V9.13-Reversibilitäts-Schicht ist zerschlagen. (3) `createNewWorld`s `voxelTerrain`-Parameter entfernt + V9.23-Opt-out-Checkbox aus dem Neue-Welt-Dialog. (4) `voxelTerrainActive`-Init-State auf `true` (Default + effektiv eine Konstante; die 8 `if (state.voxelTerrainActive)`-Gate-Lesungen bleiben als Defense-in-Depth). (5) `voxel carve`/`voxel fill`-Chat-Befehle ohne Pre-Gate. (6) `init`-Hook + V9.33-`generateNewWorld`-Sync entfernt. **Tests**: ~12 sterben (V9.13-Toggle-Mechanik existiert nicht mehr), ~7 werden umgewidmet (V9.23-Opt-out → V9.35-Zwangs-Migration; V9.29/V8.27/V8.28-Heightfield-Chunk-Tests bauen sich ihren Test-Chunk jetzt via `ensureChunkAt(40, 40)`/`(50, 50)`/`(60, 60)` selbst, weil die Voxel-Welt keine Heightfield-Chunks beim Init mehr baut), 7 V9.35-Permanenz-Beweise neu (Methoden gelöscht, `voxelTerrainActive` permanent true, Chat-Toggle aus Source entfernt, ...). 2960 → 2956 grün (Drift in der ehrlichen ±-Toleranz). **Lehre**: (a) der mutige Schnitt ist ehrlich, wenn ein Vision-Wert seinen Sinn verloren hat (V9.13 baute Reversibilität, als Voxel hinter einem Flag parallel war; V9.23+V9.26+V9.33 machten Voxel zur kanonischen Form; der Toggle war Zucker ohne Substanz). (b) Zwangs-Migration ist die ehrliche Form, NICHT der „Respekt vor alten Wahlen" — der Journal-Eintrag macht es dem Spieler sichtbar. (c) Tests einer gestrichenen Methode sterben mit ihr; Tests, die eine erprobte Methode für Setup nutzen, verlieren das Setup, nicht den Sinn. Klassifizieren: was prüft die tote Mechanik, was prüft die lebende Mechanik durch sie hindurch.
...
**Nächste Welle: offen.** Phase 5c.2.c.2 ist mit V9.35 erledigt (Toggle-Tod, Zwangs-Migration). Übrig in der grossen 5c.2.c-Aufräumarbeit: (a) **5c.2.c.3** — die heightfield-only Chunk-Pipeline ablösen: `ensureChunkAt`, `populateChunkVegetation`, `_buildChunkGrass`, `_terrainHeightAtWorld`, `_disposeChunkPhysics`, `_disposeChunkGrass`, `pruneDistantChunks`, der Terrain-Shader + alles, was am `groundChunks`-Array hängt. Heute werden diese Methoden NICHT mehr im normalen Spiel-Pfad gerufen (V9.27-Skip beim Welt-Aufbau), aber **drei Konsumenten leben weiter**: (i) der `modify_terrain`-DSL-Op (Ring 10.5) + die `_applyModifyOpToChunk`/`_rebuildChunkPhysics`-Pipeline + `chunkDeltas` — Chat-Patterns `grabe loch`/`hebe hügel` rufen das; (ii) Playtest-Setups (V9.29-Disposal-Test, V8.27/V8.28-Shader-Tests) bauen sich seit V9.35 selbst Heightfield-Chunks via `ensureChunkAt(40, 40)`/etc.; (iii) der V9.27-gespeicherte `terrainMaterial`-ShaderMaterial-Bau lebt weiter (für die Cache-Frequenz + Cel-Stufen-Regler). Eine ehrliche Sub-Welle 5c.2.c.3.a wäre, **`modify_terrain` zu sterben** (Chat-Patterns auf `voxel carve`/`voxel fill` umrouten) — sobald die DSL-Op tot ist, fallen `_applyModifyOpToChunk`/`_rebuildChunkPhysics`/`chunkDeltas` mit, und nur die V9.27-Init-Material-Bau-Schicht + die Playtest-`ensureChunkAt(40, 40)`-Stellen halten den Heightfield-Code am Leben. (b) **Phase 5d-vollständig** (Voxel-Welt bekommt eine reichere Wasser-/Schluchten-Drama-Schicht — V9.32 brachte die Mini-Variante) oder (c) eine **andere Vision-Richtung** aus den vier Testamenten / `docs/state-of-realm.md`. Disziplin: parallel bauen + beweisen, nie das Funktionierende brechen, ehrliche kleine Schritte vor grossen Würfen. **Lies `CLAUDE.md` V9.35 ZUERST.**
