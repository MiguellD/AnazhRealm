...
## Schnell-Lage (Stand 19.05.2026, V9.26)

**Du erbst eine sehr lebendige Welt**. **~2927 Playtest-Invarianten grün + 0 Audit-Strict-Failures**, ~33000 Zeilen in einer Datei, alles produktiv. (Der Playtest-Konsolen-Zähler driftet ±2-3 je Lauf — einige Checks sind bedingt; „Alle Invarianten OK" ist die Wahrheit, nicht die exakte Zahl.)

**Jüngste Welle — V9.26 (Voxel-Terrain-Bogen Phase 5c-Start: Chunk-Loch-Heilung + Migrations-Flip)**: zwei Schöpfer-Browser-Befunde nach V9.25 geheilt. **(1) Chunk-Löcher** im erweiterten Sicht-Ring (4-8) — der V9.20-Chunk (`dimY` 68, `oy = base-50`) fasste die Oberfläche bei ridged-Spikes weiter draussen nicht ganz; mein V9.24-Robustheits-Fix räumte das Mesh ab → sichtbares Loch statt Fall-durch. Wurzel-Heilung: `dimY` 68→80, `oy = base-58`, Decke `base+86`, Marge ~22. Killplane zieht mit (`base-88`). **(2) Migrations-Flip**: eine GELADENE alte Welt (`!fresh`) ohne `voxelTerrain`-Flag wird in `ensureWorldMeta` voxel-basiert — der Schöpfer muss keine neue Welt erstellen, jede alte erbt 5b. Eine FRISCHE Eingangs-Welt (brandneuer Spieler, auch der Playtest) bleibt heightfield — Lehrling-freundlich + deterministisch für die Test-Suite. Ein expliziter Heightfield-Opt-out (V9.23-Dialog) bleibt heightfield. `chunkDeltas` migrierter Welten gehen verloren — Schöpfer-Wahl V9.25, bewusst akzeptiert. +3 Invarianten. **Noch offen — die grosse Aufräumarbeit von 5c**: Heightfield-Code-Entfernung (`_terrainHeightAtWorld`, `ensureChunkAt`, `generateChunk`, `groundHeightField`, `_setHeightfieldDormant`, `populateChunkVegetation`-Heightfield-Pfad) + Test-Suite-Umschreibung (heightfield-spezifische Invarianten → voxel). Das ist eine eigene grosse Welle, sobald die Migrations-Erfahrung gefestigt ist. Davor V9.25 (Phase 5b — Konsumenten voxel-aware).

**Vor V9.25 (Voxel-Terrain-Bogen Phase 5b — die Voxel-Welt wird höhen-ehrlich)**: vor dem grossen Heightfield-Schnitt ein Verwebungs-Audit aller heightfield-gekoppelten Systeme. Schöpfer-Wahl per AskUserQuestion: **„alte Welten zu Voxel migrieren"** (der saubere Endzustand). 5b ist die Pflicht-Vorstufe — eine Welt darf nicht auf eine fehlerhafte Voxel-Schicht migriert werden. Die Wurzel-Verwebung ist die Höhe: `getTerrainHeightAt(x,z)` ist jetzt voxel-aware (liefert `_voxelSurfaceY` statt des schlafenden `groundHeightField`, wenn `worldMeta.voxelTerrain` gesetzt) — und damit erben Wasser (`_buildWaterPlane` sampelt die Voxel-Oberfläche), der Killplane (`base-80` statt heightfield-`minHeight`), `findSurfaceAbove` (Reset-Ziel), Kreatur-Spawn + Genesis-Plattform die wahre Höhe. +2 Invarianten. **Offen — Phase 5c: der Migrations-Flip (alte/Eingangs-Welt → voxel) + die Heightfield-Code-Entfernung + die Test-Suite-Umschreibung (der Playtest läuft heute auf der Heightfield-Eingangs-Welt — beim Flip müssen die heightfield-spezifischen Invarianten auf Voxel umgeschrieben werden). Plus die Heightfield-Generierung für Voxel-Welten überspringen (heute läuft sie + ruht — wasteful).** Davor V9.24 (Verbindungen geheilt — Sicht-Ring + Vegetation), V9.23 (Voxel als Default für neue Welten), V9.22 (Voxel-Terrain grünt — Instanced-Gras), V9.21 (Voxel-Welt-Geburt), V9.20 (Grösse + Hierarchie), V9.19 (Phase 4 Politur), V9.18 (Phase 4 — Höhlen/Überhänge).

**Nächste Welle: Phase 5 — Materialien/Shader + die Ablösung des Heightfields** (`aField` per Vertex auf dem Voxel-Mesh, Terrain-Material-Harvest, Cel-Shading; das Heightfield wird abgelöst, alte Welten migrieren). Der volle Plan steht in `docs/roadmap.md` „Der Voxel-Terrain-Bogen". Disziplin: parallel bauen + beweisen, dann das Heightfield ablösen — nie das Funktionierende brechen. **Lies `CLAUDE.md` V9.26 ZUERST.**
