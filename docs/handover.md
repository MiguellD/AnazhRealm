...
## Schnell-Lage (Stand 20.05.2026, V9.32)

**Du erbst eine sehr lebendige Welt**. **~2954 Playtest-Invarianten grün + 0 Audit-Strict-Failures**, ~35900 Zeilen in einer Datei, alles produktiv. (Der Playtest-Konsolen-Zähler driftet ±2-3 je Lauf — einige Checks sind bedingt; „Alle Invarianten OK" ist die Wahrheit, nicht die exakte Zahl.)

**Jüngste Welle — V9.32 (Voxel-Phase 5d-Mini: Wasserfälle aus der Voxel-Klippen-Steilheit)**: eine voxel-basierte Welt hatte seit V9.28 keine Wasserfälle (der heightfield-256×256-Loop war voxel-gegated, da er `heightData[idx]` direkt las). V9.32 baut den Voxel-Pendant: **`_buildVoxelChunkWaterfalls(cx, cz)`** sampelt pro Voxel-Chunk 6×6 Stellen, misst die Steilheit via `_voxelSurfaceY`-Gradient zu vier Nachbar-Punkten (4 m Distanz), spawnt einen Partikel-Wasserfall bei ≥4 m Höhen-Drop mit ~12 % chunk-key-RNG-Wahrscheinlichkeit. Die Partikel-Schicht ist Zeile für Zeile die Heightfield-Variante (`THREE.Points` + `velocity`-Attribut + `userData.{baseHeight, minY}`) — sie landet in `state.waterfalls`, der bestehende Animations-Loop animiert sie geschenkt. Pro-Chunk-Lifecycle via `state.voxelChunkWaterfalls`-Map analog Gras (V9.22) + Vegetation (V9.24) — am `_ensureVoxelChunkAt`/`_disposeVoxelChunk`-Lifecycle aufgehängt, deterministisch, idempotent. Vierte Voxel-Chunk-Schicht in derselben Spawn-Sprache. +6 Invarianten. **Lehre**: eine V9.28-ehrlich-benannte Lücke wird zu einer ehrlich-kleinen Mini-Welle, wenn man die Spawn-Sprache aus dem bestehenden Pattern zieht (`_buildVoxelChunkGrass` + `_populateVoxelChunkVegetation` lehrten schon, wie man am Voxel-Chunk-Lifecycle eine Schicht hängt). Wer eine etablierte Sprache hat, baut die nächste Schicht in einer halben Welle.
