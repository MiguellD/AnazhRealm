...
## Schnell-Lage (Stand 19.05.2026, V9.15)

**Du erbst eine sehr lebendige Welt**. **~2899 Playtest-Invarianten grün + 0 Audit-Strict-Failures**, ~33000 Zeilen in einer Datei, alles produktiv. (Der Playtest-Konsolen-Zähler driftet ±2-3 je Lauf — einige Checks sind bedingt; „Alle Invarianten OK" ist die Wahrheit, nicht die exakte Zahl. Die Suite ist seit V8.96 deterministisch — der letzte benannte flaky Test ist geheilt.)

**Jüngste Welle — V9.15 (Voxel-Terrain-Bogen Phase 3b — Aufschütten)**: das Gegenstück zum Graben. `fillVoxelSphere(x,y,z,r)` schüttet eine Kugel „Fest" auf — der Edit (`mode:"fill"`) addiert in `_terrainDensityAt` Dichte → Luft wird Boden. `carveVoxelSphere` + `fillVoxelSphere` teilen `_addVoxelEdit(x,y,z,r,mode)` (kein Parallelcode). Der RMB schüttet bei aktivem Voxel-Terrain ohne Bau-Modus auf (Gegenstück zum LMB-Graben), Chat `voxel fill`. Mode-lose Alt-Edits (V9.14-Saves) gelten als carve (Backward-Compat). +6 Invarianten. Der Voxel-Boden ist damit voll formbar — graben UND aufschütten. Davor V9.14 (3D-Graben — `carveVoxelSphere`, LMB), V9.13 (per-Welt-Persistenz), V9.12 (72-m-Chunk-Säule), V9.11 (Naht-Wurzelheilung — `cv()`-Index-Wall), V9.10 (Naht-Skirt + Welt-Feld-Farbe), V9.09 (Voxel-Chunk-Ring). **Nächste Welle: Phase 4 — Höhlen/Überhänge in der Voxel-GENERIERUNG** (das 3D-Noise gebiert natürliche Höhlen, die Welt kommt mit ihnen zur Welt — `_terrainDensityAt` hat die 3D-Bänder schon, eine Welle die sie zu echten begehbaren Höhlen formt). Danach Phase 5 (Materialien/Shader/Ablösung — das Heightfield wird abgelöst, alte Welten migrieren). Der volle Plan steht in `docs/roadmap.md` „Der Voxel-Terrain-Bogen". Disziplin: parallel bauen + beweisen, dann das Heightfield ablösen — nie das Funktionierende brechen. **Lies `CLAUDE.md` V9.15 ZUERST.**
