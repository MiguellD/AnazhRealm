...
## Schnell-Lage (Stand 19.05.2026, V9.17)

**Du erbst eine sehr lebendige Welt**. **~2907 Playtest-Invarianten grün + 0 Audit-Strict-Failures**, ~33000 Zeilen in einer Datei, alles produktiv. (Der Playtest-Konsolen-Zähler driftet ±2-3 je Lauf — einige Checks sind bedingt; „Alle Invarianten OK" ist die Wahrheit, nicht die exakte Zahl. Die zwei timing-flaky Fog-Checks sind in V9.17 geheilt.)

**Jüngste Welle — V9.17 (Voxel-Terrain-Bogen Phase 3c — Material-Erhaltungs-Kreis)**: das Voxel-Aufschütten wird modus-abhängig ans Inventar gekoppelt (Schöpfer-Wahl per AskUserQuestion: „beides, modus-abhängig"). Im pfad-Modus ein Kreislauf — graben gibt Material (der V9.04-Yield, modus-unabhängig), aufschütten KOSTET 4 Material-Einheiten (`_voxelFillGate` ruft `_consumeAnyMaterial` — zieht „irgendein Material" über die Inventar-Slots; das Gate spiegelt `_buildMaterialGate`); ohne genug Material wird das Aufschütten abgelehnt. In frieden + schöpfer freies Formen ohne Kosten. RMB-Fill (`tryMousePlace`) + Chat `voxel fill` rufen das Gate. Plus: zwei timing-flaky Fog-Playtests geheilt — sie hingen am verdeckten `playerEyesUnderwater`-Eingang (erzwingt `fog.near=4`), jetzt deterministisch ausgenullt (V8.57-Disziplin). +7 Invarianten. Davor V9.16 (Dichte-Gradienten-Normalen), V9.15 (Aufschütten — `fillVoxelSphere`, RMB), V9.14 (3D-Graben — `carveVoxelSphere`, LMB), V9.13 (per-Welt-Persistenz), V9.12 (72-m-Chunk-Säule), V9.11 (Naht-Wurzelheilung), V9.10 (Naht-Skirt + Welt-Feld-Farbe), V9.09 (Voxel-Chunk-Ring).

**Nächste Welle: Phase 4 — Höhlen/Überhänge in der Voxel-GENERIERUNG** (das 3D-Noise gebiert natürliche Höhlen — `_terrainDensityAt` hat die 3D-Bänder schon, eine Welle die sie zu echten begehbaren Höhlen formt). Danach Phase 5 (Materialien/Shader/Ablösung — das Heightfield wird abgelöst, alte Welten migrieren). Der volle Plan steht in `docs/roadmap.md` „Der Voxel-Terrain-Bogen". Disziplin: parallel bauen + beweisen, dann das Heightfield ablösen — nie das Funktionierende brechen. **Lies `CLAUDE.md` V9.17 ZUERST.**
