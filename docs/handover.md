...
## Schnell-Lage (Stand 19.05.2026, V9.02)
...
**Jüngste Welle — V9.02 (W15-Politur — Slash-Branch-Namen im GitHub-Fetch)**: der letzte benannte kleine offene Punkt aus `docs/roadmap.md` §3. Ein vendortes Repo aus `github.com/owner/repo/tree/feature/x` wurde bisher als Branch `feature` + Pfad `x` fehlinterpretiert (Trees-API 404). Jetzt bleiben die Segmente nach `tree/` ein Array; `resolveGithubBranch` (save-server.js) probiert die längste auflösbare Branch zuerst gegen die GitHub-Branches-API, die zugleich die Commit-SHA liefert — Trees- + Raw-Aufrufe gehen gegen die SHA (slash-sicher). Reine save-server-Welle, keine Playtest-Invariante; der Beweis ist `smoke-vendor.cjs` (Fake-GitHub mit Slash-Repo, +2 Checks). **Lies `CLAUDE.md` V9.02 ZUERST.**
