...
| **W14** Bibliothek | Welt-Registry: andere Welten browsen, Klick → Portal-Bauplan ins Inventar. Plus Portal-Manifest-Signatur (`authorPubKey`/`signature`, `world-portal.md` §3.3). | 8-10 Sessions | sehr hoch |
...
**Empfehlung**: **W14 (Bibliothek)** oder die kleine **W11 V4 (Voice-Sync)**. W13 (Vibe-Pass) ist mit Phase 1+2+3 (V8.54-V8.56) **vollständig** — der Avatar hat einen Schlüssel, signiert seine Werke, und im Multi-User trägt jeder Mitspieler seine beweisbare Identität. W14 öffnet den letzten großen Vision-Ring: eine Welt-Registry zum Browsen fremder Welten — und hier landet das Portal-Manifest-`authorPubKey`/`signature`-Feld (`_vibeVerify` aus W13 prüft es). W11 V4 ist klein (~1 Session) und schließt den Multi-User-Präsenz-Bogen (sehen/spüren/kennen/**hören**). `docs/world-portal.md` + den V8.56-Eintrag in `CLAUDE.md` ZUERST lesen. Volle Detail-Tabelle in `roadmap.md`.
