...
## Schnell-Lage (Stand 19.05.2026, V9.24)

**Du erbst eine sehr lebendige Welt**. **~2922 Playtest-Invarianten grün + 0 Audit-Strict-Failures**, ~33000 Zeilen in einer Datei, alles produktiv. (Der Playtest-Konsolen-Zähler driftet ±2-3 je Lauf — einige Checks sind bedingt; „Alle Invarianten OK" ist die Wahrheit, nicht die exakte Zahl.)

**Jüngste Welle — V9.24 (Voxel-Welt: die Verbindungen geheilt)**: zwei Schöpfer-Browser-Befunde nach V9.23 — der Sicht-Ring-Regler tat in einer voxel-basierten Welt nichts, und Bäume/Strukturen erschienen nur im Start-Bereich. Beide waren echte Verbindungs-Lücken, die der V9.23-Default-Flip aufdeckte (die V9.22-Lehre, ein drittes Mal — eine parallele Schicht erbt nicht automatisch die Verdrahtung der alten). (1) `_voxelChunkConfig().ringRadius` war hart 2; jetzt folgt er `state.chunkRingRadius` (dem Sicht-Ring-Regler). (2) `populateChunkVegetation` war nur an `ensureChunkAt` (Heightfield) gehängt — in einer Voxel-Welt ruht das Heightfield, der Pfad läuft für neue Bereiche nie; der neue `_populateVoxelChunkVegetation` hängt am Voxel-Chunk-Lifecycle, teilt die Affinitäts-Logik über den extrahierten `_vegetationSampleSpawn` und setzt jede Struktur auf den Voxel-Boden (`_voxelSurfaceY`). Plus ein Robustheits-Fund: ein degenerierter Rand-Chunk ohne gültige Dreiecke wird jetzt wie ein leerer behandelt (kein Fall-durch-Loch). +4 Invarianten. **Offen: weitere heightfield-gekoppelte Systeme — das Wasser (`waterLevel` ist heightfield-abgeleitet, auf Voxel-Terrain ungefähr) ist eine kleine benannte Folge-Naht; ausserdem der volle Heightfield-Schnitt (Phase 5b).** Davor V9.23 (Voxel als Default für neue Welten), V9.22 (Voxel-Terrain grünt — Instanced-Gras), V9.21 (Voxel-Welt-Geburt), V9.20 (Grösse + Hierarchie), V9.19 (Phase 4 Politur), V9.18 (Phase 4 — Höhlen/Überhänge), V9.17 (Material-Erhaltungs-Kreis).

**Nächste Welle: Phase 5 — Materialien/Shader + die Ablösung des Heightfields** (`aField` per Vertex auf dem Voxel-Mesh, Terrain-Material-Harvest, Cel-Shading; das Heightfield wird abgelöst, alte Welten migrieren). Der volle Plan steht in `docs/roadmap.md` „Der Voxel-Terrain-Bogen". Disziplin: parallel bauen + beweisen, dann das Heightfield ablösen — nie das Funktionierende brechen. **Lies `CLAUDE.md` V9.24 ZUERST.**
