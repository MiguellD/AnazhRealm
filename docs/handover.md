...
## Schnell-Lage (Stand 19.05.2026, V9.08)

**Du erbst eine sehr lebendige Welt**. **~2865 Playtest-Invarianten grün + 0 Audit-Strict-Failures**, ~33000 Zeilen in einer Datei, alles produktiv. (Der Playtest-Konsolen-Zähler driftet ±2-3 je Lauf — einige Checks sind bedingt; „Alle Invarianten OK" ist die Wahrheit, nicht die exakte Zahl. Die Suite ist seit V8.96 deterministisch — der letzte benannte flaky Test ist geheilt.)

**Jüngste Welle — V9.08 (Voxel-Terrain-Bogen Phase 2a — Voxel-Chunk-Kollision)**: der gemeshte Voxel-Test-Chunk bekommt ein `btBvhTriangleMeshShape` und wird begehbar — der Spieler kann auf dem Voxel-Terrain stehen. `_buildIslandCollision` wurde zum generischen `_buildStaticTriMeshCollision(mesh, {kind, friction})` extrahiert (eine Sprache für Inseln UND Voxel-Chunks, kein Parallelcode); `_spawnVoxelTestChunk` baut die Kollision beim Spawn + räumt sie beim Ersetzen über `_disposeStaticCollision`. +5 Invarianten. Ein ehrlicher Sub-Schnitt von Roadmap-Phase 2 — die Kollision ist die Grundlage. **Nächste Welle: Phase 2b — Chunk-Streaming** (Voxel-Chunks streamen im Ring wie das Heightfield-System, der Spieler geht durchgehend auf Voxel-Terrain, eine neue Welt kann voxel-basiert sein — Flag). Danach Phase 3 (3D-`modify_terrain` — echtes Graben), Phase 4 (Höhlen/Überhänge in der Generierung), Phase 5 (Materialien/Shader/Ablösung). Der volle Plan steht in `docs/roadmap.md` „Der Voxel-Terrain-Bogen". Disziplin: parallel bauen + beweisen, dann das Heightfield ablösen — nie das Funktionierende brechen. **Lies `CLAUDE.md` V9.08 ZUERST.**
