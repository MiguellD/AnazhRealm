...
## Schnell-Lage (Stand 21.05.2026, V9.43-a)
...
**Empfehlung für den nächsten Agenten**: die nächste Welle ist **V9.44-a — Persistenz-Schema (der Stamm-Pflege-Bogen)**. Schöpfer-Wahl 21.05.2026 nach einem Code-Hygiene-Audit: ein Bogen reiner verhaltensneutraler Refactorings wird VOR die Hydrosphäre eingeschoben — „damit die Weiterentwicklung danach sauberer ist". Vor dem ersten Code-Block **`docs/code-hygiene.md`** ganz lesen (Befund, Vision-Abgleich — KEIN neues Modul, kein State-Manager; sechs Sub-Wellen a-f; bewusste Nicht-Wellen). V9.44-a baut ein `_serializeBlueprint`/`_deserializeBlueprint`-Paar als EINE Quelle des Bauplan-Feld-Satzes (spiegelt das erprobte `_serializeCreature`-Muster); `buildStateSnapshot` + `loadState` routen hindurch → die V8.59-Snapshot-Bug-Klasse ist an der Wurzel geheilt. Disziplin: reines Refactoring, Verhalten bit-genau unverändert, die ~2966 bestehenden Invarianten sind der Beweis (V9.30-Disziplin — keine neuen Invarianten nötig, nur alle alten grün + Audit-Strict 0). Danach V9.44-b..f (kanonische Setter, Mesh-Router-Dispatch-Table, Infrastruktur-Konstanten, UI-Giganten, Game-Loop-Phasen). **Erst NACH dem Stamm-Pflege-Bogen** folgt die Hydrosphäre — V9.43-b der Hydrosphären-Atlas (`_computeHydrosphere()`: Surface-Sampling → Priority-Flood → D8-Flow → Flow-Accumulation → Netz-Extraktion), dann V9.43-c/d/e; `docs/hydrosphere.md` bleibt der Plan-Anker dafür. Bekannter Edge-Case aus V9.42-d: **5 Vertices an Vier-Chunk-Ecken** mit ~1 m Rest-Naht-Versatz — falls der Schöpfer sie sieht, eine kleine V9.42-e davorziehen, sonst ignorierbar. Detail in `docs/roadmap.md` §3 „Der Stamm-Pflege-Bogen" + `docs/code-hygiene.md`.
