...
## Schnell-Lage (Stand 21.05.2026, V9.44-c)
...
**Empfehlung für den nächsten Agenten**: die nächste Welle ist **V9.44-d — Infrastruktur-Konstanten konsolidieren (Stamm-Pflege-Bogen Sub-Welle 4/6)**. V9.44-a/b/c sind erledigt (a: Persistenz-Schema vereinheitlicht — `_serializeBlueprint`/`_deserializeBlueprint`; b: `_applyPlayerSpeed(v)` als kanonischer Setter; c: `p2pHandleMessage` ist ein Dispatch-Table — `AnazhRealm.P2P_MESSAGE_HANDLERS` + 18 `_p2pMsg<Type>`-Methoden, smoke-webrtc end-to-end verifiziert). Vor dem ersten Code-Block **`docs/code-hygiene.md`** ganz lesen (Befund, Vision-Abgleich — KEIN neues Modul, kein State-Manager; sechs Sub-Wellen a-f; bewusste Nicht-Wellen). V9.44-d konsolidiert gestreute Magic Values in den bestehenden `AnazhRealm.X`-Konstanten-Block: `OLLAMA_DEFAULT_ENDPOINT` (`http://localhost:11434` ×8), `P2P_DEFAULT_WS_URL` (`ws://127.0.0.1:4313` ×8), `INVITE_PROTOCOL` (`anazh://` ×6), `P2P_BACKPRESSURE_BYTES` (`262144` ×2 — als `16 × P2P_WORLD_CHUNK_SIZE` definieren, die Beziehung sichtbar machen). Für den Spawn-Fallback `{x:0,y:50,z:0}` (×16) **KEINE geteilte Konstante** — ein geteiltes Objekt-Literal wäre ein Bug sobald jemand es mutiert; stattdessen ein `_defaultSpawnPos()`-Factory-Helper, der ein frisches Objekt liefert. Die `JSON.stringify().slice(0, N)`-Log-Truncation (fünf Limits 80/100/120/140/200) vereinheitlichen (ein `_truncForLog`-Helper oder ein fester Wert). Disziplin: reines verhaltensneutrales Refactoring, die ~2974 bestehenden Invarianten sind der Beweis (V9.30-Disziplin — keine neuen Invarianten nötig, nur alle alten grün + Audit-Strict 0). Danach V9.44-e/f (UI-Giganten, Game-Loop-Phasen — V9.44-f braucht zusätzlich einen Browser-Test). **Erst NACH dem Stamm-Pflege-Bogen** folgt die Hydrosphäre — V9.43-b der Hydrosphären-Atlas (`_computeHydrosphere()`: Surface-Sampling → Priority-Flood → D8-Flow → Flow-Accumulation → Netz-Extraktion), dann V9.43-c/d/e; `docs/hydrosphere.md` bleibt der Plan-Anker dafür. Bekannter Edge-Case aus V9.42-d: **5 Vertices an Vier-Chunk-Ecken** mit ~1 m Rest-Naht-Versatz — falls der Schöpfer sie sieht, eine kleine V9.42-e davorziehen, sonst ignorierbar. Detail in `docs/roadmap.md` §3 „Der Stamm-Pflege-Bogen" + `docs/code-hygiene.md`.
