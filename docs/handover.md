...
**Nächste Welle: offen.** Phase 5c.2.c.3.b.i ist mit V9.37 erledigt (drei tote Dispatch-Pfade weg). Übrig in der grossen 5c.2.c-Aufräumarbeit: **5c.2.c.3.b.ii** (`heightData`-Allokations-Pfad in `isVoxelWorldGen2` + `terrainMaterial`-ShaderMaterial-Bau — beides lebt noch, wird aber von keinem Spieler-Pfad mehr erreicht; Test-Anker V9.28 + V8.27 Cel-Stufen-Regler umwidmen oder streichen), **5c.2.c.3.b.iii** (die Pipeline-Methoden selbst — `ensureChunkAt`, `populateChunkVegetation`, `_buildChunkGrass`, `_terrainHeightAtWorld`, `_disposeChunkPhysics`, `pruneDistantChunks`; verlangt vorher Test-Migration der V9.29/V8.27/V8.28-Tests auf Voxel-Material-Probe). Daneben Phase 5d-vollständig (Wasser-/Schluchten-Drama-Schicht) oder eine andere Vision-Richtung. Davor — historische 5c.2.c-Aufräum-Substanz: **Lies `CLAUDE.md` V9.37 ZUERST.** Disziplin: parallel bauen + beweisen, nie das Funktionierende brechen, ehrliche kleine Schritte vor grossen Würfen.
