...
## Schnell-Lage (Stand 21.05.2026, V9.44-f — der Stamm-Pflege-Bogen ist vollständig)
...
**Empfehlung für den nächsten Agenten**: der Stamm-Pflege-Bogen (V9.44-a..f) ist **vollständig** — die sechs Code-Hygiene-Audit-Befunde sind geheilt, die drei vom Audit benannten Kontrollfluss-Giganten zerlegt. Ehrliche Rest-Grenze: ~7 weitere 200+-Kontrollfluss-Funktionen (`loadState`, `processChatCommand`, …) + vier 200+-UI-Builder standen nicht im Audit-Scope und bleiben — ein Kandidat für einen späteren Hygiene-Bogen, NICHT vor der Hydrosphäre. Die nächste Welle ist **die Hydrosphäre — V9.43-b, der Hydrosphären-Atlas**. Vor dem ersten Code-Block **`docs/hydrosphere.md`** ganz lesen (der Plan-Anker: Algorithmus, Datenstrukturen, Risiken, Wellen-Schnitt b/c/d/e). V9.43-b baut `_computeHydrosphere()` — die reine BERECHNUNG, headless-prüfbar, KEIN Rendering, KEIN Carven: Surface-Sampling über eine ~2-km-Region → Priority-Flood-Depression-Filling (Barnes 2014, füllt Senken → Seen) → D8-Flow-Direction → Flow-Accumulation → Netz-Extraktion (Fluss-Polylinien mit Breite ∝ √Akkumulation, See-Senken, Wasserfall-Punkte an Fluss-Klippen-Kreuzen). Reine Daten in `state.hydrosphere`, deterministisch aus dem Seed → multiplayer-sicher. Der Stamm-Pflege-Bogen hat die Andock-Stellen sauber gemacht: V9.44-a (das Persistenz-Schema `_serializeBlueprint` — als Vorbild, falls `state.hydrosphere` persistiert werden soll) + V9.44-f (der Game-Loop ist jetzt Phasen-strukturiert — ein künftiger `_loopHydrosphere`-Tick / `uTime`-Bump dockt an eine benannte Phase an, nicht an einen Monolithen). Danach V9.43-c (Flüsse/Seen rendern), V9.43-d (Flüsse carven echte Betten), V9.43-e (Klang). Bekannter Edge-Case aus V9.42-d: **5 Vertices an Vier-Chunk-Ecken** mit ~1 m Rest-Naht-Versatz — falls der Schöpfer sie sieht, eine kleine V9.42-e davorziehen, sonst ignorierbar. Detail in `docs/roadmap.md` §3 + `docs/hydrosphere.md`.
