...
## Schnell-Lage (Stand 19.05.2026, V9.04)

**Du erbst eine sehr lebendige Welt**. **~2847 Playtest-Invarianten grün + 0 Audit-Strict-Failures**, ~33000 Zeilen in einer Datei, alles produktiv. (Der Playtest-Konsolen-Zähler driftet ±2-3 je Lauf — einige Checks sind bedingt; „Alle Invarianten OK" ist die Wahrheit, nicht die exakte Zahl. Die Suite ist seit V8.96 deterministisch — der letzte benannte flaky Test ist geheilt.)

**Jüngste Welle — V9.04 (W6.G P4 — das Terrain wird Materie)**: ein Schöpfer-Browser-Befund nach V9.03, reflektiert gegen die vier Testamente. Der Kern: alles Materielle spricht die Hylomorphismus-Sprache (Compound aus Material × Form), nur das Terrain nicht — es war ein gefärbtes Heightfield-Sheet, Strukturen abbauen gab Material, den Boden abbauen gab nichts. V9.04 holt den letzten Holdout in die Sprache: ein Grabe-Hieb (`tryMouseBreak`-Terrain-Zweig) yieldet Material, dessen Identität aus `_terrainMaterialAt(x,z)` emergiert — die dominante `worldFieldAt`-Achse nennt den Boden (lebendig→`erde` neu, dichte→`stein`, glut→`glut`, magieleitung→`quarz`). Keine Biom-Tabelle — die Farbe, die das Auge sieht, ist das Material, das die Hand bekommt. Der Yield sitzt lokal in `tryMouseBreak`, NICHT im broadcastbaren `modify_terrain`-Op (Multi-User-safe). +11 Invarianten. Echte Höhlen/Tunnel (Heightfield ist pro (x,z) eindeutig) + die visuelle Struktur-Terrain-Synergie bleiben ehrlich benannt (`docs/roadmap.md` §3 Reihen 7+9). **Lies `CLAUDE.md` V9.04 ZUERST.**
