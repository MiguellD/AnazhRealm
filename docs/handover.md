...
**Nächste Welle: offen.** Phase 5c.2.c.3.a ist mit V9.36 erledigt (`modify_terrain` weg, Welt-Mod im Voxel-Feld). Übrig in der grossen 5c.2.c-Aufräumarbeit: (a) **5c.2.c.3.b** — die heightfield-only Chunk-Pipeline ablösen: `ensureChunkAt`, `populateChunkVegetation`, `_buildChunkGrass`, `_terrainHeightAtWorld`, `_disposeChunkPhysics`, `_disposeChunkGrass`, `pruneDistantChunks`, der Terrain-Shader + alles, was am `groundChunks`-Array hängt. Heute werden diese Methoden NICHT mehr im normalen Spiel-Pfad gerufen (V9.27-Skip beim Welt-Aufbau), aber **zwei Konsumenten leben weiter**: (i) Playtest-Setups (V9.29-Disposal-Test, V8.27/V8.28-Shader-Tests) bauen sich seit V9.35/V9.36 selbst Heightfield-Chunks via `ensureChunkAt(40, 40)`/`(50, 50)`/`(60, 60)`; (ii) der V9.27-gespeicherte `terrainMaterial`-ShaderMaterial-Bau lebt weiter (für die Cel-Stufen-Regler-Verkabelung). Eine ehrliche Sub-Welle 5c.2.c.3.b verlangt vorher: Test-Migration auf eine echt-Heightfield-freie Welt (V8.27/V8.28-Shader-Tests müssten Voxel-Materialien prüfen statt Heightfield-`groundChunks[0]`), Bewertung des Cel-Stufen-Reglers (gilt er für Voxel-Material auch? Wahrscheinlich ja, dann verkabeln). (b) **Phase 5d-vollständig** (Voxel-Welt bekommt eine reichere Wasser-/Schluchten-Drama-Schicht — V9.32 brachte die Mini-Variante) oder (c) eine **andere Vision-Richtung** aus den vier Testamenten / `docs/state-of-realm.md`. Disziplin: parallel bauen + beweisen, nie das Funktionierende brechen, ehrliche kleine Schritte vor grossen Würfen. **Lies `CLAUDE.md` V9.36 ZUERST.**
