...
## Schnell-Lage (Stand 19.05.2026, V8.99)

**Du erbst eine sehr lebendige Welt**. **~2810 Playtest-Invarianten grün + 0 Audit-Strict-Failures**, ~33000 Zeilen in einer Datei, alles produktiv. (Der Playtest-Konsolen-Zähler driftet ±2-3 je Lauf — einige Checks sind bedingt; „Alle Invarianten OK" ist die Wahrheit, nicht die exakte Zahl. Die Suite ist seit V8.96 deterministisch — der letzte benannte flaky Test ist geheilt.)

**Jüngste Welle — V8.99 (W10 ext. Politur — die Affordance-Stärke skaliert mit der Substanz)**: eine Schöpfer-Frage trug die Welle — „kann ich eine bessere oder schlechtere Antenne bauen, oder gibt es nur ja/nein?". Befund: der Affordance-Schwellwert war eine Klippe, der broadcasting-Relais ein flaches ×2. Jetzt: der Schwellwert in `computeBlueprintAffordances` bleibt die GATE (ist es überhaupt ein Strahler/Mast?), aber DARÜBER skaliert die Wirkung. `computeAffordanceStrength(bp)` liefert je Affordance eine Stärke 0.2..1.0 = `overScale(tag, schwelle)` (0.4..1.0, wie weit der Tag den Schwellwert übersteigt) × `precF` (0.5..1.0, `0.5 + 0.5×computeBlueprintQuality` — Vision §6.3). `_tickRadiatingAffordances` skaliert die Emotion-Intensität mit der Strahler-Stärke; das broadcasting-Relais skaliert `mult = 1 + mastStärke × (BROADCAST_RANGE_MULT−1)`. `spawnArchitecture` friert `entry.affordanceStrength` ein. Man baut eine bessere Antenne durch Material, Form UND Handwerk. +6 Invarianten. **Lies `CLAUDE.md` V8.99 ZUERST.**
