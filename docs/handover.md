...
Der Schöpfer hatte die V9.39-Welt im Browser geprüft und **fünf substanzielle Befunde** geliefert. Die Voxel-Surface-Politur (V9.40–V9.43) hat sie abgearbeitet — **alle fünf sind an der Wurzel angegangen; V9.43-a vereinheitlicht die Wasser-Sprache (Befund #5, Wasserfälle), V9.43-b/c (Strömung + Bäche) bleiben**:
...
| Wasserfälle sind Partikel, Wasser ist Shader-Plane | Zwei Wasser-Geometrien (Partikel vs. Plane) — Vision-Inkonsistenz §1.3. | **V9.43-a** (Wasserfälle), V9.43-b/c offen | 🟡 V9.43-a geheilt (Wasserfälle = vertikale Wasser-Planes, geteilter Flow-Shader); Meeres-Strömung + Bäche bleiben |

**Empfehlung für den nächsten Agenten**: Schöpfer-Browser-Test der **V9.43-a** einfordern (stürzen die Wasserfälle schön die Klippe hinab? sitzen die Planes an der Klippen-Kante?). Falls gut → **V9.43-b** (Meeres-Strömung — ein sanfter Flow-Vektor auf der bestehenden 900×900-Plane, das `uFlowDir`-Uniform steht schon) → danach **V9.43-c** (Bäche/Flüsse — Pfad-Plane von hoher `lebendig`-Affinitäts-Quelle zum Meer, eine eigene grössere Welle). Falls die Wasserfall-Plane-Platzierung daneben → V9.43-a.2 (Klippen-Kanten-Feinjustierung). Plus ein bekannter Edge-Case aus V9.42-d: **5 Vertices an Vier-Chunk-Ecken** haben einen Rest-Naht-Versatz (~1 m) — falls der Schöpfer sie im Browser sieht, eine kleine V9.42-e davorziehen; falls nicht, ignorierbar. Detail in `docs/roadmap.md` §3 „Voxel-Surface-Politur".
