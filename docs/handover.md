...
## Schnell-Lage (Stand 19.05.2026, V9.06)

**Du erbst eine sehr lebendige Welt**. **~2849 Playtest-Invarianten grün + 0 Audit-Strict-Failures**, ~33000 Zeilen in einer Datei, alles produktiv. (Der Playtest-Konsolen-Zähler driftet ±2-3 je Lauf — einige Checks sind bedingt; „Alle Invarianten OK" ist die Wahrheit, nicht die exakte Zahl. Die Suite ist seit V8.96 deterministisch — der letzte benannte flaky Test ist geheilt.)

**Jüngste Welle — V9.06 (Strukturen tragen ihre Material-Farbe)**: ein Schöpfer-Browser-Befund — die Felsbögen wirken als „weisse Tore, nicht kongruent mit der Umgebung". Wurzel: `_buildFromBlueprint` las `part.color` und fiel auf Weiss (0xffffff) zurück, wenn es fehlte — es las NIE `part.material`. Die alten V1/V2-Baupläne (Dorf/Tempel) tragen explizite `color`-Felder; jeder material-basierte Bauplan seit V7.74 (Bäume, Felsen, Geode, Glutbrunnen, Felsbogen, Felsturm) trug nur `material` → sie rendeten alle weiss. Fix: fehlt `part.color`, kommt die Farbe aus `state.materials[part.material].color`. Ein explizites `part.color` (Werkstatt-Wahl) behält Vorrang. **Lies `CLAUDE.md` V9.06 ZUERST.**
