...
## Schnell-Lage (Stand 19.05.2026, V9.23)

**Du erbst eine sehr lebendige Welt**. **~2918 Playtest-Invarianten grün + 0 Audit-Strict-Failures**, ~33000 Zeilen in einer Datei, alles produktiv. (Der Playtest-Konsolen-Zähler driftet ±2-3 je Lauf — einige Checks sind bedingt; „Alle Invarianten OK" ist die Wahrheit, nicht die exakte Zahl.)

**Jüngste Welle — V9.23 (Voxel-Terrain-Bogen Phase 5a — Voxel als Default für neue Welten)**: der erste echte Schritt der Heightfield-Ablösung (Schöpfer-Wahl per AskUserQuestion: „Voxel als Default, schrittweise"). `createNewWorld`'s `voxelTerrain`-Parameter ist jetzt Default `true` — jede über den Neue-Welt-Dialog / Chat-`neue welt` / Host erschaffene Welt ist voxel-basiert. Die V9.21-Dialog-Checkbox ist zum Opt-out gekippt (`#new-world-heightfield` „Klassisches Heightfield-Terrain"). Alte Welten ohne `worldMeta.voxelTerrain` + die Eingangs-Welt (entsteht über `ensureWorldMeta`, NICHT `createNewWorld`) behalten ihr Heightfield — schrittweise, reversibel, kein Zwangs-Migration. Invarianten-Zahl unverändert (die 3 V9.21-Checks umgeschrieben). **Offen: der volle Schnitt — Heightfield ganz raus + Save-Migration alter Welten — bleibt ein späterer eigener Schritt.** Davor V9.22 (Voxel-Terrain grünt — Instanced-Gras auf dem Voxel-Boden), V9.21 (Voxel-Welt-Geburt), V9.20 (Grösse + Hierarchie), V9.19 (Phase 4 Politur), V9.18 (Phase 4 — Höhlen/Überhänge), V9.17 (Material-Erhaltungs-Kreis), V9.16 (Dichte-Gradienten-Normalen), V9.15 (Aufschütten), V9.14 (3D-Graben).

**Nächste Welle: Phase 5 — Materialien/Shader + die Ablösung des Heightfields** (`aField` per Vertex auf dem Voxel-Mesh, Terrain-Material-Harvest, Cel-Shading; das Heightfield wird abgelöst, alte Welten migrieren). Der volle Plan steht in `docs/roadmap.md` „Der Voxel-Terrain-Bogen". Disziplin: parallel bauen + beweisen, dann das Heightfield ablösen — nie das Funktionierende brechen. **Lies `CLAUDE.md` V9.23 ZUERST.**
