...
## Schnell-Lage (Stand 18.05.2026, V8.79)

**Du erbst eine sehr lebendige Welt**. **~2723 Playtest-Invarianten grün + 0 Audit-Strict-Failures**, ~33000 Zeilen in einer Datei, alles produktiv. (Der Playtest-Konsolen-Zähler driftet ±2-3 je Lauf — einige Checks sind bedingt; „Alle Invarianten OK" ist die Wahrheit, nicht die exakte Zahl.)

**Jüngste Welle — V8.79 (W17 Phase B-JS-Compute Phase 1 — der Compute-Host)**: B-Relay (V8.76) machte das Mesh zum Server für RELAY-Welten (Server = blosser Rebroadcast). Eine JS-Compute-Welt hat eine echte autoritative Server-JS-Logik; Phase 1 baut den Compute-Host-MECHANISMUS. **Die Deklaration**: `portalMeta.serverMode` (`relay`/`js-compute`) — `_sanitizePortalMeta` whitelistet es + erzwingt `multiplayer:true`, `aimBlueprintAtWorld` trägt es aus dem Eintrag, `buildStateSnapshot` persistiert es feldweise (V8.59-Lehre — der Playtest fing das Fehlen). **Host-Wahl OHNE Präsenz-Tabelle**: wer ein js-compute-Portal direkt betritt, wird Host (`enterPortal` → `computeRole:"host"`); wer einer W17-Phase-C-Einladung folgt, wird Gast des Einladenden (`joinPortalInvite` → `{computeRole:"guest", hostPeerId: inv.peerId}` — die Phase-C-Einladung trägt die Host-Identität schon). **Der Server-Kontext**: der Host baut ein zweites, verborgenes null-origin-iframe (`?anazh-server=1`), der save-server injiziert `PORTAL_SERVER_SHIM` (ein `WebSocketServer`-Global, der `ws`-Bibliothek nachempfunden); fremder Server-Code läuft `sandbox="allow-scripts"` ALLEIN (V8.70-Wand). **Host-geroutetes Transport**: ein Gast-`ws-send` geht NUR an den Host (`subworld-srv`, kanal-exklusiv), der Host feeds es in den Server-Kontext (`srv-recv`), die Server-JS rechnet, die Antwort kommt gezielt zurück (`subworld-cli` an einen Gast / lokal beim Host). Host-Migration ist Phase 2 (Host geht → Sub-Welt endet, ehrliche Grenze). +19 Invarianten, Zwei-Browser-verifiziert (`smoke-webrtc.cjs` — A wird Host, B Gast, die laufende Summe 12=7+5 beweist die autoritative Rechnung; ein Relay hätte nur „5" zurückgespielt). **Lies `CLAUDE.md` V8.79 ZUERST.**
...
**Die grossen Roadmap-Ringe sind gebaut**, und der echte Fremd-Engine-Bogen läuft: der KI-Übersetzer ist vollständig (V8.68/V8.69), **V8.70 öffnete das Untrusted-Welt-Tor** (eine echte fremde Engine läuft null-origin sandgesichert), **V8.71 + V8.72 bauten den Auto-Vendor-Pfad komplett**, **V8.73 + V8.74 die Mesh-Welt-Verteilung komplett**, und **W17 — V8.75 Phase A der Transport-Shim, V8.76 Phase B-Relay das Mesh-als-Server, V8.77 Phase C das Gruppen-Portal, V8.78 die Multiplayer-Welt-Deklaration, V8.79 Phase B-JS-Compute Phase 1 der Compute-Host** (ein Peer führt die autoritative Server-JS einer fremden Welt aus). Offen in diesem Bogen: nur W17 Phase B-JS-Compute Phase 2 (Host-Migration) + B-WASM. Der aktuelle Stand steht im Block „Aktuelle Roadmap" weiter unten und in `docs/roadmap.md` §3.
...
| **W17 — Multiplayer-Sub-Welten** | ✅ **Phase A (V8.75) + B-Relay (V8.76) + C (V8.77) + Multiplayer-Welt-Deklaration (V8.78) + B-JS-Compute Phase 1 (V8.79)**: der Transport-Shim trägt den `WebSocket`-Verkehr über die Sandbox-Grenze, das Mesh-als-Server verteilt ihn peer-to-peer, das Gruppen-Portal bringt eine Gruppe gemeinsam hindurch, und ein Peer wird Compute-Host für eine Welt mit autoritativer Server-JS. Offen im Bogen: nur Phase B-JS-Compute Phase 2 (Host-Migration) / B-WASM. | ~1-2 Sessions (B-JS P2) | sehr hoch |

**Empfehlung**: **W17 Phase B-JS-Compute Phase 2 — Host-Migration.** Phase 1 (V8.79) baute den Compute-Host: ein Peer führt die autoritative Server-JS einer fremden Welt in einem verborgenen, sandgesicherten Server-Kontext aus, das Mesh trägt den Verkehr host-zentriert. Die ehrliche Phase-1-Grenze: verlässt der Host das Mesh, endet die Sub-Welt. Phase 2 schliesst sie — ein deterministischer Nachfolger (die kleinste peerId der Gruppe, W7s `worldRole`-Host/Guest-Mechanik kennt Host/Guest schon) übernimmt, startet einen neuen Server-Kontext; der Server-Zustand geht dabei verloren (eine ehrliche Grenze, die `docs/roadmap.md` benennt). Eine kleine Naht nebenher: `serverMode` durch die ganze Vendor-/Mesh-Kette tragen, damit eine VENDORTE js-compute-Welt sich selbst deklariert (wie V8.78 für `multiplayer` — Phase 1 schnitt das bewusst aus). Danach B-WASM (per-Projekt). **Der Phasen-Detailplan steht in `docs/roadmap.md` §3 — „W17 — Multiplayer-Sub-Welten" (Phase B-JS-Compute).** `docs/world-portal.md` ZUERST mitlesen.
