...
## Schnell-Lage (Stand 21.05.2026, V9.44-a)
...
**Empfehlung für den nächsten Agenten**: die nächste Welle ist **V9.44-b — Kanonische Setter für gekoppelte Felder (Stamm-Pflege-Bogen Sub-Welle 2/6)**. V9.44-a ist erledigt (Persistenz-Schema vereinheitlicht — `_serializeBlueprint`/`_deserializeBlueprint` als EINE Quelle des Bauplan-Feld-Satzes, drei Aufrufer geroutet, die V8.59-Bug-Klasse an der Wurzel geheilt; der `inheritPlayer`-Pfad in `_buildEmptyWorldSnapshot` war schon gedriftet — Portal-Rolle + Signatur gingen still verloren — ehrlich mitgeheilt). Vor dem ersten Code-Block **`docs/code-hygiene.md`** ganz lesen (Befund, Vision-Abgleich — KEIN neues Modul, kein State-Manager; sechs Sub-Wellen a-f; bewusste Nicht-Wellen). V9.44-b baut einen `_applyPlayerSpeed(v)`-Setter, der `speed` + `sprintSpeed` konsistent hält (`sprintSpeed = speed*2` — die V7.72-Sprint-Bug-Klasse); alle Schreibpfade (`player_speed`-DSL-Op + `recomputePlayerStats`) routen hindurch. Den Code nach weiteren echten Kopplungen absuchen, KEIN State-Manager — nur ein Setter pro echter Kopplung. Disziplin: reines verhaltensneutrales Refactoring, die ~2974 bestehenden Invarianten sind der Beweis (V9.30-Disziplin — keine neuen Invarianten nötig, nur alle alten grün + Audit-Strict 0). Danach V9.44-c..f (Mesh-Router-Dispatch-Table, Infrastruktur-Konstanten, UI-Giganten, Game-Loop-Phasen). **Erst NACH dem Stamm-Pflege-Bogen** folgt die Hydrosphäre — V9.43-b der Hydrosphären-Atlas (`_computeHydrosphere()`: Surface-Sampling → Priority-Flood → D8-Flow → Flow-Accumulation → Netz-Extraktion), dann V9.43-c/d/e; `docs/hydrosphere.md` bleibt der Plan-Anker dafür. Bekannter Edge-Case aus V9.42-d: **5 Vertices an Vier-Chunk-Ecken** mit ~1 m Rest-Naht-Versatz — falls der Schöpfer sie sieht, eine kleine V9.42-e davorziehen, sonst ignorierbar. Detail in `docs/roadmap.md` §3 „Der Stamm-Pflege-Bogen" + `docs/code-hygiene.md`.
