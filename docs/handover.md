...
## Schnell-Lage (Stand 21.05.2026, V9.43-c.2 — das Wasser wird synergetisch)
...
**Du erbst eine sehr lebendige Welt.** **~3000 Playtest-Invarianten grün + 0 Audit-Strict-Failures**, ~35700 Zeilen in einer Datei, alles produktiv. (Der Playtest-Konsolen-Zähler driftet ±3–8 je Lauf — „Alle Invarianten OK" ist die Wahrheit, nicht die exakte Zahl.) Vor dem ersten Code-Block: `CLAUDE.md` V9.43-c.2 lesen (die jüngste Welle) + den Schöpfer-Audit-Block ganz oben. Disziplin: parallel bauen + beweisen, nie das Funktionierende brechen, ehrliche kleine Schritte vor grossen Würfen.
...
**Empfehlung für den nächsten Agenten**: V9.43-b (der Hydrosphären-Atlas) ist **fertig** — `_computeHydrosphere()` baut das Drainage-Netz als reine Daten (`state.hydrosphere`: 6 Flüsse, 12 Seen, deterministisch). Die nächste Welle ist **V9.43-c — Flüsse + Seen werden sichtbar**. Vor dem ersten Code-Block **`docs/hydrosphere.md`** ganz lesen (§7 Rendering, §9 V9.43-c). `_buildHydrosphereMeshes()` rendert: See-Planes (zur Senke geformt, auf `lake.level`) + Fluss-Ribbon-Meshes (Quad-Streifen entlang `river.points`, Breite ∝ den schon berechneten `point.width`, `uFlowDir` = `point.flowX/flowZ`) — alle mit der V9.43-a-Wasser-Material-Familie (`_ensureWaterfallMaterial`-Stil, geteilte `uDeep`/`uShallow`/`uSunDir`/`fog*`-Uniforms). **Die Wasserfälle**: V9.43-b liefert `state.hydrosphere.waterfalls` LEER (die hydrologische Makro-Surface ist glatt) — V9.43-c muss die Wasserfälle gegen die ECHTE Voxel-Surface re-verankern: entlang jeder `river.points`-Polylinie `_voxelSurfaceY(x,z)` sampeln und steile Drops finden (das ist der Fluss-Klippen-Kreuz, plan §7). Der per-Chunk-Zufalls-Wasserfall-Spawner (`_buildVoxelChunkWaterfalls`, V9.43-a) wird dabei abgelöst — Material + Plane wiederverwendet. Danach V9.43-d (Flüsse carven echte Betten via `_terrainDensityAt` — der `_hydroExtractRivers`-Code legt schon `point.width`/`point.y` bereit, V9.43-d braucht den `riverBuckets`-Spatial-Index), V9.43-e (Klang). Die `state.hydrosphere`-Datenstruktur ist in `docs/hydrosphere.md` §6 dokumentiert; `_computeHydrosphere` liefert zusätzlich `stats` (diagnostisch). Bekannter Edge-Case aus V9.42-d: **5 Vertices an Vier-Chunk-Ecken** mit ~1 m Rest-Naht-Versatz — falls der Schöpfer sie sieht, eine kleine V9.42-e davorziehen, sonst ignorierbar. Detail in `docs/roadmap.md` §3 + `docs/hydrosphere.md`.
