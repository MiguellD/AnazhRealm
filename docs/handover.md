...
## Schnell-Lage (Stand 19.05.2026, V9.20)
...
**Jüngste Welle — V9.20 (Voxel-Terrain Grösse + Hierarchie)**: ein Schöpfer-Browser-Befund nach V9.19 — „deutlich besser, Höhlen + Berge — aber alles dürfte grösser & weiter sein, kein Berg der sich über viele Chunks aufbaut, riesige Felswände, Grösse fehlt". Wurzel: die Oberfläche `surf` trug nur EINE mittlere 2D-Oktave (`0.012 × 14`) — jeder Berg gleich gross, keine Hierarchie. V9.20 gibt der Oberfläche drei Oktaven: eine **kontinentale** (`noise2D(x*0.0042) × 26` — ~1500 m Wellenlänge ≈ 35 Chunks, die grosse Gebirgs-Masse), eine **ridged** (`(1−|noise2D(x*0.013)|)² × 22` — scharfe Grate + Felswände) + eine feine Detail-Oktave. Das Oberflächen-Band wuchs auf ~`base-42..base+64`; der Voxel-Chunk wurde entsprechend höher (`dimY` 40→68, eine 122-m-Säule, `oy=base-50`) — die V9.12-Garantie hält mit Marge 8. Invarianten-Zahl unverändert (die V9.12-Probe + die drei Phase-4-Checks auf die neuen Bounds gezogen). Davor V9.19 (Phase 4 Politur — feines Band restauriert + Höhlen aus EINEM ridged-Feld), V9.18 (Phase 4 — Höhlen/Überhänge in der Generierung), V9.17 (Material-Erhaltungs-Kreis), V9.16 (Dichte-Gradienten-Normalen), V9.15 (Aufschütten), V9.14 (3D-Graben), V9.13 (per-Welt-Persistenz), V9.12 (Chunk-Säule fasst das Band).

**Nächste Welle: Phase 5 — Materialien/Shader + die Ablösung des Heightfields** (`aField` per Vertex auf dem Voxel-Mesh, Terrain-Material-Harvest, Cel-Shading; das Heightfield wird abgelöst, alte Welten migrieren). Der volle Plan steht in `docs/roadmap.md` „Der Voxel-Terrain-Bogen". Disziplin: parallel bauen + beweisen, dann das Heightfield ablösen — nie das Funktionierende brechen. **Lies `CLAUDE.md` V9.20 ZUERST.**
