...
## Schnell-Lage (Stand 21.05.2026, V9.44-b)
...
**Empfehlung für den nächsten Agenten**: die nächste Welle ist **V9.44-c — Der Mesh-Router wird ein Dispatch-Table (Stamm-Pflege-Bogen Sub-Welle 3/6)**. V9.44-a + V9.44-b sind erledigt (a: Persistenz-Schema vereinheitlicht — `_serializeBlueprint`/`_deserializeBlueprint` als EINE Quelle, drei Aufrufer geroutet, der gedriftete `inheritPlayer`-Pfad ehrlich mitgeheilt; b: `_applyPlayerSpeed(v)` als kanonischer Setter für die `sprintSpeed`↔`speed`-Kopplung, beide Schreibpfade geroutet, `hp`/`stamina`/`jumpPower` als Nicht-Kopplungen ehrlich verworfen). Vor dem ersten Code-Block **`docs/code-hygiene.md`** ganz lesen (Befund, Vision-Abgleich — KEIN neues Modul, kein State-Manager; sechs Sub-Wellen a-f; bewusste Nicht-Wellen). V9.44-c zerlegt `p2pHandleMessage` (290 Z., 19 sequentielle `if (msg.type === …)`-Branches, bis 6 Ebenen tief): ein `_p2pMessageHandlers`-Objekt `{ welcome: …, "peer-join": …, … }`, jeder Branch wird eine `_p2pHandle<Type>(msg, p2p)`-Methode, der Router schrumpft auf einen ~5-Zeilen-Dispatcher. Die Extraktion ist mechanisch sicher (jeder Branch ist schon return-terminiert + in sich geschlossen). **Hinweis**: die kanal-exklusiven Typen (`world-pull`, `llm-request`, `subworld-srv`, …) leben in `_p2pHandleChannelMessage` — die Trennung bleibt unangetastet, V9.44-c berührt nur `p2pHandleMessage`. Disziplin: reines verhaltensneutrales Refactoring, die ~2974 bestehenden Invarianten sind der Beweis (V9.30-Disziplin — keine neuen Invarianten nötig, nur alle alten grün + Audit-Strict 0). Danach V9.44-d..f (Infrastruktur-Konstanten, UI-Giganten, Game-Loop-Phasen). **Erst NACH dem Stamm-Pflege-Bogen** folgt die Hydrosphäre — V9.43-b der Hydrosphären-Atlas (`_computeHydrosphere()`: Surface-Sampling → Priority-Flood → D8-Flow → Flow-Accumulation → Netz-Extraktion), dann V9.43-c/d/e; `docs/hydrosphere.md` bleibt der Plan-Anker dafür. Bekannter Edge-Case aus V9.42-d: **5 Vertices an Vier-Chunk-Ecken** mit ~1 m Rest-Naht-Versatz — falls der Schöpfer sie sieht, eine kleine V9.42-e davorziehen, sonst ignorierbar. Detail in `docs/roadmap.md` §3 „Der Stamm-Pflege-Bogen" + `docs/code-hygiene.md`.
