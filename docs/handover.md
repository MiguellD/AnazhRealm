...
## Schnell-Lage (Stand 20.05.2026, V9.29)

**Du erbst eine sehr lebendige Welt**. **~2940 Playtest-Invarianten grün + 0 Audit-Strict-Failures**, ~36000 Zeilen in einer Datei, alles produktiv. (Der Playtest-Konsolen-Zähler driftet ±2-3 je Lauf — einige Checks sind bedingt; „Alle Invarianten OK" ist die Wahrheit, nicht die exakte Zahl.)

**Jüngste Welle — V9.29 (Ammo-Heap-Leck-Fix der Heightfield-Chunks + Gotcha-Sektion-Hygiene)**: der in V9.28 entdeckte pre-existing Bug ehrlich geheilt. `Ammo.destroy(body)` cascadiert NICHT zu seinen `btBvhTriangleMeshShape`/`btDefaultMotionState`/`btTriangleMesh`-Auxiliars; sie leakten pro Chunk-Lifecycle. V9.29 zentralisiert den Cleanup in **`_disposeChunkPhysics(mesh)`** — drei Konsumenten teilen ihn: `groundChunks`-Cleanup (Welt-Regen), `pruneDistantChunks` (Spieler verlässt Region), `_rebuildChunkPhysics` (Ring 10.5 terrain-Mod). Spiegel des `_disposeStaticCollision`-Helpers (Inseln/Bäume/Voxel) + V8.26-§6.4-`wallBoxes-Cleanup`-Musters. Build-Pfade legen jetzt `physicsShape`+`physicsMotionState` in userData ab (analog `physicsBody`+`physicsMesh`); inline-`btVector3` + `transform` werden direkt destroyed. **Plus: drei neue Bullets in der „Wichtige Gotchas (technisch)"-Sektion (V9.27/V9.28/V9.29 + Meta-Lehre)**. Plus stale Roadmap-Eintrag geheilt (W10 ext. „🔴 offen" → ✅ live seit V9.01). +5 Invarianten. **Lehre**: ich verletzte in V9.28 die V8.65/V9.27-Meta-Lehre („dokumentierte Lektion schützt nur, wenn aktiv abgerufen") — V8.26-§6.4-`wallBoxes-Cleanup`-Muster stand seit Mai 2026 in CLAUDE.md, ich rief es nicht ab; erst der OOM-Crash zwang mich, es zu finden. Disziplin: vor JEDER Ammo-Allokations-Welle `grep -n 'new Ammo\.' anazhRealm.js | grep <kontext>` rufen, prüfen welche Auxiliars vergeben werden, ZUERST den Dispose-Helper bauen, DANN den Body.

**Vor V9.29 — V9.28 (Phase 5c.1+ — V9.25 Phase 5b ehrlich abgeschlossen + heightData-Skip)**: zwei surgische Schnitte. **(1)** `updateCreatures` war der letzte missed Höhen-Konsument aus V9.25 Phase 5b — es las `state.groundHeightField` direkt am voxel-aware `getTerrainHeightAt` vorbei; Kreaturen schwebten in einer Voxel-Welt an der schlafenden Heightfield-Höhe statt am Voxel-Boden. Jetzt route auch dieser Pfad über `getTerrainHeightAt` — eine Funktion, alle Höhen-Konsumenten, V9.25-Audit-Intent ehrlich abgeschlossen. **(2)** Nach (1) liest in einer Voxel-Welt NIEMAND mehr `groundHeightField`/`minHeight`/`maxHeight` → die 256×256×3 Float-Allokation (~768 KB) + die 65k-Noise-Schleife in `generateTerrainWithParameters` wird in einer Voxel-Welt komplett übersprungen. Der Heightfield-Waterfall-Loop (256×256-Suche nach steilen Stellen, las heightData direkt) wird mit-gegated — eine Voxel-Welt baut Wasser über `_buildWaterPlane` (V9.25). **Pre-existing Bug entdeckt**: Ammo-Heap-Leck der btBvhTriangleMeshShape-Auxiliars beim Chunk-Dispose (`Ammo.destroy(body)` cascadiert NICHT zu shape/motionState/tmesh). Die surgische Welle-Heilung: V9.27-+V9.28-Tests fusioniert (halbiert die OOM-Schwelle). Die echte Heilung des Lecks ist eine spätere Welle. +7 Invarianten, 2935 grün. **Noch offen in 5c**: (a) der Ammo-Heap-Leck-Fix (eigene saubere Welle); (b) `_setHeightfieldDormant`-Code (no-op auf leerer chunkMap, lebt aber); (c) die grosse Heightfield-Code-Entfernung + Test-Suite-Umschreibung — sobald die Migrations-Erfahrung gefestigt ist + die Eingangs-Welt geflippt wird.

**Vor V9.28 — V9.27 (Voxel-Terrain-Bogen Phase 5c.1: Heightfield-Init für Voxel-Welten übersprungen)**: der erste ehrliche Schritt der grossen 5c-Aufräumarbeit. Eine voxel-basierte Welt baut die initialen 64 Heightfield-Chunks GAR NICHT mehr — vorher bauten sie + `_restoreVoxelTerrain` legte sie Sekunden später via `_setHeightfieldDormant` schlafen (wasteful). Der Skip-Gate in `generateTerrainWithParameters` liest `worldMeta.voxelTerrain` als persistente Wahrheit. Das Heightfield-`terrainMaterial` bleibt erhalten — wer per Chat `voxel terrain off` toggelt, bekommt Chunks vom Streaming-Ring nachgeladen (Reversibilität gewahrt). Die fresh-Eingangs-Welt (Playtest) bleibt heightfield (V9.26-Disziplin). +3 Invarianten. Davor V9.26 (Phase 5c-Start — Migrations-Flip + Chunk-Loch-Heilung).

**Vor V9.26 (Voxel-Terrain-Bogen Phase 5c-Start: Chunk-Loch-Heilung + Migrations-Flip)**: zwei Schöpfer-Browser-Befunde nach V9.25 geheilt. **(1) Chunk-Löcher** im erweiterten Sicht-Ring (4-8) — der V9.20-Chunk (`dimY` 68, `oy = base-50`) fasste die Oberfläche bei ridged-Spikes weiter draussen nicht ganz; mein V9.24-Robustheits-Fix räumte das Mesh ab → sichtbares Loch statt Fall-durch. Wurzel-Heilung: `dimY` 68→80, `oy = base-58`, Decke `base+86`, Marge ~22. Killplane zieht mit (`base-88`). **(2) Migrations-Flip**: eine GELADENE alte Welt (`!fresh`) ohne `voxelTerrain`-Flag wird in `ensureWorldMeta` voxel-basiert — der Schöpfer muss keine neue Welt erstellen, jede alte erbt 5b. Eine FRISCHE Eingangs-Welt (brandneuer Spieler, auch der Playtest) bleibt heightfield — Lehrling-freundlich + deterministisch für die Test-Suite. Ein expliziter Heightfield-Opt-out (V9.23-Dialog) bleibt heightfield. Davor V9.25 (Phase 5b — Konsumenten voxel-aware), V9.24 (Verbindungen geheilt — Sicht-Ring + Vegetation), V9.23 (Voxel als Default für neue Welten), V9.22 (Voxel-Terrain grünt — Instanced-Gras), V9.21 (Voxel-Welt-Geburt), V9.20 (Grösse + Hierarchie), V9.19 (Phase 4 Politur), V9.18 (Phase 4 — Höhlen/Überhänge).

**Nächste Welle: Phase 5c.2-5c.3 — der grosse Schnitt** (Eingangs-Welt-Flip + Test-Suite-Umschreibung + Heightfield-Code raus). Eine grosse, sorgfältige Welle. Davor möglich: kleine 5c-Politur (heightData-Arrays für Voxel-Welten ebenfalls überspringen) oder eine andere Vision-Richtung. Der volle Plan steht in `docs/roadmap.md` „Der Voxel-Terrain-Bogen". Disziplin: parallel bauen + beweisen, dann das Heightfield ablösen — nie das Funktionierende brechen. **Lies `CLAUDE.md` V9.27 ZUERST.**
