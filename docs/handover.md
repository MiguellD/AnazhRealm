...
## Schnell-Lage (Stand 19.05.2026, V9.18)
...
**Jüngste Welle — V9.18 (Voxel-Terrain-Bogen Phase 4 — Höhlen + Überhänge in der Generierung)**: das 3D-Dichte-Feld gebiert natürliche Höhlen, Tunnel + Überhänge — die Welt kommt mit ihnen zur Welt. `_terrainDensityAt` bekommt drei bewusste 3D-Schichten statt zwei uniformer Rausch-Bänder: (a) ein grobes Roughness-Band, (b) ein Überhang-Band (`nearSurf`-gewichtet, wölbt knapp an der Oberfläche Dichte → echte Überhänge + Felsbögen), (c) Wurm-Höhlen (zwei dekorrelierte ridged-Noise-Felder, ihr Produkt ist nur dort hoch, wo BEIDE Ridges scharf sind → ein dünner 1D-Tunnel; Perlin-Worm-Näherung). Eine Tiefen-Hüllkurve (`caveEnv = caveFloor × caveCeil`) hält die Höhlen zwischen `surf-6` (verborgen, durch Graben zu finden) und `base-28` — bei `base-35` ist `caveEnv` beweisbar 0, der Chunk-Boden bleibt fest, die V9.12-Garantie hält. +3 Invarianten. Davor V9.17 (Material-Erhaltungs-Kreis), V9.16 (Dichte-Gradienten-Normalen), V9.15 (Aufschütten), V9.14 (3D-Graben), V9.13 (per-Welt-Persistenz), V9.12 (72-m-Chunk-Säule), V9.11 (Naht-Wurzelheilung), V9.10 (Naht-Skirt + Welt-Feld-Farbe), V9.09 (Voxel-Chunk-Ring).

**Nächste Welle: Phase 5 — Materialien/Shader + die Ablösung des Heightfields** (`aField` per Vertex auf dem Voxel-Mesh, Terrain-Material-Harvest, Cel-Shading; das Heightfield wird abgelöst, alte Welten migrieren). Der volle Plan steht in `docs/roadmap.md` „Der Voxel-Terrain-Bogen". Disziplin: parallel bauen + beweisen, dann das Heightfield ablösen — nie das Funktionierende brechen. **Lies `CLAUDE.md` V9.18 ZUERST.**
