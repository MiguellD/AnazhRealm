...
## Schnell-Lage (Stand 19.05.2026, V9.21)

**Du erbst eine sehr lebendige Welt**. **~2913 Playtest-Invarianten grün + 0 Audit-Strict-Failures**, ~33000 Zeilen in einer Datei, alles produktiv. (Der Playtest-Konsolen-Zähler driftet ±2-3 je Lauf — einige Checks sind bedingt; „Alle Invarianten OK" ist die Wahrheit, nicht die exakte Zahl.)

**Jüngste Welle — V9.21 (Voxel-Welt-Geburt)**: die in V9.13 benannte „kleine UX-Naht" geschlossen — eine Welt kann jetzt voxel-basiert GEBOREN werden, statt zur Laufzeit per Chat `voxel terrain on` umgeschaltet. Der Neue-Welt-Dialog hat eine `#new-world-voxel`-Checkbox; `createNewWorld` nimmt ein optionales `voxelTerrain`-Flag (Default false, Backward-Compat) und setzt `snap.worldMeta.voxelTerrain = true` direkt nach `_buildEmptyWorldSnapshot` — das Flag reist im Snapshot, `_restoreVoxelTerrain` (V9.13) aktiviert das Voxel-Terrain beim ersten Aufbau. +3 Invarianten. Davor V9.20 (Voxel-Terrain Grösse + Hierarchie — drei Oktaven kontinental/ridged/Detail, der Chunk wuchs auf eine 122-m-Säule), V9.19 (Phase 4 Politur — feines Band restauriert + Höhlen aus EINEM ridged-Feld), V9.18 (Phase 4 — Höhlen/Überhänge in der Generierung), V9.17 (Material-Erhaltungs-Kreis), V9.16 (Dichte-Gradienten-Normalen), V9.15 (Aufschütten), V9.14 (3D-Graben), V9.13 (per-Welt-Persistenz).

**Nächste Welle: Phase 5 — Materialien/Shader + die Ablösung des Heightfields** (`aField` per Vertex auf dem Voxel-Mesh, Terrain-Material-Harvest, Cel-Shading; das Heightfield wird abgelöst, alte Welten migrieren). Der volle Plan steht in `docs/roadmap.md` „Der Voxel-Terrain-Bogen". Disziplin: parallel bauen + beweisen, dann das Heightfield ablösen — nie das Funktionierende brechen. **Lies `CLAUDE.md` V9.21 ZUERST.**
