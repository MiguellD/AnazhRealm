...
## Schnell-Lage (Stand 21.05.2026, V9.44-d)
...
**Empfehlung für den nächsten Agenten**: die nächste Welle ist **V9.44-e — die UI-Giganten zerlegen (Stamm-Pflege-Bogen Sub-Welle 5/6)**. V9.44-a/b/c/d sind erledigt (a: Persistenz-Schema; b: `_applyPlayerSpeed`; c: Mesh-Router-Dispatch-Table; d: Infrastruktur-Konstanten + ein getrennter Flaky-Heilungs-Commit der drei Journal-Cap-Tests). Vor dem ersten Code-Block **`docs/code-hygiene.md`** ganz lesen (Befund, Vision-Abgleich — KEIN neues Modul, kein State-Manager; sechs Sub-Wellen a-f; bewusste Nicht-Wellen). V9.44-e zerlegt die zwei grössten UI-Builder: `_renderWorkshopDOM` (658 Z., 159 DOM-Operationen) + `initStatusPanel` (174 Z.) in benannte Sub-Render-Methoden (`_workshopRenderStatsPanel` existiert schon als Vorbild — so weiter schneiden); die 4× fast-identische `data-cmd`-Delegation → ein `_wireCmdDelegation(container)`-Helper. UI-Render-Code ist gut extrahierbar, weil er in sich geschlossen DOM baut. Disziplin: reines verhaltensneutrales Refactoring, die ~2974 bestehenden Invarianten sind der Beweis (V9.30-Disziplin — keine neuen Invarianten nötig, nur alle alten grün + Audit-Strict 0). Danach V9.44-f (Game-Loop-Phasen-Struktur — `startEternalLoop` 696 Z.; die heikelste, bewusst zuletzt, braucht zusätzlich einen Browser-Test). **Erst NACH dem Stamm-Pflege-Bogen** folgt die Hydrosphäre — V9.43-b der Hydrosphären-Atlas (`_computeHydrosphere()`: Surface-Sampling → Priority-Flood → D8-Flow → Flow-Accumulation → Netz-Extraktion), dann V9.43-c/d/e; `docs/hydrosphere.md` bleibt der Plan-Anker dafür. Bekannter Edge-Case aus V9.42-d: **5 Vertices an Vier-Chunk-Ecken** mit ~1 m Rest-Naht-Versatz — falls der Schöpfer sie sieht, eine kleine V9.42-e davorziehen, sonst ignorierbar. Detail in `docs/roadmap.md` §3 „Der Stamm-Pflege-Bogen" + `docs/code-hygiene.md`.
