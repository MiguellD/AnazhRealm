...
## Schnell-Lage (Stand 19.05.2026, V9.09)

**Du erbst eine sehr lebendige Welt**. **~2874 Playtest-Invarianten grün + 0 Audit-Strict-Failures**, ~33000 Zeilen in einer Datei, alles produktiv. (Der Playtest-Konsolen-Zähler driftet ±2-3 je Lauf — einige Checks sind bedingt; „Alle Invarianten OK" ist die Wahrheit, nicht die exakte Zahl. Die Suite ist seit V8.96 deterministisch — der letzte benannte flaky Test ist geheilt.)

**Jüngste Welle — V9.09 (Voxel-Terrain-Bogen Phase 2b — der Voxel-Chunk-Ring)**: das Voxel-Terrain wird begehbar. Voxel-Chunks streamen um den Spieler (`_ensureVoxelChunkAt`/`_tickVoxelChunkStreaming`/`_pruneDistantVoxelChunks` spiegeln Zeile für Zeile das erprobte Heightfield-Streaming). `state.voxelTerrainActive` (Default aus, parallel hinter dem Flag) schaltet um: aktiv → der Voxel-Ring streamt + das Heightfield ruht (`_setHeightfieldDormant` — Mesh/Gras unsichtbar, Kollision aus dem physicsWorld genommen, NICHT zerstört, reversibel); aus → die Voxel-Chunks fallen, das Heightfield erwacht. Chat `voxel terrain on`/`off`. +9 Invarianten. Honest Cut: die per-Welt-Persistenz des Flags + die voxel-basierte neue Welt sind Phase 2c. **Nächste Welle: Phase 2c — per-Welt-Persistenz** (`worldMeta.voxelTerrain` persistiert das Flag; eine neue Welt kann voxel-basiert sein). Danach Phase 3 (3D-`modify_terrain` — echtes Graben), Phase 4 (Höhlen/Überhänge in der Generierung), Phase 5 (Materialien/Shader/Ablösung). Der volle Plan steht in `docs/roadmap.md` „Der Voxel-Terrain-Bogen". Disziplin: parallel bauen + beweisen, dann das Heightfield ablösen — nie das Funktionierende brechen. **Lies `CLAUDE.md` V9.09 ZUERST.**
