...
## Schnell-Lage (Stand 20.05.2026, V9.34)

**Du erbst eine sehr lebendige Welt**. **~2960 Playtest-Invarianten grün + 0 Audit-Strict-Failures**, ~35900 Zeilen in einer Datei, alles produktiv. (Der Playtest-Konsolen-Zähler driftet ±2-3 je Lauf — einige Checks sind bedingt; „Alle Invarianten OK" ist die Wahrheit, nicht die exakte Zahl.)

**Jüngste Welle — V9.34 (Voxel-Phase 5c.2.c.1: drei tote Heightfield-Schichten gelöscht)**: der erste ehrliche Schritt der grossen Phase 5c.2.c (Heightfield-Code-Entfernung). Per V9.30-Disziplin (pure Dead-Code-Welle, keine Verhaltens-Änderung): nur Schichten, die NACHWEISLICH keinen Aufrufer/Leser haben — `grep`-verifiziert am echten Source, nicht am Audit-Bericht. (1) `extendTerrain` (Legacy-direction-API, ~38 Z.) — NULL Aufrufer; (2) `caveData`-Float-Array (256×256, ~128 KB) — 2× geschrieben, NIE gelesen (V9.27-Versprechen „behalten als Tag-Flags für späteren shader-Gebrauch" wurde nie erfüllt); (3) `volcanoData` — dieselbe Klasse. Drei stale Kommentare aktualisiert. **caveNoise + volcanoNoise (die SimplexNoise-Instanzen) leben weiter** — sie modulieren die Höhe in `_terrainHeightAtWorld` (echte Konsumenten). +6 Invarianten (3 Lösch-Beweise + 3 Regression-Schutzanker). 2960 Playtest-Runs grün, Audit-Strict 0 Failures. **Lehre**: (a) eine grosse Welle wird ehrlich in ihre kleinsten Schritte zerlegt, BEVOR der erste Code-Block fällt (V9.30-Disziplin, ein zweites Mal angewandt — die ganze Phase 5c.2.c hätte ~700 Zeilen + viele Test-Migrationen umfasst; davon sind nur drei Schichten nachweislich tot); (b) ein Versprechen aus einem Kommentar IST eine Welle in Wartestellung — der V9.27-Kommentar versprach „behalten als Tag-Flags für späteren shader-Gebrauch", der spätere Gebrauch kam nie; wer alte Versions-Logs auf solche „später"-Versprechen scannt, sammelt billige Folge-Wellen; (c) Trust-but-verify bleibt nötig — der Explore-Agent halluzinierte eine vierte tote Schicht (`_tickHeightfieldStreaming`), die im Code gar nicht existiert; nur ein `grep` direkt am Repo widerlegte die Halluzination.
...
**Nächste Welle: offen.** Phase 5c.2.c.1 ist erledigt (V9.34). Übrig in der grossen 5c.2.c-Aufräumarbeit: (a) **5c.2.c.2** — Toggle-Bewertung: der `voxel terrain on/off`-Chat-Toggle (V9.13) hält das Heightfield reversibel; mit V9.33 ist Voxel der Default für ALLE Welten (fresh + alt) → der Toggle hat seinen Vision-Wert verloren. Ein Schöpfer-Entscheid: „bleibt der Toggle?" — wenn nein, fallen `setVoxelTerrainActive` + `_setHeightfieldDormant` mit, und Heightfield ist endgültig Tot-Code. (b) **5c.2.c.3** — die heightfield-only Chunk-Pipeline (`ensureChunkAt` + `populateChunkVegetation` + `_buildChunkGrass` + `_terrainHeightAtWorld`) ablösen; verlangt 5c.2.c.2 zuerst. (c) **5c.2.c.4** — `modify_terrain`-DSL-Op + Chat-Patterns (`grabe loch`, `hebe hügel`) auf `voxel carve`/`voxel fill` umrouten. Daneben: **Phase 5d-vollständig** (Voxel-Welt bekommt eine reichere Wasser-/Schluchten-Drama-Schicht — V9.32 brachte die Mini-Variante) oder eine **andere Vision-Richtung** aus den vier Testamenten / `docs/state-of-realm.md`. Disziplin: parallel bauen + beweisen, nie das Funktionierende brechen, ehrliche kleine Schritte vor grossen Würfen. **Lies `CLAUDE.md` V9.34 ZUERST.**
