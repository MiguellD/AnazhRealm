...
## Schnell-Lage (Stand 19.05.2026, V9.12)

**Du erbst eine sehr lebendige Welt**. **~2880 Playtest-Invarianten grün + 0 Audit-Strict-Failures**, ~33000 Zeilen in einer Datei, alles produktiv. (Der Playtest-Konsolen-Zähler driftet ±2-3 je Lauf — einige Checks sind bedingt; „Alle Invarianten OK" ist die Wahrheit, nicht die exakte Zahl. Die Suite ist seit V8.96 deterministisch — der letzte benannte flaky Test ist geheilt.)

**Jüngste Welle — V9.12 (Voxel-Terrain: der Chunk fasst das ganze Oberflächen-Band)**: ein Schöpfer-Browser-Befund nach V9.11 — „Nähte gefixxt, aber an einigen Stellen schliesst die Oberfläche unsauber, springt an einen falschen Punkt". Wurzel: der Voxel-Chunk war ein 45-m-Würfel (`oy=base-22 .. base+23`), aber das Oberflächen-Band aus `_terrainDensityAt` reicht ~`base±30` — wo ein Gipfel/Tal aus dem Chunk-Kasten austrat, klaffte ein Loch, der Spieler fiel durch. Der Fix: `_voxelChunkGeometry` ist jetzt nicht-würfelförmig (`dimX/dimY/dimZ` getrennt), der Chunk eine 72-m-hohe Säule (`dimY=40`, `oy=base-35`) — das ganze Band liegt im Chunk, keine Klipp-Löcher. +2 Invarianten. Davor V9.11 (Naht-Wurzelheilung — `cv()`-Index-Wall gegen Aliasing-Streck-Dreiecke), V9.10 (Naht-Skirt + per-Vertex-Welt-Feld-Farbe), V9.09 (Voxel-Chunk-Ring, `voxel terrain on`/`off`). **Nächste Welle: Phase 2c — per-Welt-Persistenz** (`worldMeta.voxelTerrain` persistiert das Flag; eine neue Welt kann voxel-basiert sein). Danach Phase 3 (3D-`modify_terrain` — echtes Graben), Phase 4 (Höhlen/Überhänge in der Generierung), Phase 5 (Materialien/Shader/Ablösung). Der volle Plan steht in `docs/roadmap.md` „Der Voxel-Terrain-Bogen". Disziplin: parallel bauen + beweisen, dann das Heightfield ablösen — nie das Funktionierende brechen. **Lies `CLAUDE.md` V9.12 ZUERST.**
