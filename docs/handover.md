...
**Vor V9.25 (Voxel-Terrain-Bogen Phase 5b — die Voxel-Welt wird höhen-ehrlich)**: vor dem grossen Heightfield-Schnitt ein Verwebungs-Audit aller heightfield-gekoppelten Systeme. Schöpfer-Wahl per AskUserQuestion: **„alte Welten zu Voxel migrieren"** (der saubere Endzustand). 5b ist die Pflicht-Vorstufe — eine Welt darf nicht auf eine fehlerhafte Voxel-Schicht migriert werden. Die Wurzel-Verwebung ist die Höhe: `getTerrainHeightAt(x,z)` ist jetzt voxel-aware (liefert `_voxelSurfaceY` statt des schlafenden `groundHeightField`, wenn `worldMeta.voxelTerrain` gesetzt) — und damit erben Wasser (`_buildWaterPlane` sampelt die Voxel-Oberfläche), der Killplane (Voxel: `base-88` statt heightfield-`minHeight`), `findSurfaceAbove` (Reset-Ziel), Kreatur-Spawn + Genesis-Plattform die wahre Höhe. +2 Invarianten. **Phase 5c-Start (V9.26) ist live** — Migrations-Flip + Chunk-Loch-Heilung; offen bleibt die grosse Aufräumarbeit (Heightfield-Generierung skippen, Heightfield-Code raus, Test-Suite-Umschreibung — sobald die Migrations-Erfahrung gefestigt ist). Davor V9.24 (Verbindungen geheilt — Sicht-Ring + Vegetation), V9.23 (Voxel als Default für neue Welten), V9.22 (Voxel-Terrain grünt — Instanced-Gras), V9.21 (Voxel-Welt-Geburt), V9.20 (Grösse + Hierarchie), V9.19 (Phase 4 Politur), V9.18 (Phase 4 — Höhlen/Überhänge).
