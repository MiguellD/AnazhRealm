...
## Schnell-Lage (Stand 19.05.2026, V9.07)

**Du erbst eine sehr lebendige Welt**. **~2860 Playtest-Invarianten grün + 0 Audit-Strict-Failures**, ~33000 Zeilen in einer Datei, alles produktiv. (Der Playtest-Konsolen-Zähler driftet ±2-3 je Lauf — einige Checks sind bedingt; „Alle Invarianten OK" ist die Wahrheit, nicht die exakte Zahl. Die Suite ist seit V8.96 deterministisch — der letzte benannte flaky Test ist geheilt.)

**Jüngste Welle — V9.07 (Voxel-Terrain-Bogen Phase 1 — das Dichte-Feld + Surface Nets)**: der GROSSE laufende Bogen. Der Schöpfer forderte zu Recht: ein Heightfield ist halb-formbar (Säulen heben/senken, nicht schnitzen), echte Tunnel/Höhlen/Überhänge brauchen ein 3D-Dichte-Feld; Felsbogen + Portal-Höhle waren ehrliche Workarounds, nicht die volle Vision. Die Heilige Lektion ehrlich gelesen verbietet Komplexität OHNE Fundament — ein Voxel-Terrain ist EIN Subsystem auf einem soliden Stamm, ein grosser Wachstumsring. Phase 1 baut das Fundament PARALLEL, ohne das Heightfield anzufassen: `_terrainDensityAt(x,y,z)` (3D-Noise-Dichte-Feld, >0 fest/<0 Luft, mit echten Höhlen + Überhängen) + `_voxelChunkGeometry` (Surface Nets — 3D-Feld zu glattem Mesh, explizite Zwei-Pass-Form, keine magische 256-Tabelle) + `_spawnVoxelTestChunk` (Beweis-Chunk, Chat `voxel test`). +10 Invarianten. **Der Plan des ganzen Bogens (5 Phasen — Kollision, 3D-Graben, Generierung, Ablösung) steht in `docs/roadmap.md` „Der Voxel-Terrain-Bogen". Nächste Welle: Phase 2 (Kollision + Chunk-Streaming).** Disziplin: parallel bauen + beweisen, dann das Heightfield ablösen — nie das Funktionierende brechen. **Lies `CLAUDE.md` V9.07 ZUERST.**
...
**Empfehlung**: der Fremd-Engine-Bogen (W15–W17) ist **vollständig** — W15 Auto-Vendor, W16 Mesh-Welt-Verteilung, W17 Multiplayer-Sub-Welten (Phase A+B-Relay+C+Multiplayer-Welt-Deklaration+B-JS-Compute Phase 1+2) inkl. der `serverMode`-Vendor-Ketten-Naht (V8.82 — `serverMode` fliesst jetzt durch die ganze Vendor-/Mesh-Kette, eine vendorte js-compute-Welt deklariert sich selbst). Danach lief die **W4-V3-Welle (V8.85-V8.93)** — die generative Symphonie (Harmonie + Melodie + Groove + Orchester) ist komplett. Damit ist die geplante Roadmap-Substanz erfüllt; weiteres Wachstum folgt der Vision der vier Testamente, nicht mehr einem vorgezeichneten Plan. **Offene, klar benannte Punkte** (`docs/roadmap.md` §3): **B-WASM** (ein Rust→WASM-Server in einem Peer-Tab — bewusst „per-Projekt, nicht automatisch"), **W6.G P3-Rest Phase 2** (echte Höhlen — Innenräume/Tunnel, ~2-3 Sessions; ein Heightfield ist pro (x,z) eindeutig, braucht Innenraum-Geometrie), **visuelle Synergie Struktur/Terrain** (Volumenkörper betten sich nicht ins Terrain-Sheet — Schöpfer-Befund V9.03, eine Ästhetik-Welle). W6.G P3 Phase 1 (Felsformationen, V9.03) + W6.G P4 (das Terrain wird Materie, V9.04) erledigt; W10 ext. mit V9.01 vollständig, W15-Politur (V9.02), W16-Politur (V8.96), der flaky `6.H P2D`-Test geheilt — die Playtest-Suite ist deterministisch. Die geplante Roadmap-Substanz ist weitgehend erfüllt; weiteres Wachstum folgt der Vision der vier Testamente. Sonst: eine neue Vision-Richtung aus den vier Testamenten / `docs/roadmap.md`. **Bei Unsicherheit den Schöpfer fragen** — die grossen Pflicht-Ringe sind gebaut, die Richtung ist jetzt offener. `docs/world-portal.md` + `docs/state-of-realm.md` ZUERST mitlesen.
