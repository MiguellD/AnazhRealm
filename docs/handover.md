...
## Schnell-Lage (Stand 17.05.2026, V8.55)

**Du erbst eine sehr lebendige Welt**. **2362 Playtest-Invarianten grün + 0 Audit-Strict-Failures**, ~28000 Zeilen in einer Datei, alles produktiv.

**Jüngste Welle — V8.55 (W13 Phase 2: Bauplan-Signaturen)**: Phase 1 gab dem Avatar einen Schlüssel; Phase 2 wendet ihn an — ein eigener Bauplan trägt jetzt eine ed25519-Signatur. **Signiert wird die SUBSTANZ, nicht der Name** (`_canonicalBlueprint`: Rolle + Parts + Verbindungen, deterministisch) — ein Werk IST seine Substanz; eine Signatur überlebt damit Recipe-Import + Fusion (die Namen umtaufen) und bricht nur bei echter Substanz-Änderung. `verifyBlueprintSignature` liefert vier Stufen (unsigned/valid/modified/forged), **bei jedem Werkstatt-Render frisch geprüft — kein Mutations-Hook** (Robustheit durch Nicht-Kopplung). Die Signatur reist mit dem Bauplan durch Save/Welt-Tor/Fusion (`buildStateSnapshot` + `loadState` tragen die vier Felder). Werkstatt-Stats-Panel: „Signatur"-Zeile + [Signieren]-Knopf. Ein Commit, playtest-grün. **Lies `CLAUDE.md` V8.55 + `docs/world-portal.md` §4 ZUERST.**

**Welle davor — V8.54 (W13 Phase 1: der Vibe-Pass)**: W13 macht den Avatar zur **Self-Sovereign Identity**. Phase 1 ist die Schlüssel-Grundlage: ein ed25519-Schlüsselpaar, lokal beim ersten Erwachen erzeugt (WebCrypto nativ — kein Vendoring, keine CSP-Lockerung). Der private Schlüssel lebt GLOBAL in `localStorage["anazh.vibePass"]`, NIE im Welt-Save — die Sicherheits-Wand. Zwei Primitive `_vibeSign`/`_vibeVerify`, eine sichtbare + sicherbare Identität im Spieler-Drawer (Fingerprint, Export/Import).

**Offen — die nächste Welle**: **W13 Phase 3** — die Signatur-Prüfung wandert aus der Werkstatt in die Welt: ein Mitspieler / eine importierte Welt / ein Portal zeigt „signiert von <Autor>". Plus das Portal-Manifest bekommt das `authorPubKey`/`signature`-Feld aus `docs/world-portal.md` §3.3. Phase 2 lieferte das Cross-Autor-Fundament (`_vibeVerify` prüft jedes Autors Werk; eine Playtest-Invariante beweist es mit einem fremden Schlüssel). Alternativ einschiebbar: W11 V4 (Voice-Sync, ~1 Session). Siehe `docs/world-portal.md` §4. (Die älteren „Offen"-Notizen unten — Schatten/Performance — sind mit V8.48–V8.50 abgearbeitet.)
...
| **W13** Vibe-Pass — **Phase 1+2 live (V8.55)** | Phase 1: die Schlüssel-Grundlage (ed25519-Keypair, Sign/Verify-Primitive). Phase 2: Bauplan-Signaturen (`signBlueprint`/`verifyBlueprintSignature`, Signatur über die Substanz, Werkstatt-Anzeige). Offen: Phase 3 (Signatur-Prüfung in Multi-User / Welt-Tor / Portal-Manifest). | 3-5 Sessions Rest | sehr hoch |
...
**Empfehlung**: **W13 Phase 3** — Phase 1+2 (V8.54+V8.55) legten das Fundament: der Avatar hat einen Schlüssel, eigene Baupläne tragen Signaturen, die Werkstatt zeigt den Status. Phase 3 trägt die Prüfung in die Welt: ein Mitspieler / eine importierte Welt / ein Portal zeigt „signiert von <Autor>"; das Portal-Manifest bekommt das `authorPubKey`/`signature`-Feld aus `world-portal.md` §3.3. Das Cross-Autor-Fundament steht schon (`_vibeVerify` prüft jedes Autors Werk). Alternativ einschiebbar: W11 V4 (Voice-Sync, ~1 Session, klein). `docs/world-portal.md` §4 + den V8.55-Eintrag in `CLAUDE.md` ZUERST lesen. Volle Detail-Tabelle in `roadmap.md`.
