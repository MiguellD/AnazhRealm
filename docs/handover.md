...
## Schnell-Lage (Stand 19.05.2026, V9.03)

**Du erbst eine sehr lebendige Welt**. **~2826 Playtest-Invarianten grün + 0 Audit-Strict-Failures**, ~33000 Zeilen in einer Datei, alles produktiv. (Der Playtest-Konsolen-Zähler driftet ±2-3 je Lauf — einige Checks sind bedingt; „Alle Invarianten OK" ist die Wahrheit, nicht die exakte Zahl. Die Suite ist seit V8.96 deterministisch — der letzte benannte flaky Test ist geheilt.)

**Jüngste Welle — V9.03 (W6.G P3 Phase 1 — Felsformationen)**: die einzige benannte grosse offene Roadmap-Welle (Terrain-Höhlen/Überhänge/Klippen) — chirurgisch angegangen, durch Wiederverwendung statt Voxel-Rewrite. Der Reframe: ein Überhang ist ein Ding AUF dem Terrain, keine Eigenschaft DES Terrains. `felsbogen` (ein Trilithon — zwei Stein-Pfeiler + ein Sturz) IST ein Überhang: die Per-Sub-Mesh-Box-Kollision (`_buildArchitectureCollision`) lässt zwischen den Pfeilern eine echte begehbare 3.4-m-Lücke. `felsturm` ist eine ~17-m-Fels-Nadel (Klippen-Dramatik). Beide emergieren über das Welt-Affinitäts-Feld in einem eigenen seltenen Landmark-Pass von `populateChunkVegetation` (NICHT in der Streu-Affinitäts-Wahl — ein Felsbogen ist ein Superset eines Felsblocks, er hätte ihn verdrängt). +10 Invarianten. Echte Höhlen (Innenräume/Tunnel) bleiben ehrlich als Phase 2 benannt. **Lies `CLAUDE.md` V9.03 ZUERST.**
