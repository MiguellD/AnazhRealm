...
**Empfehlung für den nächsten Agenten**: die nächste Welle ist **V9.43-b — der Hydrosphären-Atlas**. Vor dem ersten Code-Block **`docs/hydrosphere.md`** ganz lesen (Algorithmus, Datenstrukturen, Wellen-Schnitt, Risiken — die ausführliche Planung des Wasser-Systems). V9.43-b baut `_computeHydrosphere()` (Surface-Sampling über eine ~2-km-Region → Priority-Flood-Depression-Filling → D8-Flow-Direction → Flow-Accumulation → Netz-Extraktion); reine Daten in `state.hydrosphere`, KEIN Rendering, KEIN Carven — vollständig headless-prüfbar. Danach V9.43-c (Flüsse/Seen rendern + Wasserfälle re-verankern), V9.43-d (Carven via `_terrainDensityAt`), V9.43-e (Klang). Plus ein bekannter Edge-Case aus V9.42-d: **5 Vertices an Vier-Chunk-Ecken** haben einen Rest-Naht-Versatz (~1 m) — falls der Schöpfer sie im Browser sieht, eine kleine V9.42-e davorziehen; falls nicht, ignorierbar. Detail in `docs/roadmap.md` §3 „Voxel-Surface-Politur" + `docs/hydrosphere.md`.
