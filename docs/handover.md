...
## Schnell-Lage (Stand 19.05.2026, V9.10)

**Du erbst eine sehr lebendige Welt**. **~2878 Playtest-Invarianten grün + 0 Audit-Strict-Failures**, ~33000 Zeilen in einer Datei, alles produktiv. (Der Playtest-Konsolen-Zähler driftet ±2-3 je Lauf — einige Checks sind bedingt; „Alle Invarianten OK" ist die Wahrheit, nicht die exakte Zahl. Die Suite ist seit V8.96 deterministisch — der letzte benannte flaky Test ist geheilt.)

**Jüngste Welle — V9.10 (Voxel-Terrain Phase 2b-Politur — Naht + Welt-Feld-Farbe)**: ein Schöpfer-Browser-Befund nach V9.09 — „die Übergänge sind nicht sauber, alles scheint das gleiche Biom". Zwei Heilungen: (1) der 1-Zellen-Naht-Skirt — `_ensureVoxelChunkAt` mesht `dim+1` Zellen statt `dim`, eine Überlappung in den Nachbar-Chunk; die Surface-Nets-Naht-Quads entstehen, die Flächen stossen nahtlos zusammen (das deterministische Dichte-Feld macht die Überlapp-Zelle in beiden Chunks identisch). (2) `_attachVoxelFieldColors` — per-Vertex-Farbe aus `worldFieldAt` (Biom-Farben wie der Heightfield-Terrain-Shader, als `color`-Attribut + `vertexColors`, kein Shader). Das Voxel-Terrain trägt dieselben Biom-Regionen wie der Boden. +4 Invarianten. Davor V9.09 (Voxel-Chunk-Ring — Voxel-Chunks streamen um den Spieler, `state.voxelTerrainActive` legt das Heightfield reversibel schlafen, Chat `voxel terrain on`/`off`). **Nächste Welle: Phase 2c — per-Welt-Persistenz** (`worldMeta.voxelTerrain` persistiert das Flag; eine neue Welt kann voxel-basiert sein). Danach Phase 3 (3D-`modify_terrain` — echtes Graben), Phase 4 (Höhlen/Überhänge in der Generierung), Phase 5 (Materialien/Shader/Ablösung). Der volle Plan steht in `docs/roadmap.md` „Der Voxel-Terrain-Bogen". Disziplin: parallel bauen + beweisen, dann das Heightfield ablösen — nie das Funktionierende brechen. **Lies `CLAUDE.md` V9.10 ZUERST.**
