...
## Schnell-Lage (Stand 19.05.2026, V9.13)

**Du erbst eine sehr lebendige Welt**. **~2886 Playtest-Invarianten grün + 0 Audit-Strict-Failures**, ~33000 Zeilen in einer Datei, alles produktiv. (Der Playtest-Konsolen-Zähler driftet ±2-3 je Lauf — einige Checks sind bedingt; „Alle Invarianten OK" ist die Wahrheit, nicht die exakte Zahl. Die Suite ist seit V8.96 deterministisch — der letzte benannte flaky Test ist geheilt.)

**Jüngste Welle — V9.13 (Voxel-Terrain-Bogen Phase 2c — per-Welt-Persistenz)**: der `voxelTerrainActive`-Zustand überlebt Reload + Welt-Wechsel. `setVoxelTerrainActive` schreibt `worldMeta.voxelTerrain` (per-Welt, im Snapshot — das `gameMode`-Muster) + ruft `saveState`; `_restoreVoxelTerrain` wird beim Welt-Aufbau VOR dem Game-Loop gerufen und aktiviert das Voxel-Terrain, wenn das Flag gesetzt ist (`opts.persist:false` — kein Save mitten im Init). Eine Welt bleibt voxel-basiert. +6 Invarianten. Honest Cut: ein „Voxel"-Häkchen im Neue-Welt-Dialog bleibt eine kleine benannte UX-Naht. Davor V9.12 (der Chunk fasst das ganze Oberflächen-Band — 72-m-Säule statt 45-m-Würfel, keine Klipp-Löcher), V9.11 (Naht-Wurzelheilung — `cv()`-Index-Wall), V9.10 (Naht-Skirt + per-Vertex-Welt-Feld-Farbe), V9.09 (Voxel-Chunk-Ring, `voxel terrain on`/`off`). **Nächste Welle: Phase 3 — 3D-`modify_terrain` (echtes Graben)** — `modify_terrain` schnitzt das Dichte-Feld in 3D (eine Kugel „Luft" subtrahiert → ein echtes Loch/Tunnel/Höhle). Danach Phase 4 (Höhlen/Überhänge in der Generierung), Phase 5 (Materialien/Shader/Ablösung). Der volle Plan steht in `docs/roadmap.md` „Der Voxel-Terrain-Bogen". Disziplin: parallel bauen + beweisen, dann das Heightfield ablösen — nie das Funktionierende brechen. **Lies `CLAUDE.md` V9.13 ZUERST.**
