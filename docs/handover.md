...
## Schnell-Lage (Stand 19.05.2026, V9.16)

**Du erbst eine sehr lebendige Welt**. **~2900 Playtest-Invarianten grün + 0 Audit-Strict-Failures**, ~33000 Zeilen in einer Datei, alles produktiv. (Der Playtest-Konsolen-Zähler driftet ±2-3 je Lauf — einige Checks sind bedingt; „Alle Invarianten OK" ist die Wahrheit, nicht die exakte Zahl. Hinweis: die V8.28/V8.31-Fog-Checks sind gelegentlich timing-flaky — sie sampeln den laufenden Spielzustand; 1 roter Lauf unter mehreren grünen ist kein echter Bruch.)

**Jüngste Welle — V9.16 (Voxel-Terrain-Politur — Dichte-Gradienten-Normalen)**: ein Schöpfer-Browser-Befund — die Voxel-Oberfläche zeigt ein hartes Rauten-/Trapez-Facetten-Muster (ausdrücklich NICHT die Cel-Stufen). Wurzel: `_voxelChunkGeometry` nutzte `computeVertexNormals` — das mittelt die Flächen-Normalen der triangulierten Quads + nimmt jede Quad-Diagonal-Faltung auf. Der Fix: die Normalen kommen aus dem Dichte-Feld-Gradienten (`−∇d` per 6-Sample-`_terrainDensityAt`-Differenz je Vertex) — die wahre, glatte Iso-Oberflächen-Normale, unabhängig von der Mesh-Facettierung. +1 Invariante. Davor V9.15 (Aufschütten — `fillVoxelSphere`, RMB), V9.14 (3D-Graben — `carveVoxelSphere`, LMB), V9.13 (per-Welt-Persistenz), V9.12 (72-m-Chunk-Säule), V9.11 (Naht-Wurzelheilung), V9.10 (Naht-Skirt + Welt-Feld-Farbe), V9.09 (Voxel-Chunk-Ring).

**Offener Schöpfer-Befund (noch nicht adressiert)**: „die gegrabenen Materialien sind im Inventar fixiert, ich kann sie nicht in die Hotbar nehmen + platzieren". Das ist kein Bug, sondern die Hylomorphismus-Design-Grenze: Roh-Materialien sind Werkstatt-Zutaten, die Hotbar trägt nur Baupläne. Die offene Frage an den Schöpfer: soll das Voxel-Terrain einen direkten „gegrabenes Material → Boden aufschütten"-Erhaltungs-Kreis bekommen (graben verbraucht Voxel + gibt Material, RMB-fill verbraucht Material)? Eine bewusste eigene Welle wert, kein Schnellschuss.

**Nächste Welle: Phase 4 — Höhlen/Überhänge in der Voxel-GENERIERUNG** (das 3D-Noise gebiert natürliche Höhlen — `_terrainDensityAt` hat die 3D-Bänder schon, eine Welle die sie zu echten begehbaren Höhlen formt) — ODER der Voxel-Material-Kreis (siehe offener Befund). Danach Phase 5 (Materialien/Shader/Ablösung — das Heightfield wird abgelöst, alte Welten migrieren). Der volle Plan steht in `docs/roadmap.md` „Der Voxel-Terrain-Bogen". Disziplin: parallel bauen + beweisen, dann das Heightfield ablösen — nie das Funktionierende brechen. **Lies `CLAUDE.md` V9.16 ZUERST.**
