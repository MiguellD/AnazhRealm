...
## Schnell-Lage (Stand 21.05.2026, V9.43-b — der Hydrosphären-Atlas)

**V9.43-b — der Hydrosphären-Atlas (Wasser-Ultiversum Welle D.2)**: die erste Welle nach dem Stamm-Pflege-Bogen, der Beginn des vollen Wasser-Systems (`docs/hydrosphere.md`). `_computeHydrosphere()` leitet aus der Voxel-Surface ein deterministisches Drainage-Netz ab — Flüsse, Seen, Wasserfälle als reine Daten in `state.hydrosphere`. Acht `_hydro*`-Phasen-Methoden im Orchestrator-Muster (pre-cut gebaut — die V9.44-f-Disziplin sofort gelebt): Surface-Sampling → Ozean-Maske → Priority-Flood (Barnes 2014, Min-Heap) → D8-Flow-Direction → Flow-Accumulation → Netz-Extraktion. KEIN Rendering (V9.43-c), KEIN Carven (V9.43-d) — reine Daten, vollständig headless-prüfbar. 12 neue Invarianten, ~2984 grün, Audit-Strict 0, Perf 17 ms. **Zwei Wurzel-Bugs an der echten Messung gefunden, nicht geraten**: (1) `_voxelSurfaceY` zu sampeln aliaste die 3D-Crags bei 16-m-Zellen zu 276 Mikro-Seen → Fix: die glatte Makro-Surface (`_terrainMacroSurfaceY(x,z,false)`, aus `_terrainDensityAt` extrahiert — eine Quelle, V9.44-a-Disziplin). (2) Jede Zelle unter dem Meeresspiegel als „Meer"-Auslass zu seeden ließ den Abfluss versickern (`maxAccum 50`, 0 Flüsse) → Fix: `_hydroMarkOcean` — der Ozean ist NUR die mit dem Region-Rand verbundene Komponente; ein abflussloses Becken mitten im Land ist ein See. Danach: `maxAccum 4521`, 6 Flüsse, 12 Seen — ein echtes Drainage-Netz. Ehrliche Grenze: 0 Wasserfälle (die Makro-Surface ist glatt — `docs/hydrosphere.md` §7 verankert sie in V9.43-c gegen die echte Voxel-Surface). **Meta-Lehre**: bei einer „0" zuerst die Sichtbarkeit bauen (Diagnostik-`stats`), dann den Fix; „Meer" ist ein begriffliches, kein numerisches Problem — „unter dem Meeresspiegel" ≠ „mit dem offenen Meer verbunden". Nächste Welle: **V9.43-c** — Flüsse + Seen werden sichtbar (`_buildHydrosphereMeshes`).
...
**Du erbst eine sehr lebendige Welt.** **~2984 Playtest-Invarianten grün + 0 Audit-Strict-Failures**, ~35200 Zeilen in einer Datei, alles produktiv. (Der Playtest-Konsolen-Zähler driftet ±3–8 je Lauf — „Alle Invarianten OK" ist die Wahrheit, nicht die exakte Zahl.) Vor dem ersten Code-Block: `CLAUDE.md` V9.43-b lesen (die jüngste Welle) + den Schöpfer-Audit-Block ganz oben. Disziplin: parallel bauen + beweisen, nie das Funktionierende brechen, ehrliche kleine Schritte vor grossen Würfen.
