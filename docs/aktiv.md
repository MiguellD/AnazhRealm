...
| **Γ3 Feld-Charakter** | **✓ FREQUENZ-FÄCHER FOUNDATION GEBAUT V18.203:** `AnazhRealm.FIELD_CHARACTER` frozen (lambdaLebendig:200, lambdaDichte:340, lambdaGlut:520, lambdaMagieleitung:160). `worldFieldAt` mit gen≥3-Gate: jede Welt-Stimme bekommt eigene Wellenlänge → Ecotone an argmax-Grenzen emergiert. Legacy gen<3 bit-identisch zu Pre-V18.203 (alte Welten behalten ihr Gesicht — V18.181/V18.193-Disziplin). Worker-Mirror hardkodiert. 9 Wände GEMESSEN, inkl. Δmagie 7.49 > Δdichte 5.01 über 200 m (kürzere λ → mehr Variation). **OFFEN als Folge-Welle:** Domain-Warp (`(x,z) += warpNoise(x,z) · A` mit eigenem `seed + "-veg-warp"`-Stream — bricht die Gitter-Parallelität der ridged-Kämme + dem Welt-Charakter mehr Tiefe). | **✓ FOUNDATION** |
