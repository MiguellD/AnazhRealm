...
> **Status: VOLLENDET (P0–P3 + P4 Stufe 1, V18.331).** Ammo.js ist PHYSISCH RAUS — die Feld-Physik ist die einzige Physik; die ERNTE (deterministischer Replay) ist eingefahren + bewiesen (bit-identisch). Kollision/Bewegung/Raycast/Struktur-Kollision sind feld-nativ (Dichtefeld + `_stepCharacter` · `_fieldRaycast` · `_stepCharacterStructures` · `entry.blockerAABBs`); der per-Chunk-BVH-Build (der Lauf-Freeze) ist weg, der 256-MB-WASM-Heap ist weg, der Boden ist deterministisch. `new Ammo` = 0; `physicsWorld` = null; kein ammo.js in `index.html`. Verifiziert: Fast-Gate 13/13 + `diag-walk-feel`/`-field-raycast`/`-structure-collide` grün. **Offen (klein):** der Sediment-Sweep (Kollisions-Stubs + tote State-Felder wegkehren · CSP `wasm-unsafe-eval` droppen) + die freie Wurf-Box-Schicht (§5.3/§5.5/§6) als Spezifikation; P4 (Replay/Determinismus-Ernte) als eigener Bogen.
> **Branch:** gebaut auf `claude/relaxed-ritchie-axa3us` (V18.325–.331). Der reservierte `claude/confident-noether-yczn0c` blieb ungenutzt.
...
| **P1b ✅ GEBAUT (Feel ✅)** | `_stepCharacter` (Kapsel) für den SPIELER; die Velocity lebt feld-eigen in `state.playerVel`/`_fieldVy` (kein Ammo-Body mehr seit P3) | `diag-walk-feel` 4/4 + Profi-Gefühl V18.326–.327 (Trägheit · View-Smoothing · Lande-Absorption) + Rand-Bugs gefischt V18.329 | erledigt |
| **P2 ✅ GEBAUT** | Struktur-Kollision feld-nativ (`_stepCharacterStructures` gegen `entry.blockerAABBs`, V18.327); **Feld-Raycast** (`_fieldRaycast` + `_segmentAABB` im EINEN `_runRaycast`-Chokepoint, V18.330) | `diag-structure-collide` ✅ + `diag-field-raycast` 4/4 (Grenze · Genauigkeit · Struktur · Himmel; die historische „vs Ammo 0,911 m"-Messung stammt aus der A/B-Phase, als Ammo noch parallel lief). Box-Schicht (freie Wurf-Boxen) bleibt Spec | erledigt (Kern) |
| **P3 ✅ GEBAUT (V18.331)** | `physicsWorld`=null, alle `new Ammo.*` weg, der 256-MB-Patch + ammo.js aus `index.html` raus, `_buildVoxelChunkBVH`/`_ensurePlayerChunkBVH`/`_pumpVoxelChunkBVH`/`_upgradeChunkBVH`/`_softFloorWhileChunkLoading`/`addRigidBody` GELÖSCHT, die Kollisions-Builder auf Ammo-freie Stubs; der Spieler-Body → `state.playerVel`+`_fieldVy`; Kreaturen body-frei | Welt baut/läuft ohne Ammo (Fast-Gate 13/13 grün); `grep 'new Ammo'` = 0; `diag-walk-feel`/`-field-raycast`/`-structure-collide` grün; volles Gate auf CI. scaleFactor bleibt als no-op-Konstante (1) — eigener Sediment-Schnitt | erledigt |
| **P4 Stufe 1 ✅ GEBAUT (V18.331)** | Deterministischer Input-Replay: `startReplayRecording`/`stopReplayRecording`/`replayRun` + `_replayCaptureFrame` (Chat `replay record`/`replay stop`). Der Replay läuft durch DENSELBEN Schritt-Pfad (`_stepCharacter`+`_loopPlayerMovement`), keine Parallel-Sim | `diag-replay-determinism` + `checkBandV18331ReplayDeterminism`: drei Replays derselben Aufnahme BIT-IDENTISCH, input-sensitiv, nicht-trivial (9,33 m). Der Determinismus ist BEWIESEN — die self-suffiziente Verifikation (kein Browser) | erledigt |
| **P4 Stufe 2/3 — Lockstep + Fixed-Point** | Stufe 2: Lockstep-MP (nur Inputs übers Netz, jeder rechnet dieselbe Welt). Stufe 3: Fixed-Point für Cross-Maschine-Bit-Gleichheit | deterministischer Replay bit-gleich ÜBER Maschinen | eigener Bogen (das Fundament steht) |
...
| 2 | Inseln: AABB-Hülle vs eigenes Feld | **AABB-Hülle zuerst** | **GEBAUT (V18.331 Backlog)** — `island.userData.fieldAABB` aus der Geometrie + `_stepCharacterIslands` über die geteilte `_resolveCapsuleVsAABB`; der Spieler steht auf der Insel + wird an der Flanke geblockt (Stand-Probe verifiziert). Eine echte Insel-Silhouette (Überhänge) bliebe ein Folgeschritt. |
| 3 | Determinismus-Tiefe P4 | Replay + Gleich-Browser-Lockstep; Fixed-Point eigener Bogen | offen (eigener Bogen — der Lohn, den P3 erst möglich macht) |
| 4 | Ammo-Schnitt (P3)-Zeitpunkt | Erst Schöpfer-Browser-Bestätigung, dann schneiden | **ERLEDIGT (Schöpfer 23.06. „es passt champ, vollende es") — Ammo ist raus (V18.331)** |

**Stand (VOLLENDET, V18.331):** P0–P3 sind GEBAUT. Ammo ist PHYSISCH RAUS — die Feld-Physik ist die einzige Physik (`physicsWorld`=null, `new Ammo`=0, kein ammo.js, kein WASM-Heap, kein per-Chunk-BVH-Build = der Lauf-Freeze tot). Der Spieler läuft/springt/landet/kollidiert/gräbt/platziert REIN feld-nativ (`state.playerVel`+`_fieldVy`+`_groundedCache`), Kreaturen erden body-frei (`_creatureGroundY`). Verifiziert: Fast-Gate 13/13 + `diag-walk-feel`/`-field-raycast`/`-structure-collide` grün; das Merge-Gate (Ammo-Bänder feld-nativ nachgezogen) auf der CI. **Stand des Backlogs (V18.331):** der Sediment-Sweep ist GESCHNITTEN (die Ammo-Kollisions-Stubs + toten Felder weg, CSP `wasm-unsafe-eval` gedroppt — V18.331 Nachschlag); **Inseln in die Feld-Kollision sind GEBAUT (Entscheid #2, V18.331 Backlog)**; P4 Stufe 1 (deterministischer Replay) ist GEBAUT (`diag-replay-determinism`). **OFFEN (eigene Folge-Bögen):** P4 Stufe 2/3 (Lockstep-MP + Fixed-Point) · die freie Wurf-Box-Schicht (§5.3/§5.5/§6) als Spezifikation.
