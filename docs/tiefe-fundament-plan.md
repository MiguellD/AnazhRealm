...
- **J4 — die Trapeze ✅ AN DER WURZEL GEHEILT (V17.105): der SCHNEE war fehlkalibriert.** Die V17.104-Eliminations-Hypothese („Biom-Zwei-Farben zu kontrastig → kohärente Paletten") war nah, aber FALSCH GEFRAMT — die Wurzel ist die VERTEILUNG, nicht der Kontrast. Schöpfer-Durchbruch: „hat nicht der SCHNEE dieses Verhalten? in fast jedem Chunk, nicht auf Gipfeln, aus einer Zeit als ein Chunk extrem war". **GEMESSEN (`scripts/diag-snowband.cjs`):** die Schnee-Zeile `mix(snow, ss(12,42,y))` ist eine ABSOLUTE Höhe, kalibriert als Gipfel bei ~40 m lagen; nach der V14/V17.96-Terrain-Weitung (Median ~30 m) war **der durchschnittliche sichtbare Boden ~53 % Schnee** (nahe 0.528) → fast-weißer Schnee (0.92) neben dunklem Stein (0.42) = der 2×-Kontrast, der tags die hellen Cel-Bänder erreicht (Trapeze) + nachts kollabiert (weg nach 18:00). **HEILUNG (V14.5-Co-Tuning, EINE Zeile in `_attachVoxelFieldColors` + Worker-Mirror): Schnee auf PROMINENZ `ss(50,115, y − cont0)` statt absolutem y** → Caps nur auf echten Gipfeln, robust gegen die regionale Höhenvariation (nahe max-Prom 70 m, weite 156 m). GEMESSEN: Schnee-Anteil nahe 0.528 → 0.002, weit 0.593 → 0.061; Playtest „Alle Invarianten OK", main↔Worker bit-identisch (maxColorDelta=0). **Der Schnee-KONTRAST bleibt (Caps sollen weiß sein) — nur der ORT ist korrigiert; NICHT desaturieren.** Der eps (V17.100/.102, zurückgerollt V17.103) war der falsche Hebel, weil die Cel-Bänder nur das VERSTÄRKEN, was der Schnee-Schmier bereitstellt. **OFFEN: Schöpfer-Browser-Sign-off** (look-ändernd + pixel-blind); die Schwelle `SNOW_PROM_START/FULL` ist in 2 Konstanten tunbar. **Residual (falls nach dem Sign-off noch milde Bänder bleiben): der SURFACE-ORDER-Bogen (§J-2) für die übrigen mid-tone-Biome — aber die DOMINANTE Trapeze-Quelle (Schnee) ist geheilt.**
