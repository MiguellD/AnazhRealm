...
- **J4 — Trapeze-Cel-Härte an der NORMALEN-WURZEL ✅ GEBAUT (V17.102):** der Schöpfer-Befund nach V17.100 „die Trapeze haben sich NICHT geändert" war richtig — eps 1.5 (2.7 m) war viel zu klein. GEMESSEN (`diag-cel-eps`): die Surface-Roughness ist `noise3D(x·0.05)·7` (λ~20 m) + `noise3D(x·0.018)·5` (λ~55 m); eps 2.7 m sampelt INNERHALB der 20-m-Beule → das Cel quantisiert die Mikro-Neigung in feine Bänder (das „Cel-Netz"). Heilung: **eps 1.5→6 Zellen (≈10.8 m, ≈λ/2 der 20-m-Roughness)** → der Normal liest die MAKRO-Neigung → die cel-relevante lokale Licht-Δ fällt −39 % (0.147→0.090) UND die Makro-Spanne bleibt (0.957 — Berge/Klippen shaden weiter) = grosse, gleichmässig licht-reaktive Cel-Regionen wie in 2.5D. Main + Worker bit-identisch; der Band-Skip-Margin (V17.96) musste mitwachsen (eps+2 Zellen, sonst kippten Gipfel-Normalen — `diag-normals` max 26°→0.02°). **OFFEN: der Browser-Sign-off** (verschwinden die Trapeze, sind die Cel-Regionen gross wie in 2.5D? eps 6 ist der gemessene Wert; mehr/weniger ist der Dial).

**Risiko.** J1/J2 render-only + pixel-blind + look-ändernd → headless beweist die VEREINHEITLICHUNG; das FEEL ist der Browser-Sign-off (V10.0-g.r). J3 ist headless beweisbar (KONSUM). J4 ändert den Determinismus (Welt-Aussehen, Normalen) → Worker-Mirror + Band-Margin-Co-Tuning, Browser-Sign-off des „2.5D-Look". Dials: `AnazhRealm.AERIAL` + `hazeBase`/`hazeTop`; eps (hart kodiert, determinismus-gebunden).
