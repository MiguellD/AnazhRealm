...
- **J4 — die Trapeze ⏳ WURZEL GEFUNDEN, Fix = SURFACE-ORDER-Bogen (NICHT der eps).** Der V17.102-eps-6-„Fix" ist ZURÜCKGEROLLT (V17.103): die Schöpfer-Augen sahen keine Änderung (mein −39 %-Headless-Maß fasste das FEEL nicht) UND eps=6 verschlechterte den Schatten (makro-Normale × `normalBias=1.0`). Die echte Wurzel (durch 5-Slider-Elimination): die BIOM-ZWEI-FARBEN auf der facettierten Geometrie (§J.2). **Der Fix ist §J-2 (kohärente Paletten + Detail-Schichten vereinen + Cel auf glatter Lichtung), KEIN Cel-/eps-Dial.** Die 5 Mess-Slider (V17.102.2–.104) sind das Instrument.
