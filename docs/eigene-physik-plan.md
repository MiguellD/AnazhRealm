...
| **P1b ✅ GEBAUT (Feel ✅)** | `_stepCharacter` (Kapsel) für den SPIELER; A/B hinter `state.fieldPhysics` (Default AN seit V18.325, Chat `feldphysik`→Ammo); Ammo trägt PARALLEL | `diag-walk-feel` 4/4 + Profi-Gefühl V18.326–.327 (Trägheit · View-Smoothing · Lande-Absorption) + Rand-Bugs gefischt V18.329 | erledigt |
| **P2 ✅ GEBAUT** | Struktur-Kollision feld-nativ (`_stepCharacterStructures` gegen `entry.blockerAABBs`, V18.327); **Feld-Raycast** (`_fieldRaycast` + `_segmentAABB` im EINEN `_runRaycast`-Chokepoint, V18.330) | `diag-structure-collide` ✅ + `diag-field-raycast` 4/4 (Grenze · vs Ammo 0,911 m · Struktur · Himmel). Box-Schicht (freie Wurf-Boxen) bleibt Spec | erledigt (Kern) |
...
**Nächster Schritt:** P0/P1a/P1b/P2 sind GEBAUT (alle Linsen grün: walk-feel · structure · inertia · edge · raycast). Der Spieler läuft, kollidiert, springt, landet, gräbt + platziert REIN feld-nativ; Ammo ist nur noch ein A/B-Schatten. **Es bleibt P3: Ammo physisch RAUS** — der gründliche Schnitt (V17.20-sed-Disziplin: Backup, awk-Methoden-Liste, `diag-page-error` vor Playtest), reiner Code-Schnitt ohne Funktions-Verlust. Entscheid #4 (Ammo-Schnitt-Zeitpunkt): nach Schöpfer-Browser-Bestätigung über mehrere Welten.
