...
**Phase 2 (V9.90, Density-async)**: async `_buildVoxelChunkData` awaitet Worker-Density-Grid, baut Iso+Cells+BVH lokal mit `preDensity`. Main-Spike sinkt von ~150ms auf ~50ms (~60% Wurzel). Streaming-Pump async-aware.

**Phase 3 (V9.91, Iso-async)**: `_voxelChunkGeometry` + `_buildVoxelChunkWaterCells` auch im Worker. Main macht nur noch BVH-Collision + Three.js-Scene. Spike ~10ms.

**Phase 4 (V9.92, Lazy-BVH)**: ferne Chunks bekommen Iso-Mesh sofort + BVH erst beim Frustum-Eintritt. 0ms Main-Cost beim Erst-Stream.
