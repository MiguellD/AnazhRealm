...
**Postscript 27.05.2026 — Perf-Bogen abgeschlossen, V10.0-Renderer-Migration darauf gefolgt**: der Performance-Plan dieses Docs ist gebaut — Perf-1 (V9.84) + Perf-2 (V9.85) + Perf-2 ext.a (V9.86) + Perf-3-Bogen (V9.87-V9.92, alle vier Sub-Wellen vollendet inkl. Worker-Migration Phase 1-3 + Lazy-BVH) + V9.96 Per-Frame-Spawn-Budget. Main-Thread-Spike vor Perf-Bogen ~150 ms → V9.92 ~5-10 ms. Danach: **V9.95-Bogen WebGPU-Compute** als Vorarbeit (a/b/c/d Foundation + Density-Pipeline + Logging) endete mit V9.95-e ehrlicher Abklemmung — die Architektur-Wurzel benannt: *WebGPU-Compute + WebGL-Renderer braucht IMMER `mapAsync`-Roundtrip → GPU langsamer als Worker*. V9.95-Code lebt weiter als Vorarbeit für die V10.0-Renderer-Migration (zero-copy GPU→Renderer ohne mapAsync), die im **V10.0-Bogen (a..g, 26-27.05.2026)** gebaut wurde: Three.js r134→r160 + WebGPURenderer-Addon vendored + Smart Hot-Swap-Safety-Net + alle 5 Material-Familien als NodeMaterials (Skybox/Stars/Waterfall/HydroSurface/Toon). Die nächsten Performance-Schritte leben jetzt im V10.0-h (Shadow-Sampling auf WebGPU via colorNode) + V10.0-i (Rim/Spec/Outline + Gras-Wind als TSL-positionNode statt onBeforeCompile), nicht mehr in einer eigenen Perf-Welle.
