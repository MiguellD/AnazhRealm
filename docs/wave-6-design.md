...
| Phase 2E V1 (V7.90) | ✅ live | **LLM-Persona aus vollem Identitäts-Anker** — Chat „Name, text" → _parseCreatureAddress → _findCreatureByName → llmCallCreature mit Persona-Prompt (bornAt + Name + Soul + Stats + Specs + Equipped + Boosts + Memory + Welt). llmCall hat optionalen system-Prompt-Override (eine Pipeline, viele Identitäten). V1 reaktiv-only — program-Field ignoriert. Memory-Eintrag „spoken" bei Konversation. 14 Tests, 1526/1526 grün. |
| Phase 2E V1.1 (V7.91) | ✅ live | **Schöpfer-Browser-Test-Bugfix + UX-Politur** — `@Name text` als eindeutige Adresse (Discord/Slack-Konvention), höchste Priorität vor Komma/Doppelpunkt-Fallback. „Bran wie gehts" wird bewusst NICHT mehr als Adresse missverstanden (Schöpfer-Bug-Fix). Soul-Farben für creature-name (Sprite cyan, Wesen brass, Geist grün) in Liste UND Chat-Output (direkter DOM-Pfad mit `<span class='chat-creature-name soul-X'>`). Chat-Placeholder vermittelt @-Geste passiv. 7 Tests inkl. expliziter Bug-Fix-Test, 1533/1533 grün. |
| Phase 2E V2 (V7.92) | ✅ live | **Proaktive Sprache** — pre-baked phrase-pool soul-aware (5 Events × 3 Souls + default). Throttle 60s/Kreatur + 8s global, silent-drop. 4 Hooks (LevelUp gather/build, Boost-NEU, no_material, no_inventory). UI-Toggle in Einstellungen. Eine Identität, viele Anlässe (Soul-Farben konsistent zwischen Liste + reaktiver Antwort + proaktiver Sprache). 13 Tests, 1546/1546 grün. |
| Phase 2E V3 (V7.93) | ✅ live | **Welt-Aktion-Vorschläge mit Sandbox + Modus-Pfad + Rare-Event-LLM** — Schöpfer-Wahl: atmosphärisch + Terrain (modify_terrain erlaubt), modus-abhängig (schöpfer auto / pfad+frieden Buttons), Defense-in-Depth (Whitelist gilt auch in schöpfer), Memory-Lifecycle (proposed/accepted/auto_executed/rejected/blocked), Rare-Event-LLM bei L5/neue-Spec mit eigenem 10-Min-Throttle. **Welle 6.H V2 vollständig: 14/14 Sub-Phasen.** 13 Tests + 1 V1-Test umgestellt, 1558/1558 grün. |
