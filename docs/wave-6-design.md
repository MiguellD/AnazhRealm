...
| Phase 2F.2 (offen) | 🔴 als nächstes | **Equipped tool + armor für Kreatur** — `creature.userData.equipped = {tool, armor}` persistent via 2D.1-Snapshot-Erweiterung, beeinflusst computeCreatureStats über existing Player-Pattern. ~1-2 Sessions. |
| Phase 2F.3 (offen) | 🔴 später | **Kreatur-Boosts** — apply_boost-Op auf Kreatur, Tränke. ~1 Session. |
