...
- **U-W4 ✅ GEBAUT (V18.6) — DIE FINALE FORM: Wasser ist eine FLÄCHE auf `L`, nicht die Zell-Iso.** Der ganze Render-Bogen drehte um die Zell-Iso (Surface-Nets über die Flood-Zellen) — sie ist die WURZEL ALLER Restsymptome (gemessen, nicht behauptet): sie sitzt ±1 m unter `L` (Granularität → „Seezentrum fällt"), ist ein VOLUMEN mit Unterseiten (→ „unter der Karte" an Nähten), definiert die Uferlinie GEOMETRISCH (→ „klettert die Küste") und behandelt SOLID **neutral** → zeichnet die Fläche QUER DURCH Strukturen auf `L` (→ Phantome an deren flachen Oberseiten — der Schöpfer-Hinweis „nur an horizontalen Ebenen" war der Schlüssel). `_buildVoxelChunkWaterSurfaceMesh` baut ein Höhenfeld: pro Eck-Vertex `L = _atlasWaterLevelAt(x,z,-Inf)` (Körper-Spiegel + Fluss-Gefälle → Drops; -Inf extended ~16 m über die Uferlinie, der Tiefen-Shader macht den Pixel-Verlauf). **Naht-frei PER KONSTRUKTION** (geteiltes kontinuierliches `L` → Nachbar-Kanten fallen zusammen; kein Smoothing/OOB/Granularität) → reitet die TERRAIN-LOD gefahrlos (W2s Ziel, hier ohne die DH-#606-Höhen-Verschiebung, weil es auf `L` nichts zu verschieben gibt). Einseitige Oberseite (-Y-Wicklung + BackSide → von unten gecullt = kein „unter der Karte"; Strukturen/Terrain verdecken die Fläche pro Pixel = kein Phantom). Zellen UNANGETASTET (Physik/Reaktivität/3D-Höhlen; die Fläche liest `L`, nicht die Zellen → korrekt auch bei Zell-Bau-Lag). GEMESSEN (`scripts/diag-water-surface-l.cjs`, Regressions-Gate): jeder Vertex sitzt EXAKT auf `L` (max|vertexY−L|=0 m, die alte Iso wich 0.505 m ab), sackt nicht unter `L` (0 m), -Y-Oberseite (area-ny −0.93), naht-frei (0 m), A/B-Schalter ok. Der Modus liegt in den Einstellungen (Fläche/Zell-Iso) + zwei Tiefen-Hebel (Ufer-Schärfe/Wasser-Tiefe). **OFFEN: der Browser-Sign-off (Pixel, V13-Lehre) — sieht das Ufer/die Tiefe/die Fluss-Drops richtig aus?** Aufgestaute Hoch-Becken über dem umgebenden `L` (gedämmt) brauchen weiter die Zellen — eigener Faden. **PERMANENTE LEHRE: genau das, was NUR der Browser validieren kann (der Wasser-RENDER), war das Ungebaute — die headless-beweisbare Zell-Seite (U-W1/U-W3) war gebaut, die browser-only Render-Seite (U-W4) aufgeschoben. Nicht den browser-only-Kern aufschieben, weil er nicht headless-grün werden kann; bauen + den Sign-off holen.**
