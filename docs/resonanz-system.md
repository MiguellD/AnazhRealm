...
- **U5/U6 [Plan-U6] — die Subsysteme:** das U2-Register enthält BEREITS Fahrzeug + Trank als volle Rollen (vom Spektrum U3 + der Rolle-Passung U2 gelesen). **Trank ✅** wird über seine volle consumable-Resonanz bewertet (die Potency-Faltung ruft `_blueprintRoleFit("consumable")`, V17.79). **Fahrzeug:** die vehicle-Signatur (spread+zäh+Antrieb) ist im Register + im Spektrum sichtbar; ein Fahrzeug-GAMEPLAY-Effekt (Bewegung) existiert noch nicht (Backlog) — wenn es landet, liest es die vehicle-Resonanz direkt (kein Sonder-Pfad nötig). **Damit ist die systematische Vereinheitlichung KOMPLETT: ein Produkt-Vektor (U1) → ein Rollen-Register (U2) → die Wirkung tief (U2) → der Katalysator sichtbar (U3) → die Klassifikation richtig (U4); alle Subsysteme lesen den EINEN Vektor.**
