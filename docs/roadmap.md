...
Stand: 20.05.2026 (V9.35, Voxel-Phase 5c.2.c.2 — der Toggle-Tod: Voxel ist permanent + irreversibel. Schöpfer-Wahl nach V9.34 — der `voxel terrain on/off`-Chat-Toggle (V9.13) ist gelöscht, `setVoxelTerrainActive` + `_setHeightfieldDormant` + `_restoreVoxelTerrain` fallen mit, alte V9.23-Heightfield-Opt-out-Welten werden in `ensureWorldMeta` zwangs-migriert. Phase 1-5b komplett, Phase 5c-Start (V9.26) + 5c.1 (V9.27) + 5c.1+ (V9.28) + Ammo-Leck-Fix (V9.29) + 5c.2.a Dead-Code (V9.30) + 5c.2.b-Vorbereitung (V9.31) + 5d-Mini Wasserfälle (V9.32) + 5c.2.b Eingangs-Welt-Flip (V9.33) + 5c.2.c.1 weitere tote Schichten (V9.34) + 5c.2.c.2 Toggle-Tod (V9.35) live; in 5c.2.c noch offen: 5c.2.c.3 (heightfield-only Chunk-Pipeline ablösen — `ensureChunkAt`/`populateChunkVegetation`/`_buildChunkGrass`/`_terrainHeightAtWorld`, jetzt nur noch von `modify_terrain`-DSL-Op + Playtest-Setups erreicht), 5c.2.c.4 (`modify_terrain`-DSL-Op + Chat-Pattern-Umrouting auf `voxel carve`/`voxel fill`). 2956 Playtest-Runs grün, Audit-Strict 0 Failures. Der Fremd-Engine-Bogen (W12-W17) ist komplett; offen darin nur B-WASM (per-Projekt). Volle Wellen-Historie in `CLAUDE.md`.)
