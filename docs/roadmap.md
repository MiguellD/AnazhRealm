...
### Welle A — Wasser ↔ Spieler-Wille (offen, Roadmap §1.1 Vision-Pfeiler)
...
**A.3 — Reaktive Hydrosphäre (V9.66)**: nach Damm-Spawn `_recomputeHydrosphere()` aufrufen (re-runs `_computeHydrosphere` + `_buildHydrosphereMeshes` + alle Chunk-Wasser-Layer). Performance-Check: muss < 1s sein.

**A.4 — Aufstauen + Überlauf (V9.67)**: wenn Damm den Fluss blockiert → das See-Becken oberhalb vergrößert sich (Priority-Flood findet neuen Auslass). Wenn der See den Damm-Top übersteigt → neuer Wasserfall am Damm.

**A.5 — Spieler-Feedback + Test (V9.68)**: Welt-Effekt-Ping beim Damm-Bau („das Wasser sucht einen neuen Weg"); Journal-Eintrag; Playtest-Invariante „Damm verschiebt Fluss".

**Erwartetes Ergebnis**: das Wasser bekommt eine Rolle. Der Spieler experimentiert mit der Welt. Wasserfälle sind sekundäre Phänomene wo Wasser über eine Klippe fließt — sie entstehen, wo sie hingehören, statt prozedural geschönt zu werden. Die V9.60-Topologie-Heilungen + V9.63-cleanup-Basis tragen dies.
