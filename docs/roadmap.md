...
Stand: **V12.0-perf.h.1 (29.05.2026) — der V12-Bogen ist durch.** r184-WebGPU-Migration (V12.0-vendor/a-f) + der Architektur-Instancing-Bogen (Instancing/HISM, budgetierter Culling-Build, Lazy-Proxy-Collision, Nexus→adaptiver Qualitäts-Governor) + die Streaming-Hitch-Heilung (Wasser-Iso deferred-Queue). **Schöpfer-Browser-Audit: 119 FPS steady — der Render ist gelöst.** Offene Reibung (Schöpfer-Browser, ehrlich diagnostiziert, nicht spekuliert): die **Wasser-Cell-Render-Pipeline ist ineffizient/inkorrekt vs die Riesen** — (1) Surface-Nets über das ganze ~71k-Zell-Grid (78 ms/Chunk) statt Grenzflächen-Meshing wie Minecraft (~1 ms); (2) „unter Pegel"-Klassifikation (`cy <= colWaterY`) erzeugt Wasserschatten an Hängen/um Strukturen statt verbunden-zur-Quelle/Atlas-strict; (3) Edit rechnet den ganzen 24×24×124-Chunk neu statt nur die berührte Sub-Region. **Nächster Bogen: V13 — Wasser-Profi-Bogen** (Voxel-Wasser EFFIZIENT + KORREKT wie die Riesen, nicht zurück zu flachen Planes — das wäre Ausweichen). Danach V14 — die lebendige Welt (Vision-Pfeiler E/F/G). Diese Doc trägt den **Plan vorwärts**; der aktuelle Stand + Gotchas in `CLAUDE.md`, die volle Wellen-Chronik in `docs/handover.md`, die Vision in `docs/state-of-realm.md`.
...
| **V13 Wasser-Profi-Bogen** ⏳ | **Voxel-Wasser EFFIZIENT + KORREKT wie die Riesen** (Schöpfer-Browser-Befund 29.05.2026, im Code verifiziert nicht spekuliert). Wasser bleibt voxel/zell-basiert (V9.69-Vision, reaktiv wie Minecraft) — aber gerendert + klassifiziert wie die Giganten, nicht zurück zu flachen Planes (= Ausweichen). Detail §1.4. | V12 ✅ | **V13.0** Wasser-Klassifikation: „unter Pegel" → Atlas-strict + verbunden-zur-Quelle (Wasserschatten an Hängen/Strukturen weg) · **V13.1** Grenzflächen-Meshing: Surface-Nets → Wasser-Luft-Boundary-Quads/Greedy (78 ms → ~1 ms, Streaming-Hitch weg) · **V13.2** Sub-Region-Edit-Remesh (nur berührte Sektion, nicht ganzer Chunk — Edit-Drops weg) · **V13.3** Wasser-Shader-Glanz (Tiefen-Schaum, Gerstner-Ripples, Reflexion auf der billigen Fläche) · **V13.4** LOD-Cross-Fade (kein Pop beim Annähern) · **V13.5** Schöpfer-Browser-Audit | Wasser positioniert + rendert wie die Riesen (Minecraft/Genshin-Lehre): kohärente Körper, keine Hang-Schatten, fast gratis, schön — UND voll reaktiv (Damm staut). |
...
1. **Falscher Mesher.** `_buildVoxelChunkWaterIsoSurface` läuft Surface-Nets über das ganze `(dim+3)·dimY·(dim+3)`-Zell-Grid (~71k Samples + extract+quads+Laplacian-smooth+crop+OOB-Density) = ~78 ms/Chunk. Eine Wasseroberfläche ist FLACH — die Riesen meshen nur die **Wasser-Luft-Grenzflächen** (Face-Culling + Greedy-Meshing, ~ein paar hundert Quads, ~1 ms). Surface-Nets ist für glatte Iso-Volumen, nicht für flache Wasserspiegel.
2. **Falsche Klassifikation (= Wasserschatten).** `_buildVoxelChunkWaterCells` Z58-59: `if (d>0) SOLID; else if (cy <= colWaterY) WATER` — Wasser = „jede nicht-solide Zelle UNTER dem Spalten-Pegel". `_waterLevelAt` gibt einer Hang-Spalte nahe eines Sees dessen Pegel → Hang-Zellen werden WATER → Wasser klebt am Hang/um Strukturen. Die Riesen füllen von QUELLEN (flood-fill / Atlas-strict) — Wasser ist nur, wo es wirklich ist.
...
| Welle | Was | Heilt |
|---|---|---|
| **V13.0** | Wasser-Profi-Diagnose: echte Wasser-Zell-Verteilung + Wasserschatten-Quantifizierung (Hang-Spalten fälschlich WATER) + Boundary-Face-Count vs Surface-Nets-Cells. Vorher-Zahl, V9.96-Disziplin. | Messen vor Schneiden |
| **V13.1** | Klassifikations-Heilung: „unter Pegel" → Atlas-strict (`_atlasWaterLevelAt`, nur wo der Hydrosphäre-Atlas Wasser markiert) + V9.86-Konnektivität (verbunden zur Quelle). | Wasserschatten an Hängen/Strukturen |
| **V13.2** | Grenzflächen-Meshing: Surface-Nets-Wasser-Iso → Wasser-Luft-Boundary-Quads (Greedy-Mesh, zell-getrieben = reaktiv). | 78 ms → ~1 ms, Streaming-Hitch, Naht |
| **V13.3** | Sub-Region-Edit-Remesh: nur die vom Carve/Fill berührten Zellen + ihre Mesh-Sektion neu, nicht der ganze Chunk. | Edit-Drops, „ganzer Chunk neu" |
| **V13.4** | Wasser-Shader-Glanz: Tiefen-Schaum (Depth-Buffer), Gerstner-Ripples (Vertex), Reflexion/Refraktion auf der billigen flachen Fläche. | Visuelle Tiefe wie die Profis |
| **V13.5** | LOD-Cross-Fade: sanftes Überblenden zwischen Detailstufen statt hartem Pop. | „sauberer werden"-Pop |
| **V13.6** | Schöpfer-Browser-Audit RDNA-3: Wasser kohärent, keine Hang-Schatten, smooth beim Streamen + Editieren. | Bogen-Schluss |
