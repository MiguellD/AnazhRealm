...
| **V14.2** | **Thermische Erosion (Angle-of-Repose)** — VORGEZOGEN (V14.1-Empirie: trifft den Schöpfer-HAUPTBEFUND „spitzig/steil"). Ein Talus-Pass in `_computeErosion` (+ Worker falls dort gespiegelt), der Material über dem Ruhewinkel abträgt + lokal deponiert → glatte natürliche Bergflanken statt 60°-Cusps; ko-evolviert mit der Stream-Power-Inzision. Ziel: Median-Hangneigung 36°→<20°, steil-Anteil 29 %→<10 %, sanft-Anteil 6 %→30 %+. (Re-erodiert die Welt.) | 60°-Wände, fehlende Talus-Hänge (Hauptbefund) |
| **V14.3** | **Gerichtete Uplift-Maske**: lineare/kurvige Gebirgsketten statt isotropes `mtn=(1−ero)²`. Eine Ridge-Distanz-Funktion entlang weniger tektonischer Linien/Kurven (aus dem Seed) moduliert die ridged-Oktave ENTLANG der Ketten → lange Anden-artige Züge statt verstreuter Zacken. Hier wächst evtl. die Hülle (hohe Ketten). | „spitzig isoliert" statt langer Ketten |
