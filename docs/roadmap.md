...
Stand: 21.05.2026 (V9.43-a — Wasser-Ultiversum Welle D.1: Wasserfälle werden Wasser-Planes mit einem geteilten Flow-Shader). Die Heightfield-Pipeline ist mit V9.39 vollständig zurückgebaut (Phase 5c.2.c.3 abgeschlossen — die Voxel-Welt trägt alles allein). Der Schöpfer-Audit nach V9.39 (fünf Browser-Befunde) ist mit der Voxel-Surface-Politur V9.40–V9.43 nahezu vollständig geheilt: V9.40 a–f (Chunk-Löcher beim Carve/Fill + Maus-Edit-P2P-Sync — Wurzel: `ammo.wasm` auf 256 MB growable gepatcht), V9.41 + V9.41-b (Trapeze/Linien — Schach-Brett-Diagonalen + Laplacian-Smooth), V9.42 a–d (Inseln teilen die Surface-Nets-Pipeline + MeshToon-Material + Naht-pad-Smooth), V9.43-a (Befund #5 Wasser — Wasserfälle von `THREE.Points`-Partikeln auf vertikale Wasser-Planes mit geteiltem Flow-Shader). **Offen: V9.43-b/c/d/e — das volle Drainage-Netz.** Schöpfer-Browser-Test der V9.43-a: die isolierten Wasserfall-Planes wirken als „fliegende Sheets" (kontextlos — kein Fluss darüber, kein Becken darunter). Schöpfer-Wahl: das komplette Hydrosphären-System — Flow-Accumulation aus der Voxel-Surface → Flüsse/Seen/Wasserfälle als ein deterministisches Netz, Flüsse carven echte Betten. **Ausführliche Planung: `docs/hydrosphere.md`.** Der Fremd-Engine-Bogen (W12–W17) ist komplett; offen darin nur B-WASM (per-Projekt). Wellen-Plan + Audit-Lehren in §3 „Voxel-Surface-Politur"; die kanonische Versions-Historie lebt in `CLAUDE.md`.

**Code-Hygiene-Audit 21.05.2026** — ein externes Code-Audit + eine eigene Vermessung am Quellcode fanden strukturelle Reibung (Kontrollfluss-Funktionen bis 696 Zeilen, Snapshot↔Restore-Handkopplung, gekoppelte Felder ohne Setter, gestreute Magic Values). **Schöpfer-Wahl: der Stamm-Pflege-Bogen (V9.44) wird VOR der Hydrosphäre (V9.43-b) eingeschoben** — „damit die Weiterentwicklung danach sauberer ist". Er strukturiert lebenden Code in kleinere benannte Methoden DERSELBEN Klasse — kein neues Modul, keine neue Abstraktionsschicht, ein verhaltensneutrales Refactoring (die ~2966 bestehenden Invarianten sind der Beweis). Ausführliche Planung: **`docs/code-hygiene.md`**; Wellen-Tabelle in §3 „Der Stamm-Pflege-Bogen".
...
| D | **V9.43 — das Wasser-Ultiversum** | **Schöpfer-Browser-Test der V9.43-a (21.05.2026): die isolierten Wasserfall-Planes wirken als „fliegende Sheets" — ein Wasserfall ohne Fluss darüber + Becken darunter ist kontextlos. Schöpfer-Wahl: das volle Drainage-Netz mit echten Fluss-Betten.** Der Profi-Weg (Gaea/World-Machine-Pattern): Flow-Accumulation aus der Voxel-Surface → Flüsse/Seen/Wasserfälle als EIN deterministisches Hydrosphären-Netz; Flüsse carven echte Betten ins Terrain, alles mündet ins Meer, alles ein Wasser-Shader mit `uFlowDir`. **Ausführliche Planung: `docs/hydrosphere.md`** (Algorithmus, Datenstrukturen, Risiken). Geschnitten in V9.43-a ✅ + b/c/d/e. | drei Wasser-Sprachen (Plane / Partikel / nichts für Flüsse+Seen) — Vision-Inkonsistenz §1.3. | ~4-5 Sessions | 🟡 a ✅, b/c/d/e geplant |
| D.2 | **V9.43-b** | **Hydrosphären-Atlas** — `_computeHydrosphere()` berechnet das Drainage-Netz: Surface-Sampling über eine ~2-km-Region → Priority-Flood-Depression-Filling (Barnes 2014, füllt Senken → Seen) → D8-Flow-Direction → Flow-Accumulation → Netz-Extraktion (Fluss-Polylinien mit Breite ∝ √Akkumulation, See-Senken, Wasserfall-Punkte an Fluss-Klippen-Kreuzen). Reine Daten in `state.hydrosphere`, KEIN Rendering, KEIN Carven — vollständig headless-prüfbar. Deterministisch aus dem Seed → multiplayer-sicher. | ein Fluss-Netz braucht eine Berechnung, bevor es Geometrie werden kann. | ~1-2 Sessions | 🔴 geplant — nach dem Stamm-Pflege-Bogen V9.44 |
| D.3 | **V9.43-c** | **Flüsse + Seen werden sichtbar** — `_buildHydrosphereMeshes()`: See-Planes (zur Senke geformt) + Fluss-Ribbon-Meshes (Breite ∝ √A, `uFlowDir` = Gefälle-Tangente), alle mit der V9.43-a-Wasser-Material-Familie. Wasserfälle re-verankert: eine V9.43-a-Plane an jedem Fluss-Klippen-Kreuz (Fluss oben + Becken unten — kein fliegendes Sheet mehr). Der per-Chunk-Zufalls-Wasserfall-Spawner wird abgelöst (Material + Plane wiederverwendet). | das Netz braucht Geometrie; die V9.43-a-Wasserfälle brauchen Fluss-Kontext. | ~1-2 Sessions | 🔴 geplant |
| D.4 | **V9.43-d** | **Flüsse carven echte Betten** — `_terrainDensityAt` fragt die Hydrosphäre (`riverBuckets`-Spatial-Index, O(1)) + senkt die Dichte im Fluss-Kanal + in See-Senken → der Chunk-Mesher produziert echte Rinnen mit Ufern. Zirkel-frei (während die Hydrosphäre rechnet, ist `state.hydrosphere` null → kein Carve → `_voxelSurfaceY` sieht die Basis-Surface). | das Wasser soll in einer echten Furche liegen, nicht aufgemalt. | ~1 Session | 🔴 geplant |
| D.5 | **V9.43-e** | **Politur + Klang** — Vision §1.4: Fluss-Rauschen + Wasserfall-Donnern (positions-abhängige Audio-Schicht, das V9.32-Audio-Versprechen eingelöst); Fluss-Mündung blendet ins Meer, See-Ufer-Schaum, Flow-Speed nach Gefälle. | ein Wasser-System ist erst fertig, wenn man es hört. | ~1 Session | 🔴 geplant |
...
**Vision-Abgleich**: dieser Bogen modularisiert NICHT — kein neues Modul, kein State-Manager, keine neue Abstraktionsschicht (das wäre der Heilige-Lektion-Verstoß). Er zerlegt Großfunktionen in kleinere benannte Methoden DERSELBEN Klasse und stellt EINE Quelle her, wo heute zwei sind. Das ist die fraktale Disziplin (§1.3) auf den Code selbst angewandt + das Einlösen aufgelaufener Struktur-Schuld (V9.40-Lehre). Eine 696-Zeilen-Funktion IST Komplexität ohne inneres Fundament — der Bogen reduziert Komplexität, er fügt keine hinzu. Präzedenz: V9.30/V9.39 (Dead-Code-Wellen). Reines, verhaltensneutrales Refactoring — der sicherste Wellen-Typ; die ~2966 bestehenden Invarianten SIND der Beweis (bleiben sie grün, ist das Verhalten unverändert).

| # | Welle | Was | Aufwand | Risiko | Stand |
|---|---|---|---|---|---|
| a | **V9.44-a** | Persistenz-Schema vereinheitlichen — `_serializeBlueprint`/`_deserializeBlueprint` als EINE Quelle des Feld-Satzes (spiegelt das erprobte `_serializeCreature`-Muster), beide Save-Pfade routen hindurch. Heilt die V8.59-Bug-Klasse an der Wurzel. | ~1 Session | niedrig | 🔴 nächste Welle |
| b | **V9.44-b** | Kanonische Setter für gekoppelte Felder — `_applyPlayerSpeed(v)` setzt `speed`+`sprintSpeed` konsistent; weitere echte Kopplungen prüfen. KEIN State-Manager. Schließt die Sprint-Bug-Klasse. | ~0.5 Session | sehr niedrig | 🔴 geplant |
| c | **V9.44-c** | Der Mesh-Router wird ein Dispatch-Table — `p2pHandleMessage`s 19-Branch-`if`-Kette → `_p2pMessageHandlers`-Tabelle + `_p2pHandle<Type>`-Methoden; der Router schrumpft auf einen ~5-Zeilen-Dispatcher. | ~1 Session | niedrig-mittel | 🔴 geplant |
| d | **V9.44-d** | Infrastruktur-Konstanten konsolidieren — Ports/URLs/Backpressure/Protokoll in den bestehenden `AnazhRealm.X`-Block; Spawn-Fallback als `_defaultSpawnPos()`-Factory (kein geteiltes Literal); Log-`slice`-Limits vereinheitlicht. | ~0.5 Session | niedrig | 🔴 geplant |
| e | **V9.44-e** | Die UI-Giganten zerlegen — `_renderWorkshopDOM` (658 Z.) + `initStatusPanel` (174 Z.) in benannte Sub-Render-Methoden; die 4× `data-cmd`-Delegation → ein `_wireCmdDelegation`-Helper. | ~1-2 Sessions | niedrig-mittel | 🔴 geplant |
| f | **V9.44-f** | Der Game-Loop bekommt Phasen-Struktur — `startEternalLoop` (696 Z.) Loop-Body in benannte Phasen-Methoden (`_loopPhysicsSync`/`_loopPlayerMovement`/`_loopCamera`/…), geteilte lokale Werte als explizite Parameter. Bewusst zuletzt; Browser-Test verpflichtend. | ~2 Sessions | mittel | 🔴 geplant |

**Bewusst NICHT im Bogen** (ehrlich benannt): Daten-/Dispatch-Tabellen (`dslEffects` 892 Z., `chatDslPatterns` 443 Z., `_defaultBlueprints` 648 Z. — Daten-Länge, kein Komplexitäts-Problem); die `playtest.cjs`-Ko-Evolution (der bewusste Preis der Test-Disziplin); die Doc-Größe (das „Projektgedächtnis"-Design); die `_p2p*`-vs-`p2p*`-Namens-Inkonsistenz (Umbenennen öffentlicher Methoden = Risiko ohne Funktionsgewinn). Detail-Begründung in `docs/code-hygiene.md` §5.
