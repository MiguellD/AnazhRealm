...
| **D** | **Wasser ↔ Kreaturen** (klein additiv, derselbe Hydro-Layer) | Perf-1 ✅ + V10.0-Bogen ✅ | ✅ **D.1** `_creatureWaterContextAt(creature)` Foundation (V11.0-d.1, 27.05.2026 — EINE Wahrheits-Quelle für inWater/depthBelow/submerged/distToShore/shoreDir aus den V9.25/V9.50/V9.59-Atlas-Funktionen, 200 Calls < 50ms, 14 Test-Invarianten grün) · ⏳ **D.2** wander-Direction reagiert auf Wasser-Kontext (Auftrieb-y + Ufer-Bias, Distance-LOD <50m vom Spieler) · ⏳ **D.3** Neuer `drink`-Task (6. Task, Aura azur/210, Ping A5/880Hz, 3 Phasen gehe→halt→trink + Happiness-Boost) · ⏳ **D.4** Schöpfer-Browser-Audit + Splash-Ping bei Wasser-Eintritt (multisensorisch §1.2) | Eine Kreatur findet ohne Skript-Hilfe Wasser, trinkt sichtbar, schwimmt durch flaches Wasser, scheut tiefes |
