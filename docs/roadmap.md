...
~~B1 Wasser-Sheet→Worker~~ ✓ GESCHLOSSEN V18.345/.373 (off-thread, byte-identisch, Nah-Wake-gegated) ·
**DAS FERN-WASSER (die Wasser-LOD-Kaskade) — der nächste eigene Bogen (benannt V18.380):** das Wasser endet am Voxel-Chunk-Ring (~194 m bei Ring 4), fog.far reicht sunny ~450 m → der Ring dazwischen zeigt SETTLED trockene LOD-Terrain-Becken („leere Seen in der Ferne" — NICHT die Lade-Race, die ist V18.380 geheilt). Der Profi-Weg: coarse FERN-WASSER-SHEETS für die LOD-Terrain-Chunks aus dem deterministischen Hydro-ATLAS (`_atlasWaterLevelAt` — braucht KEINE Zellen/CA, reine (x,z)-Funktion → worker-baubar, billig [grober Step 3,6–7,2 m], geteiltes Wasser-Material → visuell kontinuierlich). Die Naht am Ring-Übergang: das Fern-Sheet beginnt am Ring-Rand, minimal tiefer gelegt (~5–10 cm, kein Z-Fight), der Nebel-Dunst deckt die Handoff-Zone. WÄNDE: kein zweiter Wasser-Shader (dasselbe Material/Uniforms), kein CA-Eingriff (Atlas-only = ruhende Ferne), Prune-Lifecycle wie die LOD-Terrain-Chunks; der LOOK (Handoff sichtbar?) = Schöpfer-Browser.
