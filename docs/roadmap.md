...
| A.2 | **V9.40-b** | V9.24-Symptom-Geste an der Wurzel umkehren + Async-Re-Mesh: `_remeshVoxelChunksAround` markiert Chunks dirty + ein `_tickDirtyVoxelChunks`-Pfad rebuildet ≤1 Chunk/Frame; bei null-Kollision (OOM) bleibt der alte Chunk stehen statt entfernt zu werden (kein Loch). Verlangt einen Pre-Build-Pattern: `_ensureVoxelChunkAt` aufgeteilt in `_buildVoxelChunkData` (kann fehlschlagen, baut frisches Mesh + Kollision in einem temporären Pair) + Insertion (alter wird erst ersetzt, wenn der frische bestätigt ist). | V9.24-Symptom-Heilung wurde Schuld. | ~1-2 Sessions | 🔴 offen |
