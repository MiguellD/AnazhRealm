...
Stand: **V13.4 (29.05.2026) — V13-Wasser-Profi-Bogen: Diagnose + Klassifikation + Grenzflächen-Meshing + Flow-Bias + Wasser-Glättung durch.** **V13.4** heilte den V13.2-Boundary-Trade (Schöpfer-Audit „würfelartige Flüsse, vertikale Wasserkante"): `_weldAndSmoothWater` verschweißt + Laplacian-glättet das Boundary-Mesh (Rand-Vertices gepinnt → kein Seam) → weiche Ufer statt 1,8-m-Würfel, +~1-2 ms/Chunk (Build ~3 ms). Visueller Beweis = Schöpfer-Audit. **Nächste Welle: V13.5 — Sub-Region-Edit-Remesh.** Danach V13.6 Shader, V13.7 LOD, V13.8 Audit; dann V14.**

Davor: **V13.3 — Flow-Bias-Heilung (D8 drop/dist).** **V13.3** heilte den vom Schöpfer vermuteten Richtungs-Artefakt: das D8-Flow (Erosion + Hydrosphäre) wählte steilsten DROP statt steilstes GEFÄLLE → Diagonal-Bevorzugung (65 % statt 50 %) + Tie-Break-Einrichtungs-Drift (↖ 19 %) → der „Treppe/Wasserschatten seit ewig in eine Richtung". Fix `drop/dist` (√2-Diagonal-Penalty) → ↖ 19→8,1 %, eine-Richtungs-Drift weg. Determinismus-brechend (Schöpfer „jetzt fixen"), byte-identisch reproduzierbar, Hydrosphäre intakt. **Nächste Welle: V13.4 — Sub-Region-Edit-Remesh** (durch V13.3 verschoben). Danach V13.5 Shader, V13.6 LOD, V13.7 Audit; dann V14.**

Davor: **V13.2 — V13-Wasser-Profi-Bogen: Diagnose + Klassifikation + Grenzflächen-Meshing durch.** Der V12-Bogen ist durch (r184-WebGPU, Architektur-Instancing, 119 FPS steady — Render gelöst). **V13.0** maß die Wasser-Pipeline statt zu spekulieren. **V13.1** heilte die Klassifikation (`_atlasWaterLevelAt` + Depth-Gate → Hang-Schatten 51,7→0,1 % am Bergsee; Worker-Mirror; Phantom-Carve-Fill ehrlich weg, Flood-Fill als Backlog). **V13.1.1** Worker-Cache-Buster (der Fix erreichte den Browser erst dann — `new Worker(...?v=VERSION)`). **V13.2** ersetzte den Surface-Nets-Wasser-Mesher durch zell-getriebenes Boundary-Face-Meshing: Build **~150 ms → ~3 ms/Chunk** (~40×), Output **~7–10k → ~500 Dreiecke/Chunk** (~18×), der V13.1-OOB-Aufschlag weg, naht-frei via OOB-cellClass (kein Pad+Crop). Kein Greedy-Merge (per-Cell-Dichte für V13.4-Wellen). **Nächste Welle: V13.3 — Sub-Region-Edit-Remesh** (nur die berührte Sektion neu statt des ganzen 24×24×124-Chunks). Danach V13.4 Shader-Glanz, V13.5 LOD, V13.6 Schöpfer-Browser-Audit; dann V14 — die lebendige Welt (Vision-Pfeiler E/F/G). Diese Doc trägt den **Plan vorwärts**; der aktuelle Stand + Gotchas in `CLAUDE.md`, die volle Wellen-Chronik in `docs/handover.md`, die Vision in `docs/state-of-realm.md`.
...
| **V13 Wasser-Profi-Bogen** ⏳ (V13.0–.4 ✅) | **Voxel-Wasser EFFIZIENT + KORREKT wie die Riesen** (Schöpfer-Browser-Befund 29.05.2026, im Code verifiziert nicht spekuliert). Wasser bleibt voxel/zell-basiert (V9.69-Vision, reaktiv wie Minecraft) — aber gerendert + klassifiziert wie die Giganten, nicht zurück zu flachen Planes (= Ausweichen). Detail §1.4. | V12 ✅ | **V13.0–.2 ✅** Diagnose · Klassifikation (`_atlasWaterLevelAt` + Depth-Gate, Hang-Schatten 51,7→0,1 %) · Worker-Cache-Buster · Grenzflächen-Meshing (~150→~3 ms) · **V13.3 ✅** Flow-Bias: D8 `drop/dist` (Diagonal-Grain weg, determinismus-brechend) · **V13.4 ✅** Wasser-Glättung (`_weldAndSmoothWater`) — ABER Schöpfer-Reflexion: das war Schicht-Verwechslung (Geometrie für Erscheinungs-Problem) · **V13.5** Schicht-3-Tiefen-Shader (weiche Uferlinie via `viewportLinearDepth` + Emotions-Haken; V13.4-Glättung zurück) · **V13.6** Sub-Region-Edit-Remesh · **V13.7** LOD-Cross-Fade · **V13.8** Schöpfer-Browser-Audit | Wasser positioniert + rendert wie die Riesen (Minecraft/Genshin-Lehre): kohärente Körper, keine Hang-Schatten, fast gratis, schön — UND voll reaktiv (Damm staut). |
| **V14 — die lebendige Welt** ⏳ | **System-Kopplungs-Pfeiler E/F/G** (NACH V13, V9.51-Disziplin: Vision nach Fundament). Pfeiler D Kern erledigt (Wasser↔Kreaturen). | V13 ✅ | **V14.0** Pfeiler E (Emotion ↔ lokale Welt — `_localEmotionPulseAt(x,z)`, Wasser-Schaum-Tempo + Ambient + Licht lokal moduliert) · **V14.1** Pfeiler F (Hylomorphismus-Cluster-Resonanz — 3× lebendig-Bauten = sichtbare Aura, lockt Fauna) · **V14.2** Pfeiler G (Multi-Spieler-Vibe — gleiche Emotion am gleichen Ort verstärkt E+F sichtbar) · **V14.3** D.4-Polish (Splash-Ping, autonome Trink-Geste) · Backlog: IndexedDB-Welt-Gedächtnis, Predictive Prefetch | Welt fühlt sich erinnert + emotional reaktiv an, atmet als EIN Wesen. JEDE Welle additiv + klein-fokussiert, keine Meta-Schicht (Heilige Lektion). |
...
| **Backlog** | **Polish-Pool** (kein Sequenz-Zwang) | – | (a) Wasserfall-Cellular-Emergenz (vertikale water-Cell-Säulen über Klippen als „Wasserfall-Spalte" — heute noch separate Geometrie); (b) Lake-Mask-Falloff (Schöpfer-Audit-Backlog nach V9.69, weicher See-Mask-Rand); (c) lokales Regen/Pfützen-Modell (Precipitation, Standard-Carve im Hochland soll Pfütze geben); (d) Crafting-Tiefe — Welle 6.F3 (Maschinen-Antrieb) + 6.F5 (echte Ammo-Constraints), falls je gewünscht; (e) B-WASM (per-Projekt); (f) Vision-Matrix-Reste: `evolveCommunity`, VR, IndexedDB | – |
...
| **V13.5** ⏳ **next** | **Schicht-3-Shader (der eigentliche Übergangs-Fix, Drei-Schichten-Reflexion)**: Tiefen-basierte Uferlinie via `viewportLinearDepth` (weicher Schaum + Alpha-Fade pro Pixel, folgt dem Terrain exakt = „Oberflächenspannung") + Tiefen-Farbe + sanfte Gerstner-Ripples + Fresnel + **Emotions-Kopplungs-Haken** (die Welt atmet). Im selben Zug: **V13.4-Geometrie-Glättung zurücknehmen** (war die Schicht-Verwechslung — der Shader trägt die Übergänge, das Mesh bleibt dumm-flach). | „übergänge schöner", natürliches Wasser, Makro-Komik weg |
| **V13.6** | Sub-Region-Edit-Remesh: nur die vom Carve/Fill berührten Zellen + ihre Mesh-Sektion neu, nicht der ganze Chunk. | Edit-Drops, „ganzer Chunk neu" |
| **V13.7** | LOD-Cross-Fade: sanftes Überblenden zwischen Detailstufen statt hartem Pop. | „sauberer werden"-Pop |
| **V13.8** | Schöpfer-Browser-Audit RDNA-3: Wasser kohärent, keine Hang-Schatten, smooth beim Streamen + Editieren. | Bogen-Schluss |
| **V13.x (Backlog, hinten angesetzt)** | (a) **Reaktiver Carve-Fill von echter Quelle** (Welle-A-Vision in korrekter Form): ein Carve, der einen Pfad zu echtem Wasser (Ozean/See/Fluss) öffnet, füllt via Flood-Fill von der Quelle (Minecraft-Wasser-Update). Ersetzt den V9.74-Phantom-„Carve-unter-Pegel-füllt-überall", den V13.1 (zu Recht) entfernt hat. (b) **V13.1-Rest-Schatten** ~4,6 % Küste (Überhang/Steilküste). (c) **Volle Flow-Isotropie via MFD/D∞** — V13.3 drop/dist entfernt die EINE-Richtungs-Dominanz, aber eine 4-fach-symmetrische Achsen-Rest-Präferenz (33/67) bleibt; Multi-Flow-Direction würde Drainage auf mehrere Nachbarn verteilen = voll isotrop. Nur falls der Schöpfer noch einen Achsen-Grid-Grain sieht. | „Pond füllt sich", Steilküsten-Rest, Achsen-Grid-Grain |

**Riesen-Lehre verdrahtet:** Voxel-Wasser ist nicht das Problem — der Mesher (Surface-Nets statt Grenzflächen) + der Klassifikator („unter Pegel" statt verbunden-zur-Quelle) sind es. Beide fixbar IM Voxel-Ansatz. Flache Planes wären Rückschritt (kein Damm-Reaktiv). Und: **erst messen/im-Code-verifizieren, dann pronouncieren** — meine spekulative „flache Planes"-Reflexion war die Verletzung der eigenen V9.96-Disziplin, vom Schöpfer korrekt gefangen.
