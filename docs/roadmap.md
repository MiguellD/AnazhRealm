...
Stand: V9.56-j (23.05.2026). Diese Doc trägt den **Plan vorwärts** — die Ring-Übersicht 0-17, was offen ist, die Meilenstein- und Risiko-Sicht. Die anderen Zeit-Ebenen: der aktuelle Stand + die Gotchas in `CLAUDE.md`, die volle Wellen-Chronik in `docs/handover.md`, die Vision in `docs/state-of-realm.md`.
...
- **Ein zweiter Code-Hygiene-Bogen — UI-Etappe GESCHLOSSEN, Kontrollfluss-Etappe FAST FERTIG (V9.56-a..j, 23.05.2026)**. UI-Etappe: vier Sub-Wellen a/b/c/d. Kontrollfluss-Etappe: e `fuseWorlds` 239 → 114, f `_tickCreatureTaskDirection` 267 → 8, g `_voxelChunkGeometry` 269 → 25, h `processChatCommand` 285 → 21, i `_applyDayNightToScene` 305 → 16 (Datei −30 Z. netto-negativ), j `generateTerrainWithParameters` 346 → 73 (Datei −54 Z. netto-negativ — Kommentar-Konsolidierung). Alle zehn Sub-Wellen verhaltensneutral, playtest grün vor + nach. **Bleibt im Bogen — EINE Funktion**: `loadState` 380 → V9.56-k (V8.59-Bug-Klassen-Quelle). **V9.56-e/f/g/h/i/j-Lehren**: (1) Block-Grenzen-Inspektion ist eine HIERARCHIE; (2) Side-Effect-Stream + Math.random-Reihenfolge bit-identisch halten; (3) Pipeline-Kopplung kann einseitig sein; (4) Try-Pattern-Konvention für Multi-Branch-Dispatcher; (5) Cleanup-Asymmetrie preservieren; (6) Strukturelle Test-Invarianten: positive Pattern-Tests müssen mit-wandern, negative Pattern-Tests sind robust; (7) Helfer-Granularität folgt Kosten/Nutzen — State-Init-Zeilen rechtfertigen keine Extraktion. **Disziplin (V9.44-Wiederholung)**: eine Sub-Welle = ein Commit, playtest-grün, verhaltensneutral, kein Bonus-Refactoring im selben Schnitt. **Disziplin (V9.40-Lehre)**: der Bogen gehört terminiert, sobald die nächste Feature-Welle eine dieser Funktionen berührt, nicht „irgendwann".
- **Stamm-Hygiene über den Code hinaus** — seit V9.44 sind Tests + Doku Teil des Stamms und brauchen dieselbe Hygiene-Disziplin: `playtest.cjs` war 31 574 Z., EINE Funktion mit Median-Einrückung 20 (die zweitgrößte Reibung des Projekts). Der **Playtest-Pflege-Bogen ist GESCHLOSSEN** (V9.52-a..f): jetzt 29 554 Z., schlanker Orchestrator-IIFE 207 Z., 41 Band-Funktionen + 5 Helfer, Median-Einrückung 8, `safeEvaluate(page,fn)` ersetzt 196 inline evaluate-catch-Stellen. §6-Akzeptanz erfüllt, voller Bogen-Bericht in `docs/playtest-hygiene.md` §7. Die Doku-Schicht braucht weiterhin periodisches Archivieren fertiger Konzept-Dokumente — sonst wächst das „Projektgedächtnis" zur Halde, eine Schicht über der Heiligen Lektion. Voller Schnitt-Plan in `docs/playtest-hygiene.md` (§3 unten). Die Doku-Schicht braucht periodisches Archivieren fertiger Konzept-Dokumente (Konsolidierung 21.05.2026 + erneut diese Session) — sonst wächst das „Projektgedächtnis" zur Halde, eine Schicht über der Heiligen Lektion.
