...
Stand: V10.0-g (27.05.2026, **V10.0-Bogen vollendet — Three.js r134 → r160 + WebGPU-Foundation + alle 5 Material-Familien als NodeMaterials**). Sieben Sub-Wellen vom Vendor-Wechsel zum dauerhaften WebGPU-Render: V10.0-a Vendor + Bridge-Heilungen · V10.0-b ESM-Loading-Pattern · V10.0-c WebGPURenderer-Addon vendored · V10.0-d Renderer-Auswahl + async init + Fallback · V10.0-e Smart Hot-Swap · V10.0-f-1..f-4 die 4 custom ShaderMaterials zu TSL (Skybox/Stars/Waterfall/HydroSurface) · V10.0-f-5/f-6 Hot-Swap-Canvas-Replacement + Animation-Loop-Restore · V10.0-g MeshToonMaterial → `MeshBasicNodeMaterial` + manuelles Half-Lambert-Toon-Lighting im colorNode (Cel-Stufen via gradientMap-Texture-Lookup, Sun-Sync via `state.toonLightUniforms`). Headless-Beweis: WebGPU rendert mit FPS=60, 81 Chunks, 0 Errors/Warnings. **Ehrliche temporäre Begrenzungen** (Folge-Wellen): **V10.0-h Shadow-Sampling auf WebGPU** — `lights=true`-NodeMaterials crashen im r160-webgl-legacy-Patch, Schatten leben aktuell über Hot-Swap → WebGL; manuelle Shadow-Map-Sampling im colorNode ist die Welle. **V10.0-i NPR-Polish** — Rim-Light + Specular-Cel-Step + Outline-Pass (Genshin/BotW-Standard 2024+); V10.0-g zielt auf MeshToonMaterial-Parität, nicht auf NPR-Polish. Plus: `_grassInstanceMat` nutzt `onBeforeCompile`-Wind-Patch — auf WebGPU triggert das den sauberen Hot-Swap zu WebGL (kein Crash, aber heißt: „WebGPU dauerhaft" gilt solange kein Voxel-Chunk Gras enthält); V10.0-i sollte das auch via TSL-positionNode-Displacement migrieren. Davor — V9.96 (Per-Frame-Spawn-Budget gegen FPS-Burst-Spike). Davor — Welle-Perf-3-Bogen V9.87-V9.92 vollendet (Atlas-Strict-Gate + Distance-LOD + Worker-Migration Phase 1-3 + Lazy-BVH). Davor — V9.95-Bogen WebGPU-Compute-Foundation (V9.95-a/b/c/d) + V9.95-e ehrliche GPU-Abklemmung (Architektur-Wurzel benannt: WebGPU-Compute + WebGL-Renderer braucht IMMER mapAsync-Roundtrip → GPU langsamer als Worker; V9.95-Code lebt als Vorarbeit für V10.0-WebGPU-Renderer-Migration). Diese Doc trägt den **Plan vorwärts** — die Ring-Übersicht 0-17, was offen ist, die Meilenstein- und Risiko-Sicht. Die anderen Zeit-Ebenen: der aktuelle Stand + die Gotchas in `CLAUDE.md`, die volle Wellen-Chronik in `docs/handover.md`, die Vision in `docs/state-of-realm.md`.
