...
**Summe verbleibend**: die grossen Roadmap-Ringe sind gebaut. **Stand nach V8.76**: Atmosphäre ✅ → Multi-User-Tiefe ✅ → Hylomorphismus-Vollausbau ✅ (V8.35) → Welt-Portal-Protokoll ✅ (W12) → souveräne Identität ✅ (W13) → Bibliothek ✅ (W14 Phase 1-3) → Compute-Sharing ✅ (W7 Phase 1-4) → Voice-Sync ✅ (W11 V4) → KI-Übersetzer ✅ (V8.68/V8.69) → Untrusted-Welt-Tor ✅ (V8.70) → Auto-Vendor-Pfad ✅ (W15 — V8.71/V8.72) → Mesh-Welt-Verteilung ✅ (W16 — V8.73/V8.74). **In Arbeit — die letzte Schicht des Fremd-Engine-Bogens**: die Multiplayer-Sub-Welten (W17 — eine Gruppe taucht gemeinsam in eine Server-Welt). ⏳ **Phase A ✅ (V8.75 — der Transport-Shim) + Phase B-Relay ✅ (V8.76 — das Mesh-als-Server, der Sub-Welt-Verkehr fliesst peer-to-peer übers Mesh)**; offen: Phase C (das Gruppen-Portal) + B-JS-Compute.
...
| 4 — Gruppen-Portal | eine Gruppe tritt gemeinsam durch ein Tor, teilt den Mesh-Raum der Sub-Welt | offen — W17 Phase C |
...
#### Phasen-Detailplan (Sub-Schritte, ausgearbeitet — Stand V8.76; Phase A + B-Relay gebaut)
...
**Phase C — das Gruppen-Portal (~1 Session, NÄCHSTE WELLE).** Ziel: öffnet einer ein Portal, bekommen die anderen einen „mitkommen?"-Prompt.
...
- Stand V8.76: **~2676 Invarianten** in `scripts/playtest.cjs` (Headless-Chromium, ~25 s Log-Sammlung) + `audit-strict.cjs` (5 generische Audit-Schichten) + `smoke-multiuser.cjs` + `smoke-webrtc.cjs` + `smoke-translated.cjs` + `smoke-sandbox.cjs` + `smoke-vendor.cjs` + `smoke-shim.cjs`
