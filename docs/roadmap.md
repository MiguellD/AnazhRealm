...
Stand: V9.70 (24.05.2026, Welle B abgebrochen nach Browser-Audit Round 3 als Patch-Versuch; **Welle C — Wasser-Substanz-Vereinigung** als nächste Welle, die ehrliche Wurzel-Heilung der Zwei-Skalen-Sünde). Diese Doc trägt den **Plan vorwärts** — die Ring-Übersicht 0-17, was offen ist, die Meilenstein- und Risiko-Sicht. Die anderen Zeit-Ebenen: der aktuelle Stand + die Gotchas in `CLAUDE.md`, die volle Wellen-Chronik in `docs/handover.md`, die Vision in `docs/state-of-realm.md`.
...
**C.1 V9.71 — Wasser-Cell-Feld + Worldgen-Init.** Erweiterung der Voxel-Chunk-Datenstruktur: pro Cell ein `Uint8Array`-State (0=air, 1=water, 2=solid). Cell-Feld lebt im bestehenden `state.voxelChunks[key]`-Entry, lazy beim Chunk-Build allokiert. Worldgen-Init: für jeden Cell-Voxel im neuen Chunk: state = solid wenn `density>0`; sonst water wenn `cell.y ≤ state.waterLevel`; sonst air. Initialisierung läuft in `_buildVoxelChunkData` parallel zur Density-Sampling-Phase. Aktuelle `state.hydrosphere` BLEIBT parallel als Drainage-Karte (Welle-A-Mechanik) — wird in C.4/C.5 weggeräumt. Test-Band: Cell-Feld existiert; richtige Größe (dim·dimY·dim); Worldgen-Init setzt Cells korrekt (Cell unter waterLevel + air-Density → state=water; über waterLevel → state=air; positive Density → state=solid); deterministisch über Re-Build. **Verhaltens-Beweis**: bestehende Welle-A-Tests bleiben grün (paralleles System, keiner stört den anderen).

**C.2 V9.72 — Iso-Surface-Mesher für Wasser.** Der bestehende Surface-Nets-Mesher (`_voxelChunkGeometry`) bekommt einen Zwillings-Pass: nach der Boden-Iso-Surface eine zweite Iso-Surface über das Cell-Feld (state==water vs state==air; solid wird zu „außerhalb" gezählt). Ergebnis: ein zweites Mesh pro Chunk (geschwisterlich zum Boden), das die Wasser-Oberfläche als echte Iso-Geometrie trägt. **Naht-frei per Konstruktion** — gleiche Cell-Quelle wie Boden, gleicher Mesher, identische Vertex-Quantisierung über Chunk-Boundaries. Ersetzt `_buildVoxelChunkWater` (per-Chunk-Quad-Mesh). Rendering: `hydroSurfaceMaterial` bleibt unverändert (Shader + Animation), aber Geometry kommt jetzt aus Surface-Nets. Welle-A-Pfade (`_buildVoxelChunkWater` etc.) bleiben PARALLEL aktiv unter Feature-Flag → vergleichbar im Browser. **Schöpfer-Browser-Audit**: das Sheet verbindet sichtbar; das Wasser folgt der echten Terrain-Topographie (auch in Höhlen-Decken).

**C.3 V9.73 — Lokaler Cellular-Flow nach Spieler-Edit.** Spieler-Carve (`carveVoxelSphere`): Cells in der Sphere werden auf air gesetzt (Cell-Feld direkt mutiert). DANN lokaler BFS-Flow-Pass: starte BFS von den 26 Nachbarn der gecarvten Cells; eine Cell qualifiziert sich für water-State wenn (1) state=air, (2) ein Nachbar ist water UND `nachbar.y ≥ cell.y` (Wasser fließt von oben/seitlich, nicht von unten), (3) `cell.y ≤ state.waterLevel` (oder eines verbundenen Lake-Levels — Detail in C.3-Design). BFS-Iter-Cap (z.B. 10000 Cells) gegen Pathologien. Spieler-Fill: Cells werden solid (Wasser wird gelöscht). Architektur-Spawn (V9.65-Blocker-AABBs): solide Parts stempeln Cells als solid; Wasser im Stempel-Bereich verschwindet. Architektur-Remove: gestempelte Cells werden air + lokaler Flow-Pass. **KEIN globaler Recompute**, reine lokale Cell-Manipulation. Reaktion ist instantan (1 Frame, der nächste Chunk-Rebuild übernimmt die Mesh-Update). Test-Band: Carve unter waterLevel + adjacent zu water-Cell → Cell wird water; Damm-Spawn im Fluss → Cells solid, Wasser-Cells außerhalb bleiben; Damm-Remove → Cells werden water (Flow lädt zurück).

**C.4 V9.74 — Welle-A-Mechanik auf Cell-Sprache überführen.** Damm-Bauplan bleibt; `spawnArchitecture` für solide Architekturen ruft jetzt `_stampSolidCellsFromBlueprint(entry)` (statt `_blockerIndexAdd`). Alte Welle-A-Helper-Funktionen werden no-op-Stubs oder gestrichen: `_blockerIndexAdd/Remove/Top`, `_blockerComputePartAABB`, `_isPartSolid` bleibt (wird vom Cell-Stempel-Pfad genutzt); `_effectiveSurfaceY`, `_voxelEditSurfaceDelta` → STRICHEN; `_markHydroDirty`, `_tickHydroRecompute`, `_recomputeHydrosphere` → STREICHEN. `_isAboveWaterAt` liest jetzt direkt das Cell-Feld (Cell-Säule oberhalb von (x, z) ist nicht-water). `_hydroWaterLevelAt` (Buoyancy) → `_voxelWaterTopAt(x, z)` aus dem Cell-Feld. Welle-A-Vision-Beweis-Tests (V9.68-A.5-Band) werden auf Cell-Sprache umgeschrieben — die Vision (Spieler-Wille beeinflusst Wasser) BLEIBT, die Mechanik wechselt. **Verhaltens-Beweis**: alle Welle-A-Invarianten grün, jetzt empirisch cellulär. Plus: Damm in Fluss → Cell-Stempel im Fluss-Bereich → Fluss endet dort visuell (nicht nur „Drainage-Karte routet um").

**C.5 V9.75 — Alten Hydrosphäre-Atlas streichen (Aufräum-Welle).** Streichen: `_computeHydrosphere`, `_hydroInit`, `_hydroMarkOcean`, `_hydroPriorityFlood`, `_hydroFlowDirection`, `_hydroAccumulate`, `_hydroExtractLakes`, `_hydroExtractRivers`, `_hydroBuildCarveIndex`, `_hydroBuildWaterField`, `_hydroBlur`, `_hydroSeedTarns`, `_tarnDeltaAt`. `state.hydrosphere`, `state.tarns` weg. `_waterLevelAt`-5×5-MAX weg. `_voxelChunkTouchesWater`-Gate weg. `_buildVoxelChunkWater`, `_disposeVoxelChunkWater`, `state.voxelChunkWater`, `WATER_CHUNK_SKIRT` (B.1-Konstante) weg. `_buildHydrosphereMeshes`, `_disposeHydrosphereMeshes`, `state.hydrosphereMeshes` — die Wasserfall-Sub-Meshes bleiben (vertikale Sonderform), die Liste wird kleiner. **Wasserfall-Migration**: Wasserfälle könnten Cellular emergieren (vertikale water-Cell-Säulen über Klippen werden als „Wasserfall-Spalte" gerendert mit vertikalem Flow-Shader). Erste Variante: vereinfachte Logik die direkt das Cell-Feld scannt und Klippen-Wasser-Cells findet. Test-Band: alte Hydrosphäre-API-Tests entfernt oder migriert; neue Cell-API-Tests stehen. **Code-netto erwartet**: ~2000-3000 Zeilen gestrichen (der ganze Atlas-Apparat), ~500-1000 Zeilen neu (Cell-Feld + Iso-Mesher + Cellular-Flow). Plus Welle-A-Tests umgeschrieben. **Schöpfer-Browser-Audit Round 4**: das Spiel läuft sauber, das Wasser ist eine Welt-Substanz, kein Flickenteppich mehr.
