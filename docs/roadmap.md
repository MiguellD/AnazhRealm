...
**Backlog aus dem Browser-Audit (V9.70+ Kandidaten)**: (a) **Lake-Mask-Sprung in `_waterLevelAt`** — der 3×3-Mask-Rand springt binär zwischen `lake.level` und `waterLevel`. Physikalisch korrekt (Berg-See ≠ Meer), aber visuell ein Stufe wenn Chunk-Boundary die Lake-Mask-Grenze kreuzt. Heilung-Optionen: sanfter Falloff am Rand, oder das opake Terrain muss den Schnitt verdecken (V9.49-f-Lehre). Eigene Welle. (b) **Spürbarkeit eines Standard-LMB-Carves (r=3.5)** — im Hochland erzeugt das ein 3.5-m-Loch, weit über waterLevel; korrekt „kein Wasser", aber Spieler erwartet Pfütze. Vision-Welle wäre ein Precipitation-Modell.
