...
Stand: V9.56-c (23.05.2026). Diese Doc trägt den **Plan vorwärts** — die Ring-Übersicht 0-17, was offen ist, die Meilenstein- und Risiko-Sicht. Die anderen Zeit-Ebenen: der aktuelle Stand + die Gotchas in `CLAUDE.md`, die volle Wellen-Chronik in `docs/handover.md`, die Vision in `docs/state-of-realm.md`.
...
- **Ein zweiter Code-Hygiene-Bogen — V9.56-a/b/c LAUFEN (23.05.2026)**. Sub-Welle a: `renderPlayerEquipUI` 213 → 19 Z. + 7 Helfer (median 31 Z.). Sub-Welle b: `renderSoulEditorUI` 232 → 13 Z. + 4 Helfer (median 48 Z.). Sub-Welle c: `renderLibraryUI` 241 → 9 Z. + 5 Helfer (median 47 Z.) — die erste Schleifen-Builder-Sub-Welle, mit `_libraryBuildCard` als zweistufige Komposition. Alle drei verhaltensneutral, playtest grün vor + nach. **Bleibt im Bogen**: ein UI-Builder (`_workshopRenderStatsPanel` 319 → V9.56-d, der größte) + sieben Kontrollfluss-Funktionen in Größen-Reihenfolge (`fuseWorlds`, `_tickCreatureTaskDirection` 267, `_voxelChunkGeometry` 269, `processChatCommand` 285, `_applyDayNightToScene` 305, `generateTerrainWithParameters` 346, `loadState` 380). **Disziplin (V9.44-Wiederholung)**: eine Sub-Welle = ein Commit, playtest-grün, verhaltensneutral, kein Bonus-Refactoring im selben Schnitt; vom sichersten zum riskantesten (UI-Builder vor Snapshot/Game-Loop). **Disziplin (V9.40-Lehre)**: der Bogen gehört terminiert, sobald die nächste Feature-Welle eine dieser Funktionen berührt, nicht „irgendwann".
