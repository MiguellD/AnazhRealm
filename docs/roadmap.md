...
Stand: 20.05.2026 (V9.34, Voxel-Phase 5c.2.c.1 — drei nachweislich tote Heightfield-Schichten gelöscht (V9.30-Disziplin, pure Dead-Code): `extendTerrain`-Legacy-direction-API (kein Aufrufer mehr), `caveData`-Float-Array (nie gelesen — Tag-Flag-Versprechen aus V9.27 wurde nie erfüllt), `volcanoData` symmetrisch. Phase 1-5b komplett, Phase 5c-Start (V9.26) + 5c.1 (V9.27) + 5c.1+ (V9.28) + Ammo-Leck-Fix (V9.29) + 5c.2.a Dead-Code (V9.30) + 5c.2.b-Vorbereitung (V9.31) + 5d-Mini Wasserfälle (V9.32) + 5c.2.b Eingangs-Welt-Flip (V9.33) + 5c.2.c.1 weitere tote Schichten (V9.34) live; in 5c.2.c noch offen: 5c.2.c.2 (Toggle-Bewertung — `voxel terrain on/off` + `setVoxelTerrainActive` + `_setHeightfieldDormant`; ein Schöpfer-Entscheid), 5c.2.c.3 (heightfield-only Chunk-Pipeline `ensureChunkAt` + `populateChunkVegetation` + `_buildChunkGrass` + `_terrainHeightAtWorld` ablösen — verlangt 5c.2.c.2 zuerst), 5c.2.c.4 (`modify_terrain`-DSL-Op + Chat-Pattern-Umrouting auf `voxel carve`/`voxel fill`). 2960 Playtest-Runs grün, Audit-Strict 0 Failures. Der Fremd-Engine-Bogen (W12-W17) ist komplett; offen darin nur B-WASM (per-Projekt). Volle Wellen-Historie in `CLAUDE.md`.)
