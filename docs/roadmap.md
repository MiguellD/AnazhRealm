...
| B | **V9.41** ✅ | Schach-Brett-Diagonalen. `quad(a, b, c, d, parity)` triangulieren gerade Parität a→c (alt), ungerade Parität b→d. Drei Emit-Stellen geben `parity = (i+j+k) & 1` mit. Geometrie-Kosten null, Winding bleibt CCW. Browser-Headless-Beweis: 964 a-c vs 885 b-d (~52/48 — fast balanciertes Schach-Brett); vor V9.41 waren 1849 a-c, 0 b-d. 2 neue Invarianten, 5/5 Läufe grün. **Lösungs-Trias** (Roadmap-Original) lieferte Stufe 1; falls Schöpfer-Browser-Test sagt „noch nicht genug" → V9.41.b Laplacian-Smooth-Pass; Dual Contouring bleibt grosse Phase. | Surface-Nets-Quad-Diagonalen — fundamental, V9.16 versteckte nur die Beleuchtung. | ~1 Session | ✅ V9.41 |
