...
Stand: **V13.3 (29.05.2026) — V13-Wasser-Profi-Bogen: Diagnose + Klassifikation + Grenzflächen-Meshing + Flow-Bias-Heilung durch.** **V13.3** heilte den vom Schöpfer vermuteten Richtungs-Artefakt: das D8-Flow (Erosion + Hydrosphäre) wählte steilsten DROP statt steilstes GEFÄLLE → Diagonal-Bevorzugung (65 % statt 50 %) + Tie-Break-Einrichtungs-Drift (↖ 19 %) → der „Treppe/Wasserschatten seit ewig in eine Richtung". Fix `drop/dist` (√2-Diagonal-Penalty) → ↖ 19→8,1 %, eine-Richtungs-Drift weg. Determinismus-brechend (Schöpfer „jetzt fixen"), byte-identisch reproduzierbar, Hydrosphäre intakt. **Nächste Welle: V13.4 — Sub-Region-Edit-Remesh** (durch V13.3 verschoben). Danach V13.5 Shader, V13.6 LOD, V13.7 Audit; dann V14.**
...
| **V13 Wasser-Profi-Bogen** ⏳ (V13.0–.3 ✅) | **Voxel-Wasser EFFIZIENT + KORREKT wie die Riesen** (Schöpfer-Browser-Befund 29.05.2026, im Code verifiziert nicht spekuliert). Wasser bleibt voxel/zell-basiert (V9.69-Vision, reaktiv wie Minecraft) — aber gerendert + klassifiziert wie die Giganten, nicht zurück zu flachen Planes (= Ausweichen). Detail §1.4. | V12 ✅ | **V13.0 ✅** Diagnose · **V13.1 ✅** Klassifikation: `_atlasWaterLevelAt` + Depth-Gate (Hang-Schatten 51,7→0,1 %) · **V13.1.1 ✅** Worker-Cache-Buster · **V13.2 ✅** Grenzflächen-Meshing (~150→~3 ms, ~18× weniger Dreiecke) · **V13.3 ✅** Flow-Bias: D8 `drop/dist` statt `drop` (Diagonal-Einrichtungs-Grain weg, ↖ 19→8,1 %; determinismus-brechend) · **V13.4** Sub-Region-Edit-Remesh · **V13.5** Wasser-Shader-Glanz (Tiefen-Schaum, Gerstner-Ripples, Reflexion) · **V13.6** LOD-Cross-Fade · **V13.7** Schöpfer-Browser-Audit | Wasser positioniert + rendert wie die Riesen (Minecraft/Genshin-Lehre): kohärente Körper, keine Hang-Schatten, fast gratis, schön — UND voll reaktiv (Damm staut). |
...
| **V13.4** ⏳ **next** | Sub-Region-Edit-Remesh: nur die vom Carve/Fill berührten Zellen + ihre Mesh-Sektion neu, nicht der ganze Chunk. | Edit-Drops, „ganzer Chunk neu" |
| **V13.5** | Wasser-Shader-Glanz: Tiefen-Schaum (Depth-Buffer), Gerstner-Ripples (Vertex), Reflexion/Refraktion auf der billigen flachen Fläche. | Visuelle Tiefe wie die Profis |
| **V13.6** | LOD-Cross-Fade: sanftes Überblenden zwischen Detailstufen statt hartem Pop. | „sauberer werden"-Pop |
| **V13.7** | Schöpfer-Browser-Audit RDNA-3: Wasser kohärent, keine Hang-Schatten, smooth beim Streamen + Editieren. | Bogen-Schluss |
