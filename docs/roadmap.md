...
| **V13 Wasser-Profi-Bogen** ⏳ (V13.0–.2 ✅) | **Voxel-Wasser EFFIZIENT + KORREKT wie die Riesen** (Schöpfer-Browser-Befund 29.05.2026, im Code verifiziert nicht spekuliert). Wasser bleibt voxel/zell-basiert (V9.69-Vision, reaktiv wie Minecraft) — aber gerendert + klassifiziert wie die Giganten, nicht zurück zu flachen Planes (= Ausweichen). Detail §1.4. | V12 ✅ | **V13.0 ✅** Diagnose (Mesher ~124–166 ms · Schatten 43–54 % = See-Dilatations-Bleed · Edit 0,04 %/100 %) · **V13.1 ✅** Klassifikation: `_atlasWaterLevelAt` + Depth-Gate (Hang-Schatten 51,7→0,1 %) · **V13.1.1 ✅** Worker-Cache-Buster (Fix erreicht den Browser) · **V13.2 ✅** Grenzflächen-Meshing: Boundary-Quads statt Surface-Nets (~150→~3 ms, ~18× weniger Dreiecke) · **V13.3** Sub-Region-Edit-Remesh · **V13.4** Wasser-Shader-Glanz (Tiefen-Schaum, Gerstner-Ripples, Reflexion) · **V13.5** LOD-Cross-Fade · **V13.6** Schöpfer-Browser-Audit | Wasser positioniert + rendert wie die Riesen (Minecraft/Genshin-Lehre): kohärente Körper, keine Hang-Schatten, fast gratis, schön — UND voll reaktiv (Damm staut). |
...
| **V13.3** ⏳ **next** | Sub-Region-Edit-Remesh: nur die vom Carve/Fill berührten Zellen + ihre Mesh-Sektion neu, nicht der ganze Chunk. | Edit-Drops, „ganzer Chunk neu" |
