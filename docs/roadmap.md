...
Stand: 21.05.2026 (V9.43-c.2 — das Wasser wird synergetisch: Schöpfer-Browser-Befund — die Seen/Flüsse schweben ein paar Meter über dem Meer; Fix: Fluss-Mündungen erreichen sichtbar das Meer/den See, und der Spieler schwimmt in einem See wie im Meer (`_hydroWaterLevelAt`). Nächste Welle: V9.43-d — die Flüsse carven echte Betten). Davor: V9.43-c (`_buildHydrosphereMeshes()` rendert das Drainage-Netz), der Stamm-Pflege-Bogen V9.44-a..f VOLLSTÄNDIG (verhaltensneutrales Refactoring) + V9.43-b (der Hydrosphären-Atlas — das Drainage-Netz als reine Daten). Die Heightfield-Pipeline ist mit V9.39 vollständig zurückgebaut (Phase 5c.2.c.3 abgeschlossen — die Voxel-Welt trägt alles allein). Der Schöpfer-Audit nach V9.39 (fünf Browser-Befunde) ist mit der Voxel-Surface-Politur V9.40–V9.43 nahezu vollständig geheilt: V9.40 a–f (Chunk-Löcher beim Carve/Fill + Maus-Edit-P2P-Sync — Wurzel: `ammo.wasm` auf 256 MB growable gepatcht), V9.41 + V9.41-b (Trapeze/Linien — Schach-Brett-Diagonalen + Laplacian-Smooth), V9.42 a–d (Inseln teilen die Surface-Nets-Pipeline + MeshToon-Material + Naht-pad-Smooth), V9.43-a (Befund #5 Wasser — Wasserfälle von `THREE.Points`-Partikeln auf vertikale Wasser-Planes mit geteiltem Flow-Shader). **Offen: V9.43-d/e — Flüsse carven echte Betten ins Terrain, Klang.** Die V9.43-c-Browser-Verifikation brachte einen ehrlichen Befund: das V9.43-b-Drainage-Netz ist see-dominant (12 Seen, kurze 32-48-m-Fluss-Verbinder) — lange frei fließende Flüsse sind eine Netz-Qualitäts-Frage für eine Folge-Welle. **Ausführliche Planung: `docs/hydrosphere.md`.** Der Fremd-Engine-Bogen (W12–W17) ist komplett; offen darin nur B-WASM (per-Projekt). Wellen-Plan + Audit-Lehren in §3 „Voxel-Surface-Politur"; die kanonische Versions-Historie lebt in `CLAUDE.md`.
...
| D.4 | **V9.43-d** | **Flüsse carven echte Betten** — `_terrainDensityAt` fragt die Hydrosphäre (`riverBuckets`-Spatial-Index, O(1)) + senkt die Dichte im Fluss-Kanal + in See-Senken → der Chunk-Mesher produziert echte Rinnen + gemuldete Becken mit Ufern (heilt zugleich das V9.43-c.2-„flache Sheets"-Rest-Bild — das Wasser liegt dann sichtbar IN einer Furche). Zirkel-frei (während die Hydrosphäre rechnet, ist `state.hydrosphere` null → kein Carve → `_voxelSurfaceY` sieht die Basis-Surface). | das Wasser soll in einer echten Furche liegen, nicht aufgemalt. | ~1 Session | 🔴 geplant |
