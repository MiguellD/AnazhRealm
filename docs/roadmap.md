...
**B.1 — Wasser-Mesh-Skirts gegen Chunk-Naht (V9.70, nächste Welle)**: jeder Voxel-Chunk-Wasser-Mesh erweitert sein Vertex-Raster um 1-2 Cells über jeden Boundary hinaus (Stil der V9.10 Terrain-Naht-Skirts). Sichtbar: zwei adjacent Chunks überlappen ihre Quads an der Naht; jeder bedeckt den anderen Schluss. Mechanik: NV von `dim+1` → `dim+3` (oder `dim+5`); `i,j`-Range von `[0, dim]` → `[-1, dim+1]`; Welt-Koords `vx = ox + i*step` bleibt, jetzt mit i ∈ {-1, ..., dim+1}. Indices-Loop erweitert sich entsprechend. Kosten: pro Chunk ~10 % mehr Vertices (50→55 statt 50×50). Akzeptanz: zwei Test-Chunks bauen, ihre Boundary-Vertex-Positionen überlappen geometrisch (Headless-Geometry-Test). Schöpfer-Browser-Audit: das Sheet verbindet sichtbar.
