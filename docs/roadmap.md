...
Die geplante Roadmap ist **im Kern abgearbeitet.** Ring 0-11.5 (Fundament → Welten-Ultiversum) + Welle 1-14 + W7 (Compute-Sharing) + der Fremd-Engine-Bogen W12-W17 sind live. Darüber hinaus gewachsen: der Voxel-Terrain-Bogen (V9.07+ — 3D-Dichte-Feld, formbarer Boden, Höhlen/Überhänge) und das Wasser-Ultiversum / die Hydrosphäre (V9.43 — ein Drainage-Netz aus Flüssen, Seen und Wasserfällen, das echte Betten ins Terrain carvt). Plus der Stamm-Pflege-Bogen V9.44 (verhaltensneutrales Code-Hygiene-Refactoring). ~3000 Playtest-Invarianten grün, Audit-Strict 0 Failures, 120 fps.
...
### Welle B — Wasser-Naht & Performance (Browser-Audit Round 2, geplant 24.05.2026 nach V9.69)

**Schöpfer-Browser-Audit Round 2** (nach V9.69 Gate-Heilung): „die Wasseroberfläche, das Sheet scheint nicht zu verbinden, die Chunckübergänge immernoch heickel und die FPS beginnen zu stocken." Drei distinkte Symptome — eine gemeinsame strukturelle Wurzel.

**Riesen-Reflexion**: jede Wasser-Geniale Welt (Minecraft, Genshin/BOTW, NMS, Subnautica, Noita) lebt in **EINER SKALA für Wasser** — das ist die strukturelle Disziplin, nicht die Algorithmik. Minecraft: 1-m³-Cells. Genshin: pro Asset. NMS: ein globales Ozean-Mesh. Wir leben in **ZWEI SKALEN**: das Drainage-Netz auf 16-m-Karte (`state.hydrosphere`) + das per-Chunk-Wasser-Mesh auf 1.8-m-Vertices (`_buildVoxelChunkWater`). Die beiden müssen sich an jedem Punkt einig werden — was strukturell nicht glatt geht. Die Welle B heilt die DREI Symptome ohne radikale Wende; eine optionale Mega-Welle „Substanz-Vereinigung" (siehe ehrliches Backlog unten) bleibt vorbehalten.

**Akzeptanz der Welle B** (Schöpfer-Browser-Audit Round 3): das Sheet verbindet ohne sichtbare Risse über Chunk-Grenzen; FPS bleiben bei Voxel-Edit-Bursts stabil (kein wahrnehmbares Stocken); Lake-Mask-Naht wird nicht mehr als Stufe wahrgenommen.

**B.1 — Wasser-Mesh-Skirts gegen Chunk-Naht (V9.70) ✅ erledigt**: neue Konstante `WATER_CHUNK_SKIRT = 1` in `AnazhRealm`. `_buildVoxelChunkWater` erweitert sein Vertex-Raster auf `NV = dim+1+2·SKIRT = 27` (statt 25). Schleife `i,j ∈ [0, NV)` mit `vx = ox + (i−SKIRT)·step` — erster Vertex 1.8 m VOR Chunk-Anfang, letzter 1.8 m DAHINTER. Quad-Schleife auf `NQ = dim+2·SKIRT = 26` Zeilen/Spalten. Boundary-Vertices zweier adjacent Chunks fallen auf exakt dieselbe Welt-Position → beide Meshes bedecken die Naht aus ihrer Seite (Overlap statt Lücke). Memory-Kosten: pro Chunk 27² vs 25² ≈ +17 % Vertices/Quads (729 vs 625, 676 vs 576 Quads) — akzeptabel. Test-Band `checkBandWelleB1WaterSkirts` mit 9 Invarianten (Konstante=1, Source-Probe in `_buildVoxelChunkWater`, wasser-tragenden Chunk gefunden, Vertex-Count=729, erster/letzter Vertex-Position, Boundary-Match zwischen zwei adjacent Chunks an ax=bx=-216, Skirt-Bleed -214.2 = -216+step). **Verhaltens-Beweis**: Playtest „Alle Invarianten OK"; audit:strict 0 Failures. **Lehre**: das Sheet verbindet sich strukturell durch OVERLAP, nicht durch nahtlose Interpolation — Overlap ist einfacher und ehrlicher. Plus: V9.10 Terrain-Naht-Skirts (Wand-Skirts NACH UNTEN am Chunk-Rand) sind das vertikale Pendant; B.1 ist das HORIZONTALE Wasser-Pendant. **Schöpfer-Browser-Audit pendent** (Sheet verbindet sichtbar — vor B.2).

**B.2 — Sanfter Lake-Mask-Falloff in `_waterLevelAt` (V9.71)**: statt binärer 3×3-MAX einen 5×5-Gauss-Falloff. Mechanik: für (x, z) berechne Distanz `d` zur nächsten echten Lake-Cell (`waterKind===2`); innerhalb 1 Cell → voll `lake.level`; 1-3 Cells außerhalb → linear (oder Gauss) fade von `lake.level` zu `max(waterLevel, surrounding effective)`; ≥3 Cells außerhalb → `waterLevel`. Vorsicht-Disziplin: Bergsee-Spiegel bleibt scharf (er soll als See sichtbar sein), nur der TRANSITIONS-Bereich weichgezeichnet. Akzeptanz: Test-Band — 1 Cell innen = lake.level, 2 Cells außen ≈ 0.5*lake.level + 0.5*waterLevel, 4 Cells außen = waterLevel. Performance-Check: `_waterLevelAt` wird PRO Vertex aller Wasser-Meshes gerufen (25×25 × 20 Chunks = ~12 500 calls/sek bei Streaming-Last); 5×5-Scan statt 3×3 = ~2.8× teurer pro call. Wenn das Budget brennt → Optimierung via Distance-Field-Precompute pro Lake (statt per-Vertex-Scan).

**B.3 — Inkrementeller Mesh-Rebuild via dirty-bbox (V9.72)**: `_markHydroDirty(bbox?)` akkumuliert eine Welt-bbox-Union; `_recomputeHydrosphere` rebuildet beim Mesh-Phase nur die Voxel-Chunk-Wasser-Meshes deren Footprint die bbox berührt — statt ALLER ~20 Chunks. Mechanik: neuer state.hydroDirtyBBox = `{minX, maxX, minZ, maxZ}` | null; bei `_markHydroDirty(bbox)` Union; bei null → "alles dirty" (fallback). spawnArchitecture/removeArchitecture passen ihre Entry-AABB als bbox; `_addVoxelEdit` nutzt (x±r, z±r). Atlas-Phase (`_computeHydrosphere`) bleibt global (Priority-Flood ist nicht lokal zerlegbar) — die ~250-ms-Atlas-Kosten bleiben; aber die Mesh-Phase fällt auf 1-3 Chunks statt 20+ = ~80 % Mesh-Phase-Sparung pro Recompute. Akzeptanz: Test-Band — Edit an Pos P, nur Chunks im (P±r±span)-Footprint sind im Mesh-Rebuild-Set; ferner Chunk bleibt unberührt (Mesh-Identität gleich vor/nach). Schöpfer-Browser-Audit: FPS bleibt stabil bei rapidem Carve-Burst.

**B.4 — Visual-Test als Disziplin (V9.73, optional)**: Headless-Test, der nach einer Mutation den Mesh-Vertex-Count + die Wet-Cell-Anzahl misst. Ergänzt die V9.69-Truth-Source-Tests (V9.66 + V9.68) um eine ECHTE Render-Konsequenz-Prüfung. Akzeptanz: Carve unter waterLevel im Hochland-Chunk → Chunk-Water-Mesh hat ≥ N wet Vertices, war vor Edit null.

**Sub-Wellen-Disziplin**: jede mit eigenem Test-Band + Schöpfer-Browser-Audit zwischen B.1/B.2/B.3. Wenn der Audit nach B.1 zeigt „Sheet verbindet visuell", aber B.2 noch nicht nötig ist (Schöpfer akzeptiert den Lake-Sprung als Berg-See-Charakter), dann skippen wir B.2 ehrlich. Eisenhower vor jeder Sub-Welle: was treibt den Schöpfer-Befund am stärksten?

**Ehrliches Backlog hinter Welle B — Mega-Welle „Wasser-Substanz-Vereinigung" (V9.80+, vielleicht)**: die radikale Heilung der Zwei-Sprachen-Wurzel. Statt Drainage-Karte (16m) + Voxel-Mesh (1.8m) gibt's EINE Wahrheits-Quelle: das Voxel-Density-Feld trägt einen 3. Cell-Zustand (`isWater`), der Surface-Nets-Mesher baut zwei Iso-Surfaces (Solid|Else + Water|Air) aus DERSELBEN Geometrie-Quelle. Naht-frei per Konstruktion, Spieler-Carve setzt Wasser-Cells direkt im Feld (lokal-cheap), Drainage emergiert aus Cell-Connectivity statt aus globalem Priority-Flood. **Aufwand**: substantiell — `_terrainDensityAt` erweitert, `_voxelChunkGeometry` baut zwei Meshes, der ganze Hydrosphäre-Atlas wird DERIVED VIEW oder obsolet. **Risiko**: Welt-Identität (Welt-Seeds!) und Save-Backward-Compat. **Vorbedingung**: Welle B reicht NICHT — Schöpfer-Audit nach B.3 zeigt residuale Naht/Sprung-Probleme, die nur eine Sprache-Vereinigung heilen kann. Wenn Welle B die Symptome befriedet, bleibt diese Mega-Welle Backlog und wird NICHT proaktiv gebaut (V9.50-Lehre: das ehrliche Genug ist besser als das radikale Perfekt).
