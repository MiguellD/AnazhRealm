...
Stand: V9.56-i (23.05.2026). Diese Doc trägt den **Plan vorwärts** — die Ring-Übersicht 0-17, was offen ist, die Meilenstein- und Risiko-Sicht. Die anderen Zeit-Ebenen: der aktuelle Stand + die Gotchas in `CLAUDE.md`, die volle Wellen-Chronik in `docs/handover.md`, die Vision in `docs/state-of-realm.md`.
...
- **Ein zweiter Code-Hygiene-Bogen — UI-Etappe GESCHLOSSEN, Kontrollfluss-Etappe LÄUFT (V9.56-a..i, 23.05.2026)**. UI-Etappe: vier Sub-Wellen a/b/c/d. Kontrollfluss-Etappe: e `fuseWorlds` 239 → 114 + 8 Helfer, f `_tickCreatureTaskDirection` 267 → 8 + 3 Helfer, g `_voxelChunkGeometry` 269 → 25 + 6 Helfer (Surface-Nets-Pipeline), h `processChatCommand` 285 → 21 + 9 Helfer (Try-Pattern-Dispatcher), i `_applyDayNightToScene` 305 → 16 + 9 Helfer (Tint-Akkumulation + Fanout-Apply; Duplikat-Konsolidierung → Datei −30 Z. netto-negativ). Alle neun Sub-Wellen verhaltensneutral, playtest grün vor + nach. **Bleibt im Bogen — zwei Kontrollfluss-Funktionen**: `generateTerrainWithParameters` 346 → V9.56-j, `loadState` 380. **V9.56-e/f/g/h/i-Lehren**: (1) Block-Grenzen-Inspektion ist eine HIERARCHIE (pro Schicht eine optimale Granularität); (2) Side-Effect-Stream bit-identisch halten; (3) Pipeline-Kopplung kann einseitig sein; (4) Try-Pattern-Konvention für Multi-Branch-Dispatcher; (5) Cleanup-Asymmetrie preservieren; (6) Strukturelle Test-Invarianten mit-wandern lassen (Doku-Sync auf Test-Code, nicht Code-Form an Test anpassen). **Disziplin (V9.44-Wiederholung)**: eine Sub-Welle = ein Commit, playtest-grün, verhaltensneutral, kein Bonus-Refactoring im selben Schnitt. **Disziplin (V9.40-Lehre)**: der Bogen gehört terminiert, sobald die nächste Feature-Welle eine dieser Funktionen berührt, nicht „irgendwann".
