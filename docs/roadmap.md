...
Stand: V9.52-e (23.05.2026). Diese Doc trägt den **Plan vorwärts** — die Ring-Übersicht 0-17, was offen ist, die Meilenstein- und Risiko-Sicht. Die anderen Zeit-Ebenen: der aktuelle Stand + die Gotchas in `CLAUDE.md`, die volle Wellen-Chronik in `docs/handover.md`, die Vision in `docs/state-of-realm.md`.
...
- **Stamm-Hygiene über den Code hinaus** — seit V9.44 sind Tests + Doku Teil des Stamms und brauchen dieselbe Hygiene-Disziplin: `playtest.cjs` (war 31 574 Z., EINE Funktion — die zweitgrößte Reibung des Projekts; jetzt 30 137 Z. mit 41 benannten Funktionen, der `else { ... }`-Block des Orchestrators ist erschöpft als 40-Calls-Liste) — der **Playtest-Pflege-Bogen** läuft: Sub-Wellen **a + b + c + d + e sind drin** (V9.52-a/b/c/d/e: `safeEvaluate`/`ctx`-Gerüst + 4 erste Funktionen + 11 + 8 + 8 + 10 Band-Funktionen, je 265-1567 Z. plan-treu nach §5), nur Sub-Welle f (Helfer-Durchzug + ehrliche Rest-Grenze) bleibt. Voller Schnitt-Plan in `docs/playtest-hygiene.md` (§3 unten). Die Doku-Schicht braucht periodisches Archivieren fertiger Konzept-Dokumente (Konsolidierung 21.05.2026 + erneut diese Session) — sonst wächst das „Projektgedächtnis" zur Halde, eine Schicht über der Heiligen Lektion.
...
### Der Playtest-Pflege-Bogen — Test-Hygiene (läuft, 23.05.2026 — Sub-Wellen a + b + c + d + e sind drin, nur f bleibt)
...
**Status (Stand 23.05.2026):** Sub-Wellen **a + b + c + d + e sind drin** (V9.52-a: Gerüst + 4 Funktionen; V9.52-b: 11 Band-Funktionen; V9.52-c: 8 Band-Funktionen Welle 6.A..6.H Phase 2C; V9.52-d: 8 Band-Funktionen Welle 6.X+G3/G4+V8.x+W12-14; V9.52-e: 10 Band-Funktionen Welle 6.H 2B.2..Ring 6 V2, je 432-1131 Z., Skript `/tmp/extract-band4-e.cjs` — zehn Baseline-Läufe bit-identisch). **Der else-Block ist erschöpft als 40-Calls-Liste — kein Inline-Code mehr in der `### Invarianten`-Sektion.** Nur **Sub-Welle f bleibt**: der Helfer-Durchzug. Das ~190× wiederholte `await page.evaluate(...).catch(() => null)`-Boilerplate konsequent durch `safeEvaluate(page, ...)` routen (der V9.52-a-Helfer ist gebaut aber bisher ungenutzt — bewusst getrennt von der Struktur-Arbeit), Sektions-Header-Stil vereinheitlichen, ehrliche Rest-Grenze dokumentieren.
