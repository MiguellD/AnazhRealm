...
Stand: 20.05.2026 (V9.39, Voxel-Phase 5c.2.c.3.b.iii — die letzte grosse Aufräum-Welle der Heightfield-Pipeline: sieben tote Methoden gelöscht (`ensureChunkAt`, `populateChunkVegetation`, `_buildChunkGrass`+`_disposeChunkGrass`, `_terrainHeightAtWorld`, `_disposeChunkPhysics`, `pruneDistantChunks`, `_chunkGeometry`), plus `state.terrainMaterial`-Setzer + Material-Builder, plus `_buildWaterPlane`-Heightfield-Branch + SimplexNoise-Instanzen, plus Game-Loop-`pruneDistantChunks`-Aufruf + Cleanup-Loop, plus Legacy-`onMouseClick`-Block-Place-Handler (sein `[state.groundMesh]`-Fallback crashte mit `null.layers` — V9.39-Lösch deckte den pre-existing Bug auf). Plus `toggleTerrain` voxel-aware. ~500 Z. Code netto weg, 14 V9.39-Lösch-Beweise neu, 25+ Heightfield-Pipeline-Tests gestrichen oder auf Voxel-Pendant umgewidmet. 2952 → 2931 grün. **Phase 5c.2.c.3 ist mit V9.39 abgeschlossen** — die heightfield-only Chunk-Pipeline ist vollständig zurückgebaut. Davor V9.38: fünf weitere tote Heightfield-`else`-Pfade entfernt — fünf weitere tote Heightfield-`else`-Pfade entfernt (heightData-Allokations-Pfad, 256×256-Wasserfall-Loop, „Neues Terrain generiert"-Log-Gate, Heightfield-Pfad in `getTerrainHeightAt`, Heightfield-Pfad in `findSurfaceAbove`) + `calculateTerrainSteepness` ganz gelöscht (kein Aufrufer mehr nach Wasserfall-Loop-Lösch). ~200 Z. Code netto weg, 7 V9.38-Lösch-Beweise neu, 1 V9.34-Test umgewidmet (caveNoise/volcanoNoise leben jetzt in `ensureChunkAt`+`_buildChunkGrass`). 2945 → 2952 grün. Davor V9.37: drei tote Heightfield-Dispatch-Pfade entfernt (Game-Loop-Streaming-Ring, initial 64-Chunk-Loop, populateChunkVegetation-Heightfield-Pass), ~50 Z. weg. Davor V9.36: `modify_terrain` stirbt, der Welt-Mod-Bogen schlägt um zum Voxel-Feld (~289 Z. weg, ersetzt durch `voxel_carve`/`voxel_fill`). Phase 1-5b + 5c.1-5c.2.c.3.b.ii komplett, in 5c.2.c noch offen: **5c.2.c.3.b.iii** (die Pipeline-Methoden selbst — `ensureChunkAt`/`populateChunkVegetation`/`_buildChunkGrass`/`_terrainHeightAtWorld`/`_disposeChunkPhysics`/`pruneDistantChunks`; verlangt vorher Test-Migration der V9.29/V8.27/V8.28-Tests + den `terrainMaterial`-ShaderMaterial-Bau, der mit ihnen stirbt). 2952 Playtest-Runs grün, Audit-Strict 0 Failures. Der Fremd-Engine-Bogen (W12-W17) ist komplett; offen darin nur B-WASM (per-Projekt). Volle Wellen-Historie in `CLAUDE.md`.)
...
| A.3 | **V9.40-c** | Async-Rebuild: `_remeshVoxelChunksAround` markiert Chunks dirty (statt sync zu rebuilden); ein `_tickDirtyVoxelChunks`-Pfad im Game-Loop rebuildet ≤1 Chunk/Frame. Verlangt VORHER eine Test-Infrastruktur, die den Async-Pfad von den initial-Spawn-Tests trennt (z.B. ein `_testForceSyncRemesh`-Flag oder ein expliziter `_drainDirtyVoxelChunks()`-Sync-Drain für Tests). Bisect in V9.40-b ergab: bloßes Aufrufen eines Game-Loop-Hooks (auch leer) bricht die Voxel-Initial-Spawn-Tests von 3/5 grün auf 5/5 rot — die Tests warten auf Streaming-gebaute Chunks, ein zusätzlicher rAF-Hook verschiebt das Timing genug, dass voxelChunks.size zur Test-Zeit 0 ist. Das löst sich nicht durch Throttling. Lösung kommt als V9.40-c. Heilt das Ruckeln bei häufigen Edits (Schöpfer-V9.39-Befund). | Ruckel beim Carve/Fill (synchroner Rebuild aller 9 Skirt-Nachbarn). | ~2 Sessions (inkl. Test-Infrastruktur) | 🔴 offen |
...
| D | **V9.43** | **Schöpfer-Wahl 20.05.2026: a+b+c — das komplette Wasser-Ultiversum.** Ein Shader, drei Geometrien, ein `uFlowDirection`/`uFlowSpeed`-Vektor (Sea-of-Thieves-/Genshin-Flow-Map-Pattern): **(a)** Wasserfälle als vertikale Wasser-Plane mit Flow-Vektor (0,−1) — vereinheitlicht `THREE.Points`-Partikel (V9.32) mit dem Gerstner-Plane-Shader. **(b)** Meeres-Strömung: sanfter Flow (z. B. Land-Distanz oder globaler Wind-Vektor — Vision §3 Welt-Atmen) auf der bestehenden 900×900-Plane. **(c)** Bäche/Flüsse: Pfad-Generierung von hoher `lebendig`-Affinitäts-Quelle zum Meer, schmale horizontale Plane entlang des Pfads mit Flow-Vektor = Höhen-Gradient. Vision §1.3 fraktal (eine Sprache regelt alle drei Wasser-Skalen) + §1.4 multisensorisch (Klippen-Plane löst auch V9.32-Audio-Pfad aus). | drei Wasser-Sprachen (Plane / Partikel / nichts für Bäche) — Vision-Inkonsistenz. | ~4-5 Sessions | 🔴 offen, a+b+c gewählt |
