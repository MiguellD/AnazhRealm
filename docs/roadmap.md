...
Stand: 20.05.2026 (V9.27, Voxel-Terrain-Bogen Phase 5c.1 — eine voxel-basierte Welt baut die initialen 64 Heightfield-Chunks gar nicht mehr; der Gate in `generateTerrainWithParameters` liest `worldMeta.voxelTerrain` als persistente Wahrheit. Phase 1-5b komplett, Phase 5c-Start (V9.26) + 5c.1 (V9.27) live; offen in 5c: kleine heightData-Politur + die grosse Heightfield-Code-Entfernung + Test-Suite-Umschreibung — sobald die Migrations-Erfahrung gefestigt ist. ~2928 Invarianten grün, Audit-Strict 0 Failures. Der Fremd-Engine-Bogen (W12-W17) ist komplett; offen darin nur B-WASM (per-Projekt). Volle Wellen-Historie in `CLAUDE.md`.)
