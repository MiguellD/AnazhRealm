...
Stand: **V13.4 (29.05.2026) — V13-Wasser-Profi-Bogen durch** (Diagnose · Klassifikation · Grenzflächen-Meshing · Flow-Bias · Glättung). Davor durch: der V12-Genie-Bogen (Three.js r184 + WebGPU-required, 119 FPS steady — Render gelöst) + der V10.0-NodeMaterial/TSL-Bogen. Die volle Wellen-Chronik lebt in `docs/handover.md`.

**Nächste Welle: V13.5 — Schicht-3-Tiefen-Ufer-Shader** (weiche Uferlinie via `viewportLinearDepth` + Tiefen-Farbe + Gerstner-Ripples + Fresnel + Emotions-Haken; im selben Zug die V13.4-Geometrie-Glättung zurücknehmen — die V13.4-Reflexion zeigte: das war eine Schicht-Verwechslung, Wasser ist drei Schichten und die Übergänge gehören in den Shader, nicht die Geometrie). Danach **V13.6** Sub-Region-Edit-Remesh, **V13.7** LOD-Cross-Fade, **V13.8** Schöpfer-Browser-Audit; dann **V14 — die lebendige Welt** (Vision-Pfeiler E/F/G). Der volle V13-Plan steht in **§1.4**.
