...
Stand: 18.05.2026 (V8.79, W17 Phase B-JS-Compute Phase 1 — der Compute-Host: ein Peer der Gruppe führt die autoritative Server-JS einer fremden Welt aus. B-Relay (V8.76) machte das Mesh zum Server für RELAY-Welten (Server = blosser Rebroadcast); eine JS-Compute-Welt hat eine echte autoritative Server-JS-Logik. Phase 1 baut den Compute-Host-Mechanismus: `portalMeta.serverMode` (`relay`/`js-compute` — `_sanitizePortalMeta` whitelistet es, `aimBlueprintAtWorld` trägt es, `buildStateSnapshot` persistiert es feldweise); Host-Wahl OHNE Präsenz-Tabelle (wer ein js-compute-Portal direkt betritt, wird Host; wer einer W17-Phase-C-Einladung folgt, wird Gast des Einladenden — `inv.peerId` IST der Host); ein zweites, verborgenes null-origin-iframe als Server-Kontext (`?anazh-server=1`, der save-server injiziert `PORTAL_SERVER_SHIM`, ein `WebSocketServer`-Global); host-geroutetes Transport (`subworld-srv` Gast→Host + `subworld-cli` Host→Gast, kanal-exklusiv, statt Broadcast). +19 Invarianten, Zwei-Browser-verifiziert (`smoke-webrtc.cjs` — eine laufende Summe 12=7+5, von EINEM Server in EINEM Tab autoritativ berechnet; ein Relay hätte nur „5" zurückgespielt). Host-Migration ist Phase 2. Davor V8.78 W17 die Multiplayer-Welt-Deklaration — eine vendorte Welt erklärt sich selbst mehrspielerfähig (ein `multiplayer`-Flag durch JEDE Schicht der Vendor-/Mesh-Kette). Davor V8.77 W17 Phase C — das Gruppen-Portal: betritt ein Spieler ein Multiplayer-Portal, broadcastet er einen `portal-invite`; die Mitspieler bekommen einen In-Game-Banner „X öffnete ein Tor — mitkommen?"; „Mitkommen" holt das Portal + betritt es (der Overlay erzwingt `multiplayer:true` — der Empfang der Einladung beweist es), die B2-Sub-Raum-Eingrenzung verbindet die Gruppe. Damit ist W17 Phase A+B-Relay+C komplett — eine Gruppe taucht gemeinsam in eine vendorte Relay-Multiplayer-Welt. Davor V8.76 W17 Phase B-Relay — das Mesh-als-Server: ein `ws-send` aus dem Sub-Welt-iframe wird ein `subworld-net`-Mesh-Broadcast (`_portalNetReceive`); jedes andere AnazhRealm im selben Sub-Welt-Raum (B2 — der Schlüssel ist der Welt-Pfad) stellt es als `ws-recv` in SEIN iframe zu (`_portalNetDeliver`). Rate-Limit + Größen-Deckel (B3) schützen den Kanal. Davor V8.75 W17 Phase A — der Transport-Shim (der save-server-injizierte Shim `PORTAL_TRANSPORT_SHIM` ersetzt `window.WebSocket` durch eine postMessage-Brücke; der Verkehr quert die Sandbox-Grenze zu AnazhRealm). Davor V8.74, W16 Phase 2 — der Welt-Katalog: die Mesh-Bibliothek wird browsbar — jeder Mitspieler annonciert seine vendorten Welten über den `soul`-Kanal (wie `worldRole`/`voiceShared`), der Bibliothek-Drawer zeigt „Mitspieler X hat: …" mit einem Holen-Knopf; ein deterministischer sha256-Content-Hash (der save-server rechnet ihn — Hash-Autorität) gibt einer vendorten Welt eine peer-unabhängige Identität + erlaubt Dedup. Davor V8.73, W16 Phase 1 — Mesh-Welt-Verteilung: eine vendorte Welt reist peer-to-peer. Ein Mitspieler im selben Mesh-Raum holt das Bündel einer vendorten Welt über das Mesh (`world-bundle-pull`/`world-bundle-chunk`, Spiegel von W7 P2; der Sender liest sein Bündel über die neue save-server-Lese-Seite `GET /api/vendor-bundle` zurück); der Empfänger reicht es an die erprobte `vendorWorldBundle`-Schreib-Seite — ein dritter Eingang (lokal / GitHub / Mesh-Peer), eine Wand. Davor der Auto-Vendor-Pfad komplett (W15) — ein fremdes Welt-Bündel dockt OHNE Handarbeit an. Der save-server hat einen zweiten Schreib-Pfad `/api/vendor-world` mit zwei Eingängen: ein hochgeladenes Bündel `{worldId, files}` (Phase 1, V8.71) ODER eine GitHub-Repo-URL `{worldId, repoUrl}` (Phase 2, V8.72 — der Server holt die Dateien per Trees-API + Raw-Fetch, kein SSRF: die API-/Raw-URLs baut er aus operator-konfigurierbaren Bases selbst). Strenge Pfad-/Größen-/Endung-Wand, NIE wird etwas ausgeführt; die Welt wird ein `customWorlds`-Eintrag mit `trust:"sandboxed"` — sie läuft null-origin. Davor V8.70, Untrusted-Welt-Tor — eine echte, ungeprüfte fremde Engine läuft null-origin sandgesichert hinter dem Portal: `portalMeta.trust:"sandboxed"` → das iframe bekommt `allow-scripts` ALLEIN (opake null-Herkunft), fremder Code läuft voll, kann AnazhRealms localStorage/DOM/Cookies aber nicht berühren. Die `worlds/schwarm/`-Welt (eine eigenständige 2D-Boids-Engine) beweist es. Davor V8.68/V8.69 der KI-Übersetzer (Phase 1 Manifest, Phase 2 deklarative Szene + generischer Renderer — eine übersetzte Welt wird betretbar). Davor V8.67 W11 V4 Voice-Sync — der Präsenz-Bogen (sehen/spüren/kennen/**hören**) schliesst sich: ein Begleiter-Output reist via `companion-say` an alle Mitspieler, sie spielen ihn via SpeechSynthesis ab; der Begleiter bekommt eine wählbare, mitreisende Stimme. Der letzte offene Roadmap-Punkt vor dem KI-Übersetzer. Davor V8.62-V8.66 W7 Compute-Sharing KOMPLETT (Phase 1-4) — der WebRTC-Mesh + alle benannten Multi-User-Lücken. P1 (V8.62): echte WebRTC-DataChannels (signaling-server wird Rendezvous, pos/dsl/soul/aura/vibe peer-to-peer, Mesh-Komplett-Wand). P2 (V8.63): der Welt-Snapshot reist mesh-nativ in 16-KiB-Stücken (`world-pull`/`world-chunk`), Guest-Resync per Knopf. V8.64: Multi-User-Bau-Sync (`confirmBuild`/`harvestArchitecture` broadcasten via geteilter `archId` + `remove_architecture`-Op). P3 (V8.65): LLM-Pool — ein Peer teilt seine „Stimme" (Opt-in + Rate-Limit + dslRun-Sandbox), ein schlüsselloser Mitspieler routet Chat über ihn. P4 (V8.66): Public-Lobby (`lobby-publish`/`lobby-list`, Räume browsbar). Kreatur-Sicht-Sync (V8.66): jeder Peer streamt SEINE Kreaturen, Mitspieler rendern sie als Sicht-Schicht. Davor: W14 (Bibliothek) Phase 1-3 komplett (V8.58-V8.61), W13 Vibe-Pass komplett (V8.54-V8.56), W12 Welt-Portal komplett (V8.51-V8.53). ~2723 Invarianten grün (der Konsolen-Zähler driftet ±2-3 je Lauf — einige Checks sind bedingt), Audit-Strict 0 Failures. Offen: W17 Phase B-JS-Compute Phase 2 (Host-Migration). Detail in `CLAUDE.md`.)
...
**Summe verbleibend**: die grossen Roadmap-Ringe sind gebaut. **Stand nach V8.79**: Atmosphäre ✅ → Multi-User-Tiefe ✅ → Hylomorphismus-Vollausbau ✅ (V8.35) → Welt-Portal-Protokoll ✅ (W12) → souveräne Identität ✅ (W13) → Bibliothek ✅ (W14 Phase 1-3) → Compute-Sharing ✅ (W7 Phase 1-4) → Voice-Sync ✅ (W11 V4) → KI-Übersetzer ✅ (V8.68/V8.69) → Untrusted-Welt-Tor ✅ (V8.70) → Auto-Vendor-Pfad ✅ (W15 — V8.71/V8.72) → Mesh-Welt-Verteilung ✅ (W16 — V8.73/V8.74) → **Multiplayer-Sub-Welten ✅ (W17 — Phase A V8.75, B-Relay V8.76, C V8.77, Multiplayer-Welt-Deklaration V8.78, B-JS-Compute Phase 1 V8.79)**. **Offen im Fremd-Engine-Bogen**: nur W17 Phase B-JS-Compute Phase 2 (Host-Migration, wenn der Compute-Host das Mesh verlässt — über W7s `worldRole`-Mechanik) + B-WASM (per-Projekt).
...
**Stand**: das Untrusted-Welt-Tor (V8.70) ist der Schlüsselstein — eine echte, ungeprüfte fremde Engine läuft null-origin sandgesichert hinter dem Portal (`worlds/schwarm/` beweist es). **W15 — der Auto-Vendor-Pfad — ist gebaut (V8.71 Bündel-Pfad, V8.72 GitHub-Fetch)**: ein fremdes Repo dockt ohne Handarbeit an. **W16 — die Mesh-Welt-Verteilung — ist komplett (V8.73 Welt-Bündel-Transport, V8.74 der browsbare Welt-Katalog)**: eine vendorte Welt reist peer-to-peer zu einem Mitspieler, und die Mesh-Bibliothek ist browsbar (Peers annoncieren ihre Welten über den `soul`-Kanal + ein Content-Hash). Was bleibt: das Tor **mehrspielerfähig** machen (W17 — eine Gruppe taucht gemeinsam in eine fremde Welt, auch in eine, die einen Server braucht). Niemand soll die Bibliothek verlassen müssen, um gemeinsam ein Buch zu lesen.
...
| 3 — Mesh-als-Server | AnazhRealms W7-Mesh trägt den Verkehr; ein Peer hostet die Server-Logik | ✅ V8.76 Relay-Welten + V8.79 JS-Compute Phase 1 (ein Peer wird Compute-Host; Host-Migration = Phase 2) |
...
**Phasen-Reihenfolge:** A (Shim) ✅ → B-Relay (das Einfachste, voll p2p) ✅ → C (Gruppen-Portal) ✅ → B-JS-Compute Phase 1 (der Compute-Host) ✅ V8.79 → B-JS-Compute Phase 2 (Host-Migration) → B-WASM (per-Projekt). Jede Phase eine eigene, browser-verifizierte Welle.
...
**Phase B-JS-Compute Phase 2 (~1–2 Sessions — die NÄCHSTE WELLE) — Host-Migration.** Phase 1's ehrliche Grenze: verlässt der Compute-Host das Mesh, endet die Sub-Welt. Phase 2 schliesst sie — ein deterministischer Nachfolger (die kleinste peerId der Gruppe, W7s `worldRole`-Host/Guest-Mechanik steht schon) übernimmt, startet einen neuen Server-Kontext; der Server-Zustand geht dabei verloren (eine ehrliche Grenze). Eine kleine Naht nebenher: `serverMode` durch die ganze Vendor-/Mesh-Kette tragen, damit eine VENDORTE js-compute-Welt sich selbst deklariert (wie V8.78 für `multiplayer` — Phase 1 schnitt das bewusst aus). Sicherheitslastig (fremde Server-JS in einem Peer-Kontext).
...
- Stand V8.79: **~2723 Invarianten** in `scripts/playtest.cjs` (Headless-Chromium, ~25 s Log-Sammlung) + `audit-strict.cjs` (5 generische Audit-Schichten) + `smoke-multiuser.cjs` + `smoke-webrtc.cjs` + `smoke-translated.cjs` + `smoke-sandbox.cjs` + `smoke-vendor.cjs` + `smoke-shim.cjs`
