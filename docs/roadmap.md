...
Stand: V9.52-b (23.05.2026). Diese Doc trägt den **Plan vorwärts** — die Ring-Übersicht 0-17, was offen ist, die Meilenstein- und Risiko-Sicht. Die anderen Zeit-Ebenen: der aktuelle Stand + die Gotchas in `CLAUDE.md`, die volle Wellen-Chronik in `docs/handover.md`, die Vision in `docs/state-of-realm.md`.
...
- **Stamm-Hygiene über den Code hinaus** — seit V9.44 sind Tests + Doku Teil des Stamms und brauchen dieselbe Hygiene-Disziplin: `playtest.cjs` (war 31 574 Z., EINE Funktion — die zweitgrößte Reibung des Projekts; jetzt 31 304 Z. mit 15 benannten Funktionen) — der **Playtest-Pflege-Bogen** läuft: Sub-Welle **a + b sind drin** (V9.52-a/b: `safeEvaluate`/`ctx`-Gerüst + 4 erste Funktionen + 11 thematische Band-Funktionen für Band 1, je 265-1260 Z. plan-treu nach §5), Sub-Wellen c-f folgen. Voller Schnitt-Plan in `docs/playtest-hygiene.md` (§3 unten). Die Doku-Schicht braucht periodisches Archivieren fertiger Konzept-Dokumente (Konsolidierung 21.05.2026 + erneut diese Session) — sonst wächst das „Projektgedächtnis" zur Halde, eine Schicht über der Heiligen Lektion.
...
### Der Playtest-Pflege-Bogen — Test-Hygiene (gestartet, 23.05.2026 — Sub-Welle a + b sind drin)
...
**Status (Stand 23.05.2026):** Sub-Wellen **a + b sind drin** (V9.52-a: `safeEvaluate`-Helfer + `ctx` + die ersten drei Sektionen `gatherInitialFinalState`/`checkInitialState`/`checkRing1Grok`/`checkRing2Dsl`; V9.52-b: Band 1 als 11 thematische Band-Funktionen `checkBandRing2Extended` .. `checkBandLateMultiUser`, je 265-1260 Z., mechanisches Skript `/tmp/extract-band1-grouped.cjs` als reproduzierbarer Schnitt — sechs Baseline-Läufe bit-identisch). **c-f stehen aus**. Nächste Welle: Sub-Welle c — Band 2 (Welle 6.A + Voxel-Terrain-Phasen + Hydrosphäre, ~55 Sektionen, Zeilen ~7725+).
