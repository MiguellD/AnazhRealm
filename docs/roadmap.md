...
Stand: 21.05.2026 (V9.42-c/d — Voxel-Surface-Politur Welle C: Inseln teilen die Surface-Nets-Pipeline + Naht-pad-Smooth). Die Heightfield-Pipeline ist mit V9.39 vollständig zurückgebaut (Phase 5c.2.c.3 abgeschlossen — die Voxel-Welt trägt alles allein). Der Schöpfer-Audit nach V9.39 (fünf Browser-Befunde) ist mit der Voxel-Surface-Politur V9.40–V9.42 zu vier Fünfteln geheilt: V9.40 a–f (Chunk-Löcher beim Carve/Fill + Maus-Edit-P2P-Sync — Wurzel: `ammo.wasm` auf 256 MB growable gepatcht), V9.41 + V9.41-b (Trapeze/Linien — Schach-Brett-Diagonalen + Laplacian-Smooth), V9.42 a–d (Inseln teilen die Surface-Nets-Pipeline + MeshToon-Material + Naht-pad-Smooth). **Offen: V9.43** — das Wasser-Ultiversum (Wasserfälle/Strömung/Bäche als geteilter Plane-Shader). Der Fremd-Engine-Bogen (W12–W17) ist komplett; offen darin nur B-WASM (per-Projekt). Wellen-Plan + Audit-Lehren in §3 „Voxel-Surface-Politur"; die kanonische Versions-Historie lebt in `CLAUDE.md`.
