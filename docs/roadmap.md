...
Stand: V9.86 (25.05.2026, **Welle Perf-2 ext.a fertig + PR ready-to-merge + Welle Perf-3 als epochaler Bogen vollständig geplant**). PR `claude/epic-babbage-vRHsE` trägt V9.83→V9.86 (CI-Flake-Heilung + Welle Perf-1+2+ext.a), CI ist 4× ✅ grün — bereit zum Merge. **Welle Perf-3 als nächste Welle**: Schöpfer-Auftrag nach V9.86-Browser-Audit „die Bibliothek von Alexandria der Neuzeit muss selbst die AAA-Titel alt aussehen lassen". Vier Sub-Wellen geplant (Master-Plan in `docs/performance.md` §4): **3.a** Atlas-Strict-Wasser-Gate (Minecraft-Pattern), **3.b** Distance-LOD (BotW/Genshin), **3.d** Frustum-priorisiertes Build, **3.c** Worker-Migration (Subnautica/NMS/Astroneer/Vintage-Story). Welle Perf-3 zieht die WURZEL der Frame-Drops (synchroner Main-Thread-Build) strukturell weg. Davor — V9.86 in einem Commit: (1) `_remeshVoxelChunksAround` bekommt `skirt`-Parameter — Architektur-Spawn ruft mit `skirt=0`, ~89 % weniger dirty-Chunks → FPS-Drop beim Bauen drastisch reduziert; (2) Mountain-Mulden-Filter via Atmosphären-Konnektivität (ersetzt V9.85-SOLID-darüber-Pattern) — bewahrt Carve-Vision, heilt Bergwand-Wasser, C.3-Flake auf ~25 %; (3) Klassen-Konstante `AnazhRealm.VERSION` — log() + Init lesen sie, kein hardcoded „V7.82" mehr. Davor: Welle-C-Bogen (V9.71–V9.82) vollendet; V9.83 heilt den CI-Flake; V9.84 baut Welle Perf-1 (sechs Allokations-/LOD-Heilungen); V9.85 baut Welle Perf-2 Kern in vier Sub-Wellen (Frame-Time-Budget + Stable Shadow Maps + Mountain-Mulden-Filter + Gradient-preDensity). Performance-Bogen blockiert die System-Kopplungs-Wellen D/E/F/G — die Vision-Wellen kommen NACH dem Fundament-Schliff, weil sie alle Per-Frame-Arbeit hinzufügen würden. Diese Doc trägt den **Plan vorwärts** — die Ring-Übersicht 0-17, was offen ist, die Meilenstein- und Risiko-Sicht. Die anderen Zeit-Ebenen: der aktuelle Stand + die Gotchas in `CLAUDE.md`, die volle Wellen-Chronik in `docs/handover.md`, die Vision in `docs/state-of-realm.md`.
...
| **Perf-3** | **Performance-Welle 3 — die epochale Welle: Raumzeit-Performance** (V9.87+, vier Sub-Wellen, "AAA-Titel alt aussehen lassen" — Schöpfer-Auftrag nach V9.86-Audit) | Perf-2 ext.a ✅ + Browser-Audit V9.86 | **3.a** Atlas-Strict-Wasser-Gate (V9.87, ~2h — Wasser-Cells nur wo Atlas Lake/River/Ozean markiert, keine Macro-Dipp-Heuristik mehr; heilt „Wasser an Bergwänden + Strukturen" strukturell — Minecraft-Pattern) · **3.b** Distance-LOD für ferne Chunks (V9.88, ~4h — Step 1.8m→3.6m bei >80m = 8× weniger Density-Samples; emergent über `worldFieldAt`-Affinity statt Hard-Threshold; BotW/Genshin-Pattern) · **3.d** Frustum-priorisiertes Build (V9.88-Begleiter, ~2h — sichtbare Chunks zuerst, ferne im Idle-Tick; Synergie mit Perf-2.a-Frame-Budget) · **3.c** Worker-Migration für Density+Iso (V9.89+, ~10h — der STRUKTURELLE Wurf; `voxel-worker.js` baut Geometrie zero-copy-Transfer; BVH-Collision bleibt Main wegen WASM-Heap; Lazy-BVH für ferne Chunks; Subnautica/NMS/Astroneer/Vintage-Story-Pattern; Determinismus-Test als Worker-Kern-Invariante) | Browser-Audit: Spieler rennt + baut + Nexus läuft — kein sichtbarer FPS-Drop. Per-Chunk-Main-Thread-Spike ≤ 16ms (vorher 100+ ms). Wasser strukturell nur wo Atlas es benennt. Welt-Tiefe ungebrochen (kein Pop-In). Plus DSL-Debug-Ops `debug_lod_distance`, `debug_worker_stats`, `debug_atlas_strict` als Spieler-Diagnose-Werkzeuge. Master-Plan in `docs/performance.md` §4. |
