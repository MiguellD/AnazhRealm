...
Stand: V9.56-h (23.05.2026). Diese Doc trägt den **Plan vorwärts** — die Ring-Übersicht 0-17, was offen ist, die Meilenstein- und Risiko-Sicht. Die anderen Zeit-Ebenen: der aktuelle Stand + die Gotchas in `CLAUDE.md`, die volle Wellen-Chronik in `docs/handover.md`, die Vision in `docs/state-of-realm.md`.
...
- **Ein zweiter Code-Hygiene-Bogen — UI-Etappe GESCHLOSSEN, Kontrollfluss-Etappe LÄUFT (V9.56-a..h, 23.05.2026)**. UI-Etappe: vier Sub-Wellen a/b/c/d. Kontrollfluss-Etappe: e `fuseWorlds` 239 → 114 + 8 Helfer (PURE Sequenz), f `_tickCreatureTaskDirection` 267 → 8 + 3 Helfer (Task-Dispatcher), g `_voxelChunkGeometry` 269 → 25 + 6 Helfer (Surface-Nets-Pipeline, einseitig gekoppelt), h `processChatCommand` 285 → 21 + 9 Helfer (5-Stufen-Pipeline + Themen-Dispatcher mit Try-Pattern-Konvention). Alle acht Sub-Wellen verhaltensneutral, playtest grün vor + nach. **Bleibt im Bogen — drei Kontrollfluss-Funktionen** in Größen-Reihenfolge: `_applyDayNightToScene` 305 → V9.56-i, `generateTerrainWithParameters` 346, `loadState` 380. **V9.56-e/f/g/h-Lehren**: (1) Block-Grenzen-Inspektion ist eine HIERARCHIE; (2) Side-Effect-Stream bit-identisch halten; (3) Pipeline-Kopplung kann einseitig sein → saubere Helfer-Sequenz; (4) Try-Pattern-Konvention für Multi-Branch-Dispatcher; (5) Cleanup-Asymmetrie preservieren (auch wenn sie wie ein Bug aussieht — REFACTOR ≠ BEHAVIOR-CHANGE). **Disziplin (V9.44-Wiederholung)**: eine Sub-Welle = ein Commit, playtest-grün, verhaltensneutral, kein Bonus-Refactoring im selben Schnitt. **Disziplin (V9.40-Lehre)**: der Bogen gehört terminiert, sobald die nächste Feature-Welle eine dieser Funktionen berührt, nicht „irgendwann".
