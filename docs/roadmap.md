...
**Summe verbleibend**: die grossen Roadmap-Ringe sind gebaut. **Stand nach V8.78**: Atmosphäre ✅ → Multi-User-Tiefe ✅ → Hylomorphismus-Vollausbau ✅ (V8.35) → Welt-Portal-Protokoll ✅ (W12) → souveräne Identität ✅ (W13) → Bibliothek ✅ (W14 Phase 1-3) → Compute-Sharing ✅ (W7 Phase 1-4) → Voice-Sync ✅ (W11 V4) → KI-Übersetzer ✅ (V8.68/V8.69) → Untrusted-Welt-Tor ✅ (V8.70) → Auto-Vendor-Pfad ✅ (W15 — V8.71/V8.72) → Mesh-Welt-Verteilung ✅ (W16 — V8.73/V8.74) → **Multiplayer-Sub-Welten ✅ (W17 — Phase A V8.75, B-Relay V8.76, C V8.77, Multiplayer-Welt-Deklaration V8.78)**. **Offen im Fremd-Engine-Bogen**: nur W17 Phase B-JS-Compute (ein Peer wird Compute-Host für eine Welt mit autoritativer Server-Logik — Host-Migration über W7s `worldRole`-Mechanik).
...
| 3 — Mesh-als-Server | AnazhRealms W7-Mesh trägt den Verkehr; ein Peer hostet die Server-Logik | ✅ V8.76 (Relay-Welten; JS-Compute offen) |
...
**Phase B-JS-Compute (~1–2 Sessions, der harte Teil — die NÄCHSTE WELLE).** Für Welten, deren Server JS-Logik ist (nicht blosser Relay): ein Peer der Gruppe wird Compute-Host (W7s `worldRole`-Host/Guest-Mechanik steht schon), die Server-JS läuft in seinem (sandgesicherten) Tab, das Mesh trägt den Verkehr. Host-Migration, wenn der Host geht. Eigener Bogen, sicherheitslastig (fremde Server-JS in einem Peer-Kontext). Vorarbeit erledigt (V8.78): eine vendorte Welt deklariert sich selbst `multiplayer` — eine B-JS-Compute-Welt würde analog ihren Server-Modus deklarieren.
...
- Stand V8.78: **~2704 Invarianten** in `scripts/playtest.cjs` (Headless-Chromium, ~25 s Log-Sammlung) + `audit-strict.cjs` (5 generische Audit-Schichten) + `smoke-multiuser.cjs` + `smoke-webrtc.cjs` + `smoke-translated.cjs` + `smoke-sandbox.cjs` + `smoke-vendor.cjs` + `smoke-shim.cjs`
