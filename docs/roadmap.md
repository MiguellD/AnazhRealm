...
Stand: V9.56-e (23.05.2026). Diese Doc trägt den **Plan vorwärts** — die Ring-Übersicht 0-17, was offen ist, die Meilenstein- und Risiko-Sicht. Die anderen Zeit-Ebenen: der aktuelle Stand + die Gotchas in `CLAUDE.md`, die volle Wellen-Chronik in `docs/handover.md`, die Vision in `docs/state-of-realm.md`.
...
- **Ein zweiter Code-Hygiene-Bogen — UI-Etappe GESCHLOSSEN, Kontrollfluss-Etappe ERÖFFNET (V9.56-a..e, 23.05.2026)**. UI-Etappe: vier Sub-Wellen a/b/c/d (1005 Z. → 56 Z. Orchestrator-Summe, 21 neue Helfer). Kontrollfluss-Etappe: e `fuseWorlds` 239 → 114 Z. + 8 Helfer (median ~16, max 22) — eine PURE Sequenz, das Snap-Object bleibt INLINE im Orchestrator (Symmetrie-Versuchung widerstanden). Alle fünf Sub-Wellen verhaltensneutral, playtest grün vor + nach. **Bleibt im Bogen — sechs Kontrollfluss-Funktionen** in Größen-Reihenfolge: `_tickCreatureTaskDirection` 267 → V9.56-f, `_voxelChunkGeometry` 269, `processChatCommand` 285, `_applyDayNightToScene` 305, `generateTerrainWithParameters` 346, `loadState` 380. **V9.56-e-Lehre**: Kontrollfluss ist NICHT pauschal riskanter als UI — er ist riskant in SUB-Fällen mit shared State über Block-Grenzen oder Side-Effect-Reihenfolge (Math.random/Date.now/log). Vor jedem Schnitt die Block-Grenzen + Side-Effect-Stream-Reihenfolge inspizieren. **Disziplin (V9.44-Wiederholung)**: eine Sub-Welle = ein Commit, playtest-grün, verhaltensneutral, kein Bonus-Refactoring im selben Schnitt. **Disziplin (V9.40-Lehre)**: der Bogen gehört terminiert, sobald die nächste Feature-Welle eine dieser Funktionen berührt, nicht „irgendwann".
