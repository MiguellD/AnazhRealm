...
Stand: **V13.8 (30.05.2026) — Echtes verbundenes Wasser: 3D-Flood-Fill von Quellen** (Wasser-Profi-Bogen V13.0–.8). Schöpfer-Audit V13.7: Rest-Bluten an Bergwänden + Luftblasen in Unterwasserhöhlen + unsauberes Carve-Fließen — alle drei aus der 2,5D-Klassifikation (per-Spalte + 16-m-Nähe). V13.8 ersetzt sie durch einen 3D-Flood: Wasser füllt NUR Zellen, die durch nicht-soliden Raum unter dem Body-Spiegel mit einer echten Quelle VERBUNDEN sind (`_buildVoxelChunkWaterCells` + Worker-Mirror; Seeds = Atlas-Wasser in-chunk + OOB-Ring; Architektur-Stempel VOR dem Flood → Damm blockt). Heilt Bluten (Wand trennt → nicht erreicht) + Höhlen (lateral verbunden → erreicht) + Carve-Fluss (Re-Flood) in einem. Diag: Wand-Zellen 88→0, Luftblasen 0, tiefe Wände 0 %; Worker-Density bit-identisch → seam-frei. Davor: V12-Genie-Bogen (r184 + WebGPU, 119 FPS) + V10.0-TSL-Bogen. Volle Wellen-Chronik in `docs/handover.md`.

Stand-Update: **V13.9 ✅ gebaut** — Schicht-3-Cull `uMinDepth` (default 0) für das dünne Wand-Bluten (Schöpfer-Audit-V13.8-Befund 1). Im Fragment-Shader: `alphaCulled = cond(waterThick < uMinDepth, 0, alpha)` + `mat.alphaTest = 0.0001`; Uniform in `state.hydroSurfaceUniforms.minDepth`, browser-live justierbar. Headless: 3 neue Invarianten (Uniform/default-0/Source-Probe); der wirksame Schwellwert braucht den Schöpfer-Browser-Sign-off (pixel-blind-Lehre).

**Nächste Welle: V13.10 — V13.8-Rest-Befunde 2+3.** (2) tiefes Wasser an Strukturen (Turm/Tor) diagnostizieren (vom `uMinDepth`-Cull NICHT erfasst — großes `waterThick`); (3) Naht nach Carve (Edit-Rebuild-Iso-Naht). **Erkenntnis (V13.7–.9): Wasser-Ufer ist ein 16-m-Atlas-vs-1,8-m-Voxel-Tradeoff (bleed-frei↔blockig); die Glätte gehört in den Tiefenpuffer-Shader, NICHT in eine 3D-Fluid-Sim (Heilige Lektion); Render NUR im Browser-Loop justieren (Headless ist pixel-blind).** Danach **V13.11** Sub-Region-Edit-Remesh, **V13.12** LOD-Cross-Fade, Schöpfer-Browser-Audit; dann **V14 — die lebendige Welt** (Vision-Pfeiler E/F/G). Der volle V13-Plan steht in **§1.4**.
...
| **V13 Wasser-Profi-Bogen** ⏳ (V13.0–.8 ✅) | **Voxel-Wasser EFFIZIENT + KORREKT + in SYNERGIE + 3D-verbunden** (Schöpfer-Browser-Befunde, im Code verifiziert/gemessen). Wasser bleibt voxel/zell-basiert (reaktiv) + teilt den Terrain-Mesher (Surface-Nets-Iso → fließt) + ist 3D-verbunden (Flood von Quellen). Detail §1.4. | V12 ✅ | **V13.0–.5 ✅** Diagnose · Klassifikation · Grenzflächen-Meshing · Flow-Bias · Glättung(zurück) · Schicht-3-Tiefen-Shader · **V13.6 ✅** Synergie (Surface-Nets-Iso wie das Terrain) · **V13.7 ✅** Ufer-Wände weg (`terrainTopY<rim`) · **V13.8 ✅** 3D-Flood-Fill von Quellen (Höhlen-Luftblasen weg, Damm blockt, Carve flutet; Wand-Zellen 88→0) — Rest-Wand-Bluten DÜNN (16-m-vs-Voxel-Spill) · **V13.9** Wasser-Finish im Browser-Loop (dünnes Bluten im Shader cullen + Struktur-Tiefwasser + Carve-Naht) · **V13.10** Sub-Region-Edit · **V13.11** LOD · **V13.12** Browser-Audit | Wasser wie die Riesen: kohärent, fließend, 3D-verbunden, reaktiv (Damm staut, Carve flutet); Ufer-Glätte via Shader. |
...
| **V13.9** ⏳ **next** | **Wasser-Finish im Browser-Loop (Schöpfer-Wahl A).** (a) dünnes Wand-Bluten pro Pixel im Tiefenpuffer-Shader cullen (`waterThick < uMinDepth`, tunbar, default 0 = altes Bild; tiefes Wasser bleibt) — der Schöpfer-Befund „das Wand-Wasser ist nicht tief" macht das möglich; (b) tiefes Wasser an Strukturen (Turm/Tor) diagnostizieren; (c) Naht nach Carve. NUR mit dem Schöpfer im Browser justiert (Headless ist pixel-blind). | dünnes Wand-Bluten, Struktur-Tiefwasser, Carve-Naht |
| **V13.10** | Sub-Region-Edit-Remesh: nur die vom Carve/Fill berührten Zellen + ihre Mesh-Sektion neu, nicht der ganze Chunk (macht den Carve-Fluss frame-sauber). | Edit-Drops, „ganzer Chunk neu" |
| **V13.11** | LOD-Cross-Fade: sanftes Überblenden zwischen Detailstufen statt hartem Pop. | „sauberer werden"-Pop |
| **V13.12** | Schöpfer-Browser-Audit RDNA-3: Wasser kohärent + fließend, kein Bluten, smooth beim Streamen + Editieren. | Bogen-Schluss |
| **V13.x (Backlog)** | (a) **Dynamisches Equilibrium statt statischem Fill** (falls Browser-Audit ein Über-Füllen langer Abfluss-Kanäle zeigt): der Flood füllt verbundenen Raum auf den QUELL-Spiegel, nicht zum lokalen Equilibrium. (b) **Globaler statt per-Chunk-Flood** (falls exotische Schleifen-Topologie seamt). (c) **Volle Flow-Isotropie via MFD/D∞** (V13.3-Rest-Achsen-Präferenz 33/67). | Kanal-Über-Füllung, Schleifen-Seam, Achsen-Grid-Grain |
