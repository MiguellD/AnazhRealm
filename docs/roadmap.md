...
- **Hydrosphäre-Topologie — V9.60-b 🔴 nächste Welle (substantielle Wurzel-Welle, nicht angefangen).** Heilt zwei Schöpfer-Befunde gleichzeitig: (a) „viele Biome unter Wasser" (46% Ozean per `waterLevel=35-Perzentil`); (b) „Sand-Streifen zu homogen, keine Buchten/karge Stellen" (uniforme Uferlinie weil Topologie uniform). Plan: Multi-Plateau-Tektonik-Bias asymmetrisch nach oben (mehr Land OHNE waterLevel zu ändern) + See-Höhen-Differenzierung + längere Flüsse + See-Fluss-Naht. Vorbedingung: Diagnose-Pass-Erweiterung (Fluss-Längen-Verteilung, See-Höhen-Histogramm-Breite, Uferlinien-Variations-Index). Sub-Wellen-Disziplin pro Knoten (V9.58-Lehre), gekoppelte Tuning-Schritte (V9.60-Lehre).
