...
| **Vision-Reset** ⏳ | **Die wahre Bibliothek von Alexandria** (V9.94-V9.99, der epochale Wurf — was V9.86-Brainstorm visionierte, aber V9.87-V9.93 als Catch-up untergrub) | V9.93 ✅ + Schöpfer-Konfrontation 26.05.2026 | ✅ **V9.94** WebGPU-Compute-Diagnose (Tool gebaut) · ⚠️ **V9.94.r** Bürokraten-Reflexion (Diagnose war Stufe-3 für Stufe-1-Frage; Lehre verdrahtet) · ✅ **V9.95-a** WebGPU-Foundation (Adapter/Device/Trivial-Shader/Determinismus-Beweis, Phase-1 V9.89-Pattern) · ⏳ **V9.95-b** WGSL-SimplexNoise + Density-Shader + Pipeline-Cutover (~4-6h, der eigentliche 50-100× speedup) · **V9.96** IndexedDB-Persistent-Chunk-Cache (~4-5h, VISION-PFEILER „Welt-Gedächtnis") · **V9.97** Predictive Prefetch aus Velocity (~3h, Welt atmet entgegen) · **V9.98** Geometry-Stitching für LOD-Boundaries (~6h, V9.93-Workaround durch Profi-Stitching ersetzen) · **V9.99** Per-Column-Atlas-Strict (~2h, V9.92-Audit-Folge — Wasserschatten an Strukturen strukturell weg) | Browser-Audit: Density-Cost pro Chunk <1ms (statt ~50ms), Re-Visit eines Chunks <5ms (statt 30-50ms), Welt fühlt sich vorausschauend an, LOD-Übergänge nahtlos OHNE Workaround, keine Wasserschatten an Strukturen. Master-Plan in `docs/performance.md` §5. |
| **Toon-Polish + LOD-Cross-Fade** ⏳ | **Render-Schicht-Welle** (V10.0+, V9.92-Audit-Befund „Berg-Treppen-Textur in einer Richtung" + „LOD-Pop-In") | Vision-Reset ✅ | Toon-Material-Polish (Treppenstufen-Texturen weicher), Cross-Fade-Alpha über 1-2 Frames beim LOD-Wechsel, ggf. 3-4 LOD-Stufen statt 2 | Render-Schicht-Welle, separate Diagnose. |
