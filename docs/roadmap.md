...
3. **✅ ERLEDIGT (V9.14) — 3D-Graben.** `carveVoxelSphere(x,y,z,r)` schnitzt eine Kugel „Luft" ins Dichte-Feld — der Edit landet in `worldMeta.voxelEdits` (persistiert, FIFO-256), `_terrainDensityAt` zieht dort die Dichte ab, die Voxel-Chunks werden neu gemesht. Der LMB-Grabe-Hieb schnitzt bei aktivem Voxel-Terrain; Chat `voxel carve`. +7 Invarianten. (Das Füllen — Boden AUFschütten — ist Phase 3b.)
