...
| W11 V3 | **Soul-Sync (Multi-User echt körperlich)** — Peer-Avatar wird der echte Soul des Mitspielers (Mensch/Phönix/Drache/Custom), nicht Cone+Sphere. Bauplan-Snapshot via WebSocket bei Soul-Change. Antwort auf Audit-Punkt #1 (C2). | 🔴 offen | 2-3 Sessions | Ring 11.5 |
...
| W12 | **WebGPU+TSL-Migration (ferne Zukunft, nach Performance-Wand)** — Three Shading Language + WebGPU für Compute-Shader. ~3-5× schneller bei komplexen Effekten. Voraussetzung: WebGL-Fallback bleibt für Safari/Firefox+Low-End. Erlaubt 6.G6 (Höhlen via Marching Cubes), 6.G7 (Echtes Wasser-Mesh), three-fluid-fx als TSL-Variante. | 🔴 offen (aus Projekt-Reflexion 17.05.2026: line-spider + three-fluid-fx TSL-Pfad) | 4-6 Sessions | W7 oder bei Performance-Wand |

**Summe verbleibend**: ~22-32 Arbeitstage in fokussierten Sessions. **Empfohlene Sequenz nach Audit 17.05.2026**: W6.X → W6.G3 → W11 V3 → W10 ext. → W4 V2 → W11 ext. Substanz-Rolle → W11 V4 Voice-Sync → W7 → W12 (WebGPU). Logik: Polish (Reibungs-Freiheit) → Atmosphäre (Welt lebt eigen) → Multi-User-Tiefe (Peer ist echt) → kleine emergente Erweiterungen → Sinn (Musik) → großes Vision-Wort → Multi-User-Klang → Skalierung → moderne Renderpipeline.
