...
## 1. Wo wir stehen (Mai 2026, V8.93)
...
4. **✅ ERLEDIGT (V9.18 + Politur V9.19) — Höhlen + Überhänge in der Generierung.** `_terrainDensityAt` trägt die zwei V9.17-Roughness-Bänder (fein `*0.05×7` + grob `*0.018×5` — Crags, Überhänge) + Wurm-Höhlen aus EINEM ridged-Noise-Feld (`ridge = 1−|noise3D|`, der Grat folgt der Noise-Nullfläche → eine zusammenhängende, begehbare Kavern-Ebene). Eine Tiefen-Hüllkurve (`caveEnv = caveFloor × caveCeil`) hält die Höhlen zwischen `surf-6` und `base-28` — der Chunk-Boden `base-35` bleibt fest, die V9.12-Garantie hält. V9.19 heilte zwei Schöpfer-Browser-Befunde: V9.18 hatte das feine Roughness-Band versehentlich entfernt (Berge wurden flache Hügel) + das Produkt zweier ridged-Felder zerfiel in viele kleine unzugängliche Spalten — beide an der Wurzel geheilt. +3 Invarianten.
...
- Stand V8.80: **~2737 Invarianten** in `scripts/playtest.cjs` (Headless-Chromium, ~25 s Log-Sammlung) + `audit-strict.cjs` (5 generische Audit-Schichten) + `smoke-multiuser.cjs` + `smoke-webrtc.cjs` + `smoke-translated.cjs` + `smoke-sandbox.cjs` + `smoke-vendor.cjs` + `smoke-shim.cjs`
