...
Stand: V9.84 (25.05.2026, **Welle Perf-1 GEBAUT — die billigen Twists in sechs Sub-Wellen a-f**). Welle-C-Bogen (V9.71–V9.82) vollendet; V9.83 heilt den CI-Flake (rAF-Drosselung im Headless); V9.84 baut die Performance-Welle Perf-1 — ~80 % des Gewinns ohne Welt-Tiefe-Verlust. Sub-Wellen: a Shadow-PCF + Material-Singleton, b `isInFrustum`-Pool, c Kreatur-Color-Pool, d Task-Direktionen-Scratch, e Visual-Updates gated auf `inFrustum` (Distance-LOD), f Spatial-Hash für Flocking. Erwartete Wirkung: ~600 weniger Heap-Allokationen/Sekunde (GC-Spikes weg), Mid-Range-GPU ~10–20 % entlastet, Flocking-Loop 10–20× schneller bei verteilten Crowds. Browser-Audit pendent. **Welle Perf-2** (V9.85 — Chunk-Build-Hebel) folgt falls Browser-Test zeigt: Streaming-Spikes bleiben spürbar. **Welle Perf-3** (V9.86+ — Lazy BVH-Collision, Worker-Migration) optional, je nach Audit. Voller Plan in `docs/performance.md`. Performance-Welle blockiert die System-Kopplungs-Wellen D/E/F/G — die Vision-Wellen kommen NACH dem Fundament-Schliff, weil sie alle Per-Frame-Arbeit hinzufügen würden. Diese Doc trägt den **Plan vorwärts** — die Ring-Übersicht 0-17, was offen ist, die Meilenstein- und Risiko-Sicht. Die anderen Zeit-Ebenen: der aktuelle Stand + die Gotchas in `CLAUDE.md`, die volle Wellen-Chronik in `docs/handover.md`, die Vision in `docs/state-of-realm.md`.
