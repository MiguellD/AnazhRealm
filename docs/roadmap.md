...
10. ✅ **6.C1 Hylomorphismus-Inventar** V7.77 — 27-Slot-Overlay mit Tab-Toggle, Tag-Resonanz emergiert aus Compound-Tags (resoniert summt + brennend glüht + magieleitung schimmert + lebendig sprießt + dichte schattet), Audio-Hover-Ping mit Tag-spezifischen Frequenzen, Inventar→Hotbar via Click-State (selectInventorySlot → tryAssignFromInventoryToHotbar), add_to_inventory DSL-Op in NON_BROADCASTABLE_OPS, state.player.inventory persistiert. 34 neue Invarianten → 1126 total.
11. **6.A-Maus + 6.C3** ← jetzt offen (LMB/RMB-Aktionen + Keybindings-UI)
