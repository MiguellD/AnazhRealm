...
Stand: 20.05.2026 (V9.30, Voxel-Terrain-Bogen Phase 5c.2.a — der erste ehrliche Schritt der Heightfield-Code-Entfernung. V9.30 löscht `generateChunk` (~97 Zeilen) + `addTerrainPhysics` (~104 Zeilen) — der V9.28-Eintrag hatte sie ausdrücklich als „nachweislich tote Methoden ohne Aufrufer" benannt; das Versprechen aus dem Kommentar bei Zeile 13549 ist erfüllt. ~200 Zeilen Heightfield-Altcode weg, NULL Verhaltens-Änderung — die lebende Pipeline (`ensureChunkAt`, `_terrainHeightAtWorld`, `_setHeightfieldDormant`) bleibt unangetastet. Phase 1-5b komplett, Phase 5c-Start (V9.26) + 5c.1 (V9.27) + 5c.1+ (V9.28) + Ammo-Leck-Fix (V9.29) + 5c.2.a Dead-Code (V9.30) live; in 5c.2 noch offen: 5c.2.b (Eingangs-Welt-Flip auf Voxel + Test-Rewrite der ~25 heightfield-spezifischen Invarianten), 5c.2.c (das schlafende heightfield/heightData/chunkMap-Skelett ablösen). ~2945 Invarianten grün, Audit-Strict 0 Failures. Der Fremd-Engine-Bogen (W12-W17) ist komplett; offen darin nur B-WASM (per-Projekt). Volle Wellen-Historie in `CLAUDE.md`.)
