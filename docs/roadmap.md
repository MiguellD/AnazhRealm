...
~~DAS FERN-WASSER (die Wasser-LOD-Kaskade)~~ ✓ **GEBAUT V18.381** (`_ensureFarWaterSheet` — EIN Atlas-Sheet jenseits des Chunk-Rings bis fog.far+60, geteiltes Hydro-Material, Ring-Loch chebyshev-exakt, Mantel als Bett; bewiesen `gate:fern-wasser` 100 % atlas-treu/Coverage + A/B-Render). OFFEN daraus (Schöpfer-Auge): der Handoff-LOOK an der Ring-Kante + die Fern-Kräusel-Stärke; outR-Deckel 720 m (fogMult >4,3 zeigt jenseits davon wieder Mantel-Kulisse — bewusster Scope, Zone B [43,2-m-Step bis 1400 m] als Erweiterung benannt).

**DER OFF-THREAD-BAU-BOGEN — DER BÄCKER (FREEZE-FIX GESCHLOSSEN V18.314–.316; NACHTRAG V18.456: die Skin-Klasse selbst FIEL mit KONVERGENZ III — Mensch+Tier sind Studio-BÄUME [bauMensch/bauTier, keine Isosurface, kein ~4-s-Bau], bake-core/bake-worker/spec-v2 sind physisch raus; der Bogen bleibt als Muster-Archiv für den Worldgen-Kandidaten unten):**
