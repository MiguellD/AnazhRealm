...
**Summe verbleibend**: die grossen Roadmap-Ringe sind gebaut. **Stand nach V8.73**: Atmosphäre ✅ → Multi-User-Tiefe ✅ → Hylomorphismus-Vollausbau ✅ (V8.35) → Welt-Portal-Protokoll ✅ (W12) → souveräne Identität ✅ (W13) → Bibliothek ✅ (W14 Phase 1-3) → Compute-Sharing ✅ (W7 Phase 1-4) → Voice-Sync ✅ (W11 V4) → KI-Übersetzer ✅ (V8.68/V8.69) → Untrusted-Welt-Tor ✅ (V8.70 — eine echte, ungeprüfte fremde Engine läuft null-origin sandgesichert; `worlds/schwarm/` ist der Beweis) → Auto-Vendor-Pfad ✅ (W15 — V8.71 Bündel-Pfad, V8.72 GitHub-Fetch; ein fremdes Repo dockt ohne Handarbeit an) → Mesh-Welt-Verteilung W16 Phase 1 ✅ (V8.73 — eine vendorte Welt reist peer-to-peer). **Offen — der Rest des Fremd-Engine-Bogens**: W16 Phase 2 (der browsbare Welt-Katalog — Peers annoncieren ihre Bibliothek + ein Content-Hash) + die Multiplayer-Sub-Welten (W17).
...
**Stand**: das Untrusted-Welt-Tor (V8.70) ist der Schlüsselstein — eine echte, ungeprüfte fremde Engine läuft null-origin sandgesichert hinter dem Portal (`worlds/schwarm/` beweist es). **W15 — der Auto-Vendor-Pfad — ist gebaut (V8.71 Bündel-Pfad, V8.72 GitHub-Fetch)**: ein fremdes Repo dockt ohne Handarbeit an. **W16 Phase 1 — der Welt-Bündel-Transport — ist gebaut (V8.73)**: eine vendorte Welt reist peer-to-peer zu einem Mitspieler. Was bleibt: W16 **Phase 2** (den Welt-Katalog browsbar machen — Peers annoncieren ihre Bibliothek) und das Tor **mehrspielerfähig** machen (W17 — eine Gruppe taucht gemeinsam in eine fremde Welt, auch in eine, die einen Server braucht). Niemand soll die Bibliothek verlassen müssen, um gemeinsam ein Buch zu lesen.
...
**Stand:** **⏳ Phase 1 live (V8.73)**. Phase 1 baute den Welt-Bündel-Transport: zwei kanal-exklusive Nachrichten `world-bundle-pull`/`world-bundle-chunk` (Spiegel von W7 P2 `world-pull`/`world-chunk`); `requestWorldBundleFromPeer(worldId, peerId)` holt eine vendorte Welt von genau einem Peer, der Sender liest sein Bündel über die neue save-server-Lese-Seite `GET /api/vendor-bundle` zurück, der Empfänger reassembliert + reicht es an die erprobte `vendorWorldBundle`-Schreib-Seite (ein dritter Eingang nach lokalem Bündel + GitHub). Eine peer-empfangene Welt läuft `trust:"sandboxed"`. Punkt 1 unten (Content-Adressierung) + ein browsbarer Welt-Katalog (Peers annoncieren ihre Bibliothek) sind **Phase 2** — heute muss der Spieler die `worldId` kennen + den Peer wählen. Punkt 2-4 unten sind mit Phase 1 erfüllt.
...
**Akzeptanz:** zwei Spieler im selben Mesh-Raum; einer hat eine Welt vendort, der andere nicht; der zweite holt ihr Bündel peer-to-peer, beide haben sie in der Bibliothek. ✅ erfüllt von Phase 1 (V8.73 — `smoke-webrtc.cjs` beweist es).

**Offene Ränder von W16 Phase 1 (ehrlich benannt — fliessen in Phase 2 ein, kein Blocker):**
- **Der Welt-Katalog fehlt — Phase 2.** Heute muss der Spieler die `worldId` kennen + den Peer aus einem Dropdown wählen. Phase 2: Peers annoncieren ihren `customWorlds`-Katalog über den `soul`-Kanal (wie `worldRole`), der Bibliothek-Drawer wird browsbar. Die schlichte „Welt vom Mitspieler holen"-Sektion (worldId-Feld + Peer-Dropdown) ist **transitional** — Phase 2 ersetzt sie. Folge: der Knopf-Klick-Pfad (`_runMeshWorldGet` ← Button) ist nicht test-durchlaufen, nur die Methode `requestWorldBundleFromPeer` darunter (die voll geprüft ist); ein Wegwerf-Knopf zu testen, den P2 neu baut, wäre verschwendete Mühe.
- **Keine Content-Adressierung — Phase 2.** Ein Bündel hat keinen Hash. Phase 2: ein Content-Hash gibt einer Welt eine peer-unabhängige Identität (zwei Spieler mit demselben Hash haben beweisbar dieselbe Welt) + erlaubt Dedup (habe ich den Hash schon, kein Pull). Heute schützt nur die Annahme-Wand (`pendingBundlePull.worldId`) gegen Welt-Schmuggel + die Sandbox gegen ein böses Bündel — das genügt für P1, der Hash ist die Identitäts-Schicht.
- **Dev-verankert auf BEIDEN Enden.** Der Sender liest sein Bündel über `GET /api/vendor-bundle` (braucht seinen save-server), der Empfänger schreibt über `POST /api/vendor-world` (braucht seinen). Auf githack (kein save-server) degradiert es anmutig (`save_server_unreachable`/`world-bundle-fail`), aber W16 P1 lebt in der Dev-Umgebung — wie W15. Eine save-server-lose Verteilung (ein Service-Worker, der ein In-Memory-Bündel ins Portal-iframe serviert) ist eine eigene spätere Schicht.
- **Nur Text- + `vendored`-Welten.** W15-Erbe: Binär-Assets werden nicht übertragen; eine importierte Manifest-Welt (kein `vendored`) hat keine lokalen Dateien → wird nicht herausgegeben. Eine übersetzte Welt (`worlds/translated/` + `scene`) wäre metadaten-only transferierbar (kein Datei-Bündel) — eine billige Phase-2-Ergänzung.
- **Kein Pull-Timeout.** Verstummt der Sender mitten im Chunk-Strom (Tab geschlossen), bleibt `pendingBundlePull` gesetzt, bis der nächste Pull es überschreibt (der `world-bundle-fail`-Pfad deckt nur „Sender kann nicht liefern", nicht „Sender verschwand"). Mirror von W7 P2 (`pendingWorldSnapshot` hat auch keinen Timeout) — ein weicher Timeout ist eine Phase-2-Politur.
...
- Stand V8.73: **2648 Invarianten** in `scripts/playtest.cjs` (Headless-Chromium, ~25 s Log-Sammlung) + `audit-strict.cjs` (5 generische Audit-Schichten) + `smoke-multiuser.cjs` + `smoke-webrtc.cjs` + `smoke-translated.cjs` + `smoke-sandbox.cjs` + `smoke-vendor.cjs`
