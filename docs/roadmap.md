...
Stand: 17.05.2026 (V8.55, W13 Phase 1+2 LIVE — der Vibe-Pass: souveräne Avatar-Identität. Phase 1 (V8.54): ein ed25519-Schlüsselpaar, lokal beim ersten Erwachen erzeugt (WebCrypto nativ — kein Vendoring, keine CSP-Lockerung), der private Schlüssel GLOBAL in `localStorage["anazh.vibePass"]`, nie im Welt-Save. Phase 2 (V8.55): Bauplan-Signaturen — `signBlueprint`/`verifyBlueprintSignature` signieren die SUBSTANZ eines Bauplans (Rolle + Parts + Verbindungen, nicht den Namen), vier Status-Stufen (unsigned/valid/modified/forged), Werkstatt-Anzeige; die Signatur reist mit dem Bauplan durch Save/Welt-Tor/Fusion. Davor: V8.51-V8.53 W12 Welt-Portal (Phase 1+2+3 komplett). 2362/2362 Invarianten grün, Audit-Strict 0 Failures. Nächste empfohlene Welle: W13 Phase 3 (Signatur-Prüfung in Multi-User / Welt-Tor / Portal-Manifest).)
...
| W13 | **Vibe-Pass (Self-Sovereign Identity)** — Crypto-Keypair (ed25519, lokal generiert). Schöpfer signiert eigene Baupläne/Welten. Avatar-Identifier = Public-Key. Kein Coin/NFT-Hype, nur Authentizität. Welt-Memberships als signierte Records. Vorbedingung für Welt-Portale die Authentizität brauchen. | 🟡 **Phase 1+2 live (V8.54+V8.55)** — Phase 1: Schlüssel-Grundlage (ed25519-Keypair, WebCrypto nativ; Sign/Verify-Primitive; globale localStorage-Persistenz `anazh.vibePass`, nie im Welt-Save; sichtbare + sicherbare Identität). Phase 2: Bauplan-Signaturen — `signBlueprint`/`verifyBlueprintSignature` signieren die Substanz eines Bauplans (nicht den Namen), vier Status-Stufen, Werkstatt-Anzeige, Signatur reist durch Save/Welt-Tor/Fusion. **Offen**: Phase 3 (Signatur-Prüfung in Multi-User / Welt-Tor / Portal-Manifest). | 3-5 Sessions Rest | W12 ✅ |
...
**Summe verbleibend**: ~19-30 Arbeitstage in fokussierten Sessions. **Empfohlene Sequenz nach V8.55 (W13 Phase 1+2 LIVE — der Avatar hat einen Schlüssel, eigene Baupläne tragen Signaturen)**: **W13 Phase 3 (Signatur-Prüfung in Multi-User / Welt-Tor / Portal-Manifest)** → **W14 (Bibliothek)** → W7 (Compute-Sharing als verteilter Server-Layer für die Bibliothek). Optional einschiebbar: W11 V4 (Voice-Sync, ~1 Session) oder eine kleine Affordance-Welle (10b). Logik: Atmosphäre ✅ → Multi-User-Tiefe ✅ (Peer ist sein echter Soul) → Hylomorphismus-Vollausbau ✅ (V8.35 — Rolle emergiert aus der Substanz) → Welt-Portal-Protokoll ✅ (W12) → **souveräne Identität (W13 — Phase 1+2 ✅, der Avatar hat einen Schlüssel und signiert seine Werke)** → die Bibliothek (W14, fremde Welten) → Skalierung als Compute-Sharing-Layer.
