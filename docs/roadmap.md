...
Stand: **V13.7 (30.05.2026) — Verbundenes Wasser: die Ufer-Wände weg, die Mulde füllt bis zum Terrain** (Wasser-Profi-Bogen V13.0–.7). Schöpfer-Audit V13.6: an Ufern „schliesst die Oberfläche vertikal zum Grund". Gemessen (`diag-water-shore.cjs`): 33 % der Wasser-Fläche waren vertikale Wände, weil das V13.1-Rim-Gate (3,6-m-Cap) tief-unterwasser Ufer-Spalten ablehnte. V13.7-Fix (1 Zeile, main + Worker): Gate auf `terrainTopY < rim` → fülle bis zum Body-Spiegel, jede Tiefe (tiefe Wände → 0 %). Nutzt die Atlas-priority-flood-Konnektivität → kein Hang-Schatten (Terrain über Spiegel bleibt trocken), kein Phantom (nur neben echtem Wasser). Bonus: Carve nahe Quelle füllt sich reaktiv (Welle-A-Vision). Davor: V12-Genie-Bogen (r184 + WebGPU, 119 FPS) + V10.0-TSL-Bogen. Volle Wellen-Chronik in `docs/handover.md`.

**Nächste Welle: V13.8 — Sub-Region-Edit-Remesh** (nur die vom Carve/Fill berührte Zell-Sektion + ihre Mesh-Sektion neu, nicht der ganze 24×24×124-Chunk → Edit-Drops weg). Danach **V13.9** LOD-Cross-Fade, **V13.10** Schöpfer-Browser-Audit; dann **V14 — die lebendige Welt** (Vision-Pfeiler E/F/G). Der volle V13-Plan steht in **§1.4**.
...
| **V13 Wasser-Profi-Bogen** ⏳ (V13.0–.7 ✅) | **Voxel-Wasser EFFIZIENT + KORREKT + in SYNERGIE + verbunden** (Schöpfer-Browser-Befunde, im Code verifiziert/gemessen). Wasser bleibt voxel/zell-basiert (reaktiv) + teilt den Terrain-Mesher (Surface-Nets-Iso → fließt) + füllt die Mulde bis zum Ufer, kein Sheet. Detail §1.4. | V12 ✅ | **V13.0–.4 ✅** Diagnose · Klassifikation · Grenzflächen-Meshing · Flow-Bias · Glättung(zurückgerollt) · **V13.5 ✅** Schicht-3-Tiefen-Shader · **V13.6 ✅** Synergie zurück (Surface-Nets-Iso wie das Terrain, band-limitiert) · **V13.7 ✅** verbundenes Wasser (`terrainTopY < rim` → Ufer-Wände weg, tiefe Wände 33%→0%; reaktiver Carve-Fill nahe Quelle; kein Hang-Schatten/Phantom) · **V13.8** Sub-Region-Edit-Remesh · **V13.9** LOD-Cross-Fade · **V13.10** Schöpfer-Browser-Audit | Wasser wie die Riesen: kohärent, fließend, gefüllt bis zum Ufer, keine Wände/Schatten, reaktiv (Damm staut, Carve flutet). |
...
| **V13.8** ⏳ **next** | Sub-Region-Edit-Remesh: nur die vom Carve/Fill berührten Zellen + ihre Mesh-Sektion neu, nicht der ganze Chunk. | Edit-Drops, „ganzer Chunk neu" |
| **V13.9** | LOD-Cross-Fade: sanftes Überblenden zwischen Detailstufen statt hartem Pop. | „sauberer werden"-Pop |
| **V13.10** | Schöpfer-Browser-Audit RDNA-3: Wasser kohärent + fließend, keine Hang-Schatten, smooth beim Streamen + Editieren. | Bogen-Schluss |
| **V13.x (Backlog, hinten angesetzt)** | (a) **Globale Wasser-Konnektivität** (falls Browser-Audit noch ein Rest-Phantom zeigt): V13.7 nutzt die 16-m-Atlas-3×3-Nähe als Konnektivitäts-Proxy — eine Senke neben einem höheren See (<16 m, durch eine Barriere getrennt) könnte fälschlich füllen. Ein echter globaler Flood-Fill von Quellen wäre die Vollendung. (b) **Volle Flow-Isotropie via MFD/D∞** — V13.3 drop/dist entfernt die EINE-Richtungs-Dominanz, aber eine 4-fach-symmetrische Achsen-Rest-Präferenz (33/67) bleibt. Nur falls der Schöpfer noch einen Achsen-Grid-Grain sieht. | Rest-Phantom an exotischer Geometrie, Achsen-Grid-Grain |
