...
| W6.G3 | **Welt-Lebendigkeit** — Tag-Nacht-Zyklus, sanfte Wetter-Übergänge, Fauna-Lifecycle (Population kommt + geht eigen-zeitlich). Schließt 6.G nach Phase 1+2. | ✅ **live (V8.24, 17.05.2026)** — drei Schichten: (a) `state.timeOfDay 0..1` mit 7 frozen `DAY_NIGHT_STOPS`, Default 8 min/Zyklus + Slider 1-60 min, DirectionalLight wandert über Halbkreis, AmbientLight-Intensity moduliert, Skybox-Tint × Wetter-Multiplier, Status-Bar mit Emoji+24h-Uhrzeit, DSL-Op `set_time_of_day` in NON_BROADCASTABLE_OPS; (b) `WEATHER_TINTS` mit skyMul/lightMul, `requestWeatherTransition` startet 45 s Cross-Fade während `state.weather` instant gesetzt wird (logisch sofort, visuell sanft), `updateSkyboxWeather` als Schmal-Wrapper auf `_applyDayNightToScene`; (c) TARGET=8/MAX=20 Population, 10 s-Tick mit Birth-CD 25 s + Death-CD 35 s, Soul-Wahl via `_pickFaunaSoulAtPlayer` (Welt-Affinitäts-Feld), Tod mit Trauer (sorrow +0.2, loss-Journal, soul-abhängiger 1.2 s Lebewohl-Sinus-Ping), Geburt mit silent-Spawn + growth-Journal. +59 Invarianten 1879→1938. | – | W6.X |
...
**Summe verbleibend**: ~28-43 Arbeitstage in fokussierten Sessions. **Empfohlene Sequenz nach V8.24 (17.05.2026)**: W11 V3 → W11 ext. (Substanz-Rolle) → **W12 (Welt-Portal PoC mit three-fluid-fx)** → **W13 (Vibe-Pass)** → **W14 (Bibliothek)** → W7 (Compute-Sharing als verteilter Server-Layer für die Bibliothek). Logik: Polish (✅) → Atmosphäre ✅ (Welt lebt eigen) → Multi-User-Tiefe (Peer ist echt) → Hylomorphismus-Vollausbau (Substanz-Rolle) → **Welt-Brücken (12-14, das größte Vision-Wort)** → Skalierung als Compute-Sharing-Layer.
