...
Stand: V9.49 (22.05.2026). Diese Doc trägt den **Plan vorwärts** — die Ring-Übersicht 0-17, was offen ist, die Meilenstein- und Risiko-Sicht. Die anderen Zeit-Ebenen: der aktuelle Stand + die Gotchas in `CLAUDE.md`, die volle Wellen-Chronik in `docs/handover.md`, die Vision in `docs/state-of-realm.md`.
...
- **Wasser aus der Terrain-Wahrheit — V9.50, Schöpfer-entschieden 22.05.2026, Plan zur Durchsicht.** Die Browser-Audits V9.49-d/e/f zeigten einen nicht-konvergenten Kreis: jeder Fix versöhnte zwei Gelände-Wahrheiten — das 16-m-Wasser-Modell gegen das gerenderte Voxel-Terrain. V9.50 schneidet die Wurzel: das Wasser-Oberflächen-Mesh kommt aus dem Voxel-Chunk-Pipeline, derselben Quelle (`_voxelSurfaceY`) wie das Terrain → die Uferlinie ist exakt per Konstruktion (das „Visual = Collision"-Gesetz, aufs Wasser angewandt). Voller Plan: `docs/hydrosphere.md` §14 (Weg B empfohlen, Weg C ehrlich verworfen). Pensioniert das V9.49-Vereinte-Mesh — die letzte Wasser-Architektur-Welle.
