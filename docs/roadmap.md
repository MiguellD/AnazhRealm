...
Stand: V9.52-d (23.05.2026). Diese Doc trägt den **Plan vorwärts** — die Ring-Übersicht 0-17, was offen ist, die Meilenstein- und Risiko-Sicht. Die anderen Zeit-Ebenen: der aktuelle Stand + die Gotchas in `CLAUDE.md`, die volle Wellen-Chronik in `docs/handover.md`, die Vision in `docs/state-of-realm.md`.
...
- **Stamm-Hygiene über den Code hinaus** — seit V9.44 sind Tests + Doku Teil des Stamms und brauchen dieselbe Hygiene-Disziplin: `playtest.cjs` (war 31 574 Z., EINE Funktion — die zweitgrößte Reibung des Projekts; jetzt 30 415 Z. mit 31 benannten Funktionen, drei Viertel der Test-Datei strukturiert) — der **Playtest-Pflege-Bogen** läuft: Sub-Wellen **a + b + c + d sind drin** (V9.52-a/b/c/d: `safeEvaluate`/`ctx`-Gerüst + 4 erste Funktionen + 11 Band-Funktionen Band 1 + 8 Band-Funktionen Band 2 + 8 Band-Funktionen Band 3, je 265-1567 Z. plan-treu nach §5), Sub-Wellen e + f folgen. Voller Schnitt-Plan in `docs/playtest-hygiene.md` (§3 unten). Die Doku-Schicht braucht periodisches Archivieren fertiger Konzept-Dokumente (Konsolidierung 21.05.2026 + erneut diese Session) — sonst wächst das „Projektgedächtnis" zur Halde, eine Schicht über der Heiligen Lektion.
...
### Der Playtest-Pflege-Bogen — Test-Hygiene (läuft, 23.05.2026 — Sub-Wellen a + b + c + d sind drin)
...
**Status (Stand 23.05.2026):** Sub-Wellen **a + b + c + d sind drin** (V9.52-a: `safeEvaluate`/`ctx` + die ersten drei Sektionen; V9.52-b: Band 1 als 11 Band-Funktionen; V9.52-c: Band 2 als 8 Band-Funktionen `checkBandWelle6APolish` .. `checkBandWelle6HCreatures`; V9.52-d: Band 3 als 8 Band-Funktionen `checkBandWelle6XAudit` .. `checkBandV8LatePolishAnd6XContinued`, je 498-1190 Z., Skript `/tmp/extract-band3-d.cjs` — acht Baseline-Läufe bit-identisch). **e + f stehen aus**. Nächste Welle: Sub-Welle e — Band 4 (die End-Sektionen ab Welle 6.H Phase 2B.2 bis Datei-Ende: Welle 6.H 2D/2E/2F + Welle 6.B CAD + V8.00-V8.07 + Welle 9a-d/10a/b + V7.94-V7.98 LLM + Ring 3-6 + UI V1/V2, ~55 Sektionen, Zeilen ~22257-~31040).
