...
Stand: V9.75 (25.05.2026, **Welle C VOLLENDET** — C.4 (Welle-A-Maschinerie streichen) + C.5 (Atlas-Parallel-Pfad streichen) zusammen in einem chirurgischen Schnitt; Wasser ist jetzt EIN Cell-Zustand mit EINEM Iso-Surface-Mesh; netto −230 Z. anazhRealm.js, −1056 Z. playtest.cjs). Diese Doc trägt den **Plan vorwärts** — die Ring-Übersicht 0-17, was offen ist, die Meilenstein- und Risiko-Sicht. Die anderen Zeit-Ebenen: der aktuelle Stand + die Gotchas in `CLAUDE.md`, die volle Wellen-Chronik in `docs/handover.md`, die Vision in `docs/state-of-realm.md`.
...
| **C** ✅ | **Wasser-Substanz-Vereinigung** (Wurzel-Heilung der V9.69-Diagnose „zwei Wasser-Sprachen") | Welle A + B-Diagnose | C.1 V9.71 (Wasser-Cell-Feld als paralleles System) → C.2 V9.72 + V9.73-Heilung (Iso-Surface-Mesher + Toggle entweder-oder + Bergsee-Cell-Init) → C.3 V9.74 (Cellular-Reaktion via DERIVED-View, kein BFS) → **C.4 + C.5 V9.75** (Welle-A-Maschinerie + Parallel-Quad-Pfad streichen, ein chirurgischer Schnitt) | ✅ erfüllt — EIN Wasser-Mesh pro Chunk (Iso-Surface), EINE Skala (1.8 m Voxel-Cell), EINE Geometrie-Quelle (gleicher Surface-Nets-Mesher wie der Boden). Naht-frei per Konstruktion. Netto −230 Z. anazhRealm.js, −1056 Z. playtest.cjs |
| **D** | **Wasser ↔ Kreaturen** (klein additiv, derselbe Hydro-Layer) | Welle C ✅ | D1 `_tickCreatureTaskDirection` erkennt Wasser-Nähe via Cell-Feld + `_hydroWaterLevelAt` · D2 Neuer Task „trinken am Ufer" (3 Phasen: gehe-zum-Ufer → halt → trink-Animation, ~3 s Cooldown pro Kreatur) · D3 Scheu-Verhalten: Pfad-Suche meidet WATER-Cells > 2 m tief · D4 Schwimm-Animation aus Player-Soul re-use (Stroke-Pattern) | Eine Kreatur findet ohne Skript-Hilfe Wasser, trinkt sichtbar, schwimmt durch flaches Wasser, scheut tiefes |
