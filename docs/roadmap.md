...
Stand: 20.05.2026 (V9.37, Voxel-Phase 5c.2.c.3.b.i — drei tote Heightfield-Dispatch-Pfade entfernt (Game-Loop-Streaming-Ring, `generateTerrainWithParameters` initial 64-Chunk-Loop, `populateChunkVegetation`-Initial-Pass). Die Pipeline-Methoden selbst bleiben (Tests rufen sie direkt). ~50 Z. Code netto weg. Davor V9.36: `modify_terrain` stirbt, der Welt-Mod-Bogen schlägt um zum Voxel-Feld. Die ganze Ring-10.5-Heightfield-Mod-Pipeline (DSL-Op + 6 Helpers + `chunkDeltas`-Feld + CHUNK_DELTA_OPS_CAP, ~289 Zeilen netto weg) ist ersetzt durch zwei schlanke broadcastable DSL-Ops `voxel_carve(x, y, z, r)` + `voxel_fill(x, y, z, r)` im 3D-Dichte-Feld (V9.14/V9.15). Chat-Patterns `grabe loch`/`hebe hügel` umgeroutet; `tryMouseBreak`-Heightfield-Fallback weg (war seit V9.35 toter Pfad); CREATURE_PROPOSED_OPS-Whitelist trägt jetzt die zwei Voxel-Ops. Phase 1-5b komplett, Phase 5c-Start (V9.26) + 5c.1 (V9.27) + 5c.1+ (V9.28) + Ammo-Leck-Fix (V9.29) + 5c.2.a Dead-Code (V9.30) + 5c.2.b-Vorbereitung (V9.31) + 5d-Mini Wasserfälle (V9.32) + 5c.2.b Eingangs-Welt-Flip (V9.33) + 5c.2.c.1 weitere tote Schichten (V9.34) + 5c.2.c.2 Toggle-Tod (V9.35) + 5c.2.c.3.a modify_terrain weg (V9.36) live; in 5c.2.c noch offen: 5c.2.c.3.b (heightfield-only Chunk-Pipeline ablösen — `ensureChunkAt`/`populateChunkVegetation`/`_buildChunkGrass`/`_terrainHeightAtWorld`, jetzt nur noch von V9.29-Dispose-Tests + V8.27/V8.28-Shader-Tests + V9.27-`terrainMaterial`-Init erreicht). 2947 Playtest-Runs grün, Audit-Strict 0 Failures. Der Fremd-Engine-Bogen (W12-W17) ist komplett; offen darin nur B-WASM (per-Projekt). Volle Wellen-Historie in `CLAUDE.md`.)
