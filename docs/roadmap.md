...
**Nächste Welle: V13.15 — Berg-Innenwasser am echten Spielort heilen (Browser-Dump-getrieben).** Der Berg-Wasser-Befund reproduziert sich headless NICHT (Spawn seedet nur echtes See-Wasser, 0 Rim-Bleed — die V13.0-Mess-Falle: Headless misst einen anderen Ort als der Browser). → ZUERST der Schöpfer-Browser-Dump an der echten Fehlstelle (Konsolen-Befehl bereitgestellt: WATER/under-SOLID/Höhe-über-Terrain am Spielort), DANN gezielt heilen — nicht am Spawn blind raten. **Erkenntnis (V13.7–.14): Wasser-Ufer ist ein 16-m-Atlas-vs-1,8-m-Voxel-Tradeoff (bleed-frei↔blockig); die Glätte gehört in den Tiefenpuffer-Shader, NICHT in eine 3D-Fluid-Sim (Heilige Lektion); Wasser ist 3D-konnektiv klassifiziert (Sky-Open), nie per-Spalte; Render NUR im Browser-Loop justieren (Headless ist pixel-blind).** Danach **V13.16** Sub-Region-Edit-Remesh (frame-sauberer Carve), **V13.17** Schöpfer-Browser-Audit-Schluss; dann **V14 — die lebendige Welt** (Vision-Pfeiler E/F/G). Der volle V13-Plan steht in **§1.4**.
...
| **V13 Wasser-Profi-Bogen** ⏳ (V13.0–.14 ✅, V13.15 next) | **Voxel-Wasser EFFIZIENT + KORREKT + in SYNERGIE + 3D-verbunden** (Schöpfer-Browser-Befunde, im Code verifiziert/gemessen). Wasser bleibt voxel/zell-basiert (reaktiv) + teilt den Terrain-Mesher (Surface-Nets-Iso → fließt) + ist 3D-verbunden (Sky-Konnektivität). Detail §1.4. | V12 ✅ | **V13.0–.8 ✅** Diagnose→3D-Flood · **V13.9 ✅** Schicht-3-Cull · **V13.10 ✅** Klettern-Cap (verworfen) · **V13.11 ✅** Austritt-Cull (ersetzt) · **V13.12 ✅** Vertikal-offen · **V13.13.0–.2 ✅** Iso-Memo (−61 %) + scale-Stempel + Iso-liest-Wahrheit (Phantom 83→0) · **V13.14 ✅** die EINE robuste Regel (Sky-Konnektivität, Überhang-Bucht 54/54) · **V13.15** ⏳ Berg-Innenwasser am echten Spielort (Browser-Dump) · **V13.16** Sub-Region-Edit · **V13.17** Browser-Audit-Schluss | Wasser wie die Riesen: kohärent, fließend, 3D-verbunden, reaktiv (Damm staut, Carve flutet); Ufer-Glätte via Shader. |
...
| **V13.15** ⏳ **next** | **Berg-Innenwasser am echten Spielort** (Browser-Dump-getrieben — reproduziert sich headless NICHT, V13.0-Mess-Falle). ZUERST Schöpfer-Dump (Konsolen-Befehl gegeben), DANN gezielt heilen. | Berg-Innenwasser (seitlich sichtbar) |
| **V13.16** | Sub-Region-Edit-Remesh: nur die vom Carve/Fill berührten Zellen + ihre Mesh-Sektion neu, nicht der ganze Chunk (frame-sauberer Carve). | Edit-Drops, „ganzer Chunk neu" |
| **V13.17** | Schöpfer-Browser-Audit-Schluss RDNA-3: Wasser kohärent + fließend, kein Bluten, smooth beim Streamen + Editieren. | Bogen-Schluss |
