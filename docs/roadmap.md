...
| **V14.4** | **Maßstab-Streckung**: die `ranges`-Wellenlängen strecken (λ77→~150–250 m) → Mikro-Cusps glätten + Feature-Größe (>200 m) + längere Flüsse; `terrainSteepness` verdrahten/beerdigen; die `tect`-Kommentar-Lüge heilen. | Mikro-Cusps (λ77 m), kleine Feature-Größe, kurze Flüsse |
| **V14.5** | **Gerichtete Uplift-Maske (Anden-Ketten)**: lineare/kurvige Gebirgsketten statt isotropes `mtn` → lange Züge über weite Distanzen (Hochland-Kohäsion ↑↑, „über weite strecken aufbauend"). Hier wächst evtl. die Hülle (hohe Ketten). | „spitzig isoliert" statt langer Ketten |
| **V14.6** | **Schöpfer-Browser-Audit (Bogen-Schluss)**: Anden/Everest-Charakter, VIELFALT (verschiedene Terrain-Typen wie die Erde), lange Aufbau-Phasen, epische Felder, kohärente große Seen/Flüsse. Tarn-Konstanten (V9.51) an die Skala nachziehen, falls nötig. | Bogen-Schluss |
