...
Stand: 20.05.2026 (V9.29, Voxel-Terrain-Bogen Phase 5c.1+ + Ammo-Heap-Leck-Fix der Heightfield-Chunks. V9.29 heilt den in V9.28 entdeckten pre-existing Bug — `_disposeChunkPhysics(mesh)`-Helper räumt Body+Shape+MotionState+TMesh sauber (V8.26-§6.4-Muster auf Heightfield-Chunks angewandt), drei Konsumenten teilen ihn. Plus: V9.27/V9.28/V9.29-Lehren in der kanonischen „Wichtige Gotchas"-Sektion zementiert (Doku-Hygiene); W10 ext. stale „🔴 offen"-Eintrag in §2 geheilt (✅ seit V9.01). Phase 1-5b komplett, Phase 5c-Start (V9.26) + 5c.1 (V9.27) + 5c.1+ (V9.28) + Ammo-Leck-Fix (V9.29) live; offen in 5c: die grosse Heightfield-Code-Entfernung + Test-Suite-Umschreibung — sobald die Migrations-Erfahrung gefestigt ist + die Eingangs-Welt geflippt wird. ~2940 Invarianten grün, Audit-Strict 0 Failures. Der Fremd-Engine-Bogen (W12-W17) ist komplett; offen darin nur B-WASM (per-Projekt). Volle Wellen-Historie in `CLAUDE.md`.)
