...
> **Stand 28.05.2026**: V12.0-diag/vendor.1/2/3/4/a/d/e ✅ committed (alle 7 Sub-Wellen auf Branch `claude/sharp-albattani-bkJeW`). Schöpfer-Browser bestätigt FPS 60+ auf RDNA-3 + „Gras passt". **Rest**: V12.0-f-full (MeshToonNodeMaterial-Migration, ~250 Z. erwartete netto-Streichung) + V12.0-g (final Audit) + V12.0-perf (Edit-Cluster-FPS-Heilung, Folge-Welle nach g per Schöpfer-Disziplin „erst V12-Bogen sauber schließen").
...
| **V12.0-perf** (V12.0-d-Audit-Folge) | Edit-driven Chunk-Rebuild FPS-Heilung. Schöpfer-Browser-Befund V12.0-d: FPS=52→7→5 nach 19 spawnArchitecture-Cluster. Wurzel: `_rebuildVoxelChunk` läuft SYNC (~50-150 ms/Chunk) im Main-Thread, dirty-Pump arbeitet 1/Frame ab. Phase A Profilierung (welche ms gehen wohin). Phase B gezielte Heilung — edit-driven via Worker (V9.91-Pattern erweitern) ODER GPU-Density (V12.0-e-Vorarbeit für edit-Pfad) ODER Per-Frame-Time-Budget (V9.96-Pattern auf edit übertragen). Phase C Schöpfer-Browser-Audit: 19 Strukturen platzieren, FPS stabil 60+. | ~2-3h | Edit-Cluster brechen FPS nicht mehr, Welt-Bauen-Erleben smooth |
