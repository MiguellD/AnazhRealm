...
| **Vision-Reset** ⏳ | **Die wahre Bibliothek von Alexandria** (V9.94-V9.99, der epochale Wurf — was V9.86-Brainstorm visionierte, aber V9.87-V9.93 als Catch-up untergrub) | V9.93 ✅ + Schöpfer-Konfrontation 26.05.2026 | ✅ **V9.94** WebGPU-Compute-Diagnose · ⚠️ **V9.94.r** Bürokraten-Reflexion (Lehre 3-Stufen-Hierarchie verdrahtet) · ✅ **V9.95-a** WebGPU-Foundation (Adapter/Device/Trivial-Shader/Determinismus-Beweis) · ✅ **V9.95-b** WGSL-Density-Pipeline (Macro-Surface + Cave + Erosion + Tarn auf GPU, Eligibility-Gate für Trockenland-Chunks, Pipeline-Cutover als Stufe-0; Hydro+Edits fallen auf Worker) · ⏳ **V9.95-c** WGSL-Hydrosphere-Carve + Lake-Mask + voxelEdits (~3-4h, 100% Chunk-Coverage auf GPU) · ⏳ **V9.95-d** optional: GPU-Density als preDensity in den Worker einschleusen (zero-roundtrip Main-Time ~30ms) · **V9.96** IndexedDB-Persistent-Chunk-Cache (~4-5h, VISION-PFEILER „Welt-Gedächtnis") · **V9.97** Predictive Prefetch aus Velocity · **V9.98** Geometry-Stitching für LOD-Boundaries · **V9.99** Per-Column-Atlas-Strict | Browser-Audit: Density-Cost pro Chunk <1ms (statt ~50ms), Re-Visit <5ms (statt 30-50ms), Welt fühlt sich vorausschauend an, LOD nahtlos. Master-Plan in `docs/performance.md` §5. |
