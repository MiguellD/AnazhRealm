...
Stand: V9.82 (25.05.2026, **Welle C VOLLENDET + Browser-Audit-Heilungen abgeschlossen**). Welle-C-Bogen: V9.71 (Cell-Feld) → V9.72/V9.73 (Iso-Mesher + Bergsee-Cell-Init) → V9.74 (Cellular-Reaktion via DERIVED-View) → V9.75 (C.4+C.5 chirurgischer Schnitt — Welle-A-Maschinerie + Parallel-Quad-Pfad gestrichen) → V9.76 (Trocken-Gate reanimiert) → V9.77 (globales Y-Band gegen Per-Chunk-Drift) → V9.78 (Below-Band-WATER-Shortcut gestrichen, Floating-Plane weg) → V9.79 (Pad+Crop für Iso, Oberflächen-Naht endlich geheilt) → V9.80 (nur Wasser-Luft-Iso, Bottom/Side-Geister gestrichen) → V9.81 (Density-Grid-Sharing mit Boden-Mesher, ~50× Speedup) → V9.82 (Streaming-Pfad-Bug seit V9.71 gefunden, Wasser lädt jetzt synchron mit Terrain). Diese Doc trägt den **Plan vorwärts** — die Ring-Übersicht 0-17, was offen ist, die Meilenstein- und Risiko-Sicht. Die anderen Zeit-Ebenen: der aktuelle Stand + die Gotchas in `CLAUDE.md`, die volle Wellen-Chronik in `docs/handover.md`, die Vision in `docs/state-of-realm.md`.
