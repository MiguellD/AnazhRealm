...
Stand: **V13.14 (30.05.2026) — Die EINE robuste Wasser-Regel: 3D-Sky-Konnektivität** (Wasser-Profi-Bogen V13.0–.14). Schöpfer-Befund „am Ufer abgebaut, fließt nicht nach bei einem Überhang; wieso kann Wasser klettern?" entlarvte das Wasser-Modell als FÜNF gestapelte Heuristiken. Die V13.14-Regel ersetzt den per-Spalten-Trocken-Pass (V13.12) durch EINE physikalische Wahrheit: Wasser existiert nur, wo es durch Wasser mit der offenen Atmosphäre verbunden ist (`_skyOpenWaterFilter` + Worker-Mirror, bit-identisch). Synthetik-Beweis: offener See bleibt, Überhang-Bucht BLEIBT (54/54 — der Carve-Bug geheilt), eingeschlossene Blase trocknet. Direkt davor: V13.13.2 (Iso-Mesher liest die Flood-Wahrheit statt sie per-Spalte neu zu raten, Phantom 83→0), V13.13.0 (Iso-Memo −61 %, bit-identisch). Davor: V12-Genie-Bogen (r184 + WebGPU, 119 FPS) + V10.0-TSL-Bogen. Volle Wellen-Chronik in `docs/handover.md`.

**Nächste Welle: V14 — Die wahre Tiefe des Geländes (epische Geomorphologie).** Schöpfer-Auftrag 30.05.2026: „berge bilden sich nicht über lange strecken, sind spitzig und steil, kein langsam aufbauendes gebiet, anden/everest, lange flüsse, epische gegenden … genial, alles in den schatten stellen". Tiefen-Analyse (empirisch + Code, 30.05.2026): das Makro-Relief ist ein **Alpen-Miniatur-Maßstab** — die dominante Gebirgs-Frequenz liegt bei λ≈77 m; es fehlt die niederfrequente kontinentale Basis (λ ~5–20 km), die gerichtete Uplift-Maske (lineare Ketten statt isotrope Zacken) und die thermische Erosion (Hang-Glättung); `terrainSteepness` ist ein toter Parameter. Empirie (`diag-relief`): Höhen-Histogramm rechtsschief (Berge >80 m nur ~1 %), Uferlinien-Index 47 (gesund <7) = mikro-zerfranst. **Die fraktale Wurzel: Terrain- und Wasser-Problem sind EINS** — der zerklüftete Mikro-Maßstab erzeugt sowohl die spitzen Berge als auch die fragmentierte Hydrologie (50 Fitzel-Seen); eine kontinentale Form heilt beide. Darum: erst das Fundament (V14), **dann der Wasser-Finish (V15)** gegen die neue Topologie. Der volle V14-Plan steht in **§1.5**.
...
| **V14 — Die wahre Tiefe des Geländes** ⏳ **(next)** | **Epische Geomorphologie** (Schöpfer-Auftrag 30.05.2026: anden/everest, lange Flüsse, epische Felder, weite Aufbau-Phasen). Wurzel (empirisch + Code): Alpen-Miniatur-Maßstab — dominante Gebirgs-λ≈77 m, fehlende kontinentale Basis + Uplift-Maske + thermische Erosion; `terrainSteepness` tot. Heilt das Terrain UND die fragmentierte Hydrologie an EINER Wurzel (fraktale Verbindung). Detail §1.5. | V13.14 ✅ | **V14.0** Geomorphologie-Diagnose (Ziel-Metriken + Berg-Shot) · **V14.1** kontinentale Basis-Oktave (λ ~6–12 km) · **V14.2** gerichtete Uplift-Maske (lineare Ketten) · **V14.3** thermische Erosion (Angle-of-Repose, glättet Hänge) · **V14.4** Maßstab-Streckung + `terrainSteepness` heilen · **V14.5** Schöpfer-Browser-Audit | Berge bauen sich über weite Distanzen auf (Anden/Everest), epische Felder, lange Flüsse, große kohärente Seen, glatte Küsten (Uferlinien-Index ↓). Determinismus-Bruch gewollt. |
| **V15 — Der Wasser-Finish** ⏳ | **Die drei Synchronisations-Wurzeln** (aus der V13.14-Analyse, gegen die NEUE V14-Topologie justiert) + Berg-Innenwasser + Sub-Region-Edit. | V14 ✅ + Schöpfer-Browser-Dump | **V15.0** Browser-Dump an der echten Fehlstelle (Vorbedingung — Spawn reproduziert nicht) · **V15.1** Arch-Spawn-Remesh deckt die Iso-OOB-Nachbarn ab (skirt-Wurzel) · **V15.2** `_enqueueWaterIso` als distanz-priorisierte Queue (statt Set) · **V15.3** Stempel-AABB rotations-aware (`_blockerComputePartAABB`) · **V15.4** Sub-Region-Edit-Remesh (frame-sauberer Carve) · **V15.5** Browser-Audit-Schluss | Kein Kapillar/Schatten an Strukturen, kein Berg-Bluten beim Streamen, frame-sauberer Carve. |
...
| Welle | Was | Heilt |
|---|---|---|
| **V14.0** | Geomorphologie-Diagnose: `diag-relief` um ZIEL-Metriken erweitern (Hochland-Plateau-Anteil, Hang-Neigungs-Verteilung, Gebirgsketten-Kohärenz/Längen, Becken-Größen-Histogramm) + ein robuster Berg-Überblick-Screenshot (Spieler-Body-Teleport zum höchsten Spot, Streaming-Pump, Schräg-Sicht + Top-Down). Die Akzeptanz wird die Metrik, nicht die Demo (V13.0-/V9.58-Disziplin: messen vor schneiden, strengste Definition). | Mess-Falle (Spawn ≠ Berg); Akzeptanz definieren |
| **V14.1** | **Kontinentale Basis-Oktave**: eine sehr niederfrequente additive Schicht (λ ~6–12 km, Amp ~150–300 m, ggf. plateau-biased à la V9.60-b Multi-Plateau) in `_terrainMacroSurfaceY` (+ Worker-Mirror bit-identisch) — formt breite Becken + Hochland-Sockel. Decken-Marge prüfen (`_voxelChunkConfig` dimY/floorDrop — V9.58/V9.26-Lehre: Surface-Spike über der Decke = Loch). | „keine weiten Phasen", flacher Median |
| **V14.2** | **Gerichtete Uplift-Maske**: lineare/kurvige Gebirgsketten statt isotropes `mtn=(1−ero)²`. Eine Ridge-Distanz-Funktion entlang weniger tektonischer Linien/Kurven (aus dem Seed) moduliert die ridged-Oktave ENTLANG der Ketten → lange Anden-artige Züge statt verstreuter Zacken. | „spitzig isoliert" statt langer Ketten |
| **V14.3** | **Thermische Erosion (Angle-of-Repose)**: ein Talus-Pass in `_computeErosion` (+ Worker, falls die Erosion dort gespiegelt ist), der Material über dem Ruhewinkel abträgt + lokal deponiert → glatte natürliche Bergflanken statt 60°-Cusps; ko-evolviert mit der Stream-Power-Inzision. (Re-erodiert die Welt.) | 60°-Wände, fehlende Talus-Hänge |
| **V14.4** | **Maßstab-Streckung + Aufräumen**: die ridged-Oktaven-Wellenlängen strecken (λ 77→~150–250 m), damit die Verzierung zur neuen großen Form passt; `terrainSteepness` verdrahten (multiplikativ auf die Relief-Amplituden) ODER ehrlich beerdigen; die `tect`-Kommentar-Lüge heilen. | Skala-Mismatch Verzierung↔Form; toter Parameter |
| **V14.5** | **Schöpfer-Browser-Audit**: Anden/Everest-Charakter, lange Aufbau-Phasen, epische Felder, kohärente große Seen/Flüsse (Uferlinien-Index fällt deutlich, lange Flüsse steigen). Tarn-Konstanten (V9.51) an die neue Skala nachziehen, falls nötig. | Bogen-Schluss |
...
- **Decken-Marge** (V9.58/V9.26): höhere Amplituden brauchen evtl. eine größere Voxel-Hülle (`dimY`/`floorDrop`). Vor jeder Amplituden-Erhöhung die `diag-relief`-Decken-Marge prüfen (heute: Surface-max 118 m, Decke 133 m → nur 14 m Marge!).
