...
**Stand:** **Phase 1 ✅ live (V8.71)** — die netzfreie Hälfte: ein LOKALES Bündel dockt an. Der save-server hat den `/api/vendor-world`-Endpunkt (Schreib-Seite, strenge Wand — Punkt 1+2 unten), `vendorWorldBundle` registriert die Welt als `customWorlds`-Eintrag mit `trust:"sandboxed"`, eine „Welt andocken"-Sektion im Bibliothek-Drawer nimmt einen Welt-Ordner. **Offen — Phase 2: der GitHub-Fetch** — der Endpunkt bekommt einen zweiten Eingang (eine Repo-URL statt eines Bündels; der save-server holt die Dateien selbst, URL-Whitelist) + optional der LLM-Manifest-Schritt (Punkt 3). Die Schreib-Seite (Phase 1) bleibt unverändert.
