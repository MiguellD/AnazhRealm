...
| 7 | **W6.G P3-Rest** | Terrain-Höhlen/Überhänge/Klippen. **Phase 1 ✅ (V9.03 — Felsformationen)**: `felsbogen` (ein begehbarer Trilithon-Überhang) + `felsturm` (eine vertikale Fels-Nadel) als emergente Compound-Architekturen über das Welt-Affinitäts-Feld — KEIN Voxel-Rewrite, der Überhang lebt auf der Per-Sub-Mesh-Box-Kollision. Damit sind Überhänge + Klippen-Dramatik geliefert. **Phase 2 (offen)**: echte Höhlen — ein Hohlraum/Tunnel, in den man eintritt; braucht Innenraum-Geometrie (ein Höhlen-Eingang-Compound mit begehbarem Innenraum, oder eine eigene Sub-Welt-Höhle). | Phase 1 erledigt; Phase 2 = 2-3 Sessions | ✅ Phase 1 (V9.03) |
