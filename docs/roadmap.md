...
Stand: 17.05.2026 (V8.60, W13 Vibe-Pass KOMPLETT (Phase 1+2+3) — souveräne Avatar-Identität. Phase 1 (V8.54): ein ed25519-Schlüsselpaar, lokal erzeugt (WebCrypto nativ), privat in `localStorage["anazh.vibePass"]`, nie im Welt-Save. Phase 2 (V8.55): Bauplan-Signaturen — `signBlueprint`/`verifyBlueprintSignature` signieren die SUBSTANZ (Rolle + Parts + Verbindungen, nicht den Namen), vier Status-Stufen, Werkstatt-Anzeige, Signatur reist durch Save/Welt-Tor/Fusion. Phase 3 (V8.56): Vibe-Pass-Identität im Multi-User — ein neuer WS-Typ `vibe` trägt vibePassId + einen Beweis (Signatur über die eigene peerId); der Mitspieler ist beweisbar sein Schlüssel, das Name-Schild zeigt „✓ <Fingerprint>". Davor: V8.51-V8.53 W12 Welt-Portal komplett. V8.57 heilte einen flaky CI-Playtest (3rd-Person-Kamera-Pitch) an der Wurzel; V8.58 brachte **W14 Phase 1** (Bibliothek — die `WORLD_REGISTRY` wird ein browsbarer „Bibliothek"-Tab, „Portal holen" legt einen Tor-Bauplan ins Inventar); V8.59 härtete sie (`portalMeta` überlebt den Reload); V8.60 brachte **W14 Phase 2** (signierte Welt-Manifeste: der Spieler versiegelt eine Welt mit seinem Vibe-Pass, „signiert von <Autor>" beim Betreten — plus W13 V2: der Vibe-Pass trägt das Schaffen des Spielers durch ein Portal). 2423/2423 Invarianten grün, Audit-Strict 0 Failures. Nächste empfohlene Welle: W14 Phase 3 (fremde Welten empfangen + in die eigene Bibliothek importieren). Detail in `CLAUDE.md`.)
...
| W14 | **Bibliothek (Welt-Registry)** — die letzte große Vision-Schicht. Ein browsbarer Bibliothek-Tab über `WORLD_REGISTRY`, „Portal holen" → Portal-Bauplan ins Inventar (P1 ✅ V8.58); der Spieler signiert eine Welt mit dem Vibe-Pass, „signiert von <Autor>" beim Betreten + W13 V2 (das Schaffen reist mit) (P2 ✅ V8.60); fremde Welten in die eigene Bibliothek importieren (P3, Horizont). **Drei-Phasen-Detail + Schnittstellen in §3.** | 🟡 **P1+P2 live (V8.58/V8.60) — P3 nächste Welle** | 3-4 Sessions (P3) | W13 ✅ |
...
- Welle 14 (Bibliothek) 🟡 — die Welt-Registry (Phase 1+2 live V8.58/V8.60, Phase 3 offen)
