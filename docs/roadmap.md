...
Stand: V9.48 (22.05.2026). Diese Doc trägt den **Plan vorwärts** — die Ring-Übersicht 0-17, was offen ist, die Meilenstein- und Risiko-Sicht. Die anderen Zeit-Ebenen: der aktuelle Stand + die Gotchas in `CLAUDE.md`, die volle Wellen-Chronik in `docs/handover.md`, die Vision in `docs/state-of-realm.md`.
...
- **Ein vereintes Wasser-System — V9.49, terminiert (Architektur-Bogen, in Arbeit).** Schöpfer-Browser-Befund nach V9.48: das Wasser „schliesst nicht" — Meer (globale 900×900-Plane), Seen (N Planes), Flüsse (N Ribbons), Wasserfälle wirken als gestapelte durchscheinende Sheets, nicht als ein Wasserkörper. Die Wurzel ist die Architektur „globale Platte + Ausnahmen", nicht der Shader; eine Naht-Politur (depthWrite, Y-Abgleich) wäre Symptom-Heilung (Lehre 1). V9.49 wechselt das Verfahren: Wasser ist kein Körper, es ist ein **Feld** — das Priority-Flood-`filled`-Feld ist schon eine kontinuierliche Wasser-Oberfläche über die ganze Region. Ein spieler-folgendes Höhenfeld-Mesh (ersetzt Meeres-Plane + See-Planes + Fluss-Ribbons) senkt sich in die V9.43-d-gecarvten Kanäle → Fluss/See/Meer entstehen aus EINEM Mesh + EINEM Shader. Der Wasserfall bleibt die ehrliche vertikale Ausnahme. Behebt nebenbei den absorbierter-See-rendert-trocken-Bug. Voller Design: `docs/hydrosphere.md` §12, Wellen-Plan: §3 unten.
...
- **Stamm-Hygiene über den Code hinaus** — seit V9.44 sind Tests + Doku Teil des Stamms und brauchen dieselbe Hygiene-Disziplin: `playtest.cjs` (31 065 Z., EINE Funktion — die zweitgrößte Reibung des Projekts) braucht eigene innere Struktur — als **Playtest-Pflege-Bogen** geplant, voller Schnitt-Plan in `docs/playtest-hygiene.md` (§3 unten); die Doku-Schicht braucht periodisches Archivieren fertiger Konzept-Dokumente (Konsolidierung 21.05.2026 + erneut diese Session) — sonst wächst das „Projektgedächtnis" zur Halde, eine Schicht über der Heiligen Lektion.
...
| W6 | **Crafting-Polish + UX + Stats + Welt-Sinne + Kreaturen-Aufträge** — acht Sub-Blöcke (A–H) | ✅ **Welle 6.A/B/C/D/E/F(1-2)/G(1-2)/H V2 live**, 6.G Phase 3 offen; 6.F3 (Maschinen-Antrieb) + 6.F5 (echte Ammo-Constraints) nie gebaut — siehe §1 „Crafting-Tiefe" | 22-28 Sessions verteilt, ~24 abgeschlossen | W5 + Rings 8-11.5 |
...
| # | Welle | Was | Aufwand | Risiko | Stand |
|---|---|---|---|---|---|
| a | **V9.49-a** | Das Wasser-Feld als Wahrheit — `_computeHydrosphere` rechnet ein per-Zelle-Feld (`waterY` = Wasser-Oberfläche, `waterKind` = trocken/Ozean/See) aus `filled`/`isOcean`/`lakeOf` und legt es auf `state.hydrosphere`. Reine Daten, headless-prüfbar. | ~0.5 Session | niedrig | 🔵 geplant |
| b | **V9.49-b** | Das vereinte Mesh — `_buildUnifiedWaterMesh`: ein spieler-folgendes Höhenfeld-Raster (~384 m, ~3 m Zelle, auf das Raster gesnappt), je Vertex `Wy = bilinear(Wasser-Feld)` gegen das gecarvte Gelände → nasse Quads auf `Wy`. Ersetzt `_buildLakeMesh` + `_buildRiverRibbon` + `_buildWaterPlane`. `depthWrite:true`. | ~1-2 Sessions | mittel | 🔵 geplant |
| c | **V9.49-c** | Der vereinte Shader — Gerstner (Ozean, pro-Vertex `aWave`-moduliert, weich aufs Ufer ausgeblendet), Still-Schimmer (See), Flow-Schaum (`aFlow`), tiefen-getriebener Ufer-Schaum (ersetzt den V9.48-BFS). Wasserfall: `depthWrite` + Anschluss ans Feld oben/unten. | ~1 Session | mittel | 🔵 geplant |

**Akzeptanz**: das Wasser liest sich als EIN Körper — keine gestapelten Sheets, kein toter See in der Ferne, der Fluss liegt in seiner Furche und mündet nahtlos. Schöpfer-Browser-Audit. Die Physik (`state.waterLevel`, `_hydroWaterLevelAt`-Auftrieb) bleibt unverändert — sie liest Daten, kein Mesh.

---

### Der Playtest-Pflege-Bogen — Test-Hygiene (geplant, 21.05.2026)

Eine Strukturanalyse von `scripts/playtest.cjs` fand die zweitgrößte Reibung des Projekts: **31 065 Zeilen in EINER async-IIFE-Funktion**, Median-Einrückung 20 Spaces, 196 `### `-Sektionen als bloße Inline-Kommentare (keine Funktionsgrenzen), 3 157 `check()`-Invarianten, ~200 `page.evaluate`-Roundtrips mit wiederholtem Boilerplate. Strukturell ist die Test-Datei in schlechterer Verfassung, als `anazhRealm.js` vor V9.44 war (dort war der schlimmste Gegner 696 Z.). Es ist die V9.44-Wurzel — Wachstum ohne Funktions-Größengrenze — eine Schicht versetzt: der Code-Stamm bekam mit V9.44 seine Grenze, der Test-Stamm nie.

**Der Bogen** zerlegt die eine IIFE in benannte Sektions-Funktionen DERSELBEN Datei — KEIN Test-Runner (Jest/Mocha = Re-Komplexifizierung), KEIN Datei-Split. Reines verhaltensneutrales Refactoring, sechs Sub-Wellen a-f (Gerüst + `ctx`/`safeEvaluate`-Helfer, dann vier Bänder, dann Helfer-Durchzug), ~7-8 Sessions. **Riskanter als V9.44**: der Playtest IST das Sicherheitsnetz — der Beweis ist der Diff der geordneten Invarianten-Namen-Liste vor/nach jedem Schnitt (bit-identisch modulo dem ±2-3-Konsolen-Drift). **Vollständiger Schnitt-Plan: `docs/playtest-hygiene.md`** (Befund, Vision-Abgleich, Disziplin, Sub-Wellen, bewusste Nicht-Wellen) — vor dem ersten Code-Block ZUERST lesen.
