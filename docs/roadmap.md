...
Stand: **V14.6 (30.05.2026) — Terrain-Bogen „Die wahre Tiefe des Geländes" (epische Geomorphologie, V14.0–.6, Browser-Audit-getrieben).** Das Makro-Relief von uniform-spitz-alpin zu abwechslungsreich-wie-die-Erde: V14.0 Diagnose (7 Akzeptanz-Metriken) · V14.1 kontinentale Basis-Oktave `cont0` (lange Flüsse, median 128→1152 m) · V14.2 thermische Erosion (Angle-of-Repose/Talus, glättet die Hänge) · V14.3 regionale Terrain-Typ-Differenzierung (Multi-Noise, die Vielfalt) · V14.4 echte Ebenen (`mtn` moduliert ALLE Oktaven) + See-Oberflächen-Queue-Heilung · V14.5 Harmonie-Heilung (Wurm-Höhlen-Decke `surf−6`→`surf−16`) · V14.6 Fern-Heilung (Voxel-Decke dimY 124→136 + tanh-Soft-Clamp + stale GPU-Mirror abgeklemmt — die Welt zerfiel „in eine Richtung", weil cont0 fern bis 235 m über die 133-m-Decke wuchs). **Über den Bogen (gemessen): Anteil steil (>45°) 26 %→4,4 %, Median-Hangneigung 35°→18,7° (sanfte Welt!), sanft-Anteil 6,7 %→24,1 % (echte Ebenen), Plateau-Anteil 2,7 %→11,2 % (Hochebenen), 9 lange Flüsse, FPS 119.** Vier Schöpfer-Browser-Audits eingearbeitet (Vielfalt, Ebenen+See-Oberfläche, Harmonie, Fern-Chunks). Davor: V13-Wasser-Profi-Bogen (pausiert @ V13.14, Finish → nach dem Terrain) + V12-Genie-Bogen (r184 + WebGPU, 119 FPS) + V10.0-TSL-Bogen. Volle Wellen-Chronik in `docs/handover.md`.

**Nächste Welle: V14.7 — Maßstab-Streckung.** Die `ranges`-Wellenlängen strecken (λ77→~150–250 m) → die Mikro-Cusps glätten + die Feature-Größe heben (176→>400 m) + längere Flüsse; `terrainSteepness` verdrahten oder ehrlich beerdigen; die `tect`-Kommentar-Lüge heilen. Danach **V14.8** (gerichtete Uplift-Maske: Anden-Ketten über weite Distanzen) + **V14.9** (Browser-Audit-Bogen-Schluss). Dann **V15 — Der Wasser-Finish** (die drei Render-Sync-Wurzeln gegen die neue Topologie) und **V16 — die lebendige Welt** (Pfeiler E/F/G). Der volle V14-Plan steht in **§1.5**, der Bogen-Fortschritt in der §1.1-Tabelle. **Bogen-Lehre (V14.5+V14.6): wer die Terrain-HÖHE ändert, muss ALLE höhen-co-getuneten Annahmen nachziehen — die surf-relativen Schichten (Höhlen-Decke, Wasser-Band, Tarn), die Voxel-DECKE, UND jeden parallelen Density-Mirror (Worker + GPU-WGSL). Und: Decken-Diagnose ist radial bis > λ_cont0 (7100 m), kein Spawn-Fenster.**
...
| **V14 — Die wahre Tiefe des Geländes** 🔄 (V14.0–.6 ✅, V14.7 next) | **Epische Geomorphologie** (Schöpfer-Auftrag 30.05.2026: anden/everest, lange Flüsse, epische Felder, Vielfalt wie die Erde). Wurzel (empirisch + Code): Alpen-Miniatur-Maßstab — dominante Gebirgs-λ≈77 m, fehlende kontinentale Basis + Differenzierung + thermische Erosion. Heilt das Terrain UND die fragmentierte Hydrologie an EINER Wurzel. Detail §1.5. | V13.14 ✅ | **V14.0 ✅** Diagnose · **V14.1 ✅** kontinentale Basis `cont0` (lange Flüsse 128→1152 m) · **V14.2 ✅** thermische Erosion (Talus glättet, steil 29→13 %) · **V14.3 ✅** regionale Differenzierung (Multi-Noise, Vielfalt) · **V14.4 ✅** echte Ebenen (mtn moduliert alle Oktaven, sanft →24 %, Plateau →11 %) + See-Oberflächen-Queue · **V14.5 ✅** Harmonie-Heilung (Höhlen-Decke surf−6→surf−16) · **V14.6 ✅** Fern-Heilung (Decke dimY 124→136 + tanh-Soft-Clamp + stale GPU-Mirror ab; cont0 wuchs fern bis 235 m über die 133-m-Decke) · **V14.7** Maßstab-Streckung · **V14.8** Uplift-Maske (Anden-Ketten) · **V14.9** Browser-Audit | Über den Bogen: steil 26 %→4,4 %, Median 35°→18,7°, sanft →24 %, Plateau →11 %, Vielfalt (Ebenen↔Gebirge↔Hochebenen), 9 lange Flüsse, FPS 119. Determinismus-Bruch gewollt. |
...
| **V14.7** | **Maßstab-Streckung**: die `ranges`-Wellenlängen strecken (λ77→~150–250 m) → Mikro-Cusps glätten + Feature-Größe (176→>400 m) + längere Flüsse; `terrainSteepness` verdrahten/beerdigen; die `tect`-Kommentar-Lüge heilen. | Mikro-Cusps (λ77 m), kleine Feature-Größe |
| **V14.8** | **Gerichtete Uplift-Maske (Anden-Ketten)**: lineare/kurvige Gebirgsketten statt isotropes `mtn` → lange Züge über weite Distanzen (Hochland-Kohäsion ↑↑, „über weite strecken aufbauend"). Hülle/Soft-Clamp (V14.6) ggf. an höhere Ketten nachziehen. | „spitzig isoliert" statt langer Ketten |
| **V14.9** | **Schöpfer-Browser-Audit (Bogen-Schluss)**: Anden/Everest-Charakter, VIELFALT (verschiedene Terrain-Typen wie die Erde), lange Aufbau-Phasen, epische Felder, kohärente große Seen/Flüsse. Tarn-Konstanten (V9.51) an die Skala nachziehen, falls nötig. | Bogen-Schluss |
