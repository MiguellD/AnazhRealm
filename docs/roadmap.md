...
Stand: 19.05.2026 (V9.20, Voxel-Terrain-Bogen — der Boden ist ein formbares 3D-Dichte-Feld. Phase 1-3 (Dichte-Feld + Surface Nets, Kollision + Chunk-Streaming, 3D-Graben + Aufschütten + Material-Erhaltungs-Kreis) sind live; Phase 4 (V9.18-V9.20) gibt der Generierung Höhlen, Überhänge und Massstab: drei 2D-Oktaven (kontinental, ridged, Detail) für eine fraktale Gebirgs-Hierarchie, zwei 3D-Roughness-Bänder, Wurm-Höhlen aus einem ridged-Noise-Feld; eine Tiefen-Hüllkurve schützt die V9.12-Boden-Garantie, der Voxel-Chunk ist eine 122-m-Säule. Offen: Phase 5 (Materialien/Shader + die Ablösung des Heightfields). ~2910 Invarianten grün, Audit-Strict 0 Failures. Der Fremd-Engine-Bogen (W12-W17) ist komplett; offen darin nur B-WASM (per-Projekt). Volle Wellen-Historie in `CLAUDE.md`.)
...
5. **Materialien + Shader + Politur + Ablösung.** Cel-Shading (MeshToonMaterial), per-Vertex-Welt-Feld-Farbe (V9.10), Terrain-Material-Harvest (V9.04, geteilt seit V9.14) sind ✅ live. **Voxel-Gras ✅ V9.22** — der Voxel-Boden grünt mit eigenem Instanced-Gras. **Phase 5a ✅ V9.23 — Voxel als Default für neue Welten** (Schöpfer-Wahl „schrittweise"): `createNewWorld` baut neue Welten per Default voxel-basiert, die Dialog-Checkbox ist zum Heightfield-Opt-out gekippt; alte Welten + die Eingangs-Welt behalten ihr Heightfield. **V9.24 — die Verbindungen geheilt**: der Sicht-Ring-Regler greift jetzt in einer Voxel-Welt (`_voxelChunkConfig().ringRadius` folgt `chunkRingRadius`), und die Welt-Affinitäts-Vegetation streamt mit den Voxel-Chunks (`_populateVoxelChunkVegetation`, Strukturen auf dem Voxel-Boden). **Phase 5b ✅ V9.25 — die Voxel-Welt wird höhen-ehrlich**: vor dem grossen Schnitt ein Verwebungs-Audit; dann die heightfield-gekoppelten Höhen-Konsumenten geheilt — `getTerrainHeightAt` ist voxel-aware (`_voxelSurfaceY`), Wasser/Killplane/`findSurfaceAbove`/Kreatur-Spawn/Genesis-Plattform erben die wahre Höhe. Schöpfer-Wahl per AskUserQuestion: **alte Welten zu Voxel migrieren** (der saubere Endzustand). **Offen — Phase 5c: der volle Schnitt** — (a) der Migrations-Flip: eine alte/Eingangs-Welt ohne `worldMeta.voxelTerrain` wird beim Laden voxel-basiert; (b) die Heightfield-Generierung für Voxel-Welten überspringen (heute läuft sie + ruht — wasteful); (c) die Heightfield-Code-Entfernung (`_terrainHeightAtWorld`, `ensureChunkAt`, `generateChunk`, `groundHeightField`, `_setHeightfieldDormant`, `populateChunkVegetation`-Heightfield-Pfad); (d) die Test-Suite-Umschreibung — der Playtest läuft heute auf der Heightfield-Eingangs-Welt; beim Flip werden alle heightfield-spezifischen Invarianten auf Voxel umgeschrieben. Ein Mehr-Schritt-Bogen, kein Schnellschuss — 5c sauber phasieren.
