...
- **Wasserfall-Relikt — V9.62 ✅ erledigt (23.05.2026, drei orthogonale Korrekturen kombiniert nach Schöpfer-Befund „vertikaler Wassersheet altes Relikt").** (a) Top-Lip-Foam-Aufruf aus build-loop entfernt (Schöpfer-Verdikt „ebene Fläche darüber"; V9.60-d.3 ersetzt strukturell). (b) Cross-Geometry: `_buildHydroWaterfall` mit `rotOffset`-Parameter, zweimal aufgerufen (0 und π/2) → X-Form-Volume statt 2D-Folie. (c) Alpha 0.62 → 0.38 + foam 0.33 → 0.45 + Y-Gradient (oben weicher, unten dichter). hydroKind `"waterfall_cross"` ersetzt `"waterfall_lip"`; Mesh-Count bleibt × 4. **Offen für V9.63 (Befund 3 aus Audit)**: Wasserfälle hängen auf Bergkanten ohne echte Schluchten — Worldgen-Terrain-Carve am Wasserfall-Spot. Substantielle Welle.
