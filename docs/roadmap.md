...
**Summe verbleibend**: die grossen Roadmap-Ringe sind gebaut. **Stand nach V8.72**: Atmosphäre ✅ → Multi-User-Tiefe ✅ → Hylomorphismus-Vollausbau ✅ (V8.35) → Welt-Portal-Protokoll ✅ (W12) → souveräne Identität ✅ (W13) → Bibliothek ✅ (W14 Phase 1-3) → Compute-Sharing ✅ (W7 Phase 1-4) → Voice-Sync ✅ (W11 V4) → KI-Übersetzer ✅ (V8.68/V8.69) → Untrusted-Welt-Tor ✅ (V8.70 — eine echte, ungeprüfte fremde Engine läuft null-origin sandgesichert; `worlds/schwarm/` ist der Beweis) → Auto-Vendor-Pfad ✅ (W15 — V8.71 Bündel-Pfad, V8.72 GitHub-Fetch; ein fremdes Repo dockt ohne Handarbeit an). **Offen — der Rest des Fremd-Engine-Bogens**: die Mesh-Welt-Verteilung (W16 — vendorte Welten reisen peer-to-peer, die Spieler tragen die Bibliothek) + die Multiplayer-Sub-Welten (W17).
...
**Stand**: das Untrusted-Welt-Tor (V8.70) ist der Schlüsselstein — eine echte, ungeprüfte fremde Engine läuft null-origin sandgesichert hinter dem Portal (`worlds/schwarm/` beweist es). **W15 — der Auto-Vendor-Pfad — ist gebaut (V8.71 Bündel-Pfad, V8.72 GitHub-Fetch)**: ein fremdes Repo dockt ohne Handarbeit an. Was bleibt: das Tor **verteilt** machen (W16 — vendorte Welten reisen peer-to-peer) und **mehrspielerfähig** (W17 — eine Gruppe taucht gemeinsam in eine fremde Welt, auch in eine, die einen Server braucht). Niemand soll die Bibliothek verlassen müssen, um gemeinsam ein Buch zu lesen.
...
- Stand V8.72: **2629 Invarianten** in `scripts/playtest.cjs` (Headless-Chromium, ~25 s Log-Sammlung) + `audit-strict.cjs` (5 generische Audit-Schichten) + `smoke-multiuser.cjs` + `smoke-webrtc.cjs` + `smoke-translated.cjs` + `smoke-sandbox.cjs` + `smoke-vendor.cjs`
