...
Stand: V9.86 (25.05.2026, **Welle Perf-2 ext.a: drei Heilungen nach V9.85-Browser-Audit, pre-existierender C.3-Flake nebenbei geheilt**). V9.86 in einem Commit: (1) `_remeshVoxelChunksAround` bekommt `skirt`-Parameter — Architektur-Spawn ruft mit `skirt=0`, ~89 % weniger dirty-Chunks → FPS-Drop beim Bauen drastisch reduziert; (2) Mountain-Mulden-Filter via Atmosphären-Konnektivität (ersetzt V9.85-SOLID-darüber-Pattern) — bewahrt Carve-Vision, heilt Bergwand-Wasser, C.3-Flake weg; (3) Klassen-Konstante `AnazhRealm.VERSION` — log() + Init lesen sie, kein hardcoded „V7.82" mehr. Davor: Welle-C-Bogen (V9.71–V9.82) vollendet; V9.83 heilt den CI-Flake; V9.84 baut Welle Perf-1 (sechs Allokations-/LOD-Heilungen); V9.85 baut Welle Perf-2 Kern in vier Sub-Wellen: a Frame-Time-Budget (Streaming-Spikes weg), b Stable Shadow Maps via Texel-Snap (Schatten-Rauschen geheilt, Witcher-3-Lehre), c Mountain-Mulden-Filter (Wasserflecken in Bergwänden weg), d `_voxelGradientNormals` nutzt V9.81-`preDensity`-Grid (~30–45 ms/Chunk gespart). Browser-Audit pendent. **Welle Perf-2 ext.** im Backlog (Macro-Surface-Grid pre-sample + Distance-priorisierte Min-Heap-Streaming) — wartet auf Audit-Befund. **Welle Perf-3** (V9.86+ — Lazy BVH-Collision, Worker-Migration) optional. Voller Plan in `docs/performance.md`. Performance-Welle blockiert die System-Kopplungs-Wellen D/E/F/G — die Vision-Wellen kommen NACH dem Fundament-Schliff, weil sie alle Per-Frame-Arbeit hinzufügen würden. Diese Doc trägt den **Plan vorwärts** — die Ring-Übersicht 0-17, was offen ist, die Meilenstein- und Risiko-Sicht. Die anderen Zeit-Ebenen: der aktuelle Stand + die Gotchas in `CLAUDE.md`, die volle Wellen-Chronik in `docs/handover.md`, die Vision in `docs/state-of-realm.md`.
