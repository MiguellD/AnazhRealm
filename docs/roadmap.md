...
Stand: V9.60-a (23.05.2026). Diese Doc trägt den **Plan vorwärts** — die Ring-Übersicht 0-17, was offen ist, die Meilenstein- und Risiko-Sicht. Die anderen Zeit-Ebenen: der aktuelle Stand + die Gotchas in `CLAUDE.md`, die volle Wellen-Chronik in `docs/handover.md`, die Vision in `docs/state-of-realm.md`.
...
### Welle A — Wasser ↔ Spieler-Wille (Roadmap §1.1 Vision-Pfeiler, A.1 erledigt, A.2..A.5 offen)
...
**A.2 — Generischer Blocker-Index (V9.65, nächste Welle)**: Refactor `state.damIndex` → `state.blockerIndex`. Indexiert ALLE Architekturen mit soliden Material-Tags (stein, holz, metall, kristall — NICHT laub, NICHT magie-Aura). Pro-Part-AABB (jeder `part` einer Compound-Architektur eigenständig indexiert → Baum-Stamm blockiert, Baum-Krone nicht). `_damTopAt` → `_blockerTopAt`. Damm-Bauplan bleibt als built-in (eine Anwendung der Regel). Test-Band wird generisch (testet Damm + Stein-Block + Felsturm gemeinsam). **Vision**: jede solide Bau-Geste blockiert Wasser, nicht nur die mit Namen „damm".

**A.3 — Effective-Surface mit Voxel-Edit-Awareness (V9.66)**: neue Wahrheits-Quelle `_effectiveSurfaceY(x, z)` kombiniert:
  1. Worldgen-Surface (`_terrainMacroSurfaceY`)
  2. + Voxel-Edit-Delta (aus `state.voxelEdits`-Map — fill hebt, carve senkt)
  3. + Blocker-Top (aus `_blockerTopAt`)

Hydrosphäre-Sample-Pfad (`_computeHydrosphere`, `_hydroSeedTarns`, `_hydroBuildCarveIndex`) ersetzt ALLE Aufrufe von `_terrainMacroSurfaceY` durch `_effectiveSurfaceY`. **Vision**: die Hydrosphäre sieht die WIRKLICHE Welt-Geometrie (was der Spieler sieht), nicht die Worldgen-Originale. Voxel-Carve in Trockenland → Mulde, Wasser sammelt sich. Voxel-Fill im Fluss → Blockade. Architektur im Fluss → Umleitung.

**A.4 — Reaktive Recompute mit Debounce (V9.67)**: neue Methode `_recomputeHydrosphere()` ruft `_computeHydrosphere` + `_buildHydrosphereMeshes` + alle Voxel-Chunk-Wasser-Layer neu. Trigger: `spawnArchitecture` (wenn solid), `removeArchitecture` (wenn solid), Voxel-Edit-Pfad. Debounce via `state.hydroDirty = true`-Flag + periodic-Tick-Check (~300ms) → vermeidet Recompute pro Frame bei Multi-Voxel-Carve. Performance-Budget: < 300ms pro Recompute (laut V9.43-b-Diagnose im Playtest).

**A.5 — Aufstauen + Überlauf + visueller Beweis + Tests (V9.68)**: folgt teilweise automatisch aus A.3+A.4 (Priority-Flood findet bei Damm-Blockade einen neuen Pfad → See vergrößert sich oberhalb, neuer Wasserfall am Überlauf). Plus Spieler-Feedback: Welt-Effekt-Ping „Wasser sucht einen neuen Weg", Journal-Eintrag bei `_recomputeHydrosphere`. Plus Playtest-Invarianten: (i) Damm in Fluss → Fluss-Endpunkt ≠ vorher; (ii) Stein-Block in Fluss → dasselbe; (iii) Carve im Trockenland + Voxel-Surface < waterLevel → Wasser-Mesh-Cell entsteht; (iv) Architektur-Remove → Hydrosphäre kehrt zum Original-Pfad zurück. **Vision-Beweis empirisch**: das Wasser kennt jetzt die Welt.

**Erwartetes Ergebnis nach A.5**: Wasser ist nicht mehr Dekoration. Der Spieler experimentiert: baut Damm, sieht Aufstauen; bricht Damm ab, sieht Fließen; carvt Loch, sieht Pfütze. Wasserfälle entstehen wo die echte Topologie (Worldgen + Edits + Bauten) sie ergibt — sekundäre Phänomene, keine prozedurale Schmierschicht.
