...
Stand: 20.05.2026 (V9.33, Voxel-Phase 5c.2.b — der Eingangs-Welt-Flip. Die fresh-Eingangs-Welt ist voxel-default; V9.31 nannte das Versprechen, V9.33 liefert es plus fünf pre-existing Heilungen die der Flip ans Licht zog: `ensureWorldMeta`-Flip + `createNewWorld`-explicit-set + `chunkMap.clear()`-Cleanup-Symmetrie + `generateNewWorld` voxel-Sync + `_buildStaticTriMeshCollision` Partial-Build-Cleanup. Phase 1-5b komplett, Phase 5c-Start (V9.26) + 5c.1 (V9.27) + 5c.1+ (V9.28) + Ammo-Leck-Fix (V9.29) + 5c.2.a Dead-Code (V9.30) + 5c.2.b-Vorbereitung (V9.31) + 5d-Mini Wasserfälle (V9.32) + 5c.2.b Eingangs-Welt-Flip (V9.33) live; in 5c.2 noch offen: 5c.2.c (das schlafende heightfield/heightData/chunkMap-Skelett ablösen — `_terrainHeightAtWorld`, `ensureChunkAt`, `groundHeightField`, `_setHeightfieldDormant`, `populateChunkVegetation`-Heightfield-Pfad). 5/5 Playtest-Runs grün, Audit-Strict 0 Failures. Der Fremd-Engine-Bogen (W12-W17) ist komplett; offen darin nur B-WASM (per-Projekt). Volle Wellen-Historie in `CLAUDE.md`.)
