...
Stand: V9.56-g (23.05.2026). Diese Doc trägt den **Plan vorwärts** — die Ring-Übersicht 0-17, was offen ist, die Meilenstein- und Risiko-Sicht. Die anderen Zeit-Ebenen: der aktuelle Stand + die Gotchas in `CLAUDE.md`, die volle Wellen-Chronik in `docs/handover.md`, die Vision in `docs/state-of-realm.md`.
...
- **Ein zweiter Code-Hygiene-Bogen — UI-Etappe GESCHLOSSEN, Kontrollfluss-Etappe LÄUFT (V9.56-a..g, 23.05.2026)**. UI-Etappe: vier Sub-Wellen a/b/c/d. Kontrollfluss-Etappe: e `fuseWorlds` 239 → 114 + 8 Helfer (PURE Sequenz), f `_tickCreatureTaskDirection` 267 → 8 + 3 Helfer (Task-Dispatcher), g `_voxelChunkGeometry` 269 → 25 + 6 Helfer (Surface-Nets-Pipeline, einseitig gekoppelt). Alle sieben Sub-Wellen verhaltensneutral, playtest grün vor + nach. **Bleibt im Bogen — vier Kontrollfluss-Funktionen** in Größen-Reihenfolge: `processChatCommand` 285 → V9.56-h, `_applyDayNightToScene` 305, `generateTerrainWithParameters` 346, `loadState` 380. **V9.56-e/f/g-Lehren**: (1) Block-Grenzen-Inspektion ist eine HIERARCHIE; (2) Side-Effect-Stream bit-identisch halten; (3) Pipeline-Kopplung ist einseitig → saubere Helfer-Sequenz mit return/mutate-in-place-Pattern. **Disziplin (V9.44-Wiederholung)**: eine Sub-Welle = ein Commit, playtest-grün, verhaltensneutral, kein Bonus-Refactoring im selben Schnitt. **Disziplin (V9.40-Lehre)**: der Bogen gehört terminiert, sobald die nächste Feature-Welle eine dieser Funktionen berührt, nicht „irgendwann".
