...
**Nächste Welle: V13.9 — Wasser-Finish im Browser-Loop (Schöpfer-Wahl A).** Schöpfer-Audit V13.8: (1) Wand-Bluten besteht, aber DÜNN (am Tiefenpuffer-Shader erkennbar → 16-m-Atlas-Spiegel minimal über dem Voxel-Grat → dünnes Spill-Blatt); (2) tiefes Wasser an Strukturen (Turm/Tor); (3) Naht nach Carve. Plan: (a) dünnes Wand-Bluten pro Pixel im Shader cullen (`waterThick` < `uMinDepth`, tunbar, default 0; tiefes Wasser bleibt), live mit dem Schöpfer justiert; (b) Struktur-Tiefwasser diagnostizieren; (c) Carve-Naht. **Erkenntnis (V13.7–.9): Wasser-Ufer ist ein 16-m-Atlas-vs-1,8-m-Voxel-Tradeoff (bleed-frei↔blockig); die Glätte gehört in den Tiefenpuffer-Shader, NICHT in eine 3D-Fluid-Sim (Heilige Lektion); Render NUR im Browser-Loop justieren (Headless ist pixel-blind).** Danach **V13.10** Sub-Region-Edit-Remesh, **V13.11** LOD-Cross-Fade, **V13.12** Schöpfer-Browser-Audit; dann **V14 — die lebendige Welt** (Vision-Pfeiler E/F/G). Der volle V13-Plan steht in **§1.4**.
