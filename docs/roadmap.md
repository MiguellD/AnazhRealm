...
Stand: 17.05.2026 (V8.52, W12 Phase 1+2 KOMPLETT — das Welt-Portal ist real: zwei fremde Engines (three-fluid-fx, ein 2D-Fluid; three.terrain.js, eine 3D-Landschaft) docken über ein generisches DSL-Protokoll an, eine Brücke trägt alle Welten, ein Manifest pro Welt ist das Wörterbuch. Die Welt-Registry + `aimBlueprintAtWorld` machen das Portal-Zielen spieler-erreichbar. Davor: V8.48 Terrain-Schatten, V8.49 Performance-Politur, V8.50 Flaky-Test-Heilung, V8.51 Portal-Skelett. 2291/2291 Invarianten grün, Audit-Strict 0 Failures. Nächste empfohlene Welle: W12 Phase 3 (Ereignisse zurück + native Manifest-Stufe) oder W13 (Vibe-Pass).)
...
| W12 | **Welt-Portal (Bibliothek von Alexandria, V8.23 umbenannt von WebGPU)** — Bauplan-Rolle „portal" + portalMeta (engine + manifest). Sub-Engine-Adapter in iframe/Worker mit Sandbox. DSL als Universal-Bridge zwischen Engines (jede Engine implementiert DSL-Subset). Welt-Manifest-Schema (engine + dsl_subset + signature). PoC mit `three-fluid-fx` (13 KB, klare DSL-Subset, visueller WOW). Detail in `docs/world-portal.md`. | 🟢 **Phase 1+2 live (V8.51+V8.52)** — Phase 1: Portal-Skelett (emergente Rolle „portal", sandboxed iframe, Betreten/Pause/Rückkehr). Phase 2: zwei fremde Engines (three-fluid-fx + three.terrain.js), das generische DSL-Protokoll (Manifest pro Welt, Drei-Stufen-Klarheit), die `WORLD_REGISTRY` + spieler-erreichbares Portal-Zielen, avatar-abgeleitete Tor-Größe. Schöpfer-Browser-Test bestätigt. Phase 3 (Ereignisse zurück + native Manifest-Stufe) offen. | Phase 1+2 ✅, Phase 3 ~2-4 Sessions | W11 ext. + audit:strict |
