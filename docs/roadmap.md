...
Stand: V9.83 (25.05.2026, **CI-Playtest-Flake an der Wurzel geheilt + Performance-Welle eingelegt**). Welle-C-Bogen (V9.71–V9.82) ist vollendet; V9.83 heilt den CI-Flake (rAF-Drosselung im Headless + Per-Chunk-Cost-Anstieg durch die Welle-C-Wellen) via synchroner `_gameLoopTick`-Pumpe im Test-Warmup + PLAYTEST_SECONDS-Budget von 20 → 30 s. **Performance-Welle** (Welle Perf-1 V9.84, Perf-2 V9.85, optional Perf-3 V9.86+) ist als nächster Schritt eingeplant — drei parallele Code-Audits (Allokations-Sturm im Hot-Path, Chunk-Build mit 107 k Density-Samples, Renderer-Settings auf High-End geklemmt) liefern 14 priorisierte Heilungen. Voller Plan in `docs/performance.md`. Performance-Welle blockiert die System-Kopplungs-Wellen D/E/F/G — die Vision-Wellen kommen NACH dem Fundament-Schliff, weil sie alle Per-Frame-Arbeit hinzufügen würden. Diese Doc trägt den **Plan vorwärts** — die Ring-Übersicht 0-17, was offen ist, die Meilenstein- und Risiko-Sicht. Die anderen Zeit-Ebenen: der aktuelle Stand + die Gotchas in `CLAUDE.md`, die volle Wellen-Chronik in `docs/handover.md`, die Vision in `docs/state-of-realm.md`.
...
| **Perf-1** | **Performance-Welle 1 — billige Twists** (V9.84, ~1 Tag, 80 % des Gewinns ohne Welt-Tiefe-Verlust) | C-Brücke ✅ | 1 `isInFrustum` poolt Frustum + Matrix4 · 2 Kreatur-Color-Pool (1 `THREE.Color` pro Emotion) · 3 Task-Direktionen auf `scratchDir/A/B` · 4 `MeshToonMaterial` Singleton statt 625 Instanzen · 5 Shadow-Map 2048→1024 + PCFSoft→PCF · 6 `pixelRatio = Math.min(devicePixelRatio, 1.5)` · 7 Spatial-Hash für Flocking (16×16 Grid) · 8 Distance-LOD: ferne Kreaturen >80 m skippen flocking+raycast | Allokations-Counter im Playtest ≤ N/100 Frames; Browser-Test 60 fps stabil über 60 s; audit:strict 0 Failures |
| **Perf-2** | **Performance-Welle 2 — Chunk-Build-Hebel** (V9.85, ~1 Tag, weitere ~15 %) | Perf-1 ✅ | 9 Macro-Surface 40×40-Grid pre-sample + trilinear Lerp · 10 `_voxelGradientNormals` nutzt `preDensity`-Grid statt resampeln · 11 Frame-Time-Budget statt `MAX_PER_FRAME=1` (build solange `now()-frameStart < 4 ms`) · `scripts/diag-pump.cjs` als Stamm-Mess-Werkzeug wieder aufnehmen | Per-Chunk-Build-Wall-Time ≤ 60 ms (vorher 100–150 ms); Streaming-Ring (81 Chunks) füllt sich in < 5 s auf 60-FPS-Maschine; audit:strict 0 Failures |
| **Perf-3** | **Performance-Welle 3 — tiefe Operationen** (optional V9.86+, je 1–2 Tage) | Perf-2 ✅ + Schöpfer-Wahl | 12 Lazy BVH-Collision: nur Chunks im 2-Ring bekommen sofort, der Rest später (auf Teleport-Trigger) · 13 Chunk-Build in Worker (Density + Iso-Surface) | nur wenn Perf-1+Perf-2 das gefühlte Ruckeln nicht eliminieren |
