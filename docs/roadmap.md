...
Stand: 20.05.2026 (V9.31, Vorbereitung von Phase 5c.2.b — zwei vorgelagerte Bug-Heilungen, NICHT der Flip selbst. Der erste Versuch von 5c.2.b deckte zwei vorbestehende Bugs auf: (1) `_buildStaticTriMeshCollision` (Inseln/Voxel-Chunks) leakte motionState — `_disposeStaticCollision` räumte ihn nicht; mit 81 Voxel-Chunks initial × Welt-Regens kam OOM. (2) Boden-Fehlt-Selbstanalyse war heightfield-only und hätte in einer Voxel-Welt eine Welt-Regen-Death-Spiral ausgelöst. V9.31 heilt beide. Phase 5c.2.b (Eingangs-Welt-Flip + Test-Migration) wartet auf eine eigene saubere Welle. Phase 1-5b komplett, Phase 5c-Start (V9.26) + 5c.1 (V9.27) + 5c.1+ (V9.28) + Ammo-Leck-Fix (V9.29) + 5c.2.a Dead-Code (V9.30) + 5c.2.b-Vorbereitung (V9.31) live; in 5c.2 noch offen: 5c.2.b (Eingangs-Welt-Flip + Test-Rewrite der ~25 heightfield-spezifischen Invarianten), 5c.2.c (das schlafende heightfield/heightData/chunkMap-Skelett ablösen). ~2948 Invarianten grün, Audit-Strict 0 Failures. Der Fremd-Engine-Bogen (W12-W17) ist komplett; offen darin nur B-WASM (per-Projekt). Volle Wellen-Historie in `CLAUDE.md`.)
