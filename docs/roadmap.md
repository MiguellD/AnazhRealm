...
Stand: **V13.6 (30.05.2026) — Wasser-Synergie zurück: die Surface-Nets-Iso wie das Terrain** (Wasser-Profi-Bogen V13.0–.6). Schöpfer-Audit V13.5 fand: das V13.2-Grenzflächen-Meshing (flache Quads, aus Perf) hatte die Synergie mit dem Terrain geopfert („das Terrain hat mehr Flow als das Wasser"). V13.6 holt die **gemeinsame Surface-Nets-Iso** zurück (derselbe Mesher wie der Boden → es fließt) + band-limitiert den Y-Bereich aufs globale `hydroBand` (~28 statt 124 Zellen, ~4-5×; seam-frei) → Synergie UND Perf. Alle Siege bleiben (V13.1-strikte Zellen, V13.3-Flow, V13.5-Tiefen-Shader oben drauf). Headless grün; visuelle Wahrheit (fließt es wie das Terrain?) wartet auf den Schöpfer-Browser-Audit (RDNA-3). Davor: der V12-Genie-Bogen (Three.js r184 + WebGPU, 119 FPS steady) + der V10.0-NodeMaterial/TSL-Bogen. Volle Wellen-Chronik in `docs/handover.md`.

**Nächste Welle: V13.7 — Sub-Region-Edit-Remesh** (nur die vom Carve/Fill berührte Zell-Sektion + ihre Mesh-Sektion neu, nicht der ganze 24×24×124-Chunk → Edit-Drops weg). Danach **V13.8** LOD-Cross-Fade, **V13.9** Schöpfer-Browser-Audit; dann **V14 — die lebendige Welt** (Vision-Pfeiler E/F/G). Der volle V13-Plan steht in **§1.4**.
...
| **V13 Wasser-Profi-Bogen** ⏳ (V13.0–.6 ✅) | **Voxel-Wasser EFFIZIENT + KORREKT + in SYNERGIE mit dem Terrain** (Schöpfer-Browser-Befund 29.05.2026, im Code verifiziert). Wasser bleibt voxel/zell-basiert (reaktiv) + teilt den Terrain-Mesher (Surface-Nets-Iso → fließt), kein Sheet (= Rückschritt). Detail §1.4. | V12 ✅ | **V13.0–.3 ✅** Diagnose · Klassifikation (`_atlasWaterLevelAt`, Hang-Schatten 51,7→0,1 %) · Cache-Buster · Grenzflächen-Meshing · Flow-Bias (D8 `drop/dist`) · **V13.4 ✅** Glättung (zurückgerollt) · **V13.5 ✅** Schicht-3-Tiefen-Shader (`viewportLinearDepth − linearDepth(depth)` → weiche Uferlinie + Tiefen-Farbe + Emotions-Haken) · **V13.6 ✅** Synergie zurück: Surface-Nets-Iso wie das Terrain, band-limitiert aufs `hydroBand` (~28 statt 124 Zellen; der V13.2-Boundary-Mesher hatte die Synergie geopfert) · **V13.7** Sub-Region-Edit-Remesh · **V13.8** LOD-Cross-Fade · **V13.9** Schöpfer-Browser-Audit | Wasser wie die Riesen: kohärent, fließend (teilt den Terrain-Mesher), keine Hang-Schatten, reaktiv (Damm staut). |
...
| **V13.7** ⏳ **next** | Sub-Region-Edit-Remesh: nur die vom Carve/Fill berührten Zellen + ihre Mesh-Sektion neu, nicht der ganze Chunk. | Edit-Drops, „ganzer Chunk neu" |
| **V13.8** | LOD-Cross-Fade: sanftes Überblenden zwischen Detailstufen statt hartem Pop. | „sauberer werden"-Pop |
| **V13.9** | Schöpfer-Browser-Audit RDNA-3: Wasser kohärent + fließend, keine Hang-Schatten, smooth beim Streamen + Editieren. | Bogen-Schluss |
