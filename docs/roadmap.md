...
Stand: **V13.1 (29.05.2026) — V13-Wasser-Profi-Bogen: Diagnose + Klassifikations-Heilung durch.** Der V12-Bogen ist durch (r184-WebGPU, Architektur-Instancing, Streaming-Hitch-Heilung; Schöpfer-Browser 119 FPS steady — Render gelöst). **V13.0** maß die Wasser-Pipeline statt zu spekulieren (`scripts/diag-water-perf.cjs`, 2 Orte). **V13.1** heilte die Klassifikations-Wurzel: `_atlasWaterLevelAt(x,z,terrainTopY)` ersetzt den 16-m-See-Dilatations-Bleed durch exact-Atlas + Depth-Gate (`WATER_RIM_BAND_M`=3.6 m) — der tiefe Hang-Schatten fiel **36,7→4,6 %** (Küste) / **51,7→0,1 %** (Bergsee), der flache Quantisierungs-Rand blieb (kein Mesh-Loch). Worker-Mirror mit-gewandert, Determinismus grün. Verhaltens-Wende ehrlich: Phantom-Carve-Fill weg (war derselbe Mechanismus) → reaktiver Flood-Fill als Backlog hinten in §1.4. **Nächste Welle: V13.2 — Grenzflächen-Meshing** (Surface-Nets ~150 ms → ~1 ms, eliminiert auch den transienten V13.1-OOB-Aufschlag). Danach V13.3 Edit, V13.4 Shader, V13.5 LOD, V13.6 Audit; dann V14 — die lebendige Welt (Vision-Pfeiler E/F/G). Diese Doc trägt den **Plan vorwärts**; der aktuelle Stand + Gotchas in `CLAUDE.md`, die volle Wellen-Chronik in `docs/handover.md`, die Vision in `docs/state-of-realm.md`.
...
| **V13 Wasser-Profi-Bogen** ⏳ (V13.0 ✅) | **Voxel-Wasser EFFIZIENT + KORREKT wie die Riesen** (Schöpfer-Browser-Befund 29.05.2026, im Code verifiziert nicht spekuliert). Wasser bleibt voxel/zell-basiert (V9.69-Vision, reaktiv wie Minecraft) — aber gerendert + klassifiziert wie die Giganten, nicht zurück zu flachen Planes (= Ausweichen). Detail §1.4. | V12 ✅ | **V13.0 ✅** Diagnose (Vorher-Zahl: Mesher ~124–166 ms/Chunk · Schatten 43–54 % = See-Dilatations-Bleed · Edit 0,04 % berührt/100 % gebaut) · **V13.1** Klassifikation: `_atlasWaterLevelAt` ohne 16-m-See-Bleed + Rim-Handling (Hang-Schatten weg) · **V13.2** Grenzflächen-Meshing: Surface-Nets → Wasser-Luft-Boundary-Quads/Greedy (~150 ms → ~1 ms) · **V13.3** Sub-Region-Edit-Remesh · **V13.4** Wasser-Shader-Glanz (Tiefen-Schaum, Gerstner-Ripples, Reflexion) · **V13.5** LOD-Cross-Fade · **V13.6** Schöpfer-Browser-Audit | Wasser positioniert + rendert wie die Riesen (Minecraft/Genshin-Lehre): kohärente Körper, keine Hang-Schatten, fast gratis, schön — UND voll reaktiv (Damm staut). |
...
| **V13.2** ⏳ **next** | Grenzflächen-Meshing: Surface-Nets-Wasser-Iso → Wasser-Luft-Boundary-Quads (Greedy-Mesh, zell-getrieben = reaktiv). Eliminiert auch den V13.1-OOB-`_voxelSurfaceY`-Aufschlag. | ~150 ms → ~1 ms, Streaming-Hitch, Naht |
...
| **V13.x (Backlog, hinten angesetzt)** | **Reaktiver Carve-Fill von echter Quelle** (Welle-A-Vision in korrekter Form): ein Carve, der einen Pfad zu echtem Wasser (Ozean/See/Fluss) öffnet, füllt via Flood-Fill von der Quelle (Minecraft-Wasser-Update). Ersetzt den V9.74-Phantom-„Carve-unter-Pegel-füllt-überall", den V13.1 (zu Recht) entfernt hat. Plus optionaler V13.1-Rest: der ~4,6 %-Küsten-Rest-Schatten (Überhang/Steilküste, wo „höchste solide Cell" die Wasser-Auflage überschätzt) — Konnektivitäts-/per-Column-Verfeinerung. | „Pond füllt sich", Steilküsten-Rest |
