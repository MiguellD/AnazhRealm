...
Stand: 20.05.2026 (V9.32, Voxel-Phase 5d-Mini — Wasserfälle aus der Voxel-Klippen-Steilheit. Eine voxel-basierte Welt hatte seit V9.28 keine Wasserfälle (heightfield-Loop voxel-gegated); V9.32 baut den Voxel-Pendant `_buildVoxelChunkWaterfalls`: 6×6-Sample-Raster pro Voxel-Chunk, Steilheit aus `_voxelSurfaceY`-Gradient (4 Nachbar-Punkte, 4 m Distanz), Partikel-Wasserfall bei ≥4 m Höhen-Drop. Partikel teilen `state.waterfalls`-Animation mit Heightfield; Pro-Chunk-Lifecycle analog Gras + Vegetation. Vierte Voxel-Chunk-Schicht. Phase 1-5b komplett, Phase 5c-Start (V9.26) + 5c.1 (V9.27) + 5c.1+ (V9.28) + Ammo-Leck-Fix (V9.29) + 5c.2.a Dead-Code (V9.30) + 5c.2.b-Vorbereitung (V9.31) + 5d-Mini Wasserfälle (V9.32) live; in 5c.2 noch offen: 5c.2.b (Eingangs-Welt-Flip + Test-Rewrite der ~25 heightfield-spezifischen Invarianten), 5c.2.c (das schlafende heightfield/heightData/chunkMap-Skelett ablösen). ~2954 Invarianten grün, Audit-Strict 0 Failures. Der Fremd-Engine-Bogen (W12-W17) ist komplett; offen darin nur B-WASM (per-Projekt). Volle Wellen-Historie in `CLAUDE.md`.)
