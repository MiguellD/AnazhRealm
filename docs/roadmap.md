...
Stand: V9.56-f (23.05.2026). Diese Doc trägt den **Plan vorwärts** — die Ring-Übersicht 0-17, was offen ist, die Meilenstein- und Risiko-Sicht. Die anderen Zeit-Ebenen: der aktuelle Stand + die Gotchas in `CLAUDE.md`, die volle Wellen-Chronik in `docs/handover.md`, die Vision in `docs/state-of-realm.md`.
...
- **Ein zweiter Code-Hygiene-Bogen — UI-Etappe GESCHLOSSEN, Kontrollfluss-Etappe LÄUFT (V9.56-a..f, 23.05.2026)**. UI-Etappe: vier Sub-Wellen a/b/c/d (1005 Z. → 56 Z. Orchestrator-Summe). Kontrollfluss-Etappe: e `fuseWorlds` 239 → 114 + 8 Helfer (PURE Sequenz), f `_tickCreatureTaskDirection` 267 → 8 + 3 Helfer (Task-Dispatcher mit Phasen-Logik in Helfern). Alle sechs Sub-Wellen verhaltensneutral, playtest grün vor + nach. **Bleibt im Bogen — fünf Kontrollfluss-Funktionen** in Größen-Reihenfolge: `_voxelChunkGeometry` 269 → V9.56-g, `processChatCommand` 285, `_applyDayNightToScene` 305, `generateTerrainWithParameters` 346, `loadState` 380. **V9.56-e/f-Lehre**: Kontrollfluss ist NICHT pauschal riskanter als UI — die Block-Grenzen-Inspektion ist eine HIERARCHIE; such die höchste Ebene mit sauberer Trennung. Side-Effect-Stream-Reihenfolge (Math.random/Date.now/log/state-mutations) MUSS bit-identisch bleiben. **Disziplin (V9.44-Wiederholung)**: eine Sub-Welle = ein Commit, playtest-grün, verhaltensneutral, kein Bonus-Refactoring im selben Schnitt. **Disziplin (V9.40-Lehre)**: der Bogen gehört terminiert, sobald die nächste Feature-Welle eine dieser Funktionen berührt, nicht „irgendwann".
