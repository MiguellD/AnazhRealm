...
Stand: V9.56-d (23.05.2026). Diese Doc trägt den **Plan vorwärts** — die Ring-Übersicht 0-17, was offen ist, die Meilenstein- und Risiko-Sicht. Die anderen Zeit-Ebenen: der aktuelle Stand + die Gotchas in `CLAUDE.md`, die volle Wellen-Chronik in `docs/handover.md`, die Vision in `docs/state-of-realm.md`.
...
- **Ein zweiter Code-Hygiene-Bogen — UI-Builder-Etappe GESCHLOSSEN (V9.56-a/b/c/d, 23.05.2026)**. Vier Sub-Wellen: a `renderPlayerEquipUI` 213 → 19 + 7 Helfer (median 31), b `renderSoulEditorUI` 232 → 13 + 4 Helfer (median 48), c `renderLibraryUI` 241 → 9 + 5 Helfer (median 47, erste Schleifen-Komposition), d `_workshopRenderStatsPanel` 319 → 15 + 6 Helfer (median 50.5). **Bogen-Statistik**: 1005 Z. → 56 Z. Orchestrator-Summe, 21 neue benannte Helfer, Datei netto +37 Z. Alle vier verhaltensneutral, playtest grün vor + nach jeder Sub-Welle. **Bleibt im Bogen — Kontrollfluss-Etappe** (sieben Funktionen, strukturell anders: Schleifen + State-Mutation + Side-Effects, Disziplin verschärft sich): `fuseWorlds` → V9.56-e (kleinste), `_tickCreatureTaskDirection` 267, `_voxelChunkGeometry` 269, `processChatCommand` 285, `_applyDayNightToScene` 305, `generateTerrainWithParameters` 346, `loadState` 380. **Disziplin (V9.44-Wiederholung)**: eine Sub-Welle = ein Commit, playtest-grün, verhaltensneutral, kein Bonus-Refactoring im selben Schnitt; vom sichersten zum riskantesten. **Disziplin (V9.40-Lehre)**: der Bogen gehört terminiert, sobald die nächste Feature-Welle eine dieser Funktionen berührt, nicht „irgendwann".
