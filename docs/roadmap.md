...
Stand: **V13.0 (29.05.2026) — V13-Wasser-Profi-Bogen gestartet, Diagnose-Welle durch.** Der V12-Bogen ist durch (r184-WebGPU, Architektur-Instancing, Streaming-Hitch-Heilung; Schöpfer-Browser 119 FPS steady — Render gelöst). **V13.0** hat die Wasser-Pipeline gemessen statt spekuliert (`scripts/diag-water-perf.cjs`, 2 Orte) — die Vorher-Zahl der drei Wurzeln: (1) Mesher: Surface-Nets über ~90 400 Cells = **~124–166 ms/Chunk** (statt der alten 78-ms-Schätzung) für nur ~7–10k Output-Dreiecke → V13.2 Grenzflächen-Meshing zielt ~1 ms; (2) Klassifikation: **43–54 %** aller WATER-Cells sind der **16-m-See-Dilatations-Bleed** in `_waterLevelAt` (V9.73, 3×3-MAX um See-Cells) — NICHT der Ozean-Default (nur 0–2 %); → V13.1 `_atlasWaterLevelAt` mit Rim-Handling; (3) Edit: ~0,04 % der Cells berührt, 100 % gebaut → V13.3 Sub-Region. **Nächste Welle: V13.1 — Klassifikations-Heilung.** Danach V13.2 Meshing, V13.3 Edit, V13.4 Shader, V13.5 LOD, V13.6 Audit; dann V14 — die lebendige Welt (Vision-Pfeiler E/F/G). Diese Doc trägt den **Plan vorwärts**; der aktuelle Stand + Gotchas in `CLAUDE.md`, die volle Wellen-Chronik in `docs/handover.md`, die Vision in `docs/state-of-realm.md`.
...
| **V13.1** ⏳ | Klassifikations-Heilung: `_atlasWaterLevelAt` ersetzt den 16-m-See-Dilatations-Bleed durch exact-Atlas/feines Rim-Handling — WATER nur wo der Atlas Wasser markiert (+ V9.86-Konnektivität). **ACHTUNG (V13.0-Befund):** die V9.73-3×3-Dilatation schloss die 16-m-quantisierte Becken-Rand-Lücke; platte Streichung → Mesh-Lücken am See-Rand. Braucht ein feineres Rim-Handling (z.B. sub-cell Atlas-Sample oder Falloff statt vollem 16-m-Bleed). | Wasserschatten an Hängen/Strukturen (43–54 % der WATER-Cells) |
