...
## Aktueller Stand (V9.47, 22.05.2026)
...
**Letzte Arbeit — fluviale Erosion (V9.47):** gegen die see-dominante Welt (die Becken sind 12-20 m tief, echte Topographie). Ein erster Tröpfchen-Erosions-Versuch hatte einen Zielkonflikt (Becken entwässern ⟺ Gipfel abtragen). Der Profi-Weg ist **Stream-Power-Inzision** (Braun & Willett / Fastscape): `_computeErosion` lässt Terrain + Drainage über 36 Iterationen ko-evolvieren — je Iteration Priority-Flood → Flow-Accumulation → Inzision `Δh = k·A^m·S^n`, aber NUR in Kanälen (`accum ≥ channelMinArea`). Grate/Hänge bleiben unberührt, Täler schneiden tief → kein Zielkonflikt. Das Delta fliesst in `_terrainMacroSurfaceY`, die Hydrosphäre routet danach auf dem erodierten Gelände. Ergebnis: Gipfel exakt erhalten (61→61 m), Relief erhalten, See-Fläche halbiert, dendritische Tal-Netze gecarvt, Flüsse länger + offener.
...
**Nächste Welle:** offen — `docs/roadmap.md` §1 listet die Kandidaten (Crafting-Tiefe, der Playtest-Pflege-Bogen, ein zweiter Code-Hygiene-Bogen, ein vereintes Wasser-System); keine ist terminiert. Offen-kosmetisch: See-Ufer-Schaum, Flow-Speed nach Gefälle. Ehrlich offen — die fluviale Inzision carvt einen Kanal DURCH ein breites Becken, leert es aber nicht restlos (der Becken-Boden bleibt ein kleinerer See — geomorphologisch korrekt; `docs/hydrosphere.md` §10b).
...
- **Architekturkern:** Eine `AnazhRealm`-Klasse in `anazhRealm.js` (~36 000 Zeilen, Stand V9.47). State in `localStorage` + optional `anazhRealmState.json` über save-server. Globale Referenz: `window.anazhRealm`.
...
| `scripts/playtest.cjs` | Headless-Playtest mit **~3000 Invarianten** als CI-Gate (Stand V9.47; der Konsolen-Zähler driftet ±2-3 je Lauf — „Alle Invarianten OK" ist die Wahrheit) |
