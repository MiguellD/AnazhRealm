...
Letzter Stand: Ring 1 + **Ring 2 vollständig (Phase 1-7)** + **Ring 3 V1+V2** + **Ring 4 V1** live. Chunk-Physik komplett auf `btBvhTriangleMeshShape` (Commit `e612c60`) — visuelles Mesh = Kollisionsnetz. 120 fps im echten Browser, **83/83 Playtest-Invarianten grün**. Mensch ↔ Nexus ↔ Welt: eine Sprache, eine Sicherheits-Wand, ein Lernen-aus-Outcomes-Loop, ein bidirektionaler Emotions-Kanal, **eine Klangschicht**. Vier von fünf Vision-Pfeilern haben jetzt eine V1. Nächster Schritt: Ring 5 (`createPlayerSoul`), Ring 6 (architectureTemplates), oder Ring 7 (brain.js — würde auch CSP-`unsafe-eval` ablösen).
...
- `npm run playtest` startet save-server + Headless-Chromium, sammelt 20-25 s Logs, prüft **83 Invarianten** (inkl. Grok-Stimme, DSL-Effekte, Naht-Treue, CSP, Selektion, Emotionen, Symphonie-Audio-Graph), exit 1 bei Verletzung.
