...
Letzter Stand: Ring 1 + **Ring 2 (P1-7)** + **Ring 3 V1+V2** + **Ring 4 V1** + **UI V1+V2** live. Chunk-Physik komplett auf `btBvhTriangleMeshShape` (Commit `e612c60`). 120 fps im echten Browser, **132/132 Playtest-Invarianten grün**. Architektur: eine Sprache (DSL), eine Sicherheits-Wand (CSP + CI-Gate), ein Selektions-Loop (Fitness-V2), ein bidirektionaler Emotions-Kanal, eine Klangschicht (Web Audio), eine painterly-Bedien-Oberfläche (vendored Cinzel-Fonts, Tag/Nacht-Tokens, Topbar+6-Drawer, einklappbare Konsole, Brass-Scrollbars). Alle fünf Vision-Pfeiler haben jetzt eine V1+. **Nächster Schritt**: Ring 5 (`createPlayerSoul` — Spieler wählt Form: Mensch/Phönix/Drache/Riese), Ring 6 (`architectureTemplates` — Dörfer/Tempel als DSL-Primitives), oder UI V3 (Astrolabium-Live-Element + Custom-Slider-Rail/Knob als Vollendung des Mockup-Vertrags).
...
- `npm run playtest` startet save-server + Headless-Chromium, sammelt 20-25 s Logs, prüft **132 Invarianten** (inkl. Grok-Stimme, DSL-Effekte, Naht-Treue, CSP, Selektion, Emotionen, Symphonie-Audio-Graph, UI-V2-Topbar/Tabs/Drawer/Konsole), exit 1 bei Verletzung.
