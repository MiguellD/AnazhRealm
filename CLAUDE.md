...
| 5 | `createPlayerSoul` (Mensch/Phönix/Drache) | ✅ V1 live — drei Seelen, rein visuell (Geometrie + Farbe), Physik-Body unverändert. Drawer-Dropdown im Spieler-Panel (`#player-soul-select`) + Status-Bar-Item, Chat-Pattern `werde mensch/phönix/drache` → DSL-Op `player_soul`, Save persistiert `playerSoul`. Bewusst NICHT im `dslComposeAtomic`-Pool. |
...
Letzter Stand: Ring 1 + **Ring 2 (P1-7)** + **Ring 3 V1+V2** + **Ring 4 V1** + **Ring 5 V1** + **UI V1+V2** live. Chunk-Physik komplett auf `btBvhTriangleMeshShape` (Commit `e612c60`). 120 fps im echten Browser, **155/155 Playtest-Invarianten grün**. Architektur: eine Sprache (DSL), eine Sicherheits-Wand (CSP + CI-Gate), ein Selektions-Loop (Fitness-V2), ein bidirektionaler Emotions-Kanal, eine Klangschicht (Web Audio), drei wählbare Spieler-Seelen, eine painterly-Bedien-Oberfläche (vendored Cinzel-Fonts, Tag/Nacht-Tokens, Topbar+6-Drawer, einklappbare Konsole, Brass-Scrollbars). **Nächster Schritt**: Ring 6 (`architectureTemplates` — Dörfer/Tempel als DSL-Primitives), Ring 7 (brain.js — würde CSP-`unsafe-eval` ablösen), oder UI V3 (Astrolabium-Live-Element + Custom-Slider-Rail/Knob als Vollendung des Mockup-Vertrags).
...
- **`state.player.soul`** (Ring 5) ist die aktive Seele, einer aus `playerSoulDefs` (human/phoenix/dragon). `applyPlayerSoul(name)` ist der einzige Mutations-Pfad: kanonisiert (Deutsch/Englisch/Umlaut), tauscht `playerMesh.geometry` (alte wird disposed) und `material.color`, lässt **Position + Scale + Ammo-Body** unangetastet — keine Hitbox-Anpassung in V1. UI: `<select id="player-soul-select">` im Spieler-Drawer + `#status-soul`-Item in der Status-Bar; beide werden bidirektional synchronisiert. DSL-Op `player_soul` ist verfügbar, aber bewusst NICHT im `dslComposeAtomic`-Pool: Identität ist Spieler-Entscheidung.
...
- `npm run playtest` startet save-server + Headless-Chromium, sammelt 20-25 s Logs, prüft **155 Invarianten** (inkl. Grok-Stimme, DSL-Effekte, Naht-Treue, CSP, Selektion, Emotionen, Symphonie-Audio-Graph, UI-V2-Topbar/Tabs/Drawer/Konsole, Spieler-Seele), exit 1 bei Verletzung.
