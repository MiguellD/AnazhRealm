...
## Aktueller Stand (V9.45-b, 22.05.2026)
...
**Letzte Arbeit — der Gelände- & Wasser-Bogen (V9.45-a..b):** ein Schöpfer-Befund-Bogen gegen monotones Gelände + zwei-Schichten-Wasser. **V9.45-a** gab `_terrainMacroSurfaceY` die zwei Profi-Werkzeuge prozeduraler Welten — Domain-Warping (mäandernde Kämme statt gitter-paralleler Grate) + ein regionales Erosions-Feld (manche Striche Tiefland, andere alpin — gegen „alle Berge gleich hoch") — plus grössere Amplituden und eine höhere Voxel-Chunk-Hülle (base−66..base+107, zentralisiert über `_voxelChunkConfig.floorDrop`). **V9.45-b** heilte drei Wasser-Bugs: das Seebecken ist jetzt ein flach gesculpteter, wasserdichter Topf (`_hydrosphereLakeAt` blendet die Dichte zur Bett-Ebene `bedY`), dessen Boden garantiert über dem Meeresspiegel liegt → die globale Meeres-Plane scheint nicht mehr als zweites Wasser-Layer durch, und man taucht nicht mehr durch den See-Boden; die See-Plane deckt einen 1-Zellen-Ring extra → das opake Ufer-Terrain schneidet die Wasserlinie sauber.
...
- **Architekturkern:** Eine `AnazhRealm`-Klasse in `anazhRealm.js` (~36 000 Zeilen, Stand V9.45-b). State in `localStorage` + optional `anazhRealmState.json` über save-server. Globale Referenz: `window.anazhRealm`.
...
| `scripts/playtest.cjs` | Headless-Playtest mit **~3000 Invarianten** als CI-Gate (Stand V9.45-b; der Konsolen-Zähler driftet ±2-3 je Lauf — „Alle Invarianten OK" ist die Wahrheit) |
