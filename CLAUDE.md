...
## Aktueller Stand (V9.48, 22.05.2026)
...
**Letzte Arbeit — Hydrosphäre-Politur (V9.48):** die letzte kosmetische Naht des Wasser-Bogens. Zwei Mikro-Politur-Reste geschlossen: (1) **See-Ufer-Schaum** — `_lakeShoreFoamField` baut per Multi-Source-BFS ein Zell-Distanz-Feld von der Wasserlinie ins Becken, `_buildLakeMesh` mittelt es auf die Quad-Ecken (neues `aShore`-Vertex-Attribut), der Shader rendert ein wellen-pulsierendes Schaum-Band am See-Rand. (2) **Flow-Speed nach Gefälle** — `_buildRiverRibbon` rechnet je Fluss-Punkt das mittlere Gefälle (aus `ry`/`cum`) in einen Speed-Faktor ∈ [0.55, 2.2], der im BETRAG des `aFlow`-Vektors reist; der Shader liest `length(aFlow)` als Speed → ein steiles Segment scrollt den Schaum schneller als ein flacher Lauf. Kein eigener Bogen — eine optionale Nicety, 9 Invarianten grün.

**Davor — fluviale Erosion (V9.47):** gegen die see-dominante Welt (die Becken sind 12-20 m tief, echte Topographie). Ein erster Tröpfchen-Erosions-Versuch hatte einen Zielkonflikt (Becken entwässern ⟺ Gipfel abtragen). Der Profi-Weg ist **Stream-Power-Inzision** (Braun & Willett / Fastscape): `_computeErosion` lässt Terrain + Drainage über 36 Iterationen ko-evolvieren — je Iteration Priority-Flood → Flow-Accumulation → Inzision `Δh = k·A^m·S^n`, aber NUR in Kanälen (`accum ≥ channelMinArea`). Grate/Hänge bleiben unberührt, Täler schneiden tief → kein Zielkonflikt. Das Delta fliesst in `_terrainMacroSurfaceY`, die Hydrosphäre routet danach auf dem erodierten Gelände. Ergebnis: Gipfel exakt erhalten (61→61 m), Relief erhalten, See-Fläche halbiert, dendritische Tal-Netze gecarvt, Flüsse länger + offener.

**Davor — lange Flüsse (V9.46):** Flüsse fliessen durch Seen HINDURCH als eine logische Polylinie (`_hydroExtractRivers` walkt `flowTo` durch die See-Zellen; `inLake`-Punkte). Davor der Gelände- & Wasser-Bogen V9.45-a..c (Domain-Warp + Erosions-Feld gegen monotones Gelände; Seebecken als wasserdichte Töpfe `_hydrosphereLakeAt`; Auftrieb an die See-Plane gekoppelt) + das Wasser-Ultiversum V9.43-b..e (die Hydrosphäre — `docs/hydrosphere.md`) + der Stamm-Pflege-Bogen V9.44.

**Nächste Welle:** V9.49 — das **vereinte Wasser-System** (Architektur-Bogen, in Arbeit). Schöpfer-Befund: das Wasser „schliesst nicht" — Meer/Seen/Flüsse/Wasserfälle sind vier getrennte durchscheinende Mesh-Schichten („globale Platte + Ausnahmen"). V9.49 wechselt das Verfahren: Wasser wird ein **Feld** — das Priority-Flood-`filled`-Feld ist schon eine kontinuierliche Wasser-Oberfläche; ein spieler-folgendes Höhenfeld-Mesh, das sich in die gecarvten Kanäle senkt, ersetzt Meeres-Plane + See-Planes + Fluss-Ribbons (ein Mesh, ein Shader). Wasserfall bleibt die vertikale Ausnahme. Plan: `docs/roadmap.md` §3, Design + Lernschlüsse: `docs/hydrosphere.md` §12.
...
- **Architekturkern:** Eine `AnazhRealm`-Klasse in `anazhRealm.js` (~36 000 Zeilen, Stand V9.48). State in `localStorage` + optional `anazhRealmState.json` über save-server. Globale Referenz: `window.anazhRealm`.
...
- Es gibt EINE globale Meeres-Plane bei `state.waterLevel`. Jede separate See-Wasserfläche braucht ein festes, flach gesculptetes Bett ≥ `waterLevel` (`_hydrosphereLakeAt` liefert `{bedY, w}`, `_terrainDensityAt` blendet die Dichte zur Bett-Ebene) — sonst scheint die Meeres-Plane als zweite Wasserschicht durch (V9.45-b-Bug-Klasse). Der See-Blend gilt VOR den Voxel-Edits → ein Spieler-Edit im Becken gewinnt. Wer die See-Plane-Region ändert (Dilatation, Absorptions-Schwelle), MUSS `_hydroWaterLevelAt` (Auftrieb) mitziehen — Optik + Physik lesen EINE Region (`lakeNear`), sonst klafft „gesehen ≠ gespürt" am Ufer (V9.45-c-Lehre).
...
- Das geteilte `_ensureHydroSurfaceMaterial` speist ZWEI Geometrien — See-Plane (`_buildLakeMesh`) + Fluss-Ribbon (`_buildRiverRibbon`). Ein neues per-Vertex-Attribut MUSS auf BEIDEN gesetzt werden, sonst liest der ShaderMaterial bei der einen Geometrie Müll (V9.48-Disziplin: `aShore` ist auf dem Ribbon explizit 0). Und seit V9.48 trägt der BETRAG von `aFlow` die Flow-Speed nach Gefälle (∈ [0.55, 2.2]) — der Vektor ist NICHT mehr normiert; wer `aFlow` liest, nimmt `length==1` nicht an, sondern teilt für die Richtung durch `length`.
...
| `docs/hydrosphere.md` | **Wasser-Design** — das Drainage-Netz (Flüsse/Seen/Wasserfälle/Meer als ein Hydrosphären-System). Algorithmus, Datenstrukturen, Wellen-Schnitt V9.43-b..e + der V9.45-b-See-Schliff (§8 — Seebecken als wasserdichte Töpfe). Offen nur eine kosmetische Politur-Naht. |
...
| `scripts/playtest.cjs` | Headless-Playtest mit **~3000 Invarianten** als CI-Gate (Stand V9.48; der Konsolen-Zähler driftet ±2-3 je Lauf — „Alle Invarianten OK" ist die Wahrheit) |
