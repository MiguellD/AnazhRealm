...
**Letzte Arbeit — das vereinte Wasser-System (V9.49, Architektur-Bogen):** Schöpfer-Befund — das Wasser „schliesst nicht" (Meer/Seen/Flüsse/Wasserfälle als gestapelte durchscheinende Sheets). Wurzel: vier getrennte Render-Schichten. V9.49 wechselt das Verfahren — **Wasser ist ein Feld, kein Körper.** (a) `_hydroBuildWaterField` legt das Feld `{waterY, waterKind}` auf `state.hydrosphere.water`: `waterY` ist der flache Spiegel je Wasser-Körper (Ozean → `waterLevel`, See → `lake.level`, trockene Zelle → Meeresspiegel-Default), `waterKind` die Klassifikation (0 Land/1 Ozean/2 See). (b) `_buildUnifiedWaterMesh` baut EIN spieler-folgendes Höhenfeld (~384 m, 3-m-Raster, gesnappt) — ersetzt Meeres-Plane + See-Planes + Fluss-Ribbons; nasse Quads aus dem Feld (Ozean/See) + dem `riverBuckets`-Index (Fluss). Ein Höhenfeld kann sich nicht selbst überlappen → kein Sheet-Stapeln, strukturell. (c) `_ensureHydroSurfaceMaterial` ist EIN Shader: Gerstner (Ozean, `aWave`-moduliert), Still-Schimmer, Flow-/Ufer-Schaum, `depthWrite:true`. (d) V9.49-e schliesst die Naht (Schöpfer-Audit „Wasser/Ufer/Flusslauf in der Luft"): das Wasser-Mesh berührt die Uferlinie NICHT — es taucht unter das opake Terrain, das die Wasserlinie als emergenten Schnitt schneidet (`depthWrite:true`). Der trockene Rand-Skirt sitzt auf dem Meeresspiegel-Default und taucht ab; der Fluss liegt im gecarvten Bett (`_terrainMacroSurfaceY − 0.4·D`). `terrainY` ist damit wieder entfallen. Der Wasserfall bleibt die vertikale Ausnahme. (e) V9.49-f klemmt die Fläche an die Terrain-Wahrheit (zweiter Audit — „die Fläche durchschiesst einen Hügel ins nächste Tal"): nass nur, wo `_terrainMacroSurfaceY < Spiegel + 12 m` — die nasse Maske liest jetzt je Vertex das Gelände, kein Grat-Bleed mehr; `polygonOffset` gegen Schnitt-Flimmern. Volles Design + Lernschlüsse: `docs/hydrosphere.md` §12 + §13, Chronik: `docs/handover.md`.

**Davor — Hydrosphäre-Politur (V9.48):** zwei kosmetische Mikro-Politur-Reste des V9.43-Wasser-Bogens — See-Ufer-Schaum (`_lakeShoreFoamField`) + Flow-Speed nach Gefälle. (V9.49 hat den Ufer-Schaum-BFS wieder abgelöst — im Feld ist „Ufer" gratis aus der Wasser-Tiefe.)
...
**Nächste Welle:** offen. V9.49-f wartet zuerst auf den Schöpfer-Browser-Audit (jede Wasser-Welle wird im Browser geprüft, bevor die nächste startet — Headless prüft Mechanik, das Schöpfer-Auge die Ästhetik). Danach: V9.49-g (der Tarn-Pass — Bergseen als additive Hochmulden, `hydrosphere.md` §13.6), oder die offenen Roadmap-Kandidaten (`docs/roadmap.md` §1 — Crafting-Tiefe, der Playtest-Pflege-Bogen, ein zweiter Code-Hygiene-Bogen, die Becken-Restleerung `hydrosphere.md` §10b).
...
| `docs/hydrosphere.md` | **Wasser-Design** — das Drainage-Netz (Flüsse/Seen/Wasserfälle/Meer als ein Hydrosphären-System). Algorithmus, Datenstrukturen, Wellen-Schnitt V9.43-b..e + der V9.45-b-See-Schliff (§8) + **§12: das vereinte Wasser-System (V9.49 — Wasser ist ein Feld, ein Höhenfeld-Mesh, ein Shader)**. |
