...
## Aktueller Stand (V9.52-c, 23.05.2026)
...
**Letzte Arbeit — Playtest-Pflege-Bogen Sub-Welle c (V9.52-c):** Band 2 ist extrahiert. Von der ehemaligen 31 574-Zeilen-IIFE sind jetzt die ersten ~16 143 Zeilen der `### Invarianten`-Sektion (95 Original-Sektionen) in 23 benannte Funktionen aufgeteilt — die HALBE Test-Datei ist strukturiert. Sub-Welle c brachte 8 weitere Band-Funktionen (Welle 6.A bis Ende Welle 6.H Phase 2C, ~8400 Zeilen, 53 Sektionen): `checkBandWelle6APolish` (6.A1-A5 + 6.E1/E2 + 6.F1/F2 Interaktions-Politur), `checkBandWelle6DSoul` (alle 6.D Etappen + Reflexions-Fixes, das Soul/Stats/Equipment-System), `checkBandWelle6GHylomorphism` (6.G P1.5 + P2 + P3 + P4 — Hylomorphismus-Unification, Welt-Affinität, Felsen, Terrain-als-Materie), `checkBandVoxelTerrainCore` (Voxel-Terrain-Bogen P1+P2b + V9.x-Lösch-Beweise), `checkBandHydrosphere` (V9.43-a/b/c/c.2/d/e + V9.47 Erosion + V9.50-b Chunk-Wasser), `checkBandVoxelP3AndInventory` (Voxel P3/P3b/P3c 3D-Graben/Aufschütten/Material-Kreis + 6.C1/C2 Inventar+Spielmodi), `checkBandWelle6Keybindings` (6.A6 + 6.C3 Tastenbelegung), `checkBandWelle6HCreatures` (6.H Kreatur-Aufträge + Compounds + Bring-Phase + Material-Konsum). Je 475-1567 Zeilen. Plan-treu zu §5. Mechanisches `/tmp/extract-band2-c.cjs`-Skript (direkte Adaption des b-Skripts mit neuer Band-Tabelle). Verhaltens-identisch bewiesen (3060-Invarianten-Satz bit-identisch, nur Drift-Bänder). Datei: 31 304 → 30 832 (−472 netto). Orchestrator (Zeile ~15643) führt jetzt 3 a-Calls + 11 b-Calls + 8 c-Calls. Sub-Wellen d-f ziehen Welle 6.X Audit-Fixes + 6.G3/G4 Atmosphäre + V8.x + W12-W17 Welt-Portal-Bogen + die End-Rings (Ring 3/4/5/6) + Helfer-Durchzug nach. Voller Plan: `docs/playtest-hygiene.md`.
...
**Nächste Welle:** Playtest-Pflege Sub-Welle d — Band 3 (Welle 6.X Audit-Fixes + 6.G3/G4 Welt-Lebendigkeit/Atmosphäre + V8.24-V8.93 Politur + W12-W17 Welt-Portal-Bogen + KI-Übersetzer + V8.70-V8.72 Untrusted-Tor/Vendor, ~55 Sektionen, Zeilen ~16144 bis ~22300+). Das c-Skript ist die Vorlage. Daneben offen + auf den Schöpfer-Browser-Audit wartend: V9.50 + V9.51 (jede Wasser-Welle wird im Browser geprüft). Weiter dahinter: die offenen Roadmap-Kandidaten (`docs/roadmap.md` §1 — System-Kopplungen ab „Wasser ↔ Spieler-Wille" als nächste Vision-Vertiefung, Crafting-Tiefe, ein zweiter Code-Hygiene-Bogen).
