...
## Aktueller Stand (V9.53, 23.05.2026 — Drift-Heilung im Playtest, die Namen-Liste ist jetzt deterministisch)

**Wo wir stehen:** eine Voxel-basierte 3D-Browser-Sandbox in einer Datei (`anazhRealm.js`, ~36 000 Zeilen). **3060 Playtest-Invarianten grün, die geordnete Namen-Liste ist seit V9.53 BIT-IDENTISCH zwischen Läufen** (war ±2-3 lauf-abhängig, drei aufeinanderfolgende Läufe bewiesen die Determinismus-Wende). Audit-Strict 0 Failures.

**Letzte Arbeit — V9.53 Drift-Heilung im Playtest:** Reflexions-Antwort auf den V9.52-f-Befund („timing/robustheit NICHT verbessert"). Der Test-Name war 9× variabel — perf-ms-Werte und worldgen-Counts steckten als `${X}` IN den Test-Namen statt im Detail-String. Jede Welt-Generation würfelte minimal andere Zahlen → die geordnete Invarianten-Namen-Liste war PRINZIPIELL nie bit-identisch zwischen Läufen, jeder Diff zeigte „Drift" statt „echte Regression". V9.53 verschiebt die 9 driftenden Werte aus dem Test-Namen-Argument in den Detail-String-Argument (3. `check()`-Argument, zeigt nach `—` in der Konsole). Konkret: `Welle 6.G P1.5: Worldgen-Bäume sind in state.architectures` (war: `… (n=${count}) sind …`), `… Architekturen über Affinity gespawnt` (war: `… (n=${count})`), `… populatedChunks-Cache befüllt` (war: `… (size=${size})`), `Voxel V9.47: _computeErosion im Perf-Budget (< 600 ms)` (war: `… (${ms} ms < 600)`), `Voxel V9.43-b: Berechnung im Perf-Budget (< 500 ms)`, `Voxel V9.43-d: der Carve ist im Perf-Budget (50k Aufrufe < 300 ms)`, `Voxel V9.51: der Tarn-Pass setzt Bergsee-Mulden`, `Welle 6.G P1.5: initiale Welt-Inseln haben Kollision`, `W6.G P3: Felsbogen-Kollision … (n=${count})`. **Verhaltens-Beweis**: drei aufeinanderfolgende Läufe nach dem Schnitt — `diff` zeigte ZERO Diff in 3060 Invarianten-Namen. Die Test-Disziplin-Wende: ein Lauf-Diff bedeutet jetzt ECHTE Regression, nicht harmlosen Zahlen-Drift. Datei: 29 554 → 29 563 (+9 Zeilen — eine pro Detail-String). **Was BLEIBT** (und außerhalb dieses Schritts war): der echte flaky `Kreatur-Sync: _p2pBroadcastCreatures` (P2P-Broadcast-Timing-Race) ist immer noch potentiell flaky — drei V9.53-Läufe waren grün, aber das ist ein anderer Bug.


**Davor:** V9.52-a..f der Playtest-Pflege-Bogen (31 574 → 29 554 Z., 1 Monolith → 47 benannte Funktionen, Median-Einrückung 20 → 8) · V9.51 der Tarn-Pass Bergseen (additive Gauss-Mulden, `_hydroSeedTarns` + Klemme auf `waterLevel+1`) · V9.50 Wasser aus der Terrain-Wahrheit (Chunk-Wasser-Mesh statt eigenem Modell — „Visual = Collision per Konstruktion") · V9.49 das vereinte Wasser-System (4 Meshes → 1) · V9.48 Hydrosphäre-Politur · V9.47 fluviale Stream-Power-Erosion · V9.46 lange Flüsse · V9.45-a..c Gelände-Relief + wasserdichte See-Töpfe · V9.43-b..e das Wasser-Ultiversum · V9.44 der Stamm-Pflege-Bogen. Die volle Wellen-Chronik mit Lernschlüssen: `docs/handover.md`.
