...
## Aktueller Stand (V9.43-e, 21.05.2026)
...
**Letzte Arbeit — das Wasser-Ultiversum vollendet (V9.43-b..e):** die Hydrosphäre. `_computeHydrosphere` leitet aus der Voxel-Surface ein deterministisches Drainage-Netz ab (Flüsse/Seen/Wasserfälle); `_buildHydrosphereMeshes` rendert es; die Flüsse carven echte Betten ins Terrain (`_hydrosphereCarveAt`, Phase 7), Seen sind schwimmbar; V9.43-e gab dem Wasser Klang — `_tickHydrosphereAudio` moduliert zwei positions-abhängige White-Noise-Layer (Bach-Rauschen + Wasserfall-Donnern) nach der Spieler-Distanz. Algorithmus + Wellen-Schnitt: `docs/hydrosphere.md`.

**Davor — der Stamm-Pflege-Bogen (V9.44-a..f), abgeschlossen:** ein Code-Hygiene-Bogen, reines verhaltensneutrales Refactoring (`docs/archiv/code-hygiene.md`). Die drei Kontrollfluss-Giganten zerlegt (`startEternalLoop` 696→157, `_renderWorkshopDOM`, `p2pHandleMessage`), Persistenz-Schema vereinheitlicht, kanonische Setter, Mesh-Router-Dispatch-Table, Konstanten konsolidiert. KEIN neues Modul, kein State-Manager — der Heiligen Lektion treu.

**Nächste Welle:** offen — `docs/roadmap.md` §1 listet die Kandidaten (Hydrosphäre-Netz-Qualität für lange Flüsse, Crafting-Tiefe, der Playtest-Pflege-Bogen, ein zweiter Code-Hygiene-Bogen); keine ist terminiert. V9.43 ist mit V9.43-e vollständig — offen bleibt nur eine kosmetische Wasser-Politur-Naht (See-Ufer-Schaum, Flow-Speed nach Gefälle).
...
- **Architekturkern:** Eine `AnazhRealm`-Klasse in `anazhRealm.js` (~36 000 Zeilen, Stand V9.43-e). State in `localStorage` + optional `anazhRealmState.json` über save-server. Globale Referenz: `window.anazhRealm`.
...
| `docs/hydrosphere.md` | **V9.43-Wasser-Design** — das Drainage-Netz (Flüsse/Seen/Wasserfälle/Meer als ein Hydrosphären-System). Algorithmus, Datenstrukturen, Wellen-Schnitt b/c/d/e. V9.43 vollständig; offen nur eine kosmetische Politur-Naht. |
...
| `scripts/playtest.cjs` | Headless-Playtest mit **~3000 Invarianten** als CI-Gate (Stand V9.43-e; der Konsolen-Zähler driftet ±2-3 je Lauf — „Alle Invarianten OK" ist die Wahrheit) |
