...
**Letzte Arbeit — das vereinte Wasser-System (V9.49, Architektur-Bogen):** Schöpfer-Befund — das Wasser „schliesst nicht" (Meer/Seen/Flüsse/Wasserfälle als gestapelte durchscheinende Sheets). Wurzel: vier getrennte Render-Schichten. V9.49 wechselt das Verfahren — **Wasser ist ein Feld, kein Körper.** (a) `_hydroBuildWaterField` hebt zwei per-Zelle-Höhen auf `state.hydrosphere.water`: `waterY` (Flut-Oberfläche `filled` — stehendes Wasser) + `terrainY` (Makro-Gelände `surf` — das Fluss-Bett), plus `waterKind` (0 Land/1 Ozean/2 See). (b) `_buildUnifiedWaterMesh` baut EIN spieler-folgendes Höhenfeld (~384 m, 3-m-Raster, gesnappt) — ersetzt Meeres-Plane + See-Planes + Fluss-Ribbons; nasse Quads aus dem Feld (Ozean/See) + dem `riverBuckets`-Index (Fluss). Ein Höhenfeld kann sich nicht selbst überlappen → kein Sheet-Stapeln, strukturell. (c) `_ensureHydroSurfaceMaterial` ist EIN Shader: Gerstner (Ozean, `aWave`-moduliert), Still-Schimmer, Flow-/Ufer-Schaum, `depthWrite:true`. (d) Browser-Audit-Fix: ein Fluss-Vertex + die Ufer-Kante sitzen auf `terrainY` (folgen dem Hang), nur See/Ozean auf `waterY` — sonst schwebt fliessendes Wasser an steilen Stellen (`filled` ist fast flach). `_buildLakeMesh`/`_buildRiverRibbon`/`_lakeShoreFoamField` + der alte Plane-Block gelöscht — netto −117 Zeilen. Der Wasserfall bleibt die vertikale Ausnahme. Volles Design + Lernschlüsse: `docs/hydrosphere.md` §12, Chronik: `docs/handover.md`.
...
**Nächste Welle:** offen. V9.49 wartet zuerst auf den Schöpfer-Browser-Audit (jede Wasser-Welle wird im Browser geprüft, bevor die nächste startet — Headless prüft Mechanik, das Schöpfer-Auge die Ästhetik). Danach: eine etwaige V9.49-Politur aus dem Audit, oder die offenen Roadmap-Kandidaten (`docs/roadmap.md` §1 — Crafting-Tiefe, der Playtest-Pflege-Bogen, ein zweiter Code-Hygiene-Bogen, die Becken-Restleerung `hydrosphere.md` §10b).
