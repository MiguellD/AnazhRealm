...
Letzter Stand: Ring 1 + **Ring 2 (P1-7)** + **Ring 3 V1+V2** + **Ring 4 V1** + **Ring 5 V1+V2** + **Ring 6 V1+V2 inkl. Bauplan-System** + **UI V1+V2** live. Chunk-Physik komplett auf `btBvhTriangleMeshShape` (Commit `e612c60`). 120 fps im echten Browser, **285/285 Playtest-Invarianten grün**. Architektur: eine Sprache (DSL), eine Sicherheits-Wand (CSP + CI-Gate), ein Selektions-Loop (Fitness-V2), ein bidirektionaler Emotions-Kanal, eine Klangschicht (Web Audio), drei animierte Spieler-Seelen mit Third-Person-Toggle, ein **Bauplan-Universum** aus 8 Primitiven mit Hotbar + Werkstatt, eine painterly-Bedien-Oberfläche (vendored Cinzel-Fonts, Tag/Nacht-Tokens, Topbar+7-Drawer, Hotbar, einklappbare Konsole, Brass-Scrollbars). **Nächster Schritt**: Ring 7 (`brain.js` — würde CSP-`unsafe-eval` ablösen, lernt aus Spieler-Pfad/Emotion), Ring 6 V3 (verschachtelte Baupläne = echte Fraktale), oder UI V3 (Astrolabium-Live-Element).
...
- `npm run playtest` startet save-server + Headless-Chromium, sammelt 20-25 s Logs, prüft **285 Invarianten** (inkl. Grok-Stimme, DSL-Effekte, Naht-Treue, CSP, Selektion, Emotionen, Symphonie, UI-V2, Spieler-Seele V2, Third-Person-Kamera, Architecture-V1, Distance-Culling, Fraktale, Kollision-Live-Test, Bauplan-Datenschicht, Hotbar mit 9 Slots, Werkstatt-Editor), exit 1 bei Verletzung.
