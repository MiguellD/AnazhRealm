...
## Aktueller Stand (V9.52-b, 23.05.2026)
...
**Letzte Arbeit — Playtest-Pflege-Bogen Sub-Welle b (V9.52-b):** Band 1 ist extrahiert. Von der ehemaligen 31 574-Zeilen-IIFE sind jetzt die ersten 8010 Zeilen der `### Invarianten`-Sektion in benannte Funktionen aufgeteilt: Sub-Welle a brachte 4 Funktionen (`gatherInitialFinalState`, `checkInitialState`, `checkRing1Grok`, `checkRing2Dsl`), Sub-Welle b brachte 11 weitere als plan-treue **Band-Funktionen** (nicht per-Sektion — `docs/playtest-hygiene.md` §5 verbietet das explizit als Über-Zerlegung): `checkBandRing2Extended`, `checkBandWaves1to3`, `checkBandWave4`, `checkBandWave5`, `checkBandRing8`, `checkBandRing9to10`, `checkBandRing11AndW7Mesh`, `checkBandW16Distribution`, `checkBandW17Multiplayer`, `checkBandW4LofiPad`, `checkBandLateMultiUser` (je 265-1260 Zeilen, intern eine flache Liste der `### `-Sektions-Blöcke). Erste Iteration ging zu granular (41 per-Sektion-Funktionen, ein script-basiertes mechanisches Extract); Plan-Lese-Korrektur ergab Band-Gruppierung — sauberer rückwärts kompatibel, Datei netto −282 Zeilen. Mechanisches `/tmp/extract-band1-grouped.cjs`-Skript (reproduzierbar: Band-Definitionen + 8-Space-Dedent + Header-Kommentare bleiben). Verhaltens-identisch bewiesen über zwei Baseline-Läufe nach dem Schnitt (gleicher 3060-Invarianten-Satz, gleiche Reihenfolge, gleicher Pass/Fail — nur die pre-existing Drift-Bänder: 3 perf-ms-Werte + Worldgen-Architekturen-/PopulatedChunks-Count). Orchestrator ist jetzt eine schlanke Liste von 3 a-Calls + 11 b-Calls (siehe Zeile ~7708). Sub-Wellen c-f ziehen Welle 6.A .. Datei-Ende + den Helfer-Durchzug nach. Voller Plan: `docs/playtest-hygiene.md`.
...
**Nächste Welle:** Playtest-Pflege Sub-Welle c — Band 2 (Welle 6.A .. die Voxel-Terrain-Phasen + Hydrosphäre, ~55 Sektionen, Zeilen ~7725 bis ~?) als Band-Funktionen, dem b-Muster folgend (mechanisches Extract-Skript wiederverwendbar). Daneben offen + auf den Schöpfer-Browser-Audit wartend: V9.50 + V9.51 (jede Wasser-Welle wird im Browser geprüft). Weiter dahinter: die offenen Roadmap-Kandidaten (`docs/roadmap.md` §1 — System-Kopplungen ab „Wasser ↔ Spieler-Wille" als nächste Vision-Vertiefung, Crafting-Tiefe, ein zweiter Code-Hygiene-Bogen).
