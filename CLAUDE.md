...
## Aktueller Stand (V9.46, 22.05.2026)
...
**Letzte Arbeit — lange Flüsse (V9.46):** das Drainage-Netz war see-zerstückelt — jeder Fluss endete am nächsten See (längster 45 m). V9.46 lässt Flüsse durch Seen HINDURCH fliessen, als EINE logische Polylinie vom Hang-Quell über die Seen-Kette bis zum Meer: `_hydroExtractRivers` walkt `flowTo` durch die See-Zellen (das Priority-Flood-ε routet auch im Becken zum Überlauf), See-Punkte tragen `inLake` — der Renderer überspringt ihre Quads (die See-Plane deckt die Strecke), der Carve lässt sie aus. Quellen bleiben land-only (sonst spawnt jedes flache Becken Geister-Quellen an seinen ε-Verzweigungen). Ergebnis: längster Fluss 45 m → 1361 m, alle münden ins Meer.

**Davor — der Gelände- & Wasser-Bogen (V9.45-a..c):** Domain-Warp + Erosions-Feld gegen monotones Gelände (V9.45-a, höhere Chunk-Hülle base−66..base+107 über `_voxelChunkConfig.floorDrop`); die Seebecken als wasserdichte flache Töpfe gegen zwei-Schichten-Wasser (V9.45-b, `_hydrosphereLakeAt`); der Auftrieb an die sichtbare See-Plane gekoppelt (V9.45-c). Davor das Wasser-Ultiversum V9.43-b..e (die Hydrosphäre — Drainage-Netz, Rendering, Carven, Klang; `docs/hydrosphere.md`) + der Stamm-Pflege-Bogen V9.44 (verhaltensneutrales Refactoring).

**Nächste Welle:** offen — `docs/roadmap.md` §1 listet die Kandidaten (Crafting-Tiefe, der Playtest-Pflege-Bogen, ein zweiter Code-Hygiene-Bogen, ein vereintes Wasser-System); keine ist terminiert. Offen-kosmetisch: See-Ufer-Schaum, Flow-Speed nach Gefälle. Ehrlich offen — die Welt ist see-dominant (die Flüsse fädeln Seen-Ketten statt lange offene Hang-Läufe); eine weniger basin-y Routing-Surface wäre ein eigener Bogen (`docs/hydrosphere.md` §10).
...
- **Architekturkern:** Eine `AnazhRealm`-Klasse in `anazhRealm.js` (~36 000 Zeilen, Stand V9.46). State in `localStorage` + optional `anazhRealmState.json` über save-server. Globale Referenz: `window.anazhRealm`.
...
| `scripts/playtest.cjs` | Headless-Playtest mit **~3000 Invarianten** als CI-Gate (Stand V9.46; der Konsolen-Zähler driftet ±2-3 je Lauf — „Alle Invarianten OK" ist die Wahrheit) |
