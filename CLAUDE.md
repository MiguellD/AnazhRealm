...
## Aktueller Stand (V9.50, 22.05.2026)
...
**Letzte Arbeit — Wasser aus der Terrain-Wahrheit (V9.50):** Die V9.49-Browser-Audits (d/e/f) zeigten einen nicht-konvergenten Kreis — das Wasser war ein eigenes Modell (16-m-Raster) neben dem gerenderten Voxel-Terrain; jeder Fix versöhnte zwei Gelände-Wahrheiten, die nächste Naht kam sicher. V9.50 schneidet die Wurzel: Wasser ist KEIN eigenes Mesh mehr — jeder Voxel-Chunk baut seine Wasser-Fläche selbst (`_buildVoxelChunkWater`, geschwisterlich zu `_buildVoxelChunkGrass`), nass je Zelle wo `_voxelSurfaceY < _waterLevelAt`. Wasser + Terrain teilen damit DIESELBE Geometrie-Quelle → die Uferlinie ist der exakte Schnitt, per Konstruktion (das „Visual = Collision"-Gesetz, aufs Wasser angewandt); Schweben/Bleed/Verengungs-Naht sind strukturell unmöglich, nicht poliert. Drei playtest-grüne Sub-Wellen (`_waterLevelAt`/`_hydroRiverAt` → `_buildVoxelChunkWater` → das V9.49-Vereinte-Mesh + alle Workarounds gelöscht), netto −102 Zeilen. 4 Wasser-Meshes (V9.43) → 1 (V9.49) → 0 getrennte Wasser-Modelle (V9.50). Volles Design: `docs/hydrosphere.md` §14; Chronik + Lernschlüsse: `docs/handover.md`.

**Davor:** V9.49 das vereinte Wasser-System (4 Wasser-Meshes → 1 — der Vorgänger-Schritt zu V9.50) · V9.48 Hydrosphäre-Politur · V9.47 fluviale Stream-Power-Erosion (`_computeErosion` carvt echte Täler, bewahrt die Gipfel) · V9.46 lange Flüsse (durch Seen hindurch) · V9.45-a..c Gelände-Relief + wasserdichte See-Töpfe · V9.43-b..e das Wasser-Ultiversum (die Hydrosphäre) · V9.44 der Stamm-Pflege-Bogen. Die volle Wellen-Chronik mit Lernschlüssen: `docs/handover.md`.

**Nächste Welle:** offen. V9.50 wartet zuerst auf den Schöpfer-Browser-Audit (jede Wasser-Welle wird im Browser geprüft — Headless prüft Mechanik, das Schöpfer-Auge die Ästhetik). Danach: der Tarn-Pass (Bergseen als additive Hochmulden, `hydrosphere.md` §13.6), oder die offenen Roadmap-Kandidaten (`docs/roadmap.md` §1 — Crafting-Tiefe, der Playtest-Pflege-Bogen, ein zweiter Code-Hygiene-Bogen).
