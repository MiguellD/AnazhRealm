...
**Letzte Arbeit — das vereinte Wasser-System (V9.49, Architektur-Bogen):** Schöpfer-Befund — das Wasser „schliesst nicht" (Meer/Seen/Flüsse/Wasserfälle als gestapelte durchscheinende Sheets). Wurzel: vier getrennte Render-Schichten. V9.49 wechselt das Verfahren — **Wasser ist ein Feld, kein Körper.** (a) `_hydroBuildWaterField` hebt das Priority-Flood-`filled` als per-Zelle-Feld `{waterY, waterKind}` auf `state.hydrosphere.water`. (b) `_buildUnifiedWaterMesh` baut EIN spieler-folgendes Höhenfeld (~384 m, 3-m-Raster, gesnappt) — ersetzt Meeres-Plane + See-Planes + Fluss-Ribbons; nasse Quads aus dem Feld (Ozean/See) + dem `riverBuckets`-Index (Fluss). Ein Höhenfeld kann sich nicht selbst überlappen → kein Sheet-Stapeln, strukturell. (c) `_ensureHydroSurfaceMaterial` ist EIN Shader: Gerstner (Ozean, `aWave`-moduliert), Still-Schimmer, Flow-/Ufer-Schaum, `depthWrite:true`. `_buildLakeMesh`/`_buildRiverRibbon`/`_lakeShoreFoamField` + der alte Plane-Block gelöscht — netto −117 Zeilen. Der Wasserfall bleibt die vertikale Ausnahme. **Browser-Audit durch den Schöpfer steht aus** (wie bei jeder Wasser-Welle). Volles Design + Lernschlüsse: `docs/hydrosphere.md` §12, Chronik: `docs/handover.md`.
