...
## Aktueller Stand (V9.51, 22.05.2026)
...
**Letzte Arbeit — der Tarn-Pass: Bergseen (V9.51):** Schliesst den allerersten Befund — „es entstehen Flüsse, aber keine Bergseen". Die Stream-Power-Erosion (V9.47) entwässert jedes Hochbecken → ohne Pass keine Tarns. `_hydroSeedTarns` setzt nach der Erosion gezielt Gauss-Mulden (`r 85-120m`, adaptive Tiefe `0.996·slope·r + 8`) an hohen, sanften Spots (adaptive 62.-Perzentil-Höhen-Schwelle, max Hang 0.25). Der Beitrag fliesst über `_tarnDeltaAt` in `_terrainMacroSurfaceY` (NICHT ins Erosions-Delta — Determinismus bleibt). KRITISCH: `_terrainMacroSurfaceY` klemmt das Tarn-Resultat auf `waterLevel+1` (sonst pusht der Bowl-Tail Nachbar-Zellen unter den Meeresspiegel → Border-Ozean-Flood markiert den Tarn als Ozean, keine See-Extraktion). Das bestehende Priority-Flood entdeckt die Mulde, füllt sie zum See, der See-Topf-Carve + Chunk-Wasser-Render bringen sie sichtbar — keine neue Wasser-Maschinerie, ein additiver Wachstumsring. Volles Design: `docs/hydrosphere.md` §15; Chronik + Lernschlüsse: `docs/handover.md`.

**Davor:** V9.50 Wasser aus der Terrain-Wahrheit (Chunk-Wasser-Mesh statt eigenem Modell — „Visual = Collision per Konstruktion") · V9.49 das vereinte Wasser-System (4 Meshes → 1) · V9.48 Hydrosphäre-Politur · V9.47 fluviale Stream-Power-Erosion (`_computeErosion` carvt echte Täler, bewahrt die Gipfel) · V9.46 lange Flüsse · V9.45-a..c Gelände-Relief + wasserdichte See-Töpfe · V9.43-b..e das Wasser-Ultiversum · V9.44 der Stamm-Pflege-Bogen. Die volle Wellen-Chronik mit Lernschlüssen: `docs/handover.md`.

**Nächste Welle:** offen. V9.50 + V9.51 warten zuerst auf den Schöpfer-Browser-Audit (jede Wasser-Welle wird im Browser geprüft). Der Wasser-Bogen V9.43–V9.51 ist damit funktional + ästhetisch + mit Bergseen vollständig. Danach: die offenen Roadmap-Kandidaten (`docs/roadmap.md` §1 — Crafting-Tiefe, der Playtest-Pflege-Bogen, ein zweiter Code-Hygiene-Bogen).
