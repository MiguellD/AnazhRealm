...
## Aktueller Stand (V9.45-c, 22.05.2026)
...
**Letzte Arbeit — der Gelände- & Wasser-Bogen (V9.45-a..c):** ein Schöpfer-Befund-Bogen gegen monotones Gelände + zwei-Schichten-Wasser. **V9.45-a** gab `_terrainMacroSurfaceY` die zwei Profi-Werkzeuge prozeduraler Welten — Domain-Warping (mäandernde Kämme statt gitter-paralleler Grate) + ein regionales Erosions-Feld (manche Striche Tiefland, andere alpin — gegen „alle Berge gleich hoch") — plus grössere Amplituden und eine höhere Voxel-Chunk-Hülle (base−66..base+107, zentralisiert über `_voxelChunkConfig.floorDrop`). **V9.45-b** heilte drei Wasser-Bugs: das Seebecken ist jetzt ein flach gesculpteter, wasserdichter Topf (`_hydrosphereLakeAt` blendet die Dichte zur Bett-Ebene `bedY`), dessen Boden garantiert über dem Meeresspiegel liegt → die globale Meeres-Plane scheint nicht mehr als zweites Wasser-Layer durch, und man taucht nicht mehr durch den See-Boden; die See-Plane deckt einen 1-Zellen-Ring extra → das opake Ufer-Terrain schneidet die Wasserlinie sauber. **V9.45-c** koppelte den Auftrieb (`_hydroWaterLevelAt`) an dieselbe dilatierte See-Plane-Region (Optik == Physik) und trug zwei Rest-Grenzen ehrlich nach (siehe `docs/handover.md`).

**Davor — das Wasser-Ultiversum (V9.43-b..e):** die Hydrosphäre. `_computeHydrosphere` leitet aus der Voxel-Surface ein deterministisches Drainage-Netz ab (Flüsse/Seen/Wasserfälle); `_buildHydrosphereMeshes` rendert es; die Flüsse carven echte Betten ins Terrain (`_hydrosphereCarveAt`); V9.43-e gab dem Wasser Klang (`_tickHydrosphereAudio`). Algorithmus: `docs/hydrosphere.md`. Davor der Stamm-Pflege-Bogen (V9.44-a..f) — reines verhaltensneutrales Refactoring (`docs/archiv/code-hygiene.md`), KEIN neues Modul, der Heiligen Lektion treu.

**Nächste Welle:** offen — `docs/roadmap.md` §1 listet die Kandidaten (Hydrosphäre-Netz-Qualität für lange Flüsse, Crafting-Tiefe, der Playtest-Pflege-Bogen, ein zweiter Code-Hygiene-Bogen); keine ist terminiert. Offen-kosmetisch bleibt: See-Ufer-Schaum, Flow-Speed nach Gefälle.
...
- **Architekturkern:** Eine `AnazhRealm`-Klasse in `anazhRealm.js` (~36 000 Zeilen, Stand V9.45-c). State in `localStorage` + optional `anazhRealmState.json` über save-server. Globale Referenz: `window.anazhRealm`.
...
| `scripts/playtest.cjs` | Headless-Playtest mit **~3000 Invarianten** als CI-Gate (Stand V9.45-c; der Konsolen-Zähler driftet ±2-3 je Lauf — „Alle Invarianten OK" ist die Wahrheit) |
