...
const character=new THREE.Group();var eyeL,eyeR,irisL,irisR,lidTL,lidTR,lidBL,lidBR,browL,browR,upperLipRef,lowerLipRef;
const torsoG=new THREE.Group();torsoG.position.set(0,trochanterY,0);character.add(torsoG);reg('torso',torsoG);
...
[-1,1].forEach(sd=>{const g=new THREE.Group();const l=s(0.4,matSkin,[1.4,0.6,0.8]);l.position.set(sd*0.22,nippleY+0.2-trochanterY,0.35);l.rotation.z=sd*-0.2;g.add(l);const u=s(0.3,matSkin,[1.2,0.4,0.6]);u.position.set(sd*0.55,nippleY+0.6-trochanterY,0.32);u.rotation.set(0,sd*0.2,sd*0.4);g.add(u);reg('pec'+(sd===1?'1':'-1'),g);torsoG.add(g);});
...
[-1,1].forEach(sd=>{const armG=new THREE.Group();armG.position.set(sd*baseArmX,acromionY-trochanterY,0);reg('arm'+(sd===1?'1':'-1'),armG);const delt=reg('deltoid'+(sd===1?'1':'-1'),s(0.30,matSkin,[1,1.1,1]));armG.add(delt);const bic=reg('bicep'+(sd===1?'1':'-1'),s(0.26,matSkin,[0.9,upperArmLen/0.52,1]));bic.position.set(0,-upperArmLen/2,0.1);armG.add(bic);const tri=reg('tricep'+(sd===1?'1':'-1'),s(0.27,matSkin,[0.9,upperArmLen/0.54,0.9]));tri.position.set(0,-upperArmLen/2,-0.15);armG.add(tri);const uarm=reg('uarm'+(sd===1?'1':'-1'),s(0.23,matSkin,[1.0,upperArmLen/0.46,0.96]));uarm.position.set(0,-upperArmLen/2,-0.02);armG.add(uarm);const el=s(0.165,matJoint,[1,0.75,1]);el.position.set(0,-upperArmLen,0);armG.add(el);const elbowG=new THREE.Group();elbowG.position.set(0,-upperArmLen,0);armG.add(elbowG);reg('elbow'+(sd===1?'1':'-1'),elbowG);const fa=reg('forearm'+(sd===1?'1':'-1'),s(0.24,matSkin,[1.2,forearmLen/0.48,1]));fa.position.set(sd*0.05,-forearmLen/2,0);elbowG.add(fa);const wY=-forearmLen;const wr=s(0.14,matSkin,[1,0.5,0.9]);wr.position.set(sd*0.05,wY,0);elbowG.add(wr);
const hand=new THREE.Group();hand.userData.sd=sd;hand.userData.fingers=[];hand.userData.thumbs=[];
const palmG=new THREE.Group();hand.add(palmG);hand.userData.palmG=palmG;reg('palm'+(sd===1?'1':'-1'),palmG);
const palm=s(0.25,matSkin,[1,1.2,0.5]);palmG.add(palm);
for(let i=0;i<4;i++){const fX=(i-1.5)*0.12,fZ=0.1-Math.abs(i-1.5)*0.05;const fG=new THREE.Group();fG.position.set(fX,-0.28,fZ);fG.rotation.z=(i-1.5)*0.13;reg('fA'+i+'_'+(sd===1?'1':'-1'),fG);fG.add(s(0.08,matSkin,[0.9,0.8,0.9]));const f1=c(0.06,0.05,0.18,matSkin);f1.position.y=-0.09;fG.add(f1);const jG1=new THREE.Group();jG1.position.set(0,-0.18,0);reg('fB'+i+'_'+(sd===1?'1':'-1'),jG1);jG1.add(s(0.05,matSkin,[1.0,0.8,1.0]));const f2=c(0.05,0.04,0.14,matSkin);f2.position.set(0,-0.07,-0.01);jG1.add(f2);const jG2=new THREE.Group();jG2.position.set(0,-0.14,-0.02);reg('fC'+i+'_'+(sd===1?'1':'-1'),jG2);jG2.add(s(0.04,matSkin,[1.0,0.8,1.0]));const f3=c(0.04,0.03,0.1,matSkin);f3.position.set(0,-0.05,-0.04);jG2.add(f3);jG1.add(jG2);fG.add(jG1);palmG.add(fG);hand.userData.fingers.push({mcp:fG,pip:jG1,dip:jG2});}
const thumbG=new THREE.Group();thumbG.position.set(-sd*0.3,-0.15,0.1);thumbG.rotation.set(0,sd*0.5,sd*-1.2);reg('tA_'+(sd===1?'1':'-1'),thumbG);thumbG.add(s(0.1,matSkin,[0.8,1.5,0.8]));const thumbF1=c(0.06,0.05,0.12,matSkin);thumbF1.position.y=-0.06;thumbG.add(thumbF1);const thumbJG=new THREE.Group();thumbJG.position.set(0,-0.12,0);reg('tB_'+(sd===1?'1':'-1'),thumbJG);thumbJG.add(s(0.06,matSkin,[0.8,0.8,0.8]));const thumbTip=c(0.05,0.04,0.08,matSkin);thumbTip.position.y=-0.04;thumbJG.add(thumbTip);thumbG.add(thumbJG);palmG.add(thumbG);hand.userData.thumbs.push({mcp:thumbG,ip:thumbJG});
...
const headGroup=new THREE.Group();headGroup.position.y=H-headSeg/2-trochanterY;reg('head',headGroup);var HEAD_S=0.92;headGroup.scale.setScalar(HEAD_S);base.headPosY=H-headSeg/2-headSeg*0.20*(1-HEAD_S);   // loomis-pass: 5.7->6.6 koepfe; pivot-korrektur haelt das KINN auf hoehe (halsroehre!)
...
[-1,1].forEach(sd=>{const e=new THREE.Group();e.add(s(0.12,matEye,[1,1.1,1]));const iris=s(0.07,matIris,[0.8,1,0.8]);iris.position.z=0.08;e.add(iris);const pupil=s(0.04,new THREE.MeshBasicMaterial({color:0x000000}));pupil.position.z=0.11;e.add(pupil);e.add(s(0.13,matCornea,[1,1.1,1]));const lidT=s(0.14,matSkin,[1.15,0.35,0.9]);lidT.position.y=0.07;e.add(lidT);const lidB=s(0.14,matSkin,[1.15,0.45,0.95]);lidB.position.y=-0.09;e.add(lidB);e.position.set(sd*eyeDist/2,eyeY,0.32);headGroup.add(e);if(sd<0){eyeL=e;irisL=iris;lidTL=lidT;lidBL=lidB;}else{eyeR=e;irisR=iris;lidTR=lidT;lidBR=lidB;}});
[-1,1].forEach(sd=>{const g=new THREE.Group();const bone=reg('cheekbone'+(sd===1?'1':'-1'),s(0.13,matSkin,[1.15,0.52,0.72]));bone.position.set(sd*0.22,eyeY-0.04,0.27);bone.rotation.y=sd*0.12;bone.rotation.z=sd*-0.05;g.add(bone);headGroup.add(g);});
...
[-1,1].forEach(sd=>{const e=new THREE.Group();const helix=s(0.12,matSkin,[0.35,1.7,0.95]);e.add(helix);const concha=s(0.09,matSocket,[0.55,0.95,0.5]);concha.position.set(sd*-0.02,0.0,0.03);e.add(concha);const tragus=s(0.042,matSkin,[0.6,0.75,0.5]);tragus.position.set(sd*-0.065,-0.03,0.07);e.add(tragus);const lobe=s(0.055,matSkin,[0.5,0.75,1]);lobe.position.set(sd*-0.01,-0.17,0.02);e.add(lobe);e.position.set(sd*0.43,eyeY-0.04,-0.03);e.rotation.y=sd*-0.45;headGroup.add(e);});
[-1,1].forEach(sd=>{const g=new THREE.Group();for(let i=0;i<5;i++){const b=c(0.012,0.012,0.07,matHair);b.position.set((i-2)*0.032,-Math.abs(i-2)*0.008,0);b.rotation.z=sd*0.2;g.add(b);}g.position.set(sd*eyeDist/2,browY+0.03,0.35);g.rotation.z=sd*-0.1;headGroup.add(g);if(sd<0)browL=g;else browR=g;});
torsoG.add(headGroup);scene.add(character);
...
function _landmarks(){
  ...
  var chin=P('chin'),nb=P('neckBase'),waist=P('waist'),knee=P('knee1');
  LM={chinY:chin?chin.y:5.44,
      neckTopY:(chin?chin.y:5.44)-0.04,
      collarY:nb?nb.y+0.02:4.95,
      shoulderTopY:nb?nb.y+0.04:5.06,
      neckR:Math.max(0.12,radOf('neckBase')*1.05),
      neckCX:nb?nb.x:0,neckCZ:nb?nb.z:0,
      waistY:waist?waist.y:3.9,
      wristFrac:1.36,wristOverlap:0.06,                     // handgelenks-VERTRAG: hand-shell endet bei wristFrac, koerperhaut reicht wristFrac+overlap darueber -- EIN ursprung, ueberlapp explizit
